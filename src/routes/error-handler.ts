...
export function registerErrorHandler(app: FastifyInstance): void {
  app.setErrorHandler((error, req, reply) => {
     ...
     */
    /*
     * Read through one narrowed view rather than casting at each use.
     *
     * `error` is `unknown` here — which is why every branch above reaches `.message` only
     * after an `instanceof` check. The first version of this block cast for `statusCode`
     * and `code` but then touched `error.message` directly, which is exactly the mistake
     * the `unknown` is there to prevent, and `pnpm typecheck` refused it on the pre-push
     * hook.
     *
     * Each field is checked for the type it is used as, so a thrown object with a numeric
     * `code` or a `message` that is not a string cannot put a number or an object into a
     * JSON error response.
     */
