...
const BundleInput = z.object({
  sku: z
    ...
    .regex(
      ...
      'Part # may use letters, numbers, dot, dash and underscore',
    ),
...
export function registerBundleRoutes(app: FastifyInstance): void {
  ...
  app.get('/catalog/bundles', read, async () => {
      ...
      select: {
        ...
        parentRelations: {
          ...
          select: {
            ...
            quantity: true,
            sortOrder: true,
            child: { select: { id: true, sku: true, name: true, status: true } },
          },
        },
      },
    });
    ...
    return bundles.map((b) => {
      const components = b.parentRelations.map((r) => {
        ...
        return {
          relationId: r.id,
          productId: r.child.id,
          sku: r.child.sku,
          name: r.child.name,
          status: r.child.status,
          quantity: r.quantity,
          unitPriceMinor: p?.unitPriceMinor ?? 0,
      ...
      const totals = rollup(
        components.map((c) => ({ quantity: c.quantity, sku: c.sku })),
        priced,
      );
      return {
        id: b.id,
  ...
  app.post('/catalog/bundles', admin, async (req, reply) => {
    ...
    if (!parsed.success)
      throw new ValidationError(parsed.error.issues[0]?.message ?? 'Invalid bundle');
    const d = parsed.data;
    if (await prisma.product.findUnique({ where: { sku: d.sku } }))
      throw new ConflictError(`Part # “${d.sku}” already exists`);
    ...
    const bundle = await prisma.product.create({
      data: {
        ...
        kind: 'BUNDLE',
        status: 'DRAFT',
        proposalDescription: d.proposalDescription || null,
  ...
  app.put('/catalog/bundles/:id/components', admin, async (req) => {
    ...
    if (!parsed.success)
      throw new ValidationError(parsed.error.issues[0]?.message ?? 'Invalid components');
    ...
    const priorRelations = await prisma.productRelation.findMany({
      where: { parentId: id, type: 'BUNDLE_ITEM' },
      orderBy: { sortOrder: 'asc' },
      select: { childId: true, quantity: true, child: { select: { sku: true, name: true } } },
    });

    const comps = parsed.data.components;
    if (comps.some((c) => c.productId === id))
      throw new ConflictError('A bundle cannot contain itself');
    const ids = [...new Set(comps.map((c) => c.productId))];
    if (ids.length !== comps.length)
    ...
    if (nested)
      throw new ConflictError(
        `“${nested.name}” is itself a bundle — add its parts directly instead`,
      );
    ...
    await recordAudit({
      actorId: req.user!.sub,
      action: 'catalog.bundle.components',
      entity: 'Product',
      entityId: id,
    ...
    await recordRevision({
      ...
      actorId: req.user!.sub,
      before: {
        components: priorRelations.map((r) => ({
          sku: r.child?.sku ?? r.childId,
          name: r.child?.name ?? '',
          quantity: r.quantity,
        })),
      },
  ...
  app.delete('/catalog/bundles/:id', admin, async (req, reply) => {
    ...
    const used = versions.filter(
      (v) =>
        ...
        (v.items as { sku?: string }[]).some((i) => i && i.sku === bundle.sku),
    );
    if (used.length) {
      throw new ConflictError(
        `This bundle is on ${used.length} proposal(s) (${used
          .slice(0, 3)
          .map((u) => u.proposal.number)
    ...
    await recordAudit({
      actorId: req.user!.sub,
      action: 'catalog.bundle.delete',
      entity: 'Product',
      entityId: id,
      details: { sku: bundle.sku },
    });
    reply.code(204);
    return null;
  });
