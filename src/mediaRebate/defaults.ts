...
/**
 * Typeface and layout for the printed program page, as a closed set of choices —
 * identical shape to `LegalStyle` (`src/legal/defaults.ts`) and validated with the same
 * bounds in `src/routes/mediaPartnershipProgram.ts`'s `Style` schema, so this document
 * gets the same "Layout" editor and the same guarantees: nothing here can push a
 * signature block or the acceptance page off the sheet the proposal paginator builds.
 */
export interface MediaProgramStyle {
  ...
  align?: string;
  /** Section heading size in points, 11 to 22. Default 15. */
  titlePt?: number;
}
...
/** Same values contract documents ship with — see `LEGAL_STYLE_DEFAULTS`-equivalent
 * defaults in `public/contract-pages.js`'s `STYLE_DEFAULTS`. */
export const DEFAULT_MEDIA_PROGRAM_STYLE: MediaProgramStyle = {
  font: 'plex',
  sizePt: 9,
  lineHeight: 1.35,
  align: 'justify',
...
export const DEFAULT_MEDIA_PROGRAM_CONTENT: MediaProgramContent = {
  ...
  style: DEFAULT_MEDIA_PROGRAM_STYLE,
};
