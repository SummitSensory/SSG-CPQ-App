...
export async function reconcile(productionWritesEnabled: boolean): Promise<QboReconcileReport> {
  ...
  const [txns, connections, links, failures] = await Promise.all([
    ...
    prisma.integrationSyncLog.findMany({
      where: { provider: 'quickbooks', status: { in: ['error', 'conflict'] } },
      orderBy: { createdAt: 'desc' },
      take: 50,
    }),
  ]);
  ...
  return {
    ...
    awaitingAuthorization: txns
      .filter((t) => t.status === 'PENDING_AUTHORIZATION')
      .map((t) => ({
        id: t.id,
        type: t.type,
        proposalId: t.proposalId,
    ...
    authorizedNotCreated: txns
      .filter((t) => t.status === 'AUTHORIZED')
      .map((t) => ({
        id: t.id,
        type: t.type,
        proposalId: t.proposalId,
        authorizedById: t.authorizedById,
        authorizedAt: t.authorizedAt?.toISOString() ?? null,
      })),
    erroredLinks: links.map((l) => ({ entity: l.entity, entityId: l.entityId, state: l.state })),
    recentSyncFailures: failures.map((f) => ({
      id: f.id,
      entity: f.entity,
      status: f.status,
      error: f.error,
