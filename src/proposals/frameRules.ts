...
const R = (
  ...
  name,
  group,
  terms: terms.map(([source, coefficient, when]) => ({ ...(source ? { source } : {}), coefficient, ...(when ? { when } : {}) })),
  constant: 0,
  factor: 1,
  roundMode: 'NONE',
  roundStep: 1,
...
export const DEFAULT_FRAME_RULES: FormulaRule[] = [
  R('Verticals', 'A-2245', 'Vertical Post', [['in:legs', 1], ['in:ladders', -1]]),
  R('Verticals', 'A-2246', 'Vertical Post — Ladder Bay', [['in:ladders', 1]]),

  R('Corner posts', 'A-2241', 'Corner Post — T-Shape', [[null, 2, shapeIs('T-Shape')]]),
  R('Corner posts', 'A-2242', 'Corner Post — Standard', [[null, 4, { input: 'legs', op: '>', value: 0 }], [null, 1, shapeIs('L-Shape')]]),
  ...
  R('Mid span saddle', 'A-2225', 'Mid Span Saddle', [['in:interiorBeams', 2], [null, 2, flagOn('monkeyBars')]]),

  R('Ladders', 'P-2531', 'Ladder Leg', [['in:ladders', 1]]),
  R('Ladders', 'A-2253', 'Ladder Mount', [['in:ladders', 1]]),

  R('Rungs', 'P-2330', 'Rung', [[null, 9, flagOn('monkeyBars')], ['in:ladders', 5]]),
  ...
  R('Slide', 'A-2349', 'Slide Conversion Kit', [[null, 1]], { when: flagOn('steamroller') }),

  R('Climbing wall', 'SSG-SA-CFM', 'Climbing Wall — Frame Mounted', [[null, 1]], { when: flagOn('climbFrame') }),
  R('Climbing wall', 'SSG-SA-CWM', 'Climbing Wall — Wall Mounted', [[null, 1]], { when: flagOn('climbWall') }),
  R('Climbing wall', 'P-2500', 'Climbing Wall Safety Shield', [['in:climbWalls', 1]], { when: flagOn('climbShield') }),
...
export interface FrameAnswers {
  ...
  monkeyBars?: boolean; interiorBeams?: boolean; interiorBeamsQty?: number;
  trolley?: boolean; zipLine?: boolean; zipLineQty?: number; ballRack?: boolean;
  slide?: boolean; slideGray?: boolean; steamroller?: boolean;
  climbFrame?: boolean; climbWall?: boolean; climbShield?: boolean;
  brackets?: boolean; bracketsQty?: number;
}
...
export function frameContext(a: FrameAnswers, bomQty: (part: string) => number): RuleContext {
  const raw: Record<string, unknown> = {
    legs: n(a.legs), ladders: n(a.ladders), length: n(a.length), width: n(a.width),
    ...
    monkeyBars: !!a.monkeyBars, zipLine: !!a.zipLine, ballRack: !!a.ballRack,
    slide: !!a.slide, slideGray: !!(a.slide && a.slideGray), steamroller: !!(a.slide && a.steamroller),
    climbFrame: !!a.climbFrame, climbWall: !!a.climbWall,
    climbShield: !!a.climbShield, trolley: !!a.trolley,
  };
  return {
    bom: bomQty,
    input: (key) => {
