...
export function computeAdventureBOM(a: AdvAnswers): { part: string; qty: number }[] {
  ...
  Object.keys(memLen).forEach((part) => {
    ...
    const monkeyPart = monkeyMem[len];
    const mq = monkey && L === len && monkeyPart ? 2 : 0;
    const oo = -0.5 * mq;
    add(part, jj + kk + ll + nn + oo);
    if (mq > 0 && monkeyPart) add(monkeyPart, mq);
  });
  // Monkey bar rungs
  ...
  // Zip line
  if (a.zipLine) {
    add('P-2024', 2 * n(a.zipLineQty || 1));
    add('A-2530', 4 * n(a.zipLineQty || 1));
  }
  // Ball rack
...
export function computeAdventureProposal(
  ...
  const P = (part: string, qtyOverride?: number, nameOverride?: string): void => {
    ...
    lines.push({
      lineType: 'PRODUCT',
      name: nameOverride || (rec ? rec.description : part),
      sku: part,
      description: '',
      quantity: qty,
  ...
  if (a.trolley) {
    G('Dual Trolley System (Optional)', true);
    [
      'P-2018',
      'P-2025',
      'TR2000-A07',
  ...
  if (hasComp) {
    ...
    if (a.zipLine || a.ballRack) {
      SG('Complete Zip Line Kit');
      P('P-2024');
      P('A-2530');
      P('K-5000');
    }
  ...
  if (a.matFloor || a.matColumn || a.matLadderLeg || a.matCustom) {
    G('Adventure Mat System (Highly Recommended — Optional)', true);
    SG('Adventure Mat System');
    if (a.matFloor)
      lines.push({
        lineType: 'PRODUCT',
        name: 'Adventure Mat System — Floor',
        sku: '',
    ...
    if (a.matColumn) {
      if (n(a.uShaped) > 0)
        lines.push({
          lineType: 'PRODUCT',
          name: 'U-Shaped Column Wraps',
          sku: '',
          quantity: n(a.uShaped),
          rateMinor: 0,
          weightEach: 0,
          needsPrice: true,
        });
      if (n(a.completeWrap) > 0)
        lines.push({
          lineType: 'PRODUCT',
          name: 'Complete Column Wraps',
          sku: '',
          quantity: n(a.completeWrap),
          rateMinor: 0,
          weightEach: 0,
          needsPrice: true,
        });
    ...
    if (a.matLadderLeg)
      lines.push({
        lineType: 'PRODUCT',
        name: 'Adventure Mat System — Ladder Leg',
        sku: '',
        quantity: n(a.ladders),
        rateMinor: 0,
        weightEach: 0,
        needsPrice: true,
      });
  ...
  const addlHw =
    n(a.forged) || n(a.swingHanger) || n(a.vRings) || n(a.carabiner) || n(a.webbingSling);
  if (a.brackets || addlHw) {
    G('Hardware', false);
    if (a.brackets) {
      SG('Quick Shift Saddle Bracket');
      P('P-2124');
      P('6820H-LDD', n(a.swivel360), '360 Swivel / 180 Rotational Eye Bolt');
      P('6820H-LAC-G', Math.max(0, n(a.bracketsQty) - n(a.swivel360)), '3/8" Non-Swivel Eye Bolts');
    }
    if (addlHw) {
      ...
      if (n(a.carabiner) > 0)
        lines.push({
          lineType: 'PRODUCT',
          name: 'Auto-Locking Carabiner (4-Pack)',
          sku: '',
          quantity: n(a.carabiner),
          rateMinor: 0,
          weightEach: 0,
          needsPrice: true,
        });
      if (n(a.webbingSling) > 0)
        lines.push({
          lineType: 'PRODUCT',
          name: 'Multi-Pocket Webbing Sling',
          sku: '',
          quantity: n(a.webbingSling),
          rateMinor: 0,
          weightEach: 0,
          needsPrice: true,
        });
