...
const ceilTo = (v: number, step: number): number =>
...
export const DEFAULT_HARDWARE_RULES: HardwareRule[] = [
  ...
  R(
    '6820H-LAK',
    '1/2" × 4" Titen HD Screw Anchor, Zinc',
    [
      ['bom:A-2245', 4],
      ['bom:A-2246', 4],
    ],
    { constant: 1, roundMode: 'CEIL', roundStep: 1 },
  ),
  R('6820H-LDD', 'Swing & Swivel Eye Bolt (Quick Shift Saddle Bracket)', [['in:swivel360', 1]]),
  R('6820H-LAC-G', 'Eye Bolt — Fixed (Quick Shift Saddle Bracket)', [
    ['in:bracketsQty', 1],
    ['in:swivel360', -1],
  ]),
  R('6820H-LP', 'Eye Bolt — Fixed', [
    ['in:forged', 1],
    ['bom:P-2024', 1],
  ]),
  ...
  R(
    '6820H-LC',
    'Hex Nylon Insert Lock Nut, 1/2-13',
    [
      ['hw:6820H-LP', 1],
      ['hw:6820H-LA', 1],
      ['hw:6820H-LQ', 1],
  ...
  R(
    '6820H-LF',
    'Hex Nylon Insert Lock Nut, 3/8-16',
    [
      ['bom:P-2124', 1],
      ['hw:6820H-LAF', 1],
    ],
    { factor: 1.05, roundMode: 'CEIL', roundStep: 1 },
  ),
  R(
    '6820H-LB',
    'Washer 1/2 Flat',
  ...
  R('6820H-LAG', 'Hex Bolt, 1/2-13 × 4"', [], {
    active: false,
    note: 'Hard zero in the v73 workbook.',
  }),
  R('6820H-LH', 'Tap Bolt, 3/8-16 × 2-1/4"', [], {
    active: false,
    note: 'Hard zero in the v73 workbook.',
  }),
  R(
    '6820H-LG',
    'Tap Bolt, 3/8-16 × 1-3/4"',
    [
      ['bom:P-2330', 2],
      ['bom:P-2531', -10],
    ],
    { factor: 1.02, roundMode: 'CEIL', roundStep: 1 },
  ),
  R('6820H-LI', 'Hex Bolt, 1/4-20 × 2-3/4"', [['bom:P-2531', 2]]),
  R('6820H-LJ', 'Hex Bolt, 1/4-20 × 4"', [['bom:A-2246', 2]]),
  R('6820H-LK', 'USS Flat Washer, 1/4', [
    ['hw:6820H-LY', 1],
    ['hw:6820H-LI', 2],
    ['hw:6820H-LJ', 2],
  ]),
  R('6820H-LL', 'Hex Nylon Insert Lock Nut, 1/4-20', [
    ['hw:6820H-LI', 1],
    ['hw:6820H-LJ', 1],
    ['hw:6820H-LY', 1],
  ]),
  ...
  R('6820H-LAA', 'Lock Washer, 5/16', [
    ['bom:P-2018', 2],
    ['bom:P-2028', 1],
  ]),
  R('6820H-LU', 'USS Flat Washer, 5/16', [
    ['bom:P-2025', 3],
    ['bom:P-2028', 2],
  ]),
  R('6820H-LV', 'Hex Bolt, 5/16-18 × 1"', [
    ['bom:P-2028', 1],
    ['bom:P-2025', 3],
  ]),
  ...
  R(
    ...
    { factor: 1.2, roundMode: 'CEIL', roundStep: 1 },
  ),
  R('6820H-LN', 'Hex Bolt, 1/2-13 × 3-1/2"', [], {
    active: false,
    note: 'Hard zero in the v73 workbook.',
  }),
  R('6820H-LS', 'Rubber Bumpers, 3/4 OD (25 pack)', [['bom:TRN2016', 1]], {
    mode: 'PRESENCE',
    constant: 1,
  }),
  R('6820H-LAE', 'Tap Bolt, 3/8-16 × 4"', [['in:vRings', 10]]),
  R('6820H-LAF', 'V-Ring Bolt-On Anchor', [['in:vRings', 10]]),
  R('6820H-LAH', 'Button Head Hex Drive Screw, 3/8-16 × 1-1/2"', [
    ['bom:P-2531', 10],
    ['bom:A-2253', 2],
    ['bom:A-2248', 2],
  ]),
  ...
  function resolve(part: string): number {
    ...
    if (ruleApplies) {
      for (const t of rule.terms) {
        ...
        sum += v * t.coefficient;
        parts.push(
          `${sign} ${label(t.source)} (${v})${mag === 1 ? '' : ` × ${mag}`}${cond}`.trim(),
        );
      }
    }
    ...
    if (!rule.active) {
      ...
      expr = parts.join(' ') || '0';
      if (rule.constant)
        expr = `${expr} ${rule.constant < 0 ? '−' : '+'} ${Math.abs(rule.constant)}`;
      if (rule.factor !== 1) {
        v *= rule.factor;
        expr = `(${expr}) × ${rule.factor}`;
      }
 ...
 */
export function mergeHardwareRules(
  dbRows: Partial<HardwareRule>[] | null | undefined,
): HardwareRule[] {
  ...
  for (const row of dbRows) {
    if (row.part && !out.some((r) => r.part === row.part)) {
      out.push({
        part: row.part,
        name: row.name || row.part,
        terms: (row.terms as HardwareTerm[]) || [],
        constant: row.constant ?? 0,
        factor: row.factor ?? 1,
        roundMode: row.roundMode ?? 'NONE',
        roundStep: row.roundStep ?? 1,
        mode: row.mode ?? 'SUM',
        minZero: row.minZero ?? true,
        sortOrder: row.sortOrder ?? out.length,
...
export function mergeRules(
  defaults: FormulaRule[],
  dbRows: Partial<FormulaRule>[] | null | undefined,
): FormulaRule[] {
  ...
  const out = defaults.map((d) => {
    ...
    return {
      ...d,
      ...row,
      terms: Array.isArray(row.terms) ? (row.terms as FormulaTerm[]) : d.terms,
      edited: true,
    } as FormulaRule;
  });
  for (const row of dbRows) {
    if (row.part && !out.some((r) => r.part === row.part)) {
      out.push({
        part: row.part,
        name: row.name || row.part,
        terms: (row.terms as FormulaTerm[]) || [],
        constant: row.constant ?? 0,
        factor: row.factor ?? 1,
        roundMode: row.roundMode ?? 'NONE',
        roundStep: row.roundStep ?? 1,
