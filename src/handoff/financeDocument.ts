...
export function renderFinanceHtml(d: FinanceDoc): string {
  ...
  const termCards = q.terms
    .map((t, i) => {
      ...
      return `<div style="flex:1;min-width:0;padding:14px 12px 13px;border-radius:10px;text-align:center;${
        lead
          ? `background:${DEEP};color:#fff;`
          : `background:${PAPER};border:1px solid ${RULE};color:${INK};`
      }">
        <div style="font-size:8pt;text-transform:uppercase;letter-spacing:.08em;${lead ? 'color:#b9c4cc;' : `color:${MUTED};`}">${t.termMonths} months</div>
        <div style="font-family:Georgia,serif;font-size:19pt;font-weight:700;margin:5px 0 1px;letter-spacing:-.01em;">${money(t.monthlyPaymentMinor)}</div>
        <div style="font-size:8pt;${lead ? 'color:#b9c4cc;' : `color:${MUTED};`}">per month</div>
      </div>`;
    })
    .join('');
  ...
  const detailRows = q.terms
    .map(
      (t) => `<tr>
      <td style="padding:7px 9px;border-bottom:1px solid ${RULE};font-size:9.5pt;">${t.termMonths} months</td>
      <td style="padding:7px 9px;border-bottom:1px solid ${RULE};font-size:9.5pt;text-align:right;font-weight:600;">${money(t.monthlyPaymentMinor)}</td>
      <td style="padding:7px 9px;border-bottom:1px solid ${RULE};font-size:9.5pt;text-align:right;">${money(t.totalOfPaymentsMinor)}</td>
      <td style="padding:7px 9px;border-bottom:1px solid ${RULE};font-size:9.5pt;text-align:right;color:${MUTED};">${money(t.financeChargeMinor)}</td>
    </tr>`,
    )
    .join('');
...
<body>
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:24px;padding-bottom:13px;border-bottom:2px solid ${INK};">
    <div>
      <div style="font-family:Georgia,serif;font-size:17pt;font-weight:700;letter-spacing:-.01em;">${esc(COMPANY.name)}</div>
      <div style="font-size:8.5pt;color:${MUTED};line-height:1.5;margin-top:3px;">
    ...
    <div style="text-align:right;">
      <div style="font-family:Georgia,serif;font-size:15pt;font-weight:700;">Financing Options</div>
      <div style="font-size:8.5pt;color:${MUTED};margin-top:3px;line-height:1.5;">
  ...
  <div style="margin:18px 0 6px;">
    ...
    <div style="display:flex;align-items:baseline;gap:12px;margin-top:2px;">
      <div style="font-family:Georgia,serif;font-size:26pt;font-weight:700;letter-spacing:-.015em;">${money(q.amountMinor)}</div>
      <div style="font-size:9pt;color:${MUTED};">${esc(d.proposalTitle)}</div>
    </div>
  </div>

  <div style="display:flex;gap:8px;margin:16px 0 8px;">${termCards}</div>
  <div style="font-size:8pt;color:${MUTED};">Estimated monthly payments. Final terms, rate and approval are set by Ryan Capital.</div>

  <div style="font-family:Georgia,serif;font-size:12pt;font-weight:700;margin:24px 0 8px;">Payment detail</div>
  <table style="width:100%;border-collapse:collapse;">
    <thead><tr>
      ${['Term', 'Monthly payment', 'Total of payments', 'Cost of financing']
    .map(
      (h, i) => `<th style="padding:7px 9px;text-align:${i ? 'right' : 'left'};font-size:8pt;text-transform:uppercase;letter-spacing:.05em;color:${MUTED};border-bottom:1.5px solid ${INK};font-weight:600;">${h}</th>`,
    )
    .join('')}
    </tr></thead>
  ...
  <div style="display:flex;gap:16px;margin-top:26px;align-items:stretch;">
    <div style="flex:1.15;background:${PAPER};border:1px solid ${RULE};border-radius:11px;padding:15px 16px;">
      <div style="font-family:Georgia,serif;font-size:12pt;font-weight:700;">Section 179 tax position</div>
      <div style="font-size:9pt;color:${SLATE};line-height:1.55;margin-top:5px;">${taxNote}</div>
      <table style="width:100%;border-collapse:collapse;margin-top:11px;">
        <tr>
          <td style="padding:5px 0;font-size:9.5pt;color:${SLATE};">Purchase price</td>
          <td style="padding:5px 0;font-size:9.5pt;text-align:right;font-weight:600;">${money(q.amountMinor)}</td>
        </tr>
        <tr>
          <td style="padding:5px 0;font-size:9.5pt;color:${SLATE};">Deductible this year</td>
          <td style="padding:5px 0;font-size:9.5pt;text-align:right;font-weight:600;">${money(s.deductionMinor)}</td>
        </tr>
        <tr>
          <td style="padding:5px 0;font-size:9.5pt;color:${SLATE};border-top:1px solid ${RULE};">Estimated tax saving at ${s.taxRatePct}%</td>
          <td style="padding:5px 0;font-size:9.5pt;text-align:right;font-weight:700;color:${GREEN};border-top:1px solid ${RULE};">− ${money(s.estimatedSavingsMinor)}</td>
        </tr>
        <tr>
          <td style="padding:7px 0 0;font-size:10.5pt;font-weight:700;border-top:1.5px solid ${INK};">Net cost after tax saving</td>
          <td style="padding:7px 0 0;font-size:12pt;text-align:right;font-family:Georgia,serif;font-weight:700;border-top:1.5px solid ${INK};">${money(s.netCostMinor)}</td>
        </tr>
      </table>
    </div>

    <div style="flex:.85;background:${DEEP};color:#fff;border-radius:11px;padding:15px 16px;">
      <div style="font-family:Georgia,serif;font-size:12pt;font-weight:700;">Ryan Capital</div>
      <div style="font-size:9pt;color:#c3ccd3;line-height:1.6;margin-top:6px;">
      ...
      <div style="margin-top:12px;padding-top:11px;border-top:1px solid #4a5865;font-size:9pt;line-height:1.6;">
        <div style="color:#8fa0ac;font-size:8pt;text-transform:uppercase;letter-spacing:.06em;">Contact</div>
        <div style="font-weight:600;margin-top:3px;">C. Kinsey</div>
        <div style="color:#c3ccd3;">ckinsey@ryancapital.com</div>
      </div>
      <div style="margin-top:12px;font-size:8pt;color:#8fa0ac;line-height:1.55;">
  ...
  <div style="margin-top:22px;padding-top:11px;border-top:1px solid ${RULE};font-size:7.5pt;color:${MUTED};line-height:1.6;">
