...
(function () {
  ...
  function tokens() {
    return { at: localStorage.getItem(AT), rt: localStorage.getItem(RT) };
  }
  function setTokens(at, rt) {
    if (at) localStorage.setItem(AT, at);
  ...
  function rtUnescapeTags(html) {
    ...
    return html
      .replace(open, function (m0, tag, slash) {
        return '<' + tag.toLowerCase() + (slash ? ' /' : '') + '>';
      })
      .replace(close, function (m0, tag) {
  ...
  function richTextField(id, label, value, hint) {
    var btn =
      'border:1px solid #dcded7;background:#fff;border-radius:6px;width:30px;height:28px;font-size:13px;cursor:pointer;color:#20241f;';
    return (
  ...
  function metaAmount(minor, override) {
    return Number(minor) || 0 || overrideMinor(override);
  }
  /**
   ...
   */
  function stdFreightOf(meta) {
    return meta && meta.stdFreightOn ? Number(meta.stdFreightMinor) || 0 : 0;
  }
  /**
  ...
  function discountLabel(t) {
    return t && t.discountMode === 'AMT'
      ? 'Discount'
      : 'Discount (' + (t ? t.discountPct : 0) + '%)';
  }
  ...
  function renderLogin(msg, isGood) {
    root.innerHTML =
      ...
      (msg
        ? isGood
          ? '<div style="background:#eef6f0;border:1px solid #cfe4d6;color:#2f6b4c;border-radius:10px;padding:10px 13px;font-size:13px;margin-bottom:14px;">' +
            esc(msg) +
            '</div>'
    ...
    document.getElementById('loginForm').addEventListener('submit', async function (e) {
      ...
      try {
        ...
        if (!r.ok) {
          renderLogin(
            r.status === 401 ? 'Invalid email or password.' : 'Sign-in failed (' + r.status + ').',
          );
          return;
        }
        var d = await r.json();
  ...
  function loginShell(inner) {
    return (
      '<div class="login-wrap"><form class="login-card" id="pwForm">' +
  ...
  function renderForgotPassword(prefill) {
    root.innerHTML = loginShell(
      '<h1>Reset your password</h1>' +
        ...
        '<div class="field"><label for="fpEmail">Email</label><input id="fpEmail" type="email" autocomplete="username" value="' +
        esc(prefill || '') +
        '" required></div>' +
    ...
    document.getElementById('pwForm').addEventListener('submit', async function (e) {
      ...
      root.innerHTML = loginShell(
        '<h1>Check your email</h1>' +
          '<div class="login-sub">If <b>' +
          esc(email) +
          '</b> has an account, a reset link is on its way. ' +
      ...
      document.getElementById('fpDone').addEventListener('click', function () {
        renderLogin();
      });
    });
  }
  ...
  async function renderResetPassword(token) {
    root.innerHTML = loginShell(
      '<h1>Reset your password</h1><div class="login-sub">Checking your link…</div>',
    );
    var state = 'UNKNOWN';
    try {
      var r = await api('/auth/reset-password?token=' + encodeURIComponent(token), {
        noAuth: true,
      });
    ...
    if (state !== 'VALID') {
      ...
      root.innerHTML = loginShell(
        '<h1>Link no longer works</h1>' +
          '<div class="login-sub">' +
          why +
          ' Reset links last 60 minutes and work once. Request a new one below.</div>' +
      ...
      document.getElementById('rpBack').addEventListener('click', function () {
        clearResetParam();
        renderLogin();
      });
      return;
    ...
    document.getElementById('pwForm').addEventListener('submit', async function (e) {
      ...
      var p1 = document.getElementById('rpPass').value,
        p2 = document.getElementById('rpPass2').value;
      if (p1.length < 12) {
        box.innerHTML = '<div class="err">Password must be at least 12 characters.</div>';
        return;
      }
      if (p1 !== p2) {
        box.innerHTML = '<div class="err">Those passwords do not match.</div>';
        return;
      }
      ...
      var msg = '';
      try {
        var r = await api('/auth/reset-password', {
          method: 'POST',
          noAuth: true,
          body: { token: token, newPassword: p1 },
        });
        if (r.ok) {
          clearResetParam();
      ...
      box.innerHTML =
        '<div class="err">' +
        esc(msg || 'Could not set the password. Request a new link.') +
        '</div>';
    });
  }
  ...
  function canCrmWrite(role) {
    return CRM_WRITE_ROLES.indexOf(role) !== -1;
  }
  ...
  var fxSettings = {
    ...
    legsMediumCount: 6,
    legsLargeCount: 8,
    matCostPerSqFt325: 11.78,
    matCostPerSqFt2: 7.65,
    matMarkupMultiplier: 1.4,
    matOverageIn: 14,
  };
  function loadFxSettings() {
    return authed('/formulas/settings')
      .then(function (r) {
        return r.ok ? r.json() : null;
      })
      .then(function (d) {
        if (d)
          Object.keys(d).forEach(function (k) {
            fxSettings[k] = Number(d[k]);
          });
      })
      .catch(function () {});
  }
  function depositPct() {
    return Number(fxSettings.depositPct) || 50;
  }
  function depositOf(total) {
    return Math.round(((Number(total) || 0) * depositPct()) / 100);
  }
  /**
  ...
  function optionalAmountRow(label, id, minor, tbdId, tbdVal) {
    ...
    return (
      '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:4px 0;font-size:14px;">' +
      '<span class="muted">' +
      label +
      '</span>' +
      ...
      '" value="' +
      esc(tbdVal || '') +
      '" placeholder="TBD" title="Prints on the customer proposal in place of TBD when the amount is 0" style="width:104px;' +
      box +
      'font-size:12.5px;color:#5c6157;">' +
      '<input id="' +
      id +
      '" value="' +
      m2d(minor) +
  ...
  function stdFreightRow() {
    ...
    return (
      ...
      '<input type="checkbox" id="mStdFreightOn"' +
      (on ? ' checked' : '') +
      ' style="width:15px;height:15px;accent-color:#3d4a55;cursor:pointer;flex:0 0 auto;">' +
      '<span class="muted">Standard Freight $</span>' +
      '</label>' +
      '<span style="display:flex;align-items:center;gap:6px;">' +
      '<input id="mStdFreight" value="' +
      (on ? m2d(pb.meta.stdFreightMinor || 0) : '') +
      '" placeholder="0.00"' +
      (on ? '' : ' disabled') +
      ' title="Enter the freight amount by hand" style="width:104px;' +
      box +
      'text-align:right;' +
      (on ? '' : 'background:#f4f5f1;color:#a0a49a;') +
      '">' +
      ...
      items
        ...
        .map(function (n) {
          return (
            ...
            esc(n.label) +
            '</span>' +
            (n.ready ? '' : '<span class="nav-tag">soon</span>') +
            '</button>'
          );
        })
        .join('') +
      ...
      items
        ...
        .map(function (n) {
          return (
            ...
            '<button class="nav-item" data-view="' +
            n.id +
            '"><span>' +
            esc(n.label) +
            '</span></button>' +
        ...
        .join('') +
      '</nav>' +
      '<div class="side-foot"><div class="user-row"><div class="avatar">' +
      esc(initials) +
      '</div>' +
      '<div class="user-meta"><b>' +
  ...
  function renderMockProposal(user) {
    ...
    document.getElementById('view').innerHTML =
      ...
      '</div>' +
      '</div>';
    document.getElementById('mkAdv').addEventListener('click', function () {
      startMock(user, 'adv');
    });
    document.getElementById('mkSoar').addEventListener('click', function () {
      startMock(user, 'soar');
    });
    document.getElementById('mkFlex').addEventListener('click', function () {
      startMock(user, 'flex');
    });
  }
  ...
  async function convertMockToProposal(user) {
    var orgs = [];
    try {
      var r = await authed('/crm/organizations?pageSize=200');
      if (r.ok) orgs = (await r.json()).items || [];
    } catch (e) {}
    if (!orgs.length) {
      alert('There are no customers to attach this to yet. Create one under CRM first.');
      return;
    }
    var lines = pb.lines.slice(),
      meta = pb.meta,
      title = pb.title;
    openModal(
      'Turn this into a real proposal',
      fieldRow(
        'Customer',
        '<select id="mkOrg" style="' +
          IN +
          '">' +
          orgs
            .map(function (o) {
        ...
        fieldRow(
          'Proposal title',
          '<input id="mkTitle" style="' +
            IN +
            '" value="' +
            esc(title || '') +
            '" placeholder="e.g. Adventure Series 10\' × 8\'">',
        ) +
      ...
      async function (close, showErr) {
        ...
        var cr = await authed('/proposals', {
          method: 'POST',
          body: { organizationId: document.getElementById('mkOrg').value, title: t },
        });
        if (!cr.ok)
          return showErr(
            await serverMessage(cr, 'Could not create the proposal (' + cr.status + ').'),
          );
        var created = await cr.json();
        var vid =
          created && created.versions && created.versions.length
            ? created.versions[0].id
            : created && created.versionId;
        if (!vid)
          return showErr(
            'The proposal was created but its first version could not be found — open it from Proposals and re-run the configurator.',
          );
        var payload = {
          title: t,
          sections: [
          ...
          items: lines.map(function (l, i) {
            return {
              ...
              source: l.source || '',
              freightTbd: !!l.freightTbd,
              quantity: Number(l.quantity) || 0,
              rateMinor: Number(l.rateMinor) || 0,
              costEach: Number(l.costEach) || 0,
              weightEach: Number(l.weightEach) || 0,
              group: l.group || '',
              optional: !!l.optional,
              delivery: l.delivery || '',
              returnable: l.returnable || '',
              addlFreight: l.addlFreight || '',
              freightCalc: l.freightCalc || '',
              tpFreightMinor: Number(l.tpFreightMinor) || 0,
              tpFreightLabel: l.tpFreightLabel || '',
              order: i,
            };
          }),
        ...
        if (!pr.ok)
          return showErr(
            'The proposal was created but its lines could not be saved (' +
              pr.status +
              '). Open it from Proposals.',
          );
        close();
        pb = null;
  ...
  async function renderDashboard(user) {
    ...
    document.getElementById('view').innerHTML =
      ...
      (canWrite
        ? '<button class="btn" id="dqNew" style="width:auto;padding:10px 16px;">New proposal</button>'
        : '') +
      (canCrmWrite(user.role)
        ? '<button class="link-btn" id="dqMonday" style="width:auto;padding:10px 15px;">Import a customer from monday</button>'
        : '') +
      ...
      '</div>' +
      '<div class="grid">' +
      '<div class="card"><div class="k">Signed in as</div><div class="v small">' +
      esc(user.name || user.email) +
      '</div><div class="muted" style="font-size:12.5px;margin-top:4px;">' +
      esc(user.email) +
      ' · ' +
      esc(roleLabel(user.role)) +
      '</div></div>' +
      ...
      '</div>';
    var nb = document.getElementById('dqNew');
    if (nb)
      nb.addEventListener('click', function () {
        openProposalForm(user);
      });
    var mb = document.getElementById('dqMonday');
    if (mb)
      mb.addEventListener('click', function () {
        openMondayLookup(user);
      });
    document.getElementById('dqReports').addEventListener('click', function () {
      activateNav('reports');
      renderReports(user);
    });
    document.getElementById('dqCatalog').addEventListener('click', function () {
      activateNav('catalog');
      renderCatalog(user);
    });
    try {
      ...
      if (el)
        el.innerHTML = r.ok
          ...
          : '<span class="dot bad"></span>Error ' + r.status;
    } catch (e) {
      var el2 = document.getElementById('apiStatus');
      if (el2) el2.innerHTML = '<span class="dot bad"></span>Offline';
    }
    loadDashboard(user);
  }
  ...
  async function loadDashboard(user) {
    ...
    try {
      var ro = await authed('/crm/organizations?pageSize=1');
      if (ro.ok) orgTotal = (await ro.json()).total;
    } catch (e2) {}
    ...
    var freightRows = [];
    try {
      var rfq = await authed('/proposals/freight-alerts');
      if (rfq.ok) freightRows = (await rfq.json()).alerts || [];
    } catch (e3) {}
    ...
    var followRows = [];
    try {
      var rfu = await authed('/crm/follow-ups');
      if (rfu.ok) followRows = (await rfu.json()).rows || [];
    } catch (e4) {}
    var kpis = document.getElementById('dashKpis');
    if (!kpis) return;
    if (!data) {
      kpis.innerHTML =
        '<div class="card"><div class="k">Proposals</div><div class="v small">Unavailable</div><div class="muted" style="font-size:12.5px;margin-top:4px;">Could not load reporting data.</div></div>';
      return;
    }
    var s = data.summary;
    var released = data.pipeline.filter(function (p) {
      return p.status === 'RELEASED';
    })[0] || { count: 0, value: 0 };
    var review = data.pipeline.filter(function (p) {
      return p.status === 'INTERNAL_REVIEW';
    })[0] || { count: 0, value: 0 };
    var stale = data.rows.filter(function (r) {
      return r.status === 'DRAFT' && r.daysOpen >= 14;
    });
    var attn =
      data.expiredOpen.length +
    ...
    kpis.innerHTML =
      kpi(
        ...
        fmt0(s.openValue) + ' in flight · avg ' + s.avgDaysOpen + ' days old',
        '#3d4a55',
      ) +
      kpi(
        'Out with customers',
        released.count.toLocaleString(),
        fmt0(released.value) + ' awaiting a decision',
      ) +
      kpi(
        'Accepted to date',
        fmt0(s.wonValue),
        s.won + ' proposals · ' + s.conversionRate + '% conversion',
        '#2f7d5d',
      ) +
      kpi(
        'Needs attention',
        attn.toLocaleString(),
        attn ? 'expiring, stalled or awaiting review' : 'nothing waiting on you',
        attn ? '#9c3327' : '#2f7d5d',
      );
    ...
    if (scale)
      scale.textContent =
        ...
        ' proposals · ' +
        (orgTotal == null ? data.byCustomer.length : orgTotal) +
        ' customers · ' +
    ...
    function attnGroup(label, rows, color, note) {
      ...
      return (
        '<div style="margin-bottom:10px;"><div style="display:flex;align-items:baseline;gap:8px;margin-bottom:5px;">' +
        ...
        rows.length +
        '</span>' +
        (note ? '<span class="muted" style="font-size:11.5px;">' + esc(note) + '</span>' : '') +
        '</div>' +
        ...
        rows
          ...
          .map(function (r, i) {
            return (
              ...
              (r.expiration ? 'expires ' + fmtDate(r.expiration) : r.daysOpen + ' days old') +
              '</div></div></div>'
            );
          })
          .join('') +
    ...
    function followUpGroup(rows) {
      ...
      return (
        '<div style="margin-bottom:10px;"><div style="display:flex;align-items:baseline;gap:8px;margin-bottom:5px;">' +
        '<span style="font-size:12px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:#8a6d1f;">Follow-Up Due · ' +
        rows.length +
        '</span>' +
        ...
        rows
          ...
          .map(function (r, i) {
            var win =
              r.decisionFrom || r.decisionTo
                ? 'decides ' +
                  (r.decisionFrom ? fmtDate(r.decisionFrom) : '?') +
                  ' – ' +
                  (r.decisionTo ? fmtDate(r.decisionTo) : '?')
                : 'no decision window recorded';
            return (
              '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:10px 14px;' +
              (i ? 'border-top:1px solid #f2f3ef;' : '') +
              '">' +
              '<div style="min-width:0;"><b style="font-weight:600;font-size:13.5px;">' +
              esc(r.customer) +
    ...
    var html =
      ...
      attnGroup(
        ...
        data.rows.filter(function (r) {
          return r.status === 'INTERNAL_REVIEW';
        }),
        '#3d4a55',
        '',
      ) +
    ...
    box.innerHTML =
      html ||
      '<div class="placeholder" style="padding:22px;"><p class="muted" style="margin:0;">Nothing needs attention — no expiring, stalled or unreviewed proposals.</p></div>';

    var recent = document.getElementById('dashRecent');
    recent.innerHTML = repTable(
      [['Customer'], ['Proposal'], ['Status'], ['Value', 'right'], ['Last modified']],
      data.rows
        .slice(0, 6)
        .map(function (r) {
          return (
            '<tr class="dashRow" data-id="' +
            r.id +
            ...
            rtd(
              statusChip(r.status) + (r.expired ? ' <span style="color:#9c3327;">⚑</span>' : ''),
            ) +
            rtd(fmt0(r.total), 'right', 1) +
            rtd(fmtDate(r.updatedAt)) +
            '</tr>'
          );
        })
        .join(''),
    ...
    document.querySelectorAll('.dashRow').forEach(function (el) {
      el.addEventListener('click', function () {
        activateNav('proposals');
        openProposalDetail(el.getAttribute('data-id'), user);
      });
    });
  ...
  function freightAlertGroup(rows) {
    ...
    return (
      '<div style="margin-bottom:10px;"><div style="display:flex;align-items:baseline;gap:8px;margin-bottom:5px;">' +
      '<span style="font-size:12px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:#9c3327;">Freight not requested \u00b7 ' +
      rows.length +
      '</span>' +
      ...
      rows
        ...
        .map(function (r, i) {
          ...
          return (
            ...
            r.version +
            '</div></div>' +
            '<div style="text-align:right;white-space:nowrap;font-size:12.5px;color:#9c3327;font-weight:600;">' +
            r.pendingCount +
            ' item' +
            (r.pendingCount === 1 ? '' : 's') +
            '<div class="muted" style="font-size:11.5px;font-weight:400;">' +
            esc(vend || 'freight outstanding') +
            (r.removedCount ? ' \u00b7 ' + r.removedCount + ' removed' : '') +
            '</div></div></div>'
          );
        })
        .join('') +
  ...
  async function openFreightReview(proposalId, user, versionId) {
    ...
    if (!r.ok) {
      view.innerHTML = '<div class="err">Could not load proposal.</div>';
      return;
    }
    var p = await r.json();
    ...
    var v =
      (versionId
        ? versions.filter(function (x) {
            return x.id === versionId;
          })[0]
        : null) || versions[versions.length - 1];
    if (!v) {
      view.innerHTML = '<div class="err">This proposal has no versions.</div>';
      return;
    }
    var orgName = '';
    try {
      var ro = await authed('/crm/organizations/' + p.organizationId);
      if (ro.ok) orgName = (await ro.json()).name || '';
    } catch (e) {}
    ...
    var cov = null;
    try {
      cov = await rfqApi('/proposals/versions/' + v.id + '/freight-coverage');
    } catch (e2) {}
    ...
    var headline = !cov
      ...
      : cov.needsRequest
        ? (cov.pendingCount ||
            cov.lines.filter(function (l) {
              return l.state === 'DRAFT';
            }).length) +
          ' item' +
          (cov.pendingCount === 1 ? '' : 's') +
          ' on this proposal have no freight quote requested.'
    ...
    view.innerHTML =
      ...
      '</div>' +
      '<div id="frFreight" style="' +
      (needs
        ? 'border:2px solid #c8483a;background:#fdf1ef;'
        : 'border:1px solid #cfe3d7;background:#f4faf6;') +
    ...
    document.getElementById('frBack').addEventListener('click', function () {
      renderProposals(user);
    });
    document.getElementById('frDoc').addEventListener('click', function () {
      previewProposal(p, v);
    });
    loadRfqPanel(true);
  }
  ...
  function freightReviewRow(l) {
    return (
      '<div style="display:flex;align-items:center;gap:11px;background:#fff;border:1px solid #e7e8e3;border-radius:10px;padding:9px 12px;">' +
      '<span class="bFreightMark" data-sku="' +
      esc(l.sku || '') +
      '" style="width:20px;display:flex;align-items:center;justify-content:center;">' +
      freightMarkHtml(l.sku) +
      '</span>' +
      ...
      '</div>' +
      '</div>' +
      '<div style="font-size:12.5px;color:#5c6157;white-space:nowrap;">' +
      (Number(l.quantity) || 0) +
      '\u00d7</div>' +
      freightStateChip(l.state) +
      '</div>'
    );
  }
  ...
  function freightRemovedRow(r) {
    return (
      '<div style="display:flex;align-items:center;gap:11px;background:#fdf7f6;border:1px dashed #f0ccc6;border-radius:10px;padding:9px 12px;">' +
      '<span style="width:20px;text-align:center;color:#c8483a;font-size:13px;">\u2715</span>' +
      '<div style="flex:1;min-width:0;">' +
      '<div style="font-size:13.5px;font-weight:600;color:#9c3327;text-decoration:line-through;text-decoration-color:#c8483a;">' +
      esc(r.name || r.sku) +
      '</div>' +
      '<div style="font-size:11.5px;color:#9c3327;">Removed from the proposal \u00b7 still listed on ' +
      esc(r.reference) +
      ' \u00b7 ' +
      esc(r.vendor) +
      '</div>' +
      '</div>' +
      '<div style="font-size:12.5px;color:#9c3327;white-space:nowrap;text-decoration:line-through;">' +
      (Number(r.quantity) || 0) +
      '\u00d7</div>' +
  ...
  function renderSoon(label) {
    document.getElementById('view').innerHTML =
      '<div class="placeholder"><h3>' +
      esc(label) +
      '</h3><p>This module is coming in a future slice. Its backend endpoints are already built and tested.</p></div>';
  }
  ...
  function renderCrm(user) {
    ...
    function tab(id, label) {
      ...
      return (
        '<button data-tab="' +
        id +
        '" style="border:none;border-radius:8px;padding:8px 15px;font-size:13.5px;font-weight:' +
        (on ? '600' : '500') +
        ...
        (on ? '#1c4039' : '#6b7065') +
        ';box-shadow:' +
        (on ? '0 1px 2px rgba(0,0,0,.06)' : 'none') +
        ';">' +
        label +
        '</button>'
      );
    ...
    document.getElementById('view').innerHTML =
      ...
      (crm.tab === 'orgs' ? 'organizations' : 'opportunities') +
      '…" value="' +
      esc(crm.q) +
      '" style="flex:1;max-width:340px;padding:10px 13px;border:1px solid #dcded7;border-radius:10px;font-size:14px;background:#fff;outline:none;">' +
      (writable
        ? '<button class="link-btn" id="crmMonday" style="width:auto;padding:10px 16px;">Find in monday</button>'
        : '') +
      (writable
        ? '<button class="btn" id="crmNew" style="width:auto;padding:10px 17px;">' +
          newLabel +
          '</button>'
        : '') +
      '</div>' +
    ...
    document.querySelectorAll('[data-tab]').forEach(function (b) {
      b.addEventListener('click', function () {
        crm.tab = b.getAttribute('data-tab');
        crm.q = '';
        crm.page = 1;
        renderCrm(user);
      });
    ...
    search.addEventListener('input', function () {
      ...
      t = setTimeout(function () {
        crm.q = search.value.trim();
        crm.page = 1;
        loadCrm();
      }, 300);
    });
    if (writable)
      document.getElementById('crmNew').addEventListener('click', function () {
        crm.tab === 'orgs' ? openOrgForm() : openOppForm();
      });
    if (writable)
      document.getElementById('crmMonday').addEventListener('click', function () {
        openMondayLookup(user);
      });
    loadCrm();
  }
  ...
  function openMondayLookup(user) {
    ...
    async function run() {
      ...
      if (q.length < 2) {
        box.innerHTML = '<span class="muted">Type at least 2 characters.</span>';
        return;
      }
      ...
      try {
        ...
        if (!r.ok) {
          box.innerHTML = '<div class="err">Search failed (' + r.status + ').</div>';
          return;
        }
        var rows = await r.json();
        if (!rows.length) {
          box.innerHTML = '<span class="muted">No matches in monday for “' + esc(q) + '”.</span>';
          return;
        }
        box.innerHTML = rows
          .map(function (x) {
            ...
            return (
              '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid #f2f3ef;">' +
              '<div><div style="font-weight:600;font-size:13.5px;">' +
              esc(x.name) +
              '</div>' +
              '<div class="muted" style="font-size:12px;">' +
              [x.industry, where, x.contact, x.projectId ? 'Project ' + x.projectId : '']
                .filter(Boolean)
                .map(esc)
                .join(' · ') +
              '</div></div>' +
              '<button class="link-btn mImp" data-id="' +
              esc(x.itemId) +
              '" style="width:auto;padding:6px 12px;white-space:nowrap;">Import</button>' +
              '</div>'
            );
          })
          .join('');
        box.querySelectorAll('.mImp').forEach(function (b) {
          b.addEventListener('click', async function () {
            ...
            try {
              var ir = await authed(
                '/integrations/monday/import/deal/' + b.getAttribute('data-id'),
                { method: 'POST' },
              );
  ...
  async function loadCrm() {
    var box = document.getElementById('crmList');
    if (!box) return;
    var path =
      (crm.tab === 'orgs' ? '/crm/organizations' : '/crm/opportunities') +
      '?page=' +
      crm.page +
      '&pageSize=20' +
      (crm.q ? '&q=' + encodeURIComponent(crm.q) : '');
    try {
  ...
  function shell(headCols, rows, d) {
    ...
    return (
      '<div style="background:#fbfbf9;border:1px solid #e7e8e3;border-radius:14px;overflow:hidden;">' +
      '<table style="width:100%;border-collapse:collapse;font-size:14px;"><thead><tr>' +
      headCols
        .map(function (h) {
          return (
            '<th style="text-align:left;padding:11px 16px;font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;font-weight:600;border-bottom:1px solid #eef0ea;background:#f7f8f4;">' +
            h +
            '</th>'
          );
        })
      ...
      '</tr></thead><tbody>' +
      (rows ||
        '<tr><td style="padding:22px 16px;color:#909689;" colspan="' +
          headCols.length +
      ...
      (d.total || 0) +
      ' total</span>' +
      '<span style="display:flex;gap:8px;align-items:center;"><button id="prevPg" ' +
      (crm.page <= 1 ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Prev</button>' +
      '<span>Page ' +
      (d.page || 1) +
      ' of ' +
      totalPages +
      '</span>' +
      '<button id="nextPg" ' +
      (crm.page >= totalPages ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Next</button></span>' +
      '</div>'
    );
  }
  ...
  function td(v) {
    return '<td style="padding:12px 16px;border-bottom:1px solid #f2f3ef;">' + v + '</td>';
  }
  ...
  function orgTable(d) {
    var rows = (d.items || [])
      .map(function (o) {
        return (
          '<tr>' +
          td('<b style="font-weight:600;">' + esc(o.name) + '</b>') +
          td(esc(titleCase(o.customerType))) +
          td(
            o.taxExempt ? '<span class="chip">Tax exempt</span>' : '<span class="muted">—</span>',
          ) +
          td(fmtDate(o.createdAt)) +
          '</tr>'
        );
      })
      .join('');
    ...
    var rows = (d.items || [])
      .map(function (o) {
        return (
          '<tr>' +
          td('<b style="font-weight:600;">' + esc(o.name) + '</b>') +
          td('<span class="chip">' + esc(titleCase(o.stage)) + '</span>') +
          td(esc(titleCase(o.fundingStatus))) +
          td(fmtMoney(o.budgetAmountMinor, o.budgetCurrency)) +
          td(esc(o.desiredTimeline || '—')) +
          '</tr>'
        );
      })
      .join('');
  ...
  function openModal(title, bodyHtml, onSubmit, submitLabel, opts) {
    ...
    ov.style.cssText =
      'position:fixed;inset:0;background:rgba(32,36,31,.34);display:flex;align-items:flex-start;justify-content:center;padding:48px 16px;z-index:50;overflow:auto;';
    ov.innerHTML =
      '<form id="mForm" style="width:100%;max-width:' +
      (opts.maxWidth || '460px') +
      ';background:#fbfbf9;border:1px solid #e7e8e3;border-radius:16px;box-shadow:0 24px 60px -20px rgba(32,36,31,.4);padding:24px 24px 22px;">' +
      ...
      (readOnly
        ? ''
        : '<button type="button" id="mCancel" class="link-btn" style="width:auto;padding:11px 18px;">Cancel</button>') +
      '<button type="submit" class="btn" id="mSave" style="flex:1;">' +
      (submitLabel || 'Create') +
      '</button></div></form>';
    ...
    document.getElementById('mForm').addEventListener('submit', async function (e) {
      ...
      var save = document.getElementById('mSave');
      save.disabled = true;
      save.textContent = 'Saving…';
      try {
        await onSubmit(close, function (msg) {
          document.getElementById('mErr').innerHTML = '<div class="err">' + esc(msg) + '</div>';
          save.disabled = false;
          save.textContent = submitLabel || 'Create';
        });
      } catch (err) {
        document.getElementById('mErr').innerHTML = '<div class="err">Something went wrong.</div>';
        save.disabled = false;
        save.textContent = submitLabel || 'Create';
      }
    });
  ...
  function renderKeepingTab(render, host, sel) {
    ...
    var was = document.activeElement,
      range = null;
    try {
      range = was && was.selectionStart != null ? [was.selectionStart, was.selectionEnd] : null;
    } catch (e) {}
    render();
    ...
    var same = null;
    try {
      same = sel ? root.querySelector(sel) : null;
    } catch (e2) {
      same = null;
    }
    if (!dir) {
      if (same) {
        same.focus();
        try {
          if (range) same.setSelectionRange(range[0], range[1]);
        } catch (e3) {}
      }
      return;
    }
  ...
  var IN =
    'width:100%;padding:10px 12px;border:1px solid #dcded7;border-radius:9px;font-size:14px;background:#fff;color:#20241f;outline:none;';
  function selectEl(id, opts, sel) {
    return (
      '<select id="' +
      id +
      '" style="' +
  ...
  function openOrgForm() {
    openModal(
      ...
      fieldRow('Name', '<input id="fName" style="' + IN + '" required>') +
        ...
        fieldRow(
          'Notes',
          '<textarea id="fNotes" rows="3" style="' + IN + 'resize:vertical;"></textarea>',
        ),
      async function (close, showErr) {
        ...
        var body = {
          name: name,
          customerType: document.getElementById('fType').value,
          taxExempt: document.getElementById('fTax').checked,
        ...
        if (r.status === 409) {
          ...
          if (
            confirm(
              'A similar organization may already exist (' +
                (dj.duplicates || [])
                  .map(function (x) {
                    return x.name;
                  })
  ...
  async function openOppForm() {
    var orgs = [];
    try {
      var r = await authed('/crm/organizations?pageSize=100');
      if (r.ok) orgs = (await r.json()).items || [];
    } catch (e) {}
    if (!orgs.length) {
      alert('Create an organization first — opportunities belong to one.');
      return;
    }
    var orgOpts = orgs
      .map(function (o) {
    ...
    openModal(
      ...
      fieldRow('Organization', '<select id="fOrg" style="' + IN + '">' + orgOpts + '</select>') +
        ...
        '<div style="display:flex;gap:10px;"><div class="field" style="flex:1;"><label>Budget (optional)</label><input id="fBudget" placeholder="0.00" style="' +
        IN +
        '"></div>' +
        '<div class="field" style="width:110px;"><label>Currency</label><input id="fCur" value="USD" maxlength="3" style="' +
        IN +
        'text-transform:uppercase;"></div></div>' +
        fieldRow(
          'Desired timeline',
          '<input id="fTimeline" placeholder="e.g. Q3 2026" style="' + IN + '">',
        ),
      async function (close, showErr) {
        var name = document.getElementById('fName').value.trim();
        if (name.length < 2) return showErr('Name must be at least 2 characters.');
        ...
        if (budget && !/^\d+(\.\d{1,2})?$/.test(budget))
          return showErr('Budget must be a number like 12500.00');
        var body = {
          organizationId: document.getElementById('fOrg').value,
          name: name,
          stage: document.getElementById('fStage').value,
          fundingStatus: document.getElementById('fFund').value,
          desiredTimeline: document.getElementById('fTimeline').value.trim() || undefined,
        };
        if (budget) {
          body.budgetAmount = budget;
          body.budgetCurrency = (
            document.getElementById('fCur').value.trim() || 'USD'
          ).toUpperCase();
        }
  ...
  function catOptionsTree(selectedId) {
    var rows = catCategories
      .map(function (c) {
        var p = catPath(c.id);
        return {
          id: c.id,
          depth: Math.max(0, p.length - 1),
          sortKey: p
            .map(function (x) {
  ...
  function renderCatalog(user) {
    ...
    document.getElementById('view').innerHTML =
      '<div style="display:flex;gap:5px;background:#eef0ea;padding:4px;border-radius:10px;width:max-content;margin-bottom:18px;">' +
      ctab('items', 'Catalog') +
      ctab('products', 'Product tree') +
      ctab('bundles', 'Bundles') +
      ctab('manufacturers', 'Manufacturers') +
      ctab('notes', 'Proposal notes') +
      '</div><div id="catBody"></div>';
    document.querySelectorAll('[data-ctab]').forEach(function (b) {
      b.addEventListener('click', function () {
        cat.tab = b.getAttribute('data-ctab');
        renderCatalog(user);
      });
    });
  ...
  function renderNotesTab() {
    ...
    document.getElementById('snNew').addEventListener('click', function () {
      openStandardNoteForm(null);
    });
  ...
  var itemState = {
    ...
    rows: [],
    filters: {},
    sort: { key: 'part', dir: 'asc' },
  };
  function renderItems(user) {
    var admin = canCatalogAdmin(user.role);
    document.getElementById('catBody').innerHTML =
      '<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;">' +
      '<input id="itSearch" placeholder="Search part #, name, category or manufacturer…" value="' +
      esc(itemState.q) +
      '" style="flex:1;min-width:240px;max-width:420px;padding:10px 13px;border:1px solid #dcded7;border-radius:10px;font-size:14px;background:#fff;outline:none;">' +
      (admin
        ? '<div style="margin-left:auto;display:flex;gap:8px;"><button class="link-btn" id="itImport" style="width:auto;padding:10px 15px;">Import Excel / CSV</button><button class="btn" id="itNew" style="width:auto;padding:10px 17px;">New product</button></div>'
        : '') +
      '</div>' +
    ...
    var s = document.getElementById('itSearch'),
      t;
    s.addEventListener('input', function () {
      clearTimeout(t);
      t = setTimeout(function () {
        itemState.q = s.value.trim();
        itemState.page = 1;
        loadItems(user);
      }, 300);
    });
    if (admin) {
      document.getElementById('itNew').addEventListener('click', function () {
        openSkuForm(user);
      });
      document.getElementById('itImport').addEventListener('click', function () {
        openSkuImport(user);
      });
    }
    loadItems(user);
  ...
  var FCELL =
    'width:100%;padding:5px 7px;border:1px solid #e2e5dd;border-radius:6px;font-size:12px;background:#fff;color:#3d4a55;outline:none;';
  ...
  function filterCell(cls, c, rows, filters) {
    ...
    if (c.type === 'enum') {
        ...
        enumOptions(c, rows)
          .map(function (o) {
            return (
              ...
              esc(o[0]) +
              '"' +
              (String(val) === String(o[0]) ? ' selected' : '') +
              '>' +
              esc(o[1]) +
              '</option>'
            );
      ...
      inner =
        ...
        esc(val) +
        '" placeholder="' +
        (c.type === 'num' ? '>0' : 'contains') +
        '" title="' +
        (c.type === 'num'
          ? 'Number, or an expression: >100, <=0, >=1.5'
  ...
  function colHead(cols, state, extraStyle) {
    return cols
      .map(function (c) {
        ...
        return (
          '<th' +
          (c.key ? ' data-sk="' + c.key + '" style="cursor:pointer;' : ' style="') +
          'text-align:' +
          (c.align || 'left') +
          ';padding:10px;font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:' +
          (on ? '#3d4a55' : '#8a8f85') +
          ';font-weight:600;border-bottom:1px solid #e7e8e3;white-space:nowrap;' +
          (extraStyle || '') +
          '">' +
          esc(c.label) +
          arrow +
  ...
  function wireColTable(box, cols, state, redraw) {
    box.querySelectorAll('th[data-sk]').forEach(function (th) {
      th.addEventListener('click', function () {
        ...
        if (state.sort && state.sort.key === k)
    ...
    box.querySelectorAll('.colFilter').forEach(function (el) {
      var apply = function () {
        state.filters[el.getAttribute('data-k')] = el.value;
        state.page = 1;
        redraw(el.getAttribute('data-k'));
      };
  ...
  var IT_COLS = [
    ...
    { key: 'weightLbs', label: 'Weight (lb)', w: 96, type: 'num', align: 'right' },
    {
      ...
      type: 'enum',
      options: [
        ['Product + priced', 'Product + priced'],
        ['Product only', 'Product only'],
        ['Priced only', 'Priced only'],
      ],
    },
  ...
  async function loadItems(user) {
    var box = document.getElementById('itList');
    if (!box) return;
    try {
      if (!itemState.manufacturers.length) {
        try {
          var rm = await authed('/catalog/manufacturers');
          if (rm.ok)
            itemState.manufacturers = ((await rm.json()) || []).map(function (m) {
              return m.name;
            });
        } catch (e0) {}
      }
      ...
      var d = await r.json();
      itemState.categories = (d.categories || []).map(function (c) {
        return c.name;
      });
      var all = (d.items || []).slice(),
  ...
  function drawItems(user, focusKey) {
    ...
    var rowsData = all.filter(function (r) {
      return passFilters(r, IT_COLS, itemState.filters);
    });
    ...
    var activeFilters = IT_COLS.filter(function (c) {
      return c.key && String(itemState.filters[c.key] || '').trim();
    }).length;

    var CELL =
      'width:100%;padding:6px 8px;border:1px solid #dcded7;border-radius:6px;font-size:13px;background:#fff;';
    var NUM = CELL + 'text-align:right;';
    function txt(part, field, value, style) {
      var v = value == null ? '' : String(value);
      return (
        '<input class="itEdit" data-part="' +
        esc(part) +
        '" data-f="' +
        field +
        '" value="' +
    ...
    var rows = pageRows
      .map(function (k) {
        var where =
          (k.productId ? '<span class="chip" style="font-size:10px;">Product</span>' : '') +
          (k.skuId
            ? ' <span class="chip" style="font-size:10px;background:#fdfcf7;">Priced</span>'
            : '');
        function cell(v, extra) {
          return (
            '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;vertical-align:middle;' +
            (extra || '') +
            '">' +
            v +
            '</td>'
        ...
        return (
          '<tr>' +
          cell(
            '<code style="font-size:12.5px;color:#4a4f47;white-space:nowrap;">' +
              esc(k.part) +
              '</code>',
          ) +
          cell(
            admin
              ? txt(k.part, 'name', k.name)
          ...
          cell(
            admin
              ? k.categoryOptions && itemState.categories.length
                ? sel(k.part, 'category', k.category, itemState.categories)
                : txt(k.part, 'category', k.category)
              : '<span title="' + esc(k.category) + '">' + esc(k.category) + '</span>',
          ) +
          cell(
            admin
          ...
          cell(
            admin
              ? txt(
                  k.part,
                  'unitCostMinor',
                  (Number(k.unitCostMinor) / 100).toFixed(2),
                  NUM + 'background:#fdfcf7;border-color:#e4dfd0;',
                )
              : '$' + (Number(k.unitCostMinor) / 100).toFixed(2),
            'text-align:right;',
          ) +
          cell(
            admin
              ? txt(k.part, 'unitPriceMinor', (Number(k.unitPriceMinor) / 100).toFixed(2), NUM)
              : '$' + (Number(k.unitPriceMinor) / 100).toFixed(2),
            'text-align:right;',
          ) +
          cell(
            '<span style="font-size:13px;font-weight:600;color:' +
              (k.margin >= 0 ? '#2f7d5d' : '#9c3327') +
              ';">' +
              k.margin +
              '%</span>',
            'text-align:right;',
          ...
          cell(
            admin ? txt(k.part, 'weightLbs', k.weightLbs, NUM) : String(k.weightLbs),
            'text-align:right;',
          ) +
          cell(where, 'white-space:nowrap;') +
          cell(
            '<span style="display:inline-block;background:' +
              (k.isActive ? '#eaf3ee' : '#f2f3ef') +
              ';border:1px solid ' +
              (k.isActive ? '#cfe3d7' : '#dcded7') +
              ';color:' +
              (k.isActive ? '#2f7d5d' : '#8a8f85') +
              ';border-radius:999px;padding:2px 9px;font-size:11.5px;font-weight:600;white-space:nowrap;">' +
              esc(k.statusLabel) +
              '</span>',
            'white-space:nowrap;',
          ) +
          ...
          cell(
            admin
              ? '<input type="number" min="0" class="itDefQty" data-part="' +
                  esc(k.part) +
                  '" value="' +
                  (k.defaultQty === '' || k.defaultQty == null ? '' : k.defaultQty) +
                  '" placeholder="—" title="Quantity the Adventure Series builder starts this part at. Blank for none." style="' +
                  NUM +
                  'text-align:center;">'
              : k.defaultQty === '' || k.defaultQty == null
                ? '<span style="color:#b6bab1;">—</span>'
                : '<span style="font-size:13px;font-weight:600;">' + k.defaultQty + '</span>',
            'text-align:center;',
          ) +
          ...
          cell(
            admin
              ? '<input class="itFreight" data-part="' +
                  esc(k.part) +
                  '" value="' +
                  (k.freightMinor === '' || k.freightMinor == null
                    ? ''
                    : (Number(k.freightMinor) / 100).toFixed(2)) +
                  '" placeholder="—" title="Freight added automatically when this part is put on a proposal. Blank for none." style="' +
                  NUM +
                  'text-align:right;">'
              : k.freightMinor === '' || k.freightMinor == null
                ? '<span style="color:#b6bab1;">—</span>'
                : '<span style="font-size:13px;font-weight:600;">' +
                  fmtMoney(Number(k.freightMinor), 'USD') +
                  '</span>',
            'text-align:right;',
          ) +
          ...
          cell(
            admin
              ? '<input type="checkbox" class="itFlag" data-part="' +
                  esc(k.part) +
                  '"' +
                  (k.overrideAllowed ? ' checked' : '') +
                  ' title="Allow reps to substitute this part number in the Adventure Series builder" style="width:16px;height:16px;cursor:pointer;">'
              : k.overrideAllowed
                ? '<span style="font-size:12px;color:#2f7d5d;font-weight:600;">Yes</span>'
                : '<span style="color:#b6bab1;">—</span>',
            'text-align:center;',
          ) +
          ...
          cell(
            admin
              ? '<input class="itUrl" data-part="' +
                  esc(k.part) +
                  '" value="' +
                  esc(k.productUrl || '') +
                  '" placeholder="—" title="Vendor order page. Shown as a Buy link on the Bill of Materials." style="width:100%;padding:5px 7px;border:1px solid #dcded7;border-radius:6px;font-size:12px;">'
              : k.productUrl
                ? '<a href="' +
                  esc(k.productUrl) +
                  '" target="_blank" rel="noopener" style="font-size:12px;">Buy ↗</a>'
                : '<span style="color:#b6bab1;">—</span>',
          ) +
          ...
          cell(
            admin
              ? '<input class="itBag" data-part="' +
                  esc(k.part) +
                  '" value="' +
                  esc(k.packagingBag || '') +
                  '" placeholder="—" title="Packaging bag this part ships in, e.g. Bag 7. Shown on the Bill of Materials when the bag column is turned on." style="width:100%;padding:5px 7px;border:1px solid #dcded7;border-radius:6px;font-size:12px;">'
              : k.packagingBag
          ...
          cell(
            admin
              ? '<input type="checkbox" class="itColor" data-part="' +
                  esc(k.part) +
                  '"' +
                  (k.requiresPowderColor ? ' checked' : '') +
                  ' title="Block BOM submission until this part has a powder colour" style="width:16px;height:16px;cursor:pointer;">'
              : k.requiresPowderColor
                ? '<span style="font-size:12px;color:#2f7d5d;font-weight:600;">Yes</span>'
                : '<span style="color:#b6bab1;">—</span>',
            'text-align:center;',
          ) +
          cell(
            admin
              ? '<div style="display:flex;gap:5px;justify-content:flex-end;">' +
                  '<button class="itToggle" data-part="' +
                  esc(k.part) +
                  '" data-to="' +
                  (k.isActive ? 'false' : 'true') +
                  '" title="' +
                  (k.isActive
                    ? 'Stop offering this part on new proposals'
                    : 'Offer this part again') +
                  '" style="border:1px solid #dcded7;background:#fff;border-radius:7px;padding:5px 9px;font-size:11.5px;color:#3d4a55;cursor:pointer;white-space:nowrap;">' +
                  (k.isActive ? 'Deactivate' : 'Activate') +
                  '</button>' +
                  '<button class="itDel" data-part="' +
                  esc(k.part) +
                  '" title="Delete this part" style="border:1px solid #e0e1db;background:#fff;border-radius:7px;padding:5px 8px;font-size:11.5px;color:#9c3327;cursor:pointer;">✕</button></div>'
              : '',
            'text-align:right;',
          ) +
          '</tr>'
    ...
    box.innerHTML =
      ...
      '<colgroup>' +
      IT_COLS.map(function (c) {
        return '<col' + (c.w ? ' style="width:' + c.w + 'px;"' : '') + '>';
      }).join('') +
      '</colgroup>' +
      '<thead><tr>' +
      colHead(IT_COLS, itemState) +
      '</tr>' +
      '<tr>' +
      IT_COLS.map(function (c) {
        return filterCell('colFilter', c, all, itemState.filters);
      }).join('') +
      '</tr></thead>' +
      '<tbody>' +
      (rows ||
        '<tr><td colspan="13" style="padding:28px;text-align:center;color:#8a8f85;">' +
          (all.length
            ? 'No parts match these filters.'
            : 'Nothing in the catalog yet. Import a sheet or add a product.') +
          '</td></tr>') +
      '</tbody></table></div>' +
      ...
      '<span>' +
      rowsData.length.toLocaleString() +
      (activeFilters ? ' of ' + all.length.toLocaleString() : '') +
      ' items' +
      (activeFilters
        ? ' · <button id="itClearF" class="link-btn" style="width:auto;padding:4px 10px;display:inline-block;">Clear filters</button>'
        : '') +
      '</span>' +
      '<span style="display:flex;gap:8px;align-items:center;"><button id="itPrev" ' +
      (itemState.page <= 1 ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Prev</button><span>Page ' +
      itemState.page +
      ' of ' +
      totalPages +
      '</span><button id="itNext" ' +
      (itemState.page >= totalPages ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Next</button></span></div>';

    var pv = document.getElementById('itPrev'),
      nx = document.getElementById('itNext');
    if (pv)
    ...
    wireColTable(box, IT_COLS, itemState, function (key) {
      drawItems(user, key);
    });
    if (focusKey) {
      var back = box.querySelector('.colFilter[data-k="' + focusKey + '"]');
      if (back && back.tagName === 'INPUT') {
        back.focus();
    ...
    box.querySelectorAll('.itEdit').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        var r2 = await authed('/catalog/items/' + encodeURIComponent(part), {
          method: 'PATCH',
          body: body,
        });
        ...
        if (!r2.ok) {
          var msg = '';
          try {
            msg = (await r2.json()).message || '';
          } catch (e3) {}
          alert('Could not save' + (msg ? ': ' + msg : ' (' + r2.status + ').'));
          return;
        }
        // Keep the in-memory row in step so filters and margin stay correct.
        var row = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        })[0];
        if (row) {
          row[f] = body[f];
          row.margin = row.unitPriceMinor
            ? Math.round(((row.unitPriceMinor - row.unitCostMinor) / row.unitPriceMinor) * 1000) /
              10
            : 0;
        }
        if (f === 'unitCostMinor' || f === 'unitPriceMinor') drawItems(user);
        setTimeout(function () {
          el.style.borderColor = f === 'unitCostMinor' ? '#e4dfd0' : '#dcded7';
        }, 900);
      });
    });
    box.querySelectorAll('.itFreight').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        el.style.borderColor = '#c9a227';
        var rf = await authed('/catalog/items/' + encodeURIComponent(part), {
          method: 'PATCH',
          body: { freightMinor: to },
        });
        el.style.borderColor = rf.ok ? '#3f9d78' : '#c2452f';
        if (!rf.ok) {
          var mf = '';
          try {
            mf = ((await rf.json()) || {}).message || '';
          } catch (ef) {}
          alert(
            mf ||
              'Could not save that freight amount (' +
                rf.status +
                '). If this says the column is missing, migration 0027 has not been deployed.',
          );
          return;
        }
        el.value = to == null ? '' : (to / 100).toFixed(2);
        var rowf = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        })[0];
        if (rowf) {
          rowf.freightMinor = to == null ? '' : to;
          rowf.freightDisplay = to == null ? '' : to / 100;
        }
        setTimeout(function () {
          el.style.borderColor = '#dcded7';
        }, 900);
      });
    });
    box.querySelectorAll('.itDefQty').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        el.style.borderColor = '#c9a227';
        var rq = await authed('/catalog/items/' + encodeURIComponent(part), {
          method: 'PATCH',
          body: { defaultQty: to },
        });
        ...
        if (!rq.ok) {
          var mq = '';
          try {
            mq = ((await rq.json()) || {}).message || '';
          } catch (eq) {}
          alert(
            mq ||
              'Could not save that default (' +
                rq.status +
                '). If this says the column is missing, migration 0025 has not been deployed.',
          );
          return;
        }
        el.value = to == null ? '' : String(to);
        var rowq = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        })[0];
        if (rowq) {
          rowq.defaultQty = to == null ? '' : to;
        }
        setTimeout(function () {
          el.style.borderColor = '#dcded7';
        }, 900);
    ...
    box.querySelectorAll('.itBag').forEach(function (el) {
      el.addEventListener('change', async function () {
        var part = el.getAttribute('data-part'),
          v = el.value.trim();
        el.style.borderColor = '#c9a227';
        var rb = await authed('/catalog/items/' + encodeURIComponent(part), {
          method: 'PATCH',
          body: { packagingBag: v || null },
        });
        el.style.borderColor = rb.ok ? '#3f9d78' : '#c2452f';
        if (!rb.ok) {
          var mb = '';
          try {
            mb = ((await rb.json()) || {}).message || '';
          } catch (eb) {}
          alert(
            mb ||
              'Could not save that bag number (' +
                rb.status +
                '). If this says the column is missing, migration 0033 has not been deployed.',
          );
          return;
        }
        var rowb = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        })[0];
    ...
    box.querySelectorAll('.itUrl').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        if (v && !/^https?:\/\//i.test(v)) {
          alert('A buy link must start with http:// or https://');
          el.focus();
          return;
        }
        el.style.borderColor = '#c9a227';
        var r = await authed('/catalog/items/' + encodeURIComponent(part), {
          method: 'PATCH',
          body: { productUrl: v || null },
        });
        ...
        if (!r.ok) {
          ...
          alert(m || 'Could not save the link (' + r.status + ').');
          return;
        }
        var row = (itemState.rows || []).filter(function (x) {
          return x.part === part;
    ...
    box.querySelectorAll('.itColor').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        if (!r.ok) {
          ...
          alert(m || 'Could not save (' + r.status + ').');
          return;
        }
        var row = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        ...
        if (row) {
          row.requiresPowderColor = to;
          row.colorLabel = to ? 'Yes' : 'No';
        }
      });
    });
    box.querySelectorAll('.itFlag').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        if (!r.ok) {
          ...
          var m0 = '';
          try {
            m0 = ((await r.json()) || {}).message || '';
          } catch (e2) {}
          alert(
            m0 ||
              'Could not save that change (' +
                r.status +
                '). If this says the column is missing, migration 0024 has not been deployed.',
          );
          return;
        }
        var row0 = (itemState.rows || []).filter(function (x) {
          return x.part === part;
        })[0];
        if (row0) {
          row0.overrideAllowed = to;
          row0.ovrLabel = to ? 'Yes' : 'No';
        }
      });
    });
    box.querySelectorAll('.itToggle').forEach(function (b) {
      b.addEventListener('click', async function () {
        var part = b.getAttribute('data-part'),
          to = b.getAttribute('data-to') === 'true';
        b.disabled = true;
        var r = await authed('/catalog/items/' + encodeURIComponent(part) + '/active', {
          method: 'POST',
          body: { active: to },
        });
        if (!r.ok) {
          var m1 = '';
          try {
            m1 = ((await r.json()) || {}).message || '';
          } catch (e1) {}
          alert(m1 || 'Could not change status (' + r.status + ').');
          b.disabled = false;
          return;
        }
        var row = (itemState.rows || []).filter(function (x) {
        ...
        if (row) {
          ...
          if (row.productStatus) {
            row.productStatus = to ? 'ACTIVE' : 'INACTIVE';
            row.statusLabel = to ? 'Active' : 'Inactive';
          } else {
            row.active = to;
            row.statusLabel = to ? 'Active' : 'Inactive';
          }
    ...
    box.querySelectorAll('.itDel').forEach(function (b) {
      b.addEventListener('click', function () {
        openItemDeleteForm(b.getAttribute('data-part'), user);
      });
    });
  }
  ...
  async function openItemDeleteForm(part, user) {
    var u = null;
    try {
      var r = await authed('/catalog/items/' + encodeURIComponent(part) + '/usage');
      if (r.ok) u = await r.json();
    } catch (e) {}
    if (!u) {
      alert('Could not check where this part is used.');
      return;
    }
    ...
    openModal(
      ...
      (safe
        ...
        : '<div style="background:#fbe9e6;border:1px solid #f0cdc7;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#9c3327;line-height:1.55;">' +
          esc(u.reason || 'This part cannot be deleted.') +
          '</div>') +
        '<div class="muted" style="font-size:12.5px;margin-top:10px;line-height:1.55;">' +
        (u.active === false
          ? 'This part is already inactive.'
          : 'Deactivating keeps every existing proposal exactly as priced and simply stops the part being offered on new ones.') +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-top:14px;">' +
        (u.active !== false
          ? '<button type="button" class="link-btn" id="idDeact" style="width:auto;padding:9px 15px;">Deactivate instead</button>'
          : '') +
        '</div>',
      safe
        ? async function (close, showErr) {
            var rr = await authed('/catalog/items/' + encodeURIComponent(part), {
              method: 'DELETE',
            });
            if (!rr.ok && rr.status !== 204) {
              var m = '';
              try {
                m = ((await rr.json()) || {}).message || '';
              } catch (e2) {}
              return showErr(m || 'Could not delete (' + rr.status + ').');
            }
            close();
            loadItems(user);
          }
    ...
    if (da)
      da.addEventListener('click', async function () {
        var rr = await authed('/catalog/items/' + encodeURIComponent(part) + '/active', {
          method: 'POST',
          body: { active: false },
        });
        if (!rr.ok) {
          alert('Could not deactivate (' + rr.status + ').');
          return;
        }
        var form = document.getElementById('mForm');
        if (form && form.parentNode && form.parentNode.parentNode)
  ...
  async function renderCatalogProducts(user) {
    ...
    document.getElementById('catStatus').addEventListener('change', function (e) {
      ...
      cat.status = e.target.value;
      cat.filters.status = cat.status;
      cat.page = 1;
      if ((cat.rows || []).length) drawProductTree(user);
      else loadProducts(user);
    });
    if (admin) {
      document.getElementById('catNew').addEventListener('click', function () {
        openProductForm(user);
      });
      document.getElementById('catCats').addEventListener('click', function () {
        openCategoryManager(user);
      });
      document.getElementById('catOrder').addEventListener('click', function () {
        openProductReorder(user);
      });
      ...
      document.getElementById('catSortAudit').addEventListener('click', function () {
        openSortAudit(user);
      });
      document.getElementById('catImport').addEventListener('click', function () {
        openTreeImport(user);
      });
    }
    loadProducts(user);
  ...
  function renderSkus(user) {
    ...
    document.getElementById('catBody').innerHTML =
      '<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;">' +
      '<input id="skuSearch" placeholder="Search part # or description…" value="' +
      esc(skuState.q) +
      '" style="flex:1;min-width:220px;max-width:360px;padding:10px 13px;border:1px solid #dcded7;border-radius:10px;font-size:14px;background:#fff;outline:none;">' +
      (admin
        ? '<div style="margin-left:auto;display:flex;gap:8px;"><button class="link-btn" id="skuExport" style="width:auto;padding:10px 15px;">Export Excel / CSV</button><button class="link-btn" id="skuImport" style="width:auto;padding:10px 15px;">Import Excel / CSV</button><button class="btn" id="skuNew" style="width:auto;padding:10px 17px;">New SKU</button></div>'
        : '') +
      '</div>' +
    ...
    var s = document.getElementById('skuSearch'),
      t;
    s.addEventListener('input', function () {
      clearTimeout(t);
      t = setTimeout(function () {
        skuState.q = s.value.trim();
        skuState.page = 1;
        loadSkus(user);
      }, 300);
    });
    if (admin) {
      document.getElementById('skuNew').addEventListener('click', function () {
        openSkuForm(user);
      });
      document.getElementById('skuExport').addEventListener('click', exportSkuMaster);
      document.getElementById('skuImport').addEventListener('click', function () {
        openSkuImport(user);
      });
    }
    loadSkus(user);
  ...
  function refreshCatalogList(user) {
    if (cat.tab === 'products') loadProducts(user);
    else {
      itemState.page = 1;
      loadItems(user);
    }
  ...
  async function loadSkus(user) {
    var box = document.getElementById('skuList');
    if (!box) return;
    var admin = canCatalogAdmin(user.role);
    try {
      var r = await authed(
        '/skus?page=' +
          skuState.page +
          '&pageSize=50' +
          (skuState.q ? '&q=' + encodeURIComponent(skuState.q) : ''),
      );
      if (!r.ok) {
        box.innerHTML = '<div class="err">Could not load (' + r.status + ').</div>';
        return;
      ...
      var rows = (d.items || [])
        .map(function (k) {
          var priceCell = admin
            ? '<input class="skuEdit" data-id="' +
              k.id +
              '" data-f="unitPriceMinor" value="' +
              (Number(k.unitPriceMinor) / 100).toFixed(2) +
              '" style="width:90px;padding:5px 7px;border:1px solid #dcded7;border-radius:6px;text-align:right;font-size:13px;">'
            : '$' + (Number(k.unitPriceMinor) / 100).toFixed(2);
          var costCell = admin
            ? '<input class="skuEdit" data-id="' +
              k.id +
              '" data-f="unitCostMinor" value="' +
              (Number(k.unitCostMinor || 0) / 100).toFixed(2) +
              '" style="width:90px;padding:5px 7px;border:1px solid #e4dfd0;background:#fdfcf7;border-radius:6px;text-align:right;font-size:13px;">'
            : '$' + (Number(k.unitCostMinor || 0) / 100).toFixed(2);
          var marginPct = Number(k.unitPriceMinor)
            ? Math.round(
                ((Number(k.unitPriceMinor) - Number(k.unitCostMinor || 0)) /
                  Number(k.unitPriceMinor)) *
                  1000,
              ) / 10
            : 0;
          var wtCell = admin
            ? '<input class="skuEdit" data-id="' +
              k.id +
              '" data-f="weightLbs" value="' +
              k.weightLbs +
              '" style="width:70px;padding:5px 7px;border:1px solid #dcded7;border-radius:6px;text-align:right;font-size:13px;">'
            : k.weightLbs;
          ...
          var ovrCell = admin
            ? '<input type="checkbox" class="skuFlag" data-id="' +
              k.id +
              '" data-f="overrideAllowed"' +
              (k.overrideAllowed ? ' checked' : '') +
              ' title="Allow reps to substitute this part number" style="width:15px;height:15px;cursor:pointer;">'
            : k.overrideAllowed
              ? '<span style="font-size:12px;color:#3f9d78;">Yes</span>'
              : '<span style="color:#b6bab1;">—</span>';
          return (
            '<tr>' +
            td('<code style="font-size:12.5px;color:#4a4f47;">' + esc(k.part) + '</code>') +
            td('<span style="font-size:13px;">' + esc(k.description) + '</span>') +
            td(esc(k.category)) +
            td(priceCell) +
            td(costCell) +
            td(
              '<span style="font-size:13px;color:' +
                (marginPct >= 0 ? '#2f7d5d' : '#9c3327') +
                ';font-weight:600;">' +
                marginPct +
                '%</span>',
            ) +
            ...
            td(
              admin
                ? '<button class="skuDel" data-id="' +
                    k.id +
                    '" style="border:1px solid #e0e1db;background:#fff;border-radius:7px;color:#9c3327;cursor:pointer;padding:4px 9px;font-size:12px;">Delete</button>'
                : '',
            ) +
            '</tr>'
          );
      ...
      box.innerHTML =
        tableShell(
          ...
          rows,
          9,
          'No SKUs yet. Import a sheet or add one.',
        ) +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;color:#82877d;font-size:13px;"><span>' +
        (d.total || 0) +
        ' SKUs</span>' +
        '<span style="display:flex;gap:8px;align-items:center;"><button id="skuPrev" ' +
        (skuState.page <= 1 ? 'disabled' : '') +
        ' class="link-btn" style="width:auto;padding:6px 12px;">Prev</button><span>Page ' +
        (d.page || 1) +
        ' of ' +
        totalPages +
        '</span><button id="skuNext" ' +
        (skuState.page >= totalPages ? 'disabled' : '') +
        ' class="link-btn" style="width:auto;padding:6px 12px;">Next</button></span></div>';
      var pv = document.getElementById('skuPrev'),
        nx = document.getElementById('skuNext');
      if (pv)
        pv.addEventListener('click', function () {
          if (skuState.page > 1) {
            skuState.page--;
      ...
      document.querySelectorAll('.skuEdit').forEach(function (el) {
        el.addEventListener('change', async function () {
          ...
          var body = {};
          body[f] =
            f === 'unitPriceMinor' || f === 'unitCostMinor'
              ? d2m(el.value)
              : parseFloat(el.value) || 0;
          el.style.borderColor = '#c9a227';
          var r2 = await authed('/skus/' + el.getAttribute('data-id'), {
            method: 'PATCH',
            body: body,
          });
          el.style.borderColor = r2.ok ? '#3f9d78' : '#c2452f';
      ...
      document.querySelectorAll('.skuFlag').forEach(function (el) {
        el.addEventListener('change', async function () {
          var body = {};
          body[el.getAttribute('data-f')] = el.checked;
          var r2 = await authed('/skus/' + el.getAttribute('data-id'), {
            method: 'PATCH',
            body: body,
          });
      ...
      document.querySelectorAll('.skuDel').forEach(function (b) {
        b.addEventListener('click', async function () {
          if (!confirm('Delete this SKU?')) return;
          await authed('/skus/' + b.getAttribute('data-id'), { method: 'DELETE' });
          loadSkus(user);
        });
      });
    } catch (e) {
  ...
  function openSkuForm(user) {
    openModal(
      ...
      fieldRow('Part #', '<input id="kPart" style="' + IN + '" required>') +
        ...
        '<div style="display:flex;gap:8px;"><div class="field" style="flex:1;"><label>Unit price ($)</label><input id="kPrice" value="0.00" style="' +
        IN +
        '"></div><div class="field" style="flex:1;"><label>Unit cost ($)</label><input id="kCost" value="0.00" style="' +
        IN +
        '"></div><div class="field" style="flex:1;"><label>Weight (lb)</label><input id="kWt" value="0" style="' +
        IN +
        '"></div></div>' +
      ...
      async function (close, showErr) {
        ...
        var body = {
          ...
          category: document.getElementById('kCat').value.trim() || 'OTHER',
          manufacturer: document.getElementById('kMfr').value.trim() || undefined,
          proposalGroup: document.getElementById('kGroup').value.trim() || undefined,
        };
        var r = await authed('/skus', { method: 'POST', body: body });
        if (!r.ok)
          return showErr(
            r.status === 400
              ? 'That part # may already exist.'
              : 'Could not create (' + r.status + ').',
          );
        close();
  ...
  async function exportSkuMaster() {
    ...
    if (!r.ok) {
      alert('Could not export the catalog (' + r.status + ').');
      return;
    }
    var d = await r.json();
    var cols = d.columns || [];
    var rows = [cols].concat(
  ...
  function openSkuImport(user) {
    ...
    openModal(
      ...
      async function (close, showErr) {
        var fi = document.getElementById('skuFile').files[0];
        ...
        if (!Object.prototype.hasOwnProperty.call(rows[0], 'part'))
          return showErr('The sheet needs a “part” column — it is how rows are matched.');
        var missingSel = document.getElementById('siMissing');
        var r = await authed('/skus/import', {
          method: 'POST',
          body: {
            rows: rows,
            dryRun: !skuImportConfirmed,
        ...
        if (d.issues && d.issues.length) {
          document.getElementById('siReview').innerHTML =
            ...
            '<b>' +
            d.issues.length +
            ' row(s) could not be read:</b><ul style="margin:6px 0 0;padding-left:18px;line-height:1.5;">' +
            d.issues
              .slice(0, 30)
              .map(function (i) {
                return '<li>Row ' + i.row + ': ' + esc(i.message) + '</li>';
              })
              .join('') +
            '</ul></div>';
          skuImportConfirmed = false;
        ...
        if (!skuImportConfirmed) {
          ...
          document.getElementById('siReview').innerHTML =
            ...
            (p.update || 0) +
            ' updated<br>' +
            'Columns this file will overwrite: <b>' +
            ((p.columns || []).join(', ') || 'none — part numbers only') +
            '</b>' +
            '</div>' +
            (p.missing && p.missing.length
              ? '<div style="margin-top:10px;background:#fdf6e3;border:1px solid #eadfbe;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#7a6320;line-height:1.55;">' +
                '<b>' +
                p.missing.length +
                ' active catalog part(s) are not in this file.</b>' +
                '<div style="max-height:120px;overflow:auto;margin:6px 0;">' +
                p.missing
                  .slice(0, 60)
                  .map(function (mm) {
                    return esc(mm.part + ' — ' + mm.name);
                  })
                  .join('<br>') +
                '</div>' +
  ...
  try {
    qboIncomeAccount = JSON.parse(localStorage.getItem('ssg.qboIncomeAccount') || 'null');
  } catch (e) {}
  ...
  async function loadProducts(user) {
    ...
    try {
      ...
      if (qboItemLinks === null) {
        await loadQboItemLinks();
        drawProductTree(user);
      }
    } catch (e) {
      box.innerHTML = '<div class="err">Could not reach the server.</div>';
  ...
  function drawProductTree(user, focusKey) {
    ...
    var all = cat.rows || [];
    var rowsData = all.filter(function (r) {
      return passFilters(r, PT_COLS, cat.filters);
    });
    ...
    var activeFilters = PT_COLS.filter(function (c) {
      return c.key && String(cat.filters[c.key] || '').trim();
    }).length;

    var kindOpts = PT_COLS[2];
    kindOpts.options = KINDS.map(function (k) {
      return [k, titleCase(k)];
    });
    var statusCol = PT_COLS[4];
    statusCol.options = STATUSES.map(function (st) {
      return [st, titleCase(st)];
    });
    ...
    var rows = pageRows
      .map(function (p) {
        var statusCell = admin
          ? '<select data-pid="' +
            p.id +
            '" class="rowStatus" style="padding:6px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;background:#fff;width:100%;">' +
            STATUSES.map(function (st) {
              return (
                '<option value="' +
                st +
                '"' +
                (p.status === st ? ' selected' : '') +
                '>' +
                titleCase(st) +
                '</option>'
              );
        ...
        return (
          '<tr>' +
          td('<code style="font-size:13px;color:#4a4f47;">' + esc(p.sku) + '</code>') +
          td(
            '<b style="font-weight:600;">' +
              esc(p.name) +
              '</b>' +
              (p.proposalDescription
                ? '<div class="muted" style="font-size:12px;max-width:420px;line-height:1.45;">' +
                  esc(String(p.proposalDescription).slice(0, 120)) +
                  (String(p.proposalDescription).length > 120 ? '…' : '') +
                  '</div>'
                : ''),
          ) +
          td(esc(titleCase(p.kind))) +
          td(
            '<span style="font-size:13px;">' +
              esc(p.categoryName || '—') +
              '</span>' +
              (p.categoryPath && p.categoryPath !== p.categoryName
          ...
          td(
            '<span style="font-variant-numeric:tabular-nums;font-size:13px;color:#5c6157;">' +
          ...
          td(
            admin
              ? '<button class="prodEdit" data-pid="' +
                  p.id +
                  '" style="border:1px solid #dcded7;background:#fff;border-radius:8px;padding:6px 12px;font-size:12.5px;color:#3d4a55;cursor:pointer;">Edit</button>'
              : '',
          ) +
          '</tr>'
        );
    ...
    box.innerHTML =
      ...
      '<colgroup>' +
      PT_COLS.map(function (c) {
        return '<col' + (c.w ? ' style="width:' + c.w + 'px;"' : '') + '>';
      }).join('') +
      '</colgroup>' +
      '<thead><tr>' +
      colHead(PT_COLS, cat, 'background:#f7f8f4;') +
      '</tr>' +
      '<tr>' +
      PT_COLS.map(function (c) {
        return filterCell('colFilter', c, all, cat.filters);
      }).join('') +
      '</tr></thead>' +
      '<tbody>' +
      (rows ||
        '<tr><td style="padding:22px 16px;color:#909689;" colspan="8">' +
          (all.length ? 'No products match these filters.' : 'No products yet.') +
          '</td></tr>') +
      '</tbody></table></div>' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;color:#82877d;font-size:13px;flex-wrap:wrap;gap:8px;">' +
      '<span>' +
      ...
      ' products' +
      (activeFilters
        ? ' · <button id="ptClearF" class="link-btn" style="width:auto;padding:4px 10px;display:inline-block;">Clear filters</button>'
        : '') +
      '</span>' +
      '<span style="display:flex;gap:8px;align-items:center;"><button id="cPrev" ' +
      (cat.page <= 1 ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Prev</button><span>Page ' +
      cat.page +
      ' of ' +
      totalPages +
      '</span><button id="cNext" ' +
      (cat.page >= totalPages ? 'disabled' : '') +
      ' class="link-btn" style="width:auto;padding:6px 12px;">Next</button></span></div>';

    var pv = document.getElementById('cPrev'),
      nx = document.getElementById('cNext');
    if (pv)
      pv.addEventListener('click', function () {
        if (cat.page > 1) {
          cat.page--;
    ...
    if (cf)
      cf.addEventListener('click', function () {
        ...
        cat.status = '';
        cat.page = 1;
        var sSel = document.getElementById('catStatus');
        if (sSel) sSel.value = '';
        drawProductTree(user);
      });
    wireColTable(box, PT_COLS, cat, function (key) {
      ...
      if (key === 'status') {
        cat.status = cat.filters.status || '';
        var sSel2 = document.getElementById('catStatus');
        if (sSel2) sSel2.value = cat.status;
      }
    ...
    Array.prototype.forEach.call(box.querySelectorAll('.rowStatus'), function (sel) {
      sel.addEventListener('change', async function () {
        var r2 = await authed('/catalog/products/' + sel.getAttribute('data-pid') + '/status', {
          method: 'PATCH',
          body: { status: sel.value, reason: 'changed from workspace' },
        });
        if (!r2.ok) {
          alert('Could not change status (' + r2.status + ').');
          loadProducts(user);
          return;
        }
        var row = (cat.rows || []).filter(function (x) {
          return x.id === sel.getAttribute('data-pid');
        })[0];
    ...
    Array.prototype.forEach.call(box.querySelectorAll('.qboSync'), function (b) {
      b.addEventListener('click', function () {
        var p = (cat.rows || []).filter(function (x) {
          return x.id === b.getAttribute('data-pid');
        })[0];
  ...
  function qboCell(p, admin) {
    ...
    if (link)
      return chip('#eef5f0', '#cfe3d6', '#2f7d5d', 'Linked', 'QuickBooks item ' + link.qboId);
    if (p.status !== 'ACTIVE')
      return chip(
        '#f2f3ef',
        '#e2e4dd',
        '#8a8f85',
        'Not active',
        'Only an active product can be synced to QuickBooks.',
      );
    if (!admin) return chip('#fdf6e3', '#eadfbe', '#8a6d1f', 'Not linked');
    return (
      '<button class="qboSync" data-pid="' +
      p.id +
      '" style="border:1px solid #3d4a55;background:#fff;border-radius:8px;padding:6px 11px;font-size:12.5px;color:#3d4a55;cursor:pointer;white-space:nowrap;">Sync to QBO</button>'
    );
  }
  ...
  async function openQboSyncForm(p, user) {
    ...
    try {
      ...
      if (ra.status === 409)
        err = 'QuickBooks is not connected. Connect it under Administration → Integrations first.';
      else if (!ra.ok) err = 'Could not read the account list from QuickBooks (' + ra.status + ').';
      else accounts = ((await ra.json()) || {}).accounts || [];
    } catch (e) {
      err = 'Could not reach QuickBooks.';
    }

    if (err) {
      openModal(
        'Sync ' + p.sku,
        '<div class="err" style="font-size:13px;line-height:1.6;">' + esc(err) + '</div>',
        async function (close) {
          close();
        },
        'Close',
    ...
    if (!accounts.length) {
      openModal(
        'Sync ' + p.sku,
        '<div class="err" style="font-size:13px;line-height:1.6;">This QuickBooks company has no active income accounts, so an item cannot be created.</div>',
        async function (close) {
          close();
        },
        'Close',
    ...
    var remembered =
      ...
      accounts.filter(function (x) {
        return x.id === qboIncomeAccount.id;
      })[0]
        ? qboIncomeAccount.id
        : accounts[0].id;
    openModal(
      'Sync ' + p.sku + ' to QuickBooks',
      '<div style="font-size:13.5px;line-height:1.6;">Creates a QuickBooks item for <b>' +
        esc(p.name) +
        '</b> and links it to this catalog record, so proposal lines carrying this part can reach an estimate or invoice.</div>' +
        '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-top:8px;">If an item with SKU <code>' +
        esc(p.sku) +
        '</code> already exists in QuickBooks, it is adopted rather than duplicated. ' +
        (p.kind === 'SERVICE'
          ? 'It will be created as a Service item.'
          : 'It will be created as a Non-inventory item — Summit does not track QuickBooks stock quantities.') +
        '</div>' +
      ...
      async function (close, showErr) {
        ...
        var r = await authed(
          '/integrations/quickbooks/items/' + encodeURIComponent(p.id) + '/sync',
          { method: 'POST', body: { incomeAccountRef: acctId } },
        );
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'The sync failed (' + r.status + ').');
        }
        var out = (await r.json()) || {};
        var chosen = accounts.filter(function (x) {
          return x.id === acctId;
        })[0];
        qboIncomeAccount = { id: acctId, name: chosen ? chosen.name : '' };
        try {
          localStorage.setItem('ssg.qboIncomeAccount', JSON.stringify(qboIncomeAccount));
        } catch (e) {}
  ...
  function openProductEditForm(p, user) {
    ...
    function tierPanel(categoryId) {
      ...
      var siblings = (cat.rows || [])
        .filter(function (x) {
          return x.categoryId === categoryId && x.id !== p.id;
        })
        .sort(function (a, b) {
      ...
      var crumbs = path.length
        ? path
            .map(function (node, i) {
              return (
                '<span style="' +
                (i === path.length - 1 ? 'font-weight:600;color:#20241f;' : 'color:#5c6157;') +
                '">' +
                esc(node.name) +
                '</span>'
              );
      ...
      return (
        '<div style="background:#f7f8f4;border:1px solid #eef0ea;border-radius:10px;padding:10px 12px;margin-bottom:12px;">' +
        ...
        crumbs +
        '</div>' +
        '<div class="muted" style="font-size:11.5px;margin-top:3px;">Tier ' +
        ((c && c.tierLevel) || path.length || '—') +
        (path.length > 1 ? ' · parent: ' + esc(path[path.length - 2].name) : ' · top level') +
        '</div>' +
        '<div style="margin-top:8px;font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;">Also in this category, in order (' +
        siblings.length +
        ')</div>' +
        (siblings.length
          ? '<div style="max-height:132px;overflow:auto;margin-top:4px;">' +
            siblings
              .slice(0, 40)
              .map(function (x) {
                return (
                  '<div style="display:flex;justify-content:space-between;gap:10px;font-size:12px;padding:2px 0;">' +
                  '<span style="color:#20241f;"><code style="color:#8a8f85;font-size:11px;font-variant-numeric:tabular-nums;">' +
                  (Number(x.sortOrder) || 0) +
                  '</code> ' +
                  esc(x.name) +
                  '</span>' +
                  '<code style="color:#7a7f75;font-size:11px;white-space:nowrap;">' +
                  esc(x.sku) +
                  '</code></div>'
                );
              })
              .join('') +
            (siblings.length > 40
              ? '<div class="muted" style="font-size:11.5px;">…and ' +
                (siblings.length - 40) +
                ' more</div>'
              : '') +
            '</div>'
    ...
    openModal(
      'Edit ' + p.sku,
      '<div id="ePTier">' +
        tierPanel(p.categoryId) +
        '</div>' +
        fieldRow(
          'SKU',
          '<input style="' + IN + 'background:#f2f3ef;" value="' + esc(p.sku) + '" disabled>',
        ) +
        fieldRow('Name', '<input id="ePName" style="' + IN + '" value="' + esc(p.name) + '">') +
        fieldRow(
          'Kind',
          '<select id="ePKind" style="' +
            IN +
            '">' +
        ...
        fieldRow(
          ...
          '<select id="ePCat" style="' + IN + '">' + catOpts + '</select>',
        ) +
        '<div class="field"><label>Sort order</label><input id="ePSort" type="number" min="0" step="1" style="' +
        IN +
        '" value="' +
        (Number(p.sortOrder) || 0) +
        '">' +
        ...
        '<div class="field"><label>Proposal description</label><textarea id="ePDesc" rows="3" style="' +
        IN +
        'resize:vertical;">' +
        esc(p.proposalDescription || '') +
        '</textarea>' +
        ...
        '<div class="field"><label>Internal description</label><textarea id="ePInt" rows="2" style="' +
        IN +
        'resize:vertical;">' +
        esc(p.internalDescription || '') +
        '</textarea></div>' +
        '<div style="display:flex;gap:8px;">' +
        '<div class="field" style="flex:1;"><label>Length (in)</label><input id="ePL" type="number" min="0" style="' +
        IN +
        '" value="' +
        num(p.lengthIn) +
        '"></div>' +
        '<div class="field" style="flex:1;"><label>Width (in)</label><input id="ePW" type="number" min="0" style="' +
        IN +
        '" value="' +
        num(p.widthIn) +
        '"></div>' +
        '<div class="field" style="flex:1;"><label>Height (in)</label><input id="ePH" type="number" min="0" style="' +
        IN +
        '" value="' +
        num(p.heightIn) +
        '"></div>' +
        '<div class="field" style="flex:1;"><label>Weight (oz)</label><input id="ePWt" type="number" min="0" style="' +
        IN +
        '" value="' +
        num(p.weightOz) +
        '"></div>' +
        '</div>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;"><input type="checkbox" id="ePShowDims"' +
        (p.showDimensions ? ' checked' : '') +
        '> Print dimensions on the proposal</label>' +
      ...
      async function (close, showErr) {
        ...
        if (sortRaw !== '') {
          ...
          if (!isFinite(sortNum) || sortNum < 0 || Math.floor(sortNum) !== sortNum)
            return showErr('Sort order must be a whole number, 0 or more.');
  ...
  function openProductForm(user) {
    ...
    openModal(
      ...
      fieldRow(
        'SKU',
        '<input id="pSku" placeholder="ABC-001" style="' +
          IN +
      ...
      async function (close, showErr) {
        ...
        if (!/^[A-Z0-9][A-Z0-9-]{2,39}$/.test(sku))
          return showErr('SKU must be 3–40 chars: letters, numbers, hyphens.');
  ...
  async function renderManufacturers(user) {
    ...
    document.getElementById('catBody').innerHTML =
      '<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;">' +
      '<input id="mfSearch" placeholder="Search name, city or contact…" value="' +
      esc(mfrState.q) +
      '" style="flex:1;min-width:240px;max-width:380px;padding:10px 13px;border:1px solid #dcded7;border-radius:10px;font-size:14px;background:#fff;outline:none;">' +
      '<label style="display:flex;gap:7px;align-items:center;font-size:13px;color:#5c6157;cursor:pointer;"><input type="checkbox" id="mfInactive"' +
      (mfrState.showInactive ? ' checked' : '') +
      '> Show inactive</label>' +
      (admin
        ? '<div style="margin-left:auto;"><button class="btn" id="mfNew" style="width:auto;padding:10px 17px;">New manufacturer</button></div>'
        : '') +
      '</div>' +
    ...
    s.addEventListener('input', function () {
      clearTimeout(t);
      t = setTimeout(function () {
        mfrState.q = s.value.trim();
        drawManufacturers(user);
      }, 250);
    });
    document.getElementById('mfInactive').addEventListener('change', function (e) {
      mfrState.showInactive = e.target.checked;
      loadManufacturers(user);
    });
    if (admin)
      document.getElementById('mfNew').addEventListener('click', function () {
        openManufacturerForm(null, user);
      });
    loadManufacturers(user);
  }
  ...
  async function loadManufacturers(user) {
    ...
    try {
      var r = await authed(
        '/manufacturers' + (mfrState.showInactive ? '?includeInactive=true' : ''),
      );
      if (!r.ok) {
        box.innerHTML =
          '<div class="err">Could not load manufacturers (' +
          r.status +
          '). Run the 0022 migration if this persists.</div>';
        return;
      }
  ...
  function drawManufacturers(user) {
    ...
    var rows = (mfrState.rows || []).filter(function (m) {
      return (
        !q ||
        (m.name + ' ' + (m.city || '') + ' ' + (m.contactName || '') + ' ' + (m.contactEmail || ''))
          .toLowerCase()
          .indexOf(q) !== -1
      );
    });
        ...
        return (
          ...
          td(
            '<b style="font-weight:600;">' +
              ...
              '</b>' +
              (m.isSteelFabricator
                ? ' <span class="chip" style="font-size:10.5px;background:#eef0ea;">Steel</span>'
                : '') +
              (m.isActive === false
                ? ' <span class="chip" style="font-size:10.5px;background:#f2f3ef;color:#8a8f85;">Inactive</span>'
                : '') +
              (m.accountNumber
                ? '<div class="muted" style="font-size:11.5px;">Acct ' +
                  esc(m.accountNumber) +
                  '</div>'
                : ''),
          ) +
          ...
          td(
            m.addressLine1
              ? '<span style="font-size:13px;">' +
                  esc(m.addressLine1) +
                  '</span><div class="muted" style="font-size:11.5px;">' +
                  esc(mfrCityLine(m)) +
                  '</div>'
              : '<span class="muted">—</span>',
          ) +
          ...
          td(
            admin
              ? '<div style="display:flex;gap:6px;justify-content:flex-end;">' +
                  '<button class="mfEdit link-btn" data-id="' +
                  m.id +
                  '" style="width:auto;padding:6px 12px;">Edit</button>' +
                  '<button class="mfDel link-btn" data-id="' +
                  m.id +
                  '" style="width:auto;padding:6px 10px;color:#9c3327;">Remove</button></div>'
              : '',
          ) +
          '</tr>'
        );
    ...
    box.innerHTML = tableShell(
      ['Manufacturer', 'Primary contact', 'Address', 'Terms', 'Lead time', 'Parts', ''],
      body,
      7,
      mfrState.rows.length
        ? 'No manufacturers match that search.'
        : 'No manufacturers yet. Add the vendors you buy from.',
    );
    box.querySelectorAll('.mfEdit').forEach(function (b) {
      b.addEventListener('click', function () {
        openManufacturerForm(
          (mfrState.rows || []).filter(function (x) {
    ...
    box.querySelectorAll('.mfDel').forEach(function (b) {
      b.addEventListener('click', function () {
        openManufacturerDelete(b.getAttribute('data-id'), user);
      });
    });
  }
  ...
  function openManufacturerForm(m, user) {
    ...
    function derivedAbbrev(name) {
      var words = String(name || '')
        ...
        .split(/\s+/)
        .filter(function (w) {
          return w && !/^(the|and|of|inc|llc|co|company|corp|ltd)$/i.test(w);
        });
    ...
    openModal(
      m.id ? 'Edit ' + m.name : 'New manufacturer',
      fieldRow(
        'Manufacturer name',
        '<input id="mfName" style="' + IN + '" value="' + v('name') + '" required>',
      ) +
        fieldRow(
          'Freight RFQ code',
          '<input id="mfRfqAbbrev" maxlength="8" style="' +
            IN +
            'text-transform:uppercase;" value="' +
            v('rfqAbbrev') +
            '" placeholder="' +
            esc(derivedAbbrev(m.name) || 'SE') +
            '">' +
            '<div class="muted" style="font-size:12px;margin-top:5px;line-height:1.5;">Appended to this vendor\'s freight requests so several on one project are told apart — <b>RFQ-12414494509-' +
            esc(derivedAbbrev(m.name) || 'SE') +
            '</b>, where the middle number is the monday Project ID. Leave it blank to use the initials shown.</div>',
        ) +
        ...
        two(
          fieldRow(
            'Name',
            '<input id="mfCName" style="' + IN + '" value="' + v('contactName') + '">',
          ),
          fieldRow(
            'Title',
            '<input id="mfCTitle" style="' + IN + '" value="' + v('contactTitle') + '">',
          ),
        ) +
        two(
          fieldRow(
            'Email',
            '<input id="mfCEmail" type="email" style="' +
              IN +
              '" value="' +
              v('contactEmail') +
              '">',
          ...
          fieldRow(
            'Phone',
            '<input id="mfCPhone" style="' + IN + '" value="' + v('contactPhone') + '">',
          ),
        ) +
        ...
        two(
          fieldRow(
            'Name',
            '<input id="mfAName" style="' + IN + '" value="' + v('altContactName') + '">',
          ),
          fieldRow(
            'Phone',
            '<input id="mfAPhone" style="' + IN + '" value="' + v('altContactPhone') + '">',
          ),
        ) +
        fieldRow(
          'Email',
          '<input id="mfAEmail" type="email" style="' +
            IN +
            '" value="' +
            v('altContactEmail') +
            '">',
        ...
        fieldRow(
          'Street',
          '<input id="mfAddr1" style="' + IN + '" value="' + v('addressLine1') + '">',
        ) +
        fieldRow(
          'Suite / unit',
          '<input id="mfAddr2" placeholder="Suite 100" style="' +
            IN +
            '" value="' +
            v('addressLine2') +
            '">',
        ...
        '<div style="display:flex;gap:8px;">' +
        '<div style="flex:2;">' +
        fieldRow('City', '<input id="mfCity" style="' + IN + '" value="' + v('city') + '">') +
        '</div>' +
        '<div style="flex:1;">' +
        fieldRow('State', '<input id="mfRegion" style="' + IN + '" value="' + v('region') + '">') +
        '</div>' +
        '<div style="flex:1;">' +
        fieldRow('ZIP', '<input id="mfZip" style="' + IN + '" value="' + v('postalCode') + '">') +
        '</div>' +
        '</div>' +
        two(
          fieldRow(
            'Country',
            '<input id="mfCountry" style="' +
              IN +
              '" value="' +
              esc(m.country == null ? 'USA' : m.country) +
              '">',
          ),
          fieldRow(
            'Website',
            '<input id="mfWeb" placeholder="https://" style="' +
              IN +
              '" value="' +
              v('website') +
              '">',
        ...
        fieldRow(
          'Our account #',
          '<input id="mfAcct" style="' + IN + '" value="' + v('accountNumber') + '">',
        ) +
        '</div>' +
        '<div style="flex:1;">' +
        fieldRow(
          'Payment terms',
          '<input id="mfTerms" placeholder="e.g. Net 30" style="' +
            IN +
            '" value="' +
            v('paymentTerms') +
            '">',
        ...
        fieldRow(
          'Lead time (days)',
          '<input id="mfLead" type="number" min="0" style="' +
            IN +
            '" value="' +
            (m.defaultLeadTimeDays == null ? '' : m.defaultLeadTimeDays) +
            '">',
        ) +
        '</div>' +
        '</div>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;margin-top:4px;"><input type="checkbox" id="mfSteel"' +
        (m.isSteelFabricator ? ' checked' : '') +
        '> Steel fabricator — their lines count toward total steel weight on a BOM</label>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;margin-top:6px;"><input type="checkbox" id="mfThird"' +
        (m.id ? (m.isThirdParty ? ' checked' : '') : ' checked') +
        '> Third-party vendor (unchecked = made in-house)</label>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;margin-top:6px;"><input type="checkbox" id="mfActive"' +
        (m.id ? (m.isActive !== false ? ' checked' : '') : ' checked') +
        '> Active — offered when assigning a vendor</label>' +
        '<label style="display:flex;align-items:flex-start;gap:8px;font-size:14px;cursor:pointer;margin-top:6px;"><input type="checkbox" id="mfFreightTbd" style="margin-top:3px;"' +
        (m.freightTbd ? ' checked' : '') +
        '><span>Freight quoted after approval<span class="muted" style="display:block;font-size:11.5px;line-height:1.5;">Every part from this vendor carries the standing “freight not yet determined” note on its proposal line, until a freight amount is entered on that line.</span></span></label>' +
        '<div class="field" style="margin-top:10px;"><label>Notes</label><textarea id="mfNotes" rows="2" style="' +
        IN +
        'resize:vertical;">' +
        esc(m.notes || '') +
        '</textarea></div>' +
        ...
        '<label style="display:flex;align-items:flex-start;gap:8px;font-size:14px;cursor:pointer;"><input type="checkbox" id="mfRfq" style="margin-top:3px;"' +
        (m.rfqEnabled ? ' checked' : '') +
        '><span>Can receive freight quote requests<span class="muted" style="display:block;font-size:11.5px;line-height:1.5;">Offered as an RFQ recipient on any proposal carrying their parts. Vendors who do not quote freight stay off the list.</span></span></label>' +
        ...
        two(
          fieldRow(
            'Freight contact',
            '<input id="mfRfqName" style="' + IN + '" value="' + v('rfqContactName') + '">',
          ),
          fieldRow(
            'Phone',
            '<input id="mfRfqPhone" style="' + IN + '" value="' + v('rfqContactPhone') + '">',
          ),
        ) +
        two(
          fieldRow(
            'Send RFQs to',
            '<input id="mfRfqTo" type="email" placeholder="Falls back to the contact email" style="' +
              IN +
              '" value="' +
              v('rfqEmailTo') +
              '">',
          ...
          fieldRow(
            'Cc',
            '<input id="mfRfqCc" placeholder="Optional" style="' +
              IN +
              '" value="' +
              v('rfqEmailCc') +
              '">',
        ...
        fieldRow(
          ...
          '<input id="mfRfqEmail" type="email" placeholder="Named contact, if different from the send-to address" style="' +
            IN +
            '" value="' +
            v('rfqContactEmail') +
            '">',
        ...
        fieldRow(
          'Subject',
          '<input id="mfRfqSubject" placeholder="Freight quote request {{reference}} — {{customer}}" style="' +
            IN +
            '" value="' +
            v('rfqEmailSubject') +
            '">',
        ) +
        '<div class="field"><label>Default message</label><textarea id="mfRfqBody" rows="4" placeholder="Left blank, a standard covering note is used." style="' +
        IN +
        'resize:vertical;">' +
        esc(m.rfqEmailBody || '') +
        '</textarea></div>' +
        ...
        two(
          fieldRow(
            'Send BOMs to',
            '<input id="mfBomTo" type="email" placeholder="Falls back to the contact email" style="' +
              IN +
              '" value="' +
              v('bomEmailTo') +
              '">',
          ...
          fieldRow(
            'Cc',
            '<input id="mfBomCc" placeholder="Optional" style="' +
              IN +
              '" value="' +
              v('bomEmailCc') +
              '">',
        ...
        fieldRow(
          'Subject',
          '<input id="mfBomSubject" placeholder="{{customer}}-{{order}}-{{vendor}}" style="' +
            IN +
            '" value="' +
            v('bomEmailSubject') +
            '">',
            ...
            ['PDF', 'EXCEL', 'BOTH']
              .map(function (f) {
                return (
                  ...
                  f +
                  '"' +
                  ((m.bomEmailFormat || 'PDF') === f ? ' selected' : '') +
                  '>' +
                  (f === 'BOTH' ? 'Both' : f === 'EXCEL' ? 'Excel' : 'PDF') +
                  '</option>'
                );
              })
              .join('') +
        ...
        '<div class="field"><label>Default message</label><textarea id="mfBomBody" rows="5" placeholder="Left blank, a standard covering note is used." style="' +
        IN +
        'resize:vertical;">' +
        esc(m.bomEmailBody || '') +
        '</textarea></div>',
      async function (close, showErr) {
        ...
        if (!r.ok) {
          ...
          return showErr(msg || 'Could not save (' + r.status + ').');
        }
        close();
        loadManufacturers(user);
      },
  ...
  async function openManufacturerDelete(id, user) {
    var u = null;
    try {
      var r = await authed('/manufacturers/' + id + '/usage');
      if (r.ok) u = await r.json();
    } catch (e) {}
    if (!u) {
      alert('Could not check where this vendor is used.');
      return;
    }
    openModal(
      'Remove ' + u.name,
      ...
      u.deletable
        ? async function (close, showErr) {
            ...
            if (!rr.ok && rr.status !== 204) {
              var msg = '';
              try {
                msg = ((await rr.json()) || {}).message || '';
              } catch (e) {}
              return showErr(msg || 'Could not delete (' + rr.status + ').');
            }
            close();
            loadManufacturers(user);
          }
    ...
    if (da)
      da.addEventListener('click', async function () {
        var rr = await authed('/manufacturers/' + id, {
          method: 'PATCH',
          body: { isActive: false },
        });
        if (!rr.ok) {
  ...
  async function renderBundles(user) {
    ...
    document.getElementById('catBody').innerHTML =
      '<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;">' +
      '<input id="bnSearch" placeholder="Search bundles…" value="' +
      esc(bundleState.q) +
      '" style="flex:1;min-width:220px;max-width:340px;padding:10px 13px;border:1px solid #dcded7;border-radius:10px;font-size:14px;background:#fff;outline:none;">' +
      (admin
        ? '<div style="margin-left:auto;"><button class="btn" id="bnNew" style="width:auto;padding:10px 17px;">New bundle</button></div>'
        : '') +
      '</div>' +
    ...
    var s = document.getElementById('bnSearch'),
      t;
    s.addEventListener('input', function () {
      clearTimeout(t);
      t = setTimeout(function () {
        bundleState.q = s.value.trim();
        drawBundles(user);
      }, 250);
    });
    if (admin)
      document.getElementById('bnNew').addEventListener('click', function () {
        openBundleForm(user);
      });
    loadBundles(user);
  }
  ...
  async function loadBundles(user) {
    var box = document.getElementById('bnList');
    if (!box) return;
    try {
      var r = await authed('/catalog/bundles');
      if (!r.ok) {
        box.innerHTML = '<div class="err">Could not load bundles (' + r.status + ').</div>';
        return;
      }
  ...
  function drawBundles(user) {
    ...
    var rows = (bundleState.rows || []).filter(function (b) {
      return !q || (b.name + ' ' + b.sku).toLowerCase().indexOf(q) !== -1;
    });
    if (!rows.length) {
      box.innerHTML =
        '<div class="placeholder" style="padding:22px;"><p class="muted" style="margin:0;">' +
        (bundleState.rows.length
          ? 'No bundles match that search.'
          : 'No bundles yet. Create one, then add the parts it contains.') +
        '</p></div>';
      return;
    }
    box.innerHTML = rows
      .map(function (b) {
        ...
        var inner = comp.length
          ? '<table style="width:100%;border-collapse:collapse;font-size:13px;">' +
            '<thead><tr>' +
            ['Part #', 'Component', 'Qty', 'Unit price', 'Extended']
              .map(function (h, i) {
                return (
                  '<th style="text-align:' +
                  (i > 1 ? 'right' : 'left') +
                  ';padding:7px 10px;font-size:10px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;border-bottom:1px solid #eef0ea;">' +
                  h +
                  '</th>'
                );
              })
            ...
            comp
              .map(function (c) {
                return (
                  '<tr>' +
                  '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;"><code style="font-size:12px;color:#4a4f47;">' +
                  esc(c.sku) +
                  '</code></td>' +
                  '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;">' +
                  esc(c.name) +
                  '</td>' +
                  '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;text-align:right;">' +
                  c.quantity +
                  '</td>' +
                  '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;text-align:right;">$' +
                  (Number(c.unitPriceMinor) / 100).toFixed(2) +
                  '</td>' +
                  '<td style="padding:7px 10px;border-bottom:1px solid #f2f3ef;text-align:right;">$' +
                  (Number(c.extendedPriceMinor) / 100).toFixed(2) +
                  '</td></tr>'
                );
              })
              .join('') +
        ...
        return (
          '<div style="background:#fbfbf9;border:1px solid #e7e8e3;border-radius:14px;padding:14px 16px;margin-bottom:14px;">' +
          '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">' +
          '<div><div style="font-weight:600;font-size:15px;">' +
          esc(b.name) +
          ' <code style="font-size:12px;color:#7a7f75;font-weight:400;">' +
          esc(b.sku) +
          '</code></div>' +
          ...
          ' component' +
          (comp.length === 1 ? '' : 's') +
          ' · rolls up to <b style="color:#20241f;">$' +
          (Number(b.unitPriceMinor) / 100).toFixed(2) +
          '</b>' +
          ' · cost $' +
          (Number(b.unitCostMinor) / 100).toFixed(2) +
          ' · ' +
          b.weightLbs +
          ' lb' +
          (b.missingPrice && b.missingPrice.length
            ? ' · <span style="color:#9c3327;">' +
              b.missingPrice.length +
              ' component(s) have no price</span>'
            : '') +
          '</div></div>' +
          (admin
            ? '<div style="display:flex;gap:6px;">' +
              '<button class="bnEdit link-btn" data-id="' +
              b.id +
              '" style="width:auto;padding:7px 13px;">Edit contents</button>' +
              '<button class="bnDel link-btn" data-id="' +
              b.id +
              '" style="width:auto;padding:7px 11px;color:#9c3327;">Delete</button></div>'
            : '') +
          '</div>' +
    ...
    box.querySelectorAll('.bnEdit').forEach(function (bt) {
      bt.addEventListener('click', function () {
        openBundleComponents(
          (bundleState.rows || []).filter(function (x) {
            return x.id === bt.getAttribute('data-id');
          })[0],
          user,
        );
      });
    ...
    box.querySelectorAll('.bnDel').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        ...
        var r = await authed('/catalog/bundles/' + bt.getAttribute('data-id'), {
          method: 'DELETE',
        });
        if (!r.ok && r.status !== 204) {
          ...
          alert(m || 'Could not delete (' + r.status + ').');
          return;
        }
        loadBundles(user);
      });
  ...
  async function openBundleForm(user) {
    ...
    if (!catCategories.length) {
      alert('Create a category first — a bundle is filed like any other product.');
      return;
    }
    openModal(
      'New bundle',
      fieldRow(
        'Part #',
        '<input id="bnSku" placeholder="e.g. SSG-STARTER-BUNDLE" style="' +
          IN +
          'text-transform:uppercase;" required>',
      ) +
        ...
        fieldRow(
          'Category',
          '<select id="bnCat" style="' + IN + '">' + catOptionsTree('') + '</select>',
        ) +
        fieldRow(
          'Proposal description',
          '<textarea id="bnDesc" rows="3" style="' + IN + 'resize:vertical;"></textarea>',
        ) +
      ...
      async function (close, showErr) {
        ...
        var r = await authed('/catalog/bundles', {
          ...
          body: {
            sku: sku,
            name: name,
            categoryId: document.getElementById('bnCat').value,
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'Could not create (' + r.status + ').');
        }
        var created = await r.json();
        close();
        await loadBundles(user);
        openBundleComponents(
          { id: created.id, sku: created.sku, name: created.name, components: [] },
          user,
        );
      },
      'Create bundle',
  ...
  async function openBundleComponents(bundle, user) {
    if (!bundle) return;
    var picked = (bundle.components || []).map(function (c) {
      return {
    ...
    function pickedHtml() {
      if (!picked.length)
        return '<div class="muted" style="padding:12px;font-size:13px;">Nothing added yet — search below.</div>';
      var total = picked.reduce(function (a, c) {
        return a + (Number(c.unitPriceMinor) || 0) * (Number(c.quantity) || 0);
      }, 0);
      return (
        picked
          .map(function (c, i) {
            return (
              '<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-bottom:1px solid #f2f3ef;">' +
              '<div style="flex:1;font-size:13px;">' +
              esc(c.name) +
              ' <code style="font-size:11.5px;color:#7a7f75;">' +
              esc(c.sku) +
              '</code></div>' +
              '<input class="bcQty" data-i="' +
              i +
              '" type="number" min="1" value="' +
              (c.quantity || 1) +
              '" style="width:64px;padding:5px 7px;border:1px solid #dcded7;border-radius:6px;text-align:right;font-size:13px;">' +
              '<div style="width:88px;text-align:right;font-size:12.5px;color:#5c6157;">$' +
              (((Number(c.unitPriceMinor) || 0) * (Number(c.quantity) || 0)) / 100).toFixed(2) +
              '</div>' +
              '<button type="button" class="bcRm" data-i="' +
              i +
              '" style="border:1px solid #e0e1db;background:#fff;border-radius:7px;color:#9c3327;cursor:pointer;padding:4px 9px;font-size:12px;">✕</button>' +
              '</div>'
            );
          })
          .join('') +
        '<div style="display:flex;justify-content:flex-end;gap:10px;padding:9px 10px;font-size:13px;font-weight:600;">Bundle price $' +
        (total / 100).toFixed(2) +
        '</div>'
      );
    }
    function searchHtml(list) {
      return (
        list
          ...
          .map(function (p) {
            return (
              '<button type="button" class="bcAdd" data-id="' +
              p.id +
              '" style="display:block;width:100%;text-align:left;border:none;border-bottom:1px solid #f2f3ef;background:#fff;padding:8px 11px;cursor:pointer;font-size:13px;">' +
              esc(p.name) +
              ' <code style="font-size:11.5px;color:#7a7f75;">' +
              esc(p.sku) +
              '</code></button>'
          ...
          .join('') ||
        '<div class="muted" style="padding:12px;font-size:13px;">No parts match.</div>'
      );
    }

    openModal(
      'Contents of ' + bundle.name,
      '<div style="border:1px solid #e7e8e3;border-radius:10px;overflow:hidden;margin-bottom:12px;"><div id="bcPicked">' +
        pickedHtml() +
        '</div></div>' +
        '<input id="bcSearch" placeholder="Search a part to add…" style="' +
        IN +
        'margin-bottom:8px;">' +
        '<div id="bcResults" style="max-height:220px;overflow:auto;border:1px solid #e7e8e3;border-radius:10px;">' +
        searchHtml(products) +
        '</div>',
      async function (close, showErr) {
        var r2 = await authed('/catalog/bundles/' + bundle.id + '/components', {
          method: 'PUT',
          body: {
            components: picked.map(function (c) {
              return { productId: c.productId, quantity: Number(c.quantity) || 1 };
            }),
          },
        });
        if (!r2.ok) {
          var m = '';
          try {
            m = ((await r2.json()) || {}).message || '';
          } catch (e) {}
          return showErr(m || 'Could not save (' + r2.status + ').');
        }
        close();
        loadBundles(user);
      },
    ...
    function repaint() {
      var host = document.getElementById('bcPicked');
      if (!host) return;
      host.innerHTML = pickedHtml();
      host.querySelectorAll('.bcQty').forEach(function (el) {
        el.addEventListener('change', function () {
          picked[Number(el.getAttribute('data-i'))].quantity = Math.max(
            1,
      ...
      host.querySelectorAll('.bcRm').forEach(function (el) {
        el.addEventListener('click', function () {
          picked.splice(Number(el.getAttribute('data-i')), 1);
          repaint();
        });
      });
    }
    function wireResults(list) {
      var box = document.getElementById('bcResults');
      if (!box) return;
      box.querySelectorAll('.bcAdd').forEach(function (b) {
        b.addEventListener('click', async function () {
          ...
          try {
            ...
            if (rp.ok) {
              var hit = (((await rp.json()) || {}).items || []).filter(function (x) {
                return x.part === p.sku;
              })[0];
  ...
  async function openCategoryManager(user) {
    ...
    var list = (catCategories || []).slice().sort(function (a, b) {
      return (
        (a.tierLevel || 1) - (b.tierLevel || 1) ||
        (a.sortOrder || 0) - (b.sortOrder || 0) ||
        a.name.localeCompare(b.name)
      );
    });
    var counts = {};
    (cat.rows || []).forEach(function (p) {
    ...
    function rowHtml(c, i) {
      return (
        ...
        '<button type="button" class="cmUp" data-i="' +
        i +
        '" title="Move up" style="border:1px solid #dcded7;background:#fff;border-radius:5px;cursor:pointer;font-size:10px;line-height:1;padding:2px 5px;">▲</button>' +
        '<button type="button" class="cmDown" data-i="' +
        i +
        '" title="Move down" style="border:1px solid #dcded7;background:#fff;border-radius:5px;cursor:pointer;font-size:10px;line-height:1;padding:2px 5px;">▼</button>' +
        '</div>' +
        '<input class="cmName" data-id="' +
        c.id +
        '" value="' +
        esc(c.name) +
        '" style="flex:1;padding:6px 8px;border:1px solid #dcded7;border-radius:6px;font-size:13px;">' +
        '<select class="cmTier" data-id="' +
        c.id +
        '" style="padding:6px 7px;border:1px solid #dcded7;border-radius:6px;font-size:12.5px;background:#fff;">' +
        [1, 2, 3, 4]
          .map(function (t) {
            return (
              '<option value="' +
          ...
          .join('') +
        '</select>' +
        '<span class="muted" style="font-size:11.5px;width:74px;text-align:right;">' +
        (counts[c.id] || 0) +
        ' part' +
        ((counts[c.id] || 0) === 1 ? '' : 's') +
        '</span>' +
        '<label style="display:flex;gap:5px;align-items:center;font-size:11.5px;color:#5c6157;"><input type="checkbox" class="cmActive" data-id="' +
        c.id +
        '"' +
        (c.isActive === false ? '' : ' checked') +
        '> shown</label>' +
        '<button type="button" class="cmDel" data-id="' +
        c.id +
        '" style="border:1px solid #e0e1db;background:#fff;border-radius:7px;color:#9c3327;cursor:pointer;padding:4px 8px;font-size:12px;">✕</button>' +
        '</div>'
      );
    }
    openModal(
      ...
      '<div class="muted" style="font-size:12.5px;margin-bottom:10px;line-height:1.55;">Renaming a category never moves a product — the name is only a label. The arrows set the order categories appear in; the tier is the level it sits at in the tree.</div>' +
        '<div id="cmList" style="border:1px solid #e7e8e3;border-radius:10px;max-height:380px;overflow:auto;">' +
      ...
      async function (close, showErr) {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (!nameEl) continue;
          var body = {};
          if (nameEl.value.trim() && nameEl.value.trim() !== c.name)
            body.name = nameEl.value.trim();
          if (tierEl && Number(tierEl.value) !== (c.tierLevel || 1))
            body.tierLevel = Number(tierEl.value);
          ...
          if (!r.ok) {
            ...
            return showErr(m || 'Could not save “' + c.name + '” (' + r.status + ').');
          }
        }
        var rr = await authed('/catalog/categories/reorder', {
          method: 'POST',
          body: {
            ids: list.map(function (c2) {
              return c2.id;
            }),
          },
        });
        if (!rr.ok)
          return showErr('Saved the names, but could not save the order (' + rr.status + ').');
        close();
        try {
          var rc2 = await authed('/catalog/categories');
          catCategories = rc2.ok ? await rc2.json() : catCategories;
        } catch (e3) {}
        loadProducts(user);
      },
      'Save categories',
    ...
    function repaint() {
      var host = document.getElementById('cmList');
      if (!host) return;
    ...
    function wire() {
      ...
      host.querySelectorAll('.cmDel').forEach(function (b) {
        b.addEventListener('click', async function () {
          ...
          catCategories = (catCategories || []).filter(function (c) {
            return c.id !== id;
          });
          repaint();
        });
    ...
    setTimeout(function () {
      ...
      if (add)
        add.addEventListener('click', async function () {
          ...
          var r = await authed('/catalog/categories', {
            method: 'POST',
            body: {
              name: name.trim(),
              slug: slugify(name),
          ...
          if (!r.ok) {
            alert('Could not create (' + r.status + ').');
            return;
          }
          var created = await r.json();
          list.push(created);
          catCategories.push(created);
          repaint();
        });
    }, 50);
  }
  ...
  function openProductReorder(user) {
    ...
    var tierOpts =
      ...
      (catCategories || [])
        ...
        .map(function (c) {
          return {
            id: c.id,
            label: catPathLabel(c.id),
            n: list.filter(function (p) {
        ...
        .map(function (o) {
          return '<option value="' + o.id + '">' + esc(o.label) + ' · ' + o.n + '</option>';
        })
        .join('');
    ...
    function rowsHtml() {
      var v = view();
      if (!v.length)
        return '<div class="muted" style="padding:16px;font-size:13px;">No products in this tier.</div>';
      return v
        .map(function (idx, n) {
          var p = list[idx];
          return (
            '<div style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-bottom:1px solid #f2f3ef;">' +
            '<span class="muted" style="width:26px;font-size:11.5px;text-align:right;">' +
            (n + 1) +
            '</span>' +
            '<code style="width:52px;font-size:11px;color:#8a8f85;text-align:right;font-variant-numeric:tabular-nums;">' +
            (Number(p.sortOrder) || 0) +
            '</code>' +
            '<div style="flex:1;font-size:13px;">' +
            esc(p.name) +
            ...
            (pick
              ...
              : '<div class="muted" style="font-size:11px;line-height:1.4;">' +
                ...
                '</div>') +
            '</div>' +
            '<button type="button" class="prUp" data-n="' +
            n +
            '"' +
            (n === 0 ? ' disabled' : '') +
            ' style="border:1px solid #dcded7;background:#fff;border-radius:5px;cursor:pointer;font-size:10px;padding:3px 6px;">▲</button>' +
            '<button type="button" class="prDown" data-n="' +
            n +
            '"' +
            (n === v.length - 1 ? ' disabled' : '') +
            ' style="border:1px solid #dcded7;background:#fff;border-radius:5px;cursor:pointer;font-size:10px;padding:3px 6px;">▼</button>' +
            '</div>'
          );
        })
    ...
    openModal(
      ...
      async function (close, showErr) {
        var r = await authed('/catalog/products/reorder', {
          method: 'POST',
          body: {
    ...
    function repaint() {
      var host = document.getElementById('prList');
      if (!host) return;
      host.innerHTML = rowsHtml();
      host.querySelectorAll('.prUp').forEach(function (b) {
        b.addEventListener('click', function () {
          var v = view(),
            n = Number(b.getAttribute('data-n'));
          if (n > 0) {
            swap(v[n - 1], v[n]);
            repaint();
          }
  ...
  var TREE_SHEETS = [
    {
      name: 'Categories',
      key: 'categories',
      cols: ['slug', 'name', 'parentSlug', 'tierLevel', 'sortOrder', 'isActive'],
    },
    {
      name: 'Products',
      key: 'products',
      cols: ['sku', 'name', 'categorySlug', 'kind', 'status', 'sortOrder', 'proposalDescription'],
    },
    {
      name: 'Bundles',
      key: 'bundles',
      cols: ['bundleSku', 'bundleName', 'componentSku', 'componentName', 'quantity'],
    },
  ];
  function xmlEsc(v) {
  ...
  function treeWorkbookXml(data) {
    var sheets = TREE_SHEETS.map(function (sh) {
            ...
            sh.cols
              .map(function (c) {
                ...
                return (
                  ...
                  (num ? 'Number' : 'String') +
                  '">' +
                  xmlEsc(num ? v : v === true ? 'true' : v === false ? 'false' : v) +
                  '</Data></Cell>'
                );
              })
              .join('') +
      ...
      return (
        ...
        sh.cols
          .map(function (c) {
            return '<Cell><Data ss:Type="String">' + c + '</Data></Cell>';
          })
          .join('') +
        '</Row>' +
        rows +
  ...
  function parseWorkbookXml(text) {
    ...
    for (var i = 0; i < sheets.length; i++) {
      ...
      var def = TREE_SHEETS.filter(function (s) {
      ...
      for (var r = 0; r < rows.length; r++) {
        var cells = rows[r].getElementsByTagName('Cell'),
          vals = [];
        ...
        headers.forEach(function (h, idx) {
          if (h) o[h] = vals[idx] == null ? '' : String(vals[idx]).trim();
        });
        data.push(o);
      }
      out[def.key] = data;
  ...
  function streetLine(l1, l2) {
    ...
    return (
      x +
      ', ' +
      (/^(ste|suite|apt|apartment|unit|#|bldg|building|fl|floor|rm|room|dept|po box|p\.o\.)/i.test(
        y,
      )
        ? y
        : 'Suite ' + y)
  ...
  async function openSortAudit(user) {
    ...
    if (!r.ok) {
      alert('Could not audit sort order (' + r.status + ').');
      return;
    }
    var d = await r.json();
    function block(title, list) {
      ...
      return (
        '<div style="margin-bottom:14px;"><div style="font-size:12px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;margin-bottom:6px;">' +
        title +
        '</div>' +
        list
          .map(function (c) {
            return (
              '<div style="border:1px solid #e7e8e3;border-radius:9px;padding:9px 11px;margin-bottom:6px;">' +
              '<div style="font-size:12.5px;color:#5c6157;">' +
              esc(c.scope) +
              ' · order <b>' +
              c.sortOrder +
              '</b></div>' +
              c.members
                .map(function (m) {
                  return '<div style="font-size:13px;">' + esc(m.label) + '</div>';
                })
                .join('') +
              '</div>'
            );
    ...
    var body =
      d.clashCount === 0
        ...
        : '<div style="font-size:13.5px;margin-bottom:12px;"><b>' +
          d.clashCount +
          '</b> collision' +
          (d.clashCount === 1 ? '' : 's') +
          ' across <b>' +
          d.affected +
          '</b> rows. Ties break alphabetically, so these print in an arbitrary order.</div>' +
          block('Categories', d.categoryClashes) +
          block('Products', d.productClashes) +
  ...
  async function exportCategoryProductList() {
    ...
    if (!r.ok) {
      alert('Could not export the product list (' + r.status + ').');
      return;
    }
    var d = await r.json();
    ...
    function pathOf(slug) {
      var parts = [],
        seen = {},
        cur = bySlug[slug];
      while (cur && !seen[cur.slug]) {
        seen[cur.slug] = 1;
        parts.unshift(cur.name || cur.slug);
        cur = cur.parentSlug ? bySlug[cur.parentSlug] : null;
      }
      return parts.join(' \u203a ');
    }
    ...
    if (rs.ok) {
      ...
      (sd.items || []).forEach(function (s) {
        priced[String(s.part).trim().toUpperCase()] = s;
      });
    }
  ...
  async function exportProductTree() {
    var r = await authed('/catalog/tree/export');
    if (!r.ok) {
      alert('Could not export the tree (' + r.status + ').');
      return;
    }
    var data = await r.json();
    downloadText(
      'product-tree-' + todayISO() + '.xls',
      treeWorkbookXml(data),
      ...
      async function (close, showErr) {
        ...
        var payload = {
          ...
          bundles: (parsed.bundles || [])
            ...
            .filter(function (b) {
              return b.bundleSku && b.componentSku;
            }),
        };
        ...
        if (d.issues && d.issues.length) {
          document.getElementById('tiReview').innerHTML =
            ...
            '<b>' +
            d.issues.length +
            ' problem(s) — nothing was written:</b><ul style="margin:6px 0 0;padding-left:18px;line-height:1.5;">' +
            d.issues
              .slice(0, 40)
              .map(function (i) {
                return '<li>' + esc(i.sheet + ' · ' + i.key + ': ' + i.message) + '</li>';
              })
              .join('') +
            '</ul></div>';
          treeImportConfirmed = false;
        ...
        if (!treeImportConfirmed) {
          ...
          document.getElementById('tiReview').innerHTML =
            '<div style="background:#f7f8f4;border:1px solid #eef0ea;border-radius:10px;padding:11px 13px;font-size:12.5px;line-height:1.6;">' +
            '<b>Ready to import</b><br>Categories: ' +
            (p.categories
              ? p.categories.create + ' new, ' + p.categories.update + ' updated'
              : '—') +
            '<br>Products: ' +
            (p.products ? p.products.create + ' new, ' + p.products.update + ' updated' : '—') +
            '<br>Bundle links: ' +
            ((p.bundles && p.bundles.links) || 0) +
            '</div>' +
            (p.missing && p.missing.length
              ? '<div style="margin-top:10px;background:#fdf6e3;border:1px solid #eadfbe;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#7a6320;line-height:1.55;">' +
                '<b>' +
                p.missing.length +
                ' catalog part(s) are not in this file.</b>' +
                '<div style="max-height:120px;overflow:auto;margin:6px 0;">' +
                p.missing
                  .slice(0, 60)
                  .map(function (mm) {
                    return esc(mm.sku + ' — ' + mm.name);
                  })
                  .join('<br>') +
                '</div>' +
                '<label style="display:block;margin-top:6px;">What should happen to them? ' +
  ...
  function tableShell(head, rows, cols, empty) {
    return (
      '<div style="background:#fbfbf9;border:1px solid #e7e8e3;border-radius:14px;overflow:hidden;"><table style="width:100%;border-collapse:collapse;font-size:14px;"><thead><tr>' +
      head
        .map(function (h) {
          return (
            '<th style="text-align:left;padding:11px 16px;font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;font-weight:600;border-bottom:1px solid #eef0ea;background:#f7f8f4;">' +
      ...
      (rows ||
        '<tr><td style="padding:22px 16px;color:#909689;" colspan="' +
          cols +
          '">' +
          esc(empty || 'No records.') +
          '</td></tr>') +
      '</tbody></table></div>'
    );
  }
  function sectionBlock(title, inner) {
    return '<div class="section-title">' + esc(title) + '</div>' + inner;
  }
  ...
  var props = {
    ...
    collapsed: (function () {
      try {
        return JSON.parse(localStorage.getItem('ssg.props.collapsed') || '[]');
      } catch (e) {
        return [];
      }
    })(),
  ...
  function dayDiff(v) {
    ...
    d.setHours(0, 0, 0, 0);
    return Math.round((d.getTime() - today0()) / 86400000);
  }
  function metaOfVersion(v) {
    var secs = (v && v.sections) || [];
    var m = (Array.isArray(secs) ? secs : []).filter(function (s) {
      return s && s.id === 'meta';
    })[0];
    return (m && m.data) || {};
  }
  ...
  async function renderProposals(user) {
    document.getElementById('view').innerHTML =
      ...
      '<input type="checkbox" id="propGroup"' +
      (props.grouped ? ' checked' : '') +
      '> Group by customer</label>' +
      '<input id="propSearch" placeholder="Search customer, title, number…" value="' +
      esc(props.q) +
      '" style="padding:9px 12px;border:1px solid #dcded7;border-radius:9px;font-size:13.5px;background:#fff;width:240px;">' +
      (hasRole(PROP_WRITE, user.role)
        ? '<button class="btn" id="propNew" style="width:auto;padding:10px 17px;white-space:nowrap;">New proposal</button>'
        : '') +
      '</div></div>' +
      '<div id="propList"><div class="muted" style="padding:24px;">Loading…</div></div>';
    drawPropFilters(user);
    if (hasRole(PROP_WRITE, user.role))
      document.getElementById('propNew').addEventListener('click', function () {
        openProposalForm(user);
      });
  ...
  function drawPropFilters(user) {
    var box = document.getElementById('propFilters');
    if (!box) return;
    box.innerHTML = PROP_FILTERS.map(function (f) {
      var on = props.filter === f.id;
      var count = props.rows.filter(function (r) {
        return matchFilter(r, f.id);
      }).length;
      return (
        '<button data-f="' +
        f.id +
        ...
        esc(f.label) +
        (props.rows.length ? ' <span style="opacity:.65;">' + count + '</span>' : '') +
        '</button>'
      );
    }).join('');
    box.querySelectorAll('[data-f]').forEach(function (b) {
      b.addEventListener('click', function () {
        props.filter = b.getAttribute('data-f');
        propsPersist();
        drawPropFilters(user);
        drawProposals(user);
      });
  ...
  async function loadProposals(user) {
    var box = document.getElementById('propList');
    if (!box) return;
    try {
      var r = await authed('/proposals');
      if (!r.ok) {
        box.innerHTML = '<div class="err">Could not load (' + r.status + ').</div>';
        return;
      }
      ...
      props.rows = (list || []).map(function (p) {
        ...
        return {
          ...
          title: p.title || '',
          number: p.number || '',
          version: v.version || p.currentVersion || 1,
          versionCount: p.versionCount || 1,
          status: st,
          preparedBy: p.preparedBy || '',
          created: p.createdAt,
          modified: p.lastModifiedAt || p.updatedAt,
          expires: exp,
          expDays: dd,
          ...
          invoiced: !!p.invoiced,
          invoiceDocNumber: p.invoiceDocNumber || '',
  ...
  function ptd(v, align, extra) {
    return (
      '<td style="padding:10px 11px;border-bottom:1px solid #f2f3ef;white-space:nowrap;text-align:' +
      (align || 'left') +
      ';' +
      (extra || '') +
      '">' +
  ...
  function ptdWrap(v, extra) {
    return (
      '<td style="padding:10px 11px;border-bottom:1px solid #f2f3ef;white-space:normal;overflow-wrap:anywhere;' +
      (extra || '') +
      '">' +
      v +
      '</td>'
  ...
  function drawProposals(user) {
    ...
    var rows = props.rows
      .filter(function (r) {
        return matchFilter(r, props.filter);
      })
      .filter(function (r) {
        return (
          !q ||
          (r.customer + ' ' + r.contact + ' ' + r.title + ' ' + r.number + ' ' + r.preparedBy)
            .toLowerCase()
            .indexOf(q) !== -1
        );
      });
    var sk = props.sort.key,
      dir = props.sort.dir === 'asc' ? 1 : -1;
    rows.sort(function (a, b) {
      var x = a[sk],
        y = b[sk];
      if (sk === 'created' || sk === 'modified' || sk === 'expires') {
    ...
    var head = PROP_COLS.map(function (c) {
      ...
      return (
        ...
        'text-align:' +
        (c.align || 'left') +
        ';padding:11px 14px;font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:' +
        (on ? '#3d4a55' : '#8a8f85') +
        ';font-weight:600;border-bottom:1px solid #e7e8e3;white-space:nowrap;">' +
        esc(c.label) +
        arrow +
    ...
    function rowHtml(r) {
      var expCell = r.expires
        ? r.expired
          ? '<span style="display:inline-flex;align-items:center;gap:5px;background:#fbe9e6;border:1px solid #f0cdc7;color:#9c3327;border-radius:999px;padding:3px 9px;font-size:12px;font-weight:600;" title="Expired ' +
            Math.abs(r.expDays) +
            ' day(s) ago">⚑ ' +
            fmtDate(r.expires) +
            '</span>'
          : r.expDays != null && r.expDays <= 7 && OPEN_STATUSES.indexOf(r.status) !== -1
            ? '<span style="display:inline-flex;align-items:center;gap:5px;background:#fdf6e3;border:1px solid #eadfbe;color:#8a6d1f;border-radius:999px;padding:3px 9px;font-size:12px;font-weight:600;" title="Expires in ' +
              r.expDays +
              ' day(s)">' +
              fmtDate(r.expires) +
              '</span>'
      ...
      var quick = acts.length
        ? '<select class="pQuick" data-id="' +
          r.id +
          '" data-vid="' +
          r.vid +
          '" style="padding:6px 8px;border:1px solid #dcded7;border-radius:8px;font-size:12px;background:#fff;color:#3d4a55;max-width:170px;">' +
      ...
      var reengage =
        r.status === 'EXPIRED' && !r.archivedAt
          ? '<button class="pReengage" data-id="' +
            r.id +
            '" title="Draft an email asking whether they are still interested" style="border:1px solid #dcded7;background:#fff;border-radius:8px;padding:6px 9px;font-size:12px;color:#3d4a55;cursor:pointer;white-space:nowrap;">Still interested?</button>'
          : '';
      ...
      var arch = canArchive(r, user)
        ? r.archivedAt
          ? '<button class="pRestore" data-id="' +
            r.id +
            '" title="Put this proposal back in the pipeline" style="border:1px solid #cfe3d7;background:#eaf3ee;border-radius:8px;padding:6px 9px;font-size:12px;color:#2f7d5d;cursor:pointer;white-space:nowrap;">Restore</button>'
          : '<button class="pArchive" data-id="' +
            r.id +
            '" title="Withdraw from the pipeline. Nothing is deleted and it can be restored." style="border:1px solid #dcded7;background:#fff;border-radius:8px;padding:6px 9px;font-size:12px;color:#8a8f85;cursor:pointer;white-space:nowrap;">Archive</button>'
        : '';
      return (
        ...
        ptdWrap(
          '<b style="font-weight:600;">' +
            ...
            '</b>' +
            (r.contact
              ? '<div class="muted" style="font-size:12px;">' + esc(r.contact) + '</div>'
              : ''),
        ) +
        ptdWrap(
          '<b style="font-weight:600;">' +
            ...
            ' · v' +
            r.version +
            (r.versionCount > 1 ? ' of ' + r.versionCount : '') +
            (r.preparedBy ? ' · ' + esc(r.preparedBy) : '') +
            '</div>' +
            (r.archivedAt
              ? '<div class="muted" style="font-size:11.5px;margin-top:3px;">Archived ' +
                esc(fmtDate(r.archivedAt)) +
                (r.archivedBy ? ' by ' + esc(r.archivedBy) : '') +
                (r.archiveReason ? ' — ' + esc(r.archiveReason) : '') +
                '</div>'
              : ''),
        ) +
        ptd(
          statusChip(r.status) +
            (r.invoiced && r.status === 'ACCEPTED'
              ? '<div style="margin-top:4px;font-size:11.5px;color:#2f7d5d;">Invoiced' +
                (r.invoiceDocNumber ? ' · ' + esc(r.invoiceDocNumber) : '') +
                '</div>'
              : '') +
            (r.archivedAt
        ...
        ptd(
          fmtDate(r.modified) +
            '<div class="muted" style="font-size:11px;">made ' +
            esc(fmtDate(r.created)) +
            '</div>',
        ) +
        ptd(expCell) +
        ptd(
    ...
    function bandRow(label, list, tone) {
      var sum = list.reduce(function (n, r) {
        return n + (Number(r.totalMinor) || 0);
      }, 0);
      return (
        '<tr><td colspan="7" style="padding:12px 11px 9px;border-bottom:1px solid #e7e8e3;background:' +
        tone +
        ';">' +
        '<b style="font-weight:650;font-size:13px;letter-spacing:.02em;">' +
        esc(label) +
        '</b>' +
        '<span class="muted" style="font-size:12.5px;margin-left:9px;">' +
        list.length +
        ' proposal' +
        (list.length === 1 ? '' : 's') +
        (sum ? ' · ' + fmtMoney(sum, 'USD') : '') +
        '</span></td></tr>'
      );
    }
    ...
    if (banded) {
      ...
      var lateRows = rows.filter(function (r) {
        return r.expired;
      });
      body =
        (liveRows.length
          ? bandRow('Active', liveRows, '#f1f6f2') + liveRows.map(rowHtml).join('')
          : '') +
        (lateRows.length
          ? bandRow('Past expiration', lateRows, '#fdf1ef') + lateRows.map(rowHtml).join('')
          : '');
    }
    ...
    if (!banded && props.grouped && rows.length) {
      var order = [],
        byCust = {};
      rows.forEach(function (r) {
        if (!byCust[r.customer]) {
          byCust[r.customer] = [];
          order.push(r.customer);
        }
      ...
      body = order
        .map(function (cust) {
          ...
          var open = mine.filter(function (r) {
            return OPEN_STATUSES.indexOf(r.status) !== -1 && !r.expired;
          }).length;
          var flagged = mine.filter(function (r) {
            return r.expired;
          }).length;
          var isOpen = props.collapsed.indexOf(cust) === -1;
          return (
            '<tr class="pGroup" data-cust="' +
            esc(cust) +
            '" style="cursor:pointer;background:#f4f5f1;">' +
            '<td colspan="7" style="padding:11px 14px;border-bottom:1px solid #e7e8e3;">' +
            '<span style="display:inline-block;width:14px;color:#8a8f85;">' +
            (isOpen ? '▾' : '▸') +
            '</span>' +
            '<b style="font-weight:650;font-size:13.5px;">' +
            esc(cust) +
            '</b>' +
            ...
            '</span>' +
            (flagged
              ? '<span style="margin-left:10px;font-size:12px;color:#9c3327;">⚑ ' +
                flagged +
                ' expired</span>'
              : '') +
            '</td></tr>' +
    ...
    box.innerHTML =
      lead +
      '<div style="background:#fbfbf9;border:1px solid #e7e8e3;border-radius:14px;overflow:hidden;"><table style="width:100%;border-collapse:collapse;font-size:13.5px;table-layout:auto;"><thead><tr>' +
      head +
      '</tr></thead><tbody>' +
      (body ||
        '<tr><td style="padding:22px 16px;color:#909689;" colspan="7">' +
          (props.rows.length
            ? props.filter === 'archived'
              ? 'Nothing archived.'
              : props.filter === 'closed'
                ? 'No invoiced deals yet.'
                : 'No proposals match this view.'
            : 'No proposals yet.') +
          '</td></tr>') +
      '</tbody></table></div>' +
      (props.rows.filter(function (r) {
        return r.expired && !r.archivedAt;
      }).length
        ? '<div style="margin-top:10px;font-size:12.5px;color:#9c3327;">⚑ Flagged rows are past their expiration date and still open — re-date them or mark them no longer active.</div>'
        : '');
    box.querySelectorAll('th[data-sk]').forEach(function (th) {
      th.addEventListener('click', function () {
        var k = th.getAttribute('data-sk');
        if (props.sort.key === k) props.sort.dir = props.sort.dir === 'asc' ? 'desc' : 'asc';
          ...
          props.sort.key = k;
          props.sort.dir = k === 'created' || k === 'modified' || k === 'expires' ? 'desc' : 'asc';
        }
        drawProposals(user);
      });
    });
    box.querySelectorAll('tr.pGroup').forEach(function (tr) {
      tr.addEventListener('click', function () {
        var c = tr.getAttribute('data-cust'),
          i = props.collapsed.indexOf(c);
        if (i === -1) props.collapsed.push(c);
        else props.collapsed.splice(i, 1);
        propsPersist();
        drawProposals(user);
      });
    });
    ...
    box.querySelectorAll('button.pReengage').forEach(function (bt) {
      bt.addEventListener('click', function (ev) {
        ev.stopPropagation();
        var row = props.rows.filter(function (r) {
          return r.id === bt.getAttribute('data-id');
        })[0];
    ...
    box.querySelectorAll('tr[data-id]').forEach(function (tr) {
      tr.addEventListener('click', function () {
        openProposalDetail(tr.getAttribute('data-id'), user);
      });
    });
    box.querySelectorAll('.pQuick').forEach(function (sel) {
      ...
      sel.addEventListener('change', async function (e) {
        ...
        if (
          act === 'expire' &&
          !confirm(
            'Mark this proposal no longer active? It stays on record and can be revived as a new version.',
          )
        ) {
          sel.value = '';
          return;
  ...
  async function reportRelease(act, rr) {
    ...
    if (lastReleaseDoc && m && m.pushed) {
      try {
        var fr = await authed(
          '/render/proposals/versions/' + lastReleaseDoc.versionId + '/monday-file',
          {
            method: 'POST',
        ...
        if (!fr.ok)
          fileNote = await serverMessage(fr, 'the renderer did not respond (' + fr.status + ')');
        else if (!fd || !fd.uploaded)
          fileNote = (fd && (fd.skipped || fd.error)) || 'monday did not accept the file';
      } catch (e) {
        fileNote = 'the renderer could not be reached';
      }
    }
    lastReleaseDoc = null;
    if (m && m.pushed) {
      if (fileNote)
        alert(
          'Released. The deal row was updated, but ' + who + ' did not attach: ' + fileNote + '.',
        );
      return;
    }
    if (!m) return;
    alert(
      'The proposal was released, but monday.com was not updated: ' +
  ...
  function versionTotalMinor(version) {
    ...
    var metaSec = Array.isArray(secs)
    ...
    var structureFreight = metaAmount(
    ...
    return (
      ...
      discount +
      tpFreight +
      metaAmount(meta.taxAmountMinor, meta.tbdTax) +
      structureFreight +
      metaAmount(meta.matsFreightMinor, meta.tbdMatsFreight) +
      stdFreightOf(meta)
    );
  }
  ...
  async function archiveProposal(r, user, confirmWord) {
    var reason = confirmWord
      ? r._pendingReason || ''
      : prompt(
          'Archive ' +
            (r.number || 'this proposal') +
            ' — ' +
            (r.customer || '') +
            '.' +
            '\n\n' +
    ...
    if (!rr.ok) {
      alert('Could not archive (' + rr.status + ').');
      return;
    }
    var d = null;
    try {
    ...
    if (d && d.needsConfirm) {
      ...
      if (typed.trim().toUpperCase() !== 'CONFIRM') {
        alert('Not archived — CONFIRM was not typed.');
        r._pendingReason = null;
        return;
      }
  ...
  async function restoreProposal(r, user, after) {
    ...
    if (!rr.ok) {
      alert('Could not restore (' + rr.status + ').');
      return;
    }
    if (after) after();
    else loadProposals(user);
  }
  ...
  function quickActions(r, user) {
    var a = [],
      w = hasRole(PROP_WRITE, user.role),
      rev = hasRole(PROP_REVIEW, user.role),
      rel = hasRole(PROP_RELEASE, user.role);
    if (r.status === 'DRAFT') {
      ...
      if (rev) {
        a.push(['accept', 'Proposal Signed']);
        a.push(['reject', 'Mark rejected']);
        a.push(['expire', 'Mark no longer active']);
      }
    } else if (w) {
  ...
  async function openCustomerEmail(row, user, kind) {
    ...
    var contacts = (ctx.contacts || []).filter(function (c) {
      return c.email;
    });
    ...
    if (!contacts.length) {
      return openModal(
        'Email ' + (row.customer || 'the customer'),
        '<div class="muted" style="font-size:13px;line-height:1.6;">No contact on this customer has an email address, so there is nobody to address this to. Add one under CRM → ' +
          esc(row.customer || 'the customer') +
          ' → Contacts, then come back.</div>',
        null,
        '',
        {},
      );
    ...
    openModal(
      ...
      '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-bottom:12px;">This opens a draft in your own mail client. Nothing is sent from this app — you read it, edit it and send it from your mailbox, so their reply comes back to you.</div>' +
        ...
        fieldRow(
          'Subject',
          '<input id="ceSubj" style="' + IN + '" value="' + esc(d0.subject) + '">',
        ) +
        '<div class="field"><label>Message</label><textarea id="ceBody" rows="12" style="' +
        IN +
        'resize:vertical;font-size:13.5px;line-height:1.55;">' +
        esc(d0.body) +
        '</textarea></div>' +
    ...
    if (sel)
      sel.addEventListener('change', function () {
        ...
        if (i === prevIdx) return;
        var was = defaults(contacts[prevIdx].name),
          now = defaults(contacts[i].name);
        var ta = document.getElementById('ceBody'),
          su = document.getElementById('ceSubj');
  ...
  function proposalTimelinePanel(p, latest) {
    ...
    var lbl =
      'font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;font-weight:600;margin-bottom:5px;';
    return (
      '<div class="card">' +
      '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-bottom:14px;">Kept on <b style="color:#20241f;font-weight:600;">' +
      esc(p.organizationName || 'the customer') +
      '</b>, so every proposal for this account reads the same window. Changing it here changes it in CRM.</div>' +
      '<div style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end;">' +
      '<div style="flex:1 1 150px;"><div style="' +
      lbl +
      '">Decision from</div><input type="date" id="ptFrom" value="' +
      esc(dates.decisionFrom || '') +
      '" style="' +
      IN +
      '"></div>' +
      '<div style="flex:1 1 150px;"><div style="' +
      lbl +
      '">Decision to</div><input type="date" id="ptTo" value="' +
      esc(dates.decisionTo || '') +
      '" style="' +
      IN +
      '"></div>' +
      '<div style="flex:1 1 150px;"><div style="' +
      lbl +
      '">Follow-up date</div><input type="date" id="ptFollow" value="' +
      esc(dates.followUpDate || suggested) +
      '" style="' +
      IN +
      (suggested ? 'border-color:#eadfbe;background:#fdfaf1;' : '') +
      '"></div>' +
      ...
      (suggested
        ? 'Suggested: seven days after this version was released on ' +
          esc(fmtDate(latest.releasedAt)) +
          '. Not saved until you press Save dates.'
        : latest && latest.releasedAt
          ? 'Released ' +
            esc(fmtDate(latest.releasedAt)) +
            '. New releases set this to seven days later automatically unless a later date is already booked.'
  ...
  async function openProposalDetail(id, user) {
    ...
    if (latest.id && latest.status === 'ACCEPTED') {
      try {
        var ro = await authed('/orders/by-version/' + latest.id);
        if (ro.ok) lockedOrder = await ro.json();
      } catch (e) {}
    }
    ...
    view.innerHTML =
      ...
      (p.archivedAt
        ? '<div style="margin-bottom:16px;background:#f4f5f1;border:1px solid #cbcec5;border-radius:12px;padding:13px 15px;font-size:13px;color:#5c6157;">' +
          '<b style="font-weight:650;">Archived</b> ' +
          esc(fmtDate(p.archivedAt)) +
      ...
      esc(p.organizationName || 'Customer not found') +
      '</h2>' +
      ...
      esc(p.number || '') +
      (latest.releasedAt ? ' · released ' + esc(fmtDate(latest.releasedAt)) : '') +
      '</div>' +
      '</div><div style="text-align:right;"><span class="chip">' +
      titleCase(latest.status || 'DRAFT') +
      '</span>' +
      '<div style="font-size:19px;font-weight:600;margin-top:8px;">' +
      fmtMoney(versionTotalMinor(latest), 'USD') +
      '</div>' +
      '</div></div></div>' +
      sectionBlock(
        'Versions',
        tableShell(
          ['Version', 'Status', 'Created', 'Frozen', 'Total', ''],
          versions
            .map(function (v) {
              ...
              var action = editable
                ? '<button class="btn" data-open="edit" data-vid="' +
                  v.id +
                  '" style="width:auto;padding:8px 15px;">Build / edit proposal</button>'
                : '<button class="link-btn" data-open="view" data-vid="' +
                  v.id +
                  '" style="width:auto;padding:8px 15px;">View (read only)</button>';
              var freightBtn =
                '<button class="link-btn" data-open="freight" data-vid="' +
                v.id +
                '" style="width:auto;padding:8px 13px;">Freight</button>';
              ...
              var discardBtn =
                editable && versions.length > 1
                  ? '<button class="link-btn" data-discard="' +
                    v.id +
                    '" data-v="' +
                    v.version +
                    '" style="width:auto;padding:8px 13px;color:#9c3327;">Discard draft</button>'
                  : '';
              return (
                '<tr>' +
                td('v' + v.version) +
                td('<span class="chip">' + titleCase(v.status) + '</span>') +
                td(fmtDate(v.createdAt)) +
                td(v.frozen ? 'Yes' : 'No') +
                td(
                  '<b style="font-weight:600;">' + fmtMoney(versionTotalMinor(v), 'USD') + '</b>',
                ) +
                td(
      ...
      (acceptedVersion && hasRole(QBO_VIEW_ROLES, user.role)
        ? sectionBlock(
            'QuickBooks',
            '<div id="qboBox"><div class="muted" style="padding:16px;">Loading…</div></div>',
          )
        : '') +
      sectionBlock(
        'Financing options',
        '<div id="finBox"><div class="muted" style="padding:16px;">Loading…</div></div>',
      ) +
      (actions
        ? sectionBlock(
            'Next Steps',
            '<div style="display:flex;gap:8px;flex-wrap:wrap;" id="propActions">' +
              actions +
              '</div>',
          )
        : '');
    document.getElementById('propBack').addEventListener('click', function () {
      renderProposals(user);
    });
    var prst = document.getElementById('propRestore');
    if (prst)
      prst.addEventListener('click', async function () {
    ...
    if (psd)
      psd.addEventListener('click', function () {
        openSendDocuments(p, finCache, 'customer');
      });
    var pem = document.getElementById('propEmail');
    if (pem)
      pem.addEventListener('click', function () {
        openCustomerEmail(
          {
            id: p.id,
            organizationId: p.organizationId,
            ...
            title: p.title || '',
            customer: p.organizationName || '',
            totalMinor: versionTotalMinor(latest),
            releasedAt: latest.releasedAt || null,
            expires: latest.expirationDate || null,
          },
          user,
          'general',
        );
    ...
    if (ptSave)
      ptSave.addEventListener('click', async function () {
        ...
        if (!r.ok) {
          ...
          if (msg) {
            msg.style.color = '#9c3327';
            msg.textContent = m || 'Could not save (' + r.status + ').';
          }
          return;
        }
        p.customerDates = await r.json();
        if (msg) {
          msg.style.color = '#2f7d5d';
          msg.textContent = 'Saved to ' + (p.organizationName || 'the customer') + '.';
        }
      });
    ...
    document.querySelectorAll('[data-open]').forEach(function (bt) {
      bt.addEventListener('click', function () {
        var v = versions.filter(function (x) {
          return x.id === bt.getAttribute('data-vid');
        })[0];
        if (!v) return;
    ...
    document.querySelectorAll('[data-discard]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        var vid = bt.getAttribute('data-discard'),
          vn = bt.getAttribute('data-v');
        ...
        if (
          !confirm(
            'Discard draft v' +
              vn +
              '?\n\nIts lines and pricing are deleted and cannot be recovered. The earlier versions are untouched.',
          )
        )
          return;
        bt.disabled = true;
        ...
        if (!r.ok) {
          bt.disabled = false;
          alert(await serverMessage(r, 'Could not discard this version (' + r.status + ').'));
          return;
        }
    ...
    if (puBtn)
      puBtn.addEventListener('click', function () {
        openUnlockForm({ id: lockedOrder.id, number: lockedOrder.number }, user);
      });
    document.querySelectorAll('#propActions [data-act]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        var act = bt.getAttribute('data-act'),
          vid = bt.getAttribute('data-vid');
        if (act === 'lock') {
          openLockForm(vid, user);
          return;
        }
        var path =
          act === 'new-version'
  ...
  function proposalActions(v, user, lockedOrder) {
    ...
    if (s === 'DRAFT') {
      if (hasRole(PROP_WRITE, user.role)) b.push(btn('submit-review', 'Submit for review'));
      if (hasRole(PROP_RELEASE, user.role)) b.push(btn('release', 'Ready to Send to Customer', 1));
    } else if (s === 'INTERNAL_REVIEW') {
      if (hasRole(PROP_REVIEW, user.role)) b.push(btn('return-draft', 'Return to draft'));
      if (hasRole(PROP_RELEASE, user.role)) b.push(btn('release', 'Ready to Send to Customer', 1));
    } else if (s === 'RELEASED') {
      if (hasRole(PROP_REVIEW, user.role)) {
        b.push(btn('accept', 'Proposal Signed', 1));
        b.push(btn('reject', 'Reject'));
        b.push(btn('expire', 'Expire'));
      }
    } else if (s === 'ACCEPTED') {
      if (lockedOrder && lockedOrder.id && lockedOrder.status !== 'CANCELLED') {
        b.push(
          '<span class="chip" style="align-self:center;">Locked to ' +
            esc(lockedOrder.number || 'order') +
            '</span>',
        );
        if (hasRole(ORDERS_MANAGE_ROLES, user.role) && lockedOrder.status !== 'COMPLETE') {
          b.push(
            '<button class="link-btn" id="propUnlock" style="width:auto;padding:9px 15px;color:#9c3327;">Unlock for changes</button>',
          );
        }
      } else if (hasRole(ORDERS_MANAGE_ROLES, user.role)) {
        b.push(
          '<button class="btn" data-act="lock" data-vid="' +
            v.id +
            '" style="width:auto;padding:9px 15px;">Lock to operational order</button>',
        );
      }
    }
    if (hasRole(PROP_WRITE, user.role) && (s === 'RELEASED' || s === 'REJECTED' || s === 'EXPIRED'))
      b.push(btn('new-version', 'Create new version'));
    return b.join('');
  }
  async function openProposalForm(user) {
    ...
    openModal(
      ...
      async function (close, showErr) {
        ...
        if (title.length < 2) return showErr('Title must be at least 2 characters.');
        var r = await authed('/proposals', {
          method: 'POST',
          body: {
            organizationId: document.getElementById('fOrg').value,
  ...
  function repRangeParams() {
    ...
    if (rep.range === 'ytd') from = new Date(t.getFullYear(), 0, 1);
      ...
      from = new Date(t.getTime() - Number(rep.range) * 86400000);
    }
  ...
  async function renderReports(user) {
    document.getElementById('view').innerHTML =
      ...
      REP_RANGES.map(function (r) {
        return (
          '<option value="' +
          r[0] +
          '"' +
          (rep.range === r[0] ? ' selected' : '') +
          '>' +
          r[1] +
          '</option>'
        );
      ...
      '</select>' +
      '<span id="repCustom" style="display:' +
      (rep.range === 'custom' ? 'flex' : 'none') +
      ';gap:6px;align-items:center;">' +
      '<input id="repFrom" type="date" value="' +
      esc(rep.from) +
      '" style="padding:8px 10px;border:1px solid #dcded7;border-radius:9px;font-size:13px;">' +
      '<span class="muted" style="font-size:12px;">to</span>' +
      '<input id="repTo" type="date" value="' +
      esc(rep.to) +
      '" style="padding:8px 10px;border:1px solid #dcded7;border-radius:9px;font-size:13px;"></span>' +
    ...
    ['repFrom', 'repTo'].forEach(function (id) {
      document.getElementById(id).addEventListener('change', function () {
        rep.from = document.getElementById('repFrom').value;
        rep.to = document.getElementById('repTo').value;
        loadReports();
      });
    });
  ...
  function drawRepTabs(user) {
    var box = document.getElementById('repTabs');
    if (!box) return;
  ...
  async function loadReports() {
    ...
    try {
      ...
      if (!r.ok) {
        box.innerHTML = '<div class="err">Could not load reports (' + r.status + ').</div>';
        return;
      }
  ...
  function kpi(label, value, sub, color) {
    return (
      ...
      esc(label) +
      '</div>' +
      "<div style=\"font-family:'Newsreader',serif;font-size:26px;font-weight:600;margin-top:2px;color:" +
      (color || '#20241f') +
      ';">' +
      value +
      '</div>' +
      (sub ? '<div class="muted" style="font-size:12px;margin-top:3px;">' + sub + '</div>' : '') +
      '</div>'
    );
  }
  function bar(fraction, color, height) {
    ...
    return (
      '<div style="background:#eef0ea;border-radius:999px;height:' +
      (height || 8) +
  ...
  function repTable(head, rows, empty) {
    return (
      '<div style="background:#fbfbf9;border:1px solid #e7e8e3;border-radius:14px;overflow:auto;"><table style="width:100%;border-collapse:collapse;font-size:13.5px;"><thead><tr>' +
      head
        .map(function (h) {
          return (
            '<th style="text-align:' +
            (h[1] || 'left') +
            ';padding:10px 14px;font-size:10.5px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;font-weight:600;border-bottom:1px solid #e7e8e3;white-space:nowrap;">' +
            esc(h[0]) +
            '</th>'
          );
        })
      ...
      (rows ||
        '<tr><td colspan="' +
          head.length +
          '" style="padding:20px 14px;color:#909689;">' +
          esc(empty || 'No data in this period.') +
          '</td></tr>') +
      '</tbody></table></div>'
    );
  }
  function rtd(v, align, weight) {
    return (
      '<td style="padding:10px 14px;border-bottom:1px solid #f2f3ef;text-align:' +
      (align || 'left') +
      ';' +
  ...
  function drawReports() {
    var box = document.getElementById('repBody');
    if (!box || !rep.data) return;
    var d = rep.data,
      s = d.summary;
    if (!s.total) {
      box.innerHTML =
        '<div class="placeholder"><h3>No proposals in this period</h3><p>Widen the date range to see reporting across the company.</p></div>';
      return;
    }
    if (rep.tab === 'overview') {
      var pipeMax = Math.max.apply(
        null,
        d.pipeline
          .map(function (p) {
            return p.value;
      ...
      box.innerHTML =
        ...
        kpi(
          ...
          s.winRate + '%',
          'vs ' + s.lost + ' rejected',
          s.winRate >= 50 ? '#2f7d5d' : '#9c3327',
        ) +
        kpi(
          'Open pipeline',
          fmt0(s.openValue),
          s.open + ' open · avg ' + s.avgDaysOpen + ' days old',
          '#3d4a55',
        ) +
        '</div>' +
        ...
        '</div>' +
        (s.archivedCount
          ? '<div style="margin-top:14px;background:#f4f5f1;border:1px solid #cbcec5;color:#5c6157;border-radius:12px;padding:12px 14px;font-size:13px;">' +
            s.archivedCount +
            ' archived proposal(s) worth ' +
            fmt0(s.archivedValue) +
            ' are excluded from every figure above. See the Archived tab on the proposals list.</div>'
          : '') +
        (s.expiredFlagged
          ? '<div style="margin-top:14px;background:#fbe9e6;border:1px solid #f0cdc7;color:#9c3327;border-radius:12px;padding:12px 14px;font-size:13px;">⚑ ' +
            s.expiredFlagged +
            ' open proposal(s) are past their expiration date. See the Aging tab.</div>'
          : '') +
        '<div class="section-title">Pipeline by stage</div>' +
        d.pipeline
          .map(function (p) {
            return (
              '<div style="margin-bottom:10px;"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px;"><span>' +
              esc(p.label) +
              ' <span class="muted">· ' +
              p.count +
              '</span></span><span style="font-weight:600;">' +
              fmt0(p.value) +
              '</span></div>' +
              bar(
                p.value / pipeMax,
        ...
        repTable(
          ...
          d.products
            ...
            .map(function (p) {
              return (
                '<tr>' +
                rtd(esc(p.name), 'left', 1) +
                rtd(
                  '<span style="font-family:ui-monospace,monospace;font-size:11.5px;color:#5c6157;">' +
                    esc(p.sku) +
                    '</span>',
                ) +
                rtd(p.proposals, 'center') +
                rtd(p.qty, 'center') +
                rtd(fmt0(p.proposedValue), 'right') +
                rtd(fmt0(p.wonValue), 'right') +
                '</tr>'
              );
            })
            .join(''),
    ...
    if (rep.tab === 'conversion') {
      ...
      box.innerHTML =
        ...
        kpi(
          'Conversion by value',
          s.conversionRateByValue + '%',
          fmt0(s.wonValue) + ' of ' + fmt0(s.sentValue),
        ) +
        kpi(
          'Win rate',
          s.winRate + '%',
        ...
        stages
          .map(function (st) {
            return (
              '<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px;"><span>' +
              st[0] +
              ' <span class="muted">· ' +
              st[1] +
              '</span></span><span style="font-weight:600;">' +
              fmt0(st[2]) +
              '</span></div>' +
              bar(s.total ? st[1] / s.total : 0, st[3], 12) +
              '</div>'
            );
          })
          .join('') +
        ...
        repTable(
          ...
          d.byCustomerType
            .map(function (g) {
              return (
                '<tr>' +
                rtd(esc(titleCase(g.customerType)), 'left', 1) +
                rtd(g.count, 'center') +
                rtd(fmt0(g.value), 'right') +
                rtd(g.won, 'center') +
                rtd(fmt0(g.wonValue), 'right') +
                rtd((g.count ? Math.round((g.won / g.count) * 1000) / 10 : 0) + '%', 'right', 1) +
                '</tr>'
              );
            })
            .join(''),
        ...
        repTable(
          ...
          d.byCustomer
            .map(function (g) {
              return (
                '<tr>' +
                rtd(esc(g.customer), 'left', 1) +
                rtd(g.count, 'center') +
                rtd(fmt0(g.value), 'right') +
                rtd(g.won, 'center') +
                rtd(g.lost, 'center') +
                rtd(g.winRate + '%', 'right', 1) +
                '</tr>'
              );
            })
            .join(''),
    ...
    if (rep.tab === 'aging') {
          ...
          .map(function (r) {
            return (
              ...
              rtd(
                '<b style="font-weight:600;">' +
                  esc(r.customer) +
                  '</b><div class="muted" style="font-size:11.5px;">' +
                  esc(r.title) +
                  ' · ' +
                  esc(r.number) +
                  '</div>',
              ) +
              rtd(statusChip(r.status)) +
              rtd(fmtDate(r.createdAt)) +
              rtd(r.daysOpen + ' d', 'center') +
              rtd(r.expiration ? fmtDate(r.expiration) : '<span class="muted">—</span>') +
              rtd(fmt0(r.total), 'right', 1) +
              '</tr>'
            );
          })
      ...
      box.innerHTML =
        ...
        kpi(
          'Past expiration',
          d.expiredOpen.length,
          'still open — needs action',
          d.expiredOpen.length ? '#9c3327' : '#2f7d5d',
        ) +
        ...
        d.aging
          .map(function (a) {
            return (
              ...
              fmt0(a.value) +
              '</span></div>' +
              bar(a.count / amax, '#3d4a55') +
              '</div>'
            );
          })
          .join('') +
        '<div class="section-title">Past expiration — open</div>' +
        repTable(ageHead, rowsOf(d.expiredOpen), 'Nothing past expiration. ') +
        '<div class="section-title">Expiring within 14 days</div>' +
        repTable(ageHead, rowsOf(d.expiringSoon), 'Nothing expiring soon.') +
        '<div class="section-title">Oldest open proposals</div>' +
        repTable(ageHead, rowsOf(d.oldestOpen), 'No open proposals.');
      return;
    }
    if (rep.tab === 'pipeline') {
      ...
      box.innerHTML =
        ...
        kpi(
          'Released & awaiting',
          fmt0(
            d.pipeline.filter(function (p) {
              return p.status === 'RELEASED';
            })[0].value,
          ),
          s.released + ' out with customers',
        ) +
        kpi('Closed won', fmt0(s.wonValue), s.won + ' proposals', '#2f7d5d') +
        '</div>' +
        '<div class="section-title">By stage</div>' +
        repTable(
          [['Stage'], ['Proposals', 'center'], ['Value', 'right'], ['Share of value', 'left']],
          d.pipeline
            .map(function (p) {
              return (
                '<tr>' +
                rtd(esc(p.label), 'left', 1) +
                rtd(p.count, 'center') +
                rtd(fmt0(p.value), 'right', 1) +
                '<td style="padding:10px 14px;border-bottom:1px solid #f2f3ef;min-width:180px;">' +
                bar(
                  p.value / vmax,
                  p.status === 'ACCEPTED'
                    ? '#2f7d5d'
        ...
        repTable(
          ...
          d.winLossByMonth
            .map(function (m) {
              ...
              return (
                '<tr>' +
                rtd(esc(m.month), 'left', 1) +
                rtd(m.won, 'center') +
                rtd(m.lost, 'center') +
                rtd(m.expired, 'center') +
                rtd(fmt0(m.wonValue), 'right') +
                rtd(fmt0(m.lostValue), 'right') +
                '<td style="padding:10px 14px;border-bottom:1px solid #f2f3ef;min-width:160px;"><div style="display:flex;height:8px;border-radius:999px;overflow:hidden;background:#eef0ea;"><div style="width:' +
                ((m.won / tot) * 100).toFixed(1) +
                '%;background:#2f7d5d;"></div><div style="width:' +
                ((m.lost / tot) * 100).toFixed(1) +
                '%;background:#9c3327;"></div></div></td></tr>'
              );
            })
            .join(''),
        ) +
    ...
    if (rep.tab === 'products') {
      ...
      var list = d.products.filter(function (p) {
        return !q || (p.name + ' ' + p.sku).toLowerCase().indexOf(q) !== -1;
      });
      list = list.slice().sort(function (a, b) {
        return (b[rep.psort] || 0) - (a[rep.psort] || 0);
      });
      ...
      box.innerHTML =
        ...
        kpi(
          'Proposed value',
          fmt0(
            d.products.reduce(function (a, p) {
              return a + p.proposedValue;
            }, 0),
          ),
          'product lines only',
        ) +
        ...
        '<input id="repPq" placeholder="Filter products or SKUs…" value="' +
        esc(rep.pq) +
        '" style="padding:9px 12px;border:1px solid #dcded7;border-radius:9px;font-size:13px;background:#fff;width:240px;">' +
        '<select id="repPsort" style="padding:9px 12px;border:1px solid #dcded7;border-radius:9px;font-size:13px;background:#fff;">' +
        [
          ['proposedValue', 'Sort: proposed value'],
          ['qty', 'Sort: units proposed'],
          ['proposals', 'Sort: times proposed'],
          ['wonValue', 'Sort: accepted value'],
          ['attachRate', 'Sort: attach rate'],
        ]
          .map(function (o) {
            return (
              '<option value="' +
        ...
        top
          .map(function (p) {
            return (
              '<div style="margin-bottom:10px;"><div style="display:flex;justify-content:space-between;gap:12px;font-size:13px;margin-bottom:4px;"><span>' +
              esc(p.name) +
              ' <span class="muted" style="font-family:ui-monospace,monospace;font-size:11px;">' +
              esc(p.sku) +
              '</span></span><span style="font-weight:600;">' +
              fmt0(p.proposedValue) +
              '</span></div>' +
              bar(p.proposedValue / pmax, '#3d4a55') +
              '</div>'
            );
          })
          .join('') +
          ...
          list
            .map(function (p) {
              return (
                ...
                rtd(p.proposals, 'center') +
                rtd(p.attachRate + '%', 'right') +
                rtd(p.qty, 'center') +
                rtd(fmt0(p.avgRate), 'right') +
                rtd(fmt0(p.proposedValue), 'right', 1) +
                rtd(p.wonProposals, 'center') +
                rtd(fmt0(p.wonValue), 'right') +
                '</tr>'
              );
            })
      ...
      pq.addEventListener('input', function () {
        rep.pq = pq.value;
        drawReports();
        document.getElementById('repPq').focus();
      });
      document.getElementById('repPsort').addEventListener('change', function () {
        rep.psort = this.value;
        drawReports();
      });
      return;
    }
        ...
        repTable(
          ...
          d.byPreparer
            .map(function (g) {
              return (
                '<tr>' +
                rtd(esc(g.preparedBy), 'left', 1) +
                rtd(g.count, 'center') +
                rtd(fmt0(g.value), 'right') +
                rtd(g.won, 'center') +
                rtd(fmt0(g.wonValue), 'right') +
                rtd(g.winRate + '%', 'right', 1) +
                rtd(g.avgMarginPct + '%', 'right') +
                '</tr>'
              );
            })
            .join(''),
    ...
    // detail
    box.innerHTML =
      '<div class="section-title">All proposals in range <span class="muted" style="font-weight:400;font-size:12px;">— ' +
      d.rows.length +
      ' records</span></div>' +
      repTable(
        [
        ...
        d.rows
          .map(function (r) {
            return (
              '<tr>' +
              rtd('<b style="font-weight:600;">' + esc(r.customer) + '</b>') +
              rtd(
                esc(r.title) +
                  '<div class="muted" style="font-size:11.5px;">' +
                  esc(r.number) +
              ...
              rtd(esc(r.preparedBy || '—')) +
              rtd(
                statusChip(r.status) + (r.expired ? ' <span style="color:#9c3327;">⚑</span>' : ''),
              ) +
              rtd(fmtDate(r.createdAt)) +
              rtd(fmtDate(r.updatedAt)) +
              rtd(r.expiration ? fmtDate(r.expiration) : '—') +
              rtd(r.daysOpen + ' d', 'center') +
              rtd(fmt0(r.total), 'right', 1) +
              rtd(r.marginPct + '%', 'right') +
              '</tr>'
            );
          })
          .join(''),
  ...
  function exportReportCsv() {
    ...
    var csv = [cols.join(',')]
      .concat(
        rep.data.rows.map(function (r) {
          return cols
            .map(function (c) {
              var v = r[c];
              if (c === 'total' || c === 'revenue' || c === 'cogs' || c === 'margin')
                v = (Number(v) || 0) / 100;
              if (v == null) v = '';
              var s = String(v);
  ...
  function m2d(m) {
    return (Number(m || 0) / 100).toFixed(2);
  }
  ...
  function hardwareKitLine() {
    return (
      (pb && pb.lines ? pb.lines : []).filter(function (l) {
  ...
  function hardwareQty() {
    ...
    var known = {};
    ((kit && kit.components) || []).forEach(function (c) {
      known[String(c.part || '').toUpperCase()] = true;
    });
    var out = {};
  ...
  async function refreshHardwareKit() {
    hwTimer = null;
    if (hwBusy) {
      hwTimer = setTimeout(refreshHardwareKit, 400);
      return;
    }
    var kit = hardwareKitLine();
    ...
    try {
      var r = await authed('/proposals/adventure-series/hardware', {
        method: 'POST',
        body: { answers: pb.meta.advAnswers, hwQty: hardwareQty() },
      });
      if (!r.ok) return;
      var d = await r.json();
      ...
      if (kit.description && /^All mounting hardware/.test(kit.description)) {
        kit.description =
          'All mounting hardware for this structure — ' +
          d.pieces +
          ' pieces across ' +
          d.components.length +
          ' part numbers.';
   ...
   */
  var FREIGHT_TBD_NOTE =
    'Shipping and freight charges for this item have not yet been determined. Upon approval of this proposal, current freight pricing will be obtained and added to the final invoice.';
  ...
  function normalizeLine(it) {
    ...
    if (
      !note &&
      LEAKED_INTERNAL.some(function (re) {
        return re.test(desc);
      })
    ) {
      note = desc;
    ...
    return {
      ref: it.ref || uid(),
      lineType: it.lineType || (it.isNote ? 'NOTE' : 'PRODUCT'),
      kind: it.kind || 'INCLUDED',
      productId: it.productId || null,
      sku: it.sku || '',
      name: it.name || '',
      description: desc,
      quantity: it.quantity == null ? 1 : it.quantity,
      rateMinor: it.rateMinor || 0,
      costEach: it.costEach || 0,
      weightEach: it.weightEach || 0,
      group: it.group || '',
      optional: !!it.optional,
      delivery: it.delivery || '',
      returnable: it.returnable || '',
      addlFreight: it.addlFreight || '',
      freightCalc: it.freightCalc || '',
      tpFreightMinor: it.tpFreightMinor || 0,
      tpFreightLabel: it.tpFreightLabel || '',
  ...
  function formatOrgShipTo(org) {
    ...
    var a =
      org.addresses.filter(function (x) {
        return x.type === 'SHIPPING';
      })[0] ||
      org.addresses.filter(function (x) {
        return x.type === 'BILLING';
      })[0] ||
      org.addresses[0];
    if (!a) return '';
  ...
  function primaryContactName(org) {
    ...
    var c =
      cs.filter(function (x) {
        return x.isDecisionMaker;
      })[0] || cs[0];
    if (!c) return '';
  ...
  async function openBuilder(proposal, version, user) {
    var orgName = '',
      ...
      orgContact = '';
    try {
      var rd = await authed('/crm/organizations/' + proposal.organizationId);
      if (rd.ok) {
        var org = await rd.json();
        orgName = org.name || '';
        orgShipTo = formatOrgShipTo(org);
        orgContact = primaryContactName(org);
      }
    } catch (e) {}
    if (!orgName) {
      try {
        var ro = await authed('/crm/organizations?pageSize=100');
        if (ro.ok) {
          var found = ((await ro.json()).items || []).filter(function (o) {
            return o.id === proposal.organizationId;
          })[0];
          orgName = found ? found.name : '';
        }
      } catch (e2) {}
    }
    ...
    try {
      var rp = await authed(
        '/crm/opportunities?pageSize=100' + (orgName ? '&q=' + encodeURIComponent(orgName) : ''),
      );
      if (rp.ok) {
        var opps = ((await rp.json()).items || []).filter(function (o) {
          return o.organizationId === proposal.organizationId;
        });
    ...
    var lines = hoistHardwareKit(
      (version.items || []).map(function (it) {
        return normalizeLine(it);
      }),
    );
    ...
    var stdNotes = [];
    try {
      var rn = await authed('/standard-notes');
      if (rn.ok) stdNotes = (await rn.json()) || [];
    } catch (e) {}
    if (!stdNotes.length) {
    ...
    stdNotes = stdNotes.filter(function (nn) {
      return nn.active !== false;
    });
    ...
    if (!(version.items || []).length) {
      ...
      stdNotes
        .filter(function (nn) {
          return nn.autoInclude && nn.placement !== 'FOOTER' && condOk(nn);
        })
        .forEach(function (nn) {
          lines.push(
            normalizeLine({
    ...
    if (!footerNotes) {
      ...
      footerNotes = stdNotes
        ...
        .map(function (nn) {
          return { title: nn.title, body: nn.body };
        });
    }
    pb = {
      ...
      meta: {
        contactName: meta.contactName || orgContact || '',
        shipTo: meta.shipTo || orgShipTo || '',
        billTo: meta.billTo || '',
        billSameAsShip: !meta.billTo || meta.billTo === (meta.shipTo || orgShipTo || ''),
        showTitle: meta.showTitle !== false,
        projectId: meta.projectId || importedProjectId || '',
        showProjectId: meta.showProjectId !== false,
        showDeposit: meta.showDeposit !== false,
        tbdTax: meta.tbdTax || '',
        tbdStructureFreight: meta.tbdStructureFreight || '',
        tbdMatsFreight: meta.tbdMatsFreight || '',
        proposalDate: propDate,
        taxAmountMinor: meta.taxAmountMinor || 0,
        discountPct: meta.discountPct || 0,
        discountMode: meta.discountMode === 'AMT' ? 'AMT' : 'PCT',
        discountAmountMinor: meta.discountAmountMinor || 0,
        structureFreightMinor:
          meta.structureFreightMinor != null ? meta.structureFreightMinor : meta.freightMinor || 0,
        matsFreightMinor: meta.matsFreightMinor || 0,
        stdFreightOn: !!meta.stdFreightOn,
        stdFreightMinor: meta.stdFreightMinor || 0,
        expiration: meta.expiration || addDays(propDate, 7),
        footerNotes: footerNotes,
        advAnswers: meta.advAnswers || null,
  ...
  function notesTriggeredBy(sku) {
    ...
    return ((pb && pb.stdNotes) || []).filter(function (nn) {
      ...
      return String(nn.triggerParts)
        .split(',')
        .some(function (p) {
          return p.trim().toUpperCase() === part;
        });
    });
  }
  ...
  function noteAlreadyPresent(nn) {
    return (
      (pb.lines || []).some(function (l) {
        return (
          l.lineType === 'NOTE' && String(l.name || '').trim() === String(nn.title || '').trim()
        );
      }) ||
  ...
  function applyTriggeredNotes(sku, atIndex) {
    ...
    notesTriggeredBy(sku).forEach(function (nn) {
      ...
      if (nn.placement === 'FOOTER') {
        pb.meta.footerNotes = (pb.meta.footerNotes || []).concat([
          { title: nn.title, body: nn.body },
        ]);
        added++;
        return;
      }
  ...
  function stdNoteFor(line) {
    var t = String((line && line.name) || '')
      .trim()
      .toLowerCase();
    if (!t) return null;
    return (
      ((pb && pb.stdNotes) || []).filter(function (nn) {
        return (
          String(nn.title || '')
            .trim()
            .toLowerCase() === t
    ...
    for (var i = pb.lines.length - 1; i >= 0; i--) {
      ...
      var alt = pb.stdNotes.filter(function (nn) {
        return (
          nn.placement !== 'FOOTER' &&
          nn.condition &&
          nn.condition !== src.condition &&
          noteConditionHolds(nn)
        );
      })[0];
      if (alt && !noteAlreadyPresent(alt)) {
        pb.lines[i] = normalizeLine({
          lineType: 'NOTE',
          kind: 'NOTE',
          name: alt.title,
          description: alt.body,
          quantity: 0,
          rateMinor: 0,
        });
      } else {
  ...
  function isGroupHeader(l) {
    return !!l && l.lineType === 'GROUP';
  }
  function isSubHeader(l) {
    return !!l && l.lineType === 'SUBGROUP';
  }
  /**
  ...
  function insertLineInOrder(line) {
    ...
    var gi = -1,
      si = -1;
    if (found && found.isSub) {
      si = found.index;
      gi = groupIndexBefore(si);
    ...
    if (gi === -1 && si === -1) {
      ...
      for (i = 0; i < pb.lines.length; i++) {
        ...
        if (gk !== null && gk > groupKey) {
          gAt = i;
          break;
        }
      }
      pb.lines.splice(gAt, 0, {
        ref: uid(),
        lineType: 'GROUP',
        kind: 'GROUP',
        name: groupLabel,
    ...
    if (si !== -1) {
      ...
      to = gEnd;
      for (i = from; i < gEnd; i++)
        if (isSubHeader(pb.lines[i]) || isGroupHeader(pb.lines[i])) {
          to = i;
          break;
        }
    } else if (subLabel && keys) {
      for (i = from; i < gEnd; i++) {
        if (isSubHeader(pb.lines[i]) && sameHeading(pb.lines[i].name, subLabel)) {
          si = i;
          break;
        }
      }
      if (si === -1) {
        ...
        for (i = from; i < gEnd; i++) {
          ...
          var sEnd = gEnd;
          for (j = i + 1; j < gEnd; j++)
            if (isSubHeader(pb.lines[j])) {
              sEnd = j;
              break;
            }
          var sk = spanTierKey(i + 1, sEnd, 0);
          if (sk !== null && sk > subKey) {
            sAt = i;
            break;
          }
        }
        pb.lines.splice(sAt, 0, {
          ref: uid(),
          lineType: 'SUBGROUP',
          kind: 'SUBGROUP',
          name: subLabel,
    ...
    if (key) {
      for (i = from; i < to; i++) {
        ...
        if (ld && ld.sortKey && String(ld.sortKey) > String(key)) {
          at = i;
          break;
        }
      }
    ...
    if (at === to)
      while (at > from && pb.lines[at - 1] && pb.lines[at - 1].lineType === 'NOTE') at--;
  ...
  function stalePricedLines() {
    return pb.lines.filter(function (l) {
      ...
      if (!d) return false;
      return (
        (!(Number(l.rateMinor) || 0) && d.priceMinor > 0) ||
  ...
  function freightControlsHtml() {
    ...
    var amtLabel =
      quote != null
        ? fmtMoney(quote, 'USD')
        : sent
          ? pb.meta.freightPending
            ? 'Awaiting the desk'
            : 'Not pulled yet'
          : 'Freight Needs to be Requested';
    ...
    return (
      ...
      '<button class="btn" id="bFreightReq" style="width:auto;padding:9px 16px;background:' +
      (sent ? '#2f6b4f' : '#9c3327') +
      ...
      (quote == null && !sent ? '12px' : '14px') +
      ';font-weight:600;color:' +
      (quote != null ? '#20241f' : sent ? '#8a8f85' : '#9c3327') +
      ';">' +
      ...
      '<span style="display:block;font-size:14px;font-weight:600;color:' +
      (matsFreight != null ? '#20241f' : '#8a8f85') +
      ';">' +
  ...
  async function requestFreight() {
    ...
    if (!item)
      return alert(
        'This proposal needs its Project ID — that is the monday.com deal item the weight is written to.',
      );
    var t = builderTotals();
    pb.meta.freightBusy = 'req';
    renderBuilderKeepingFocus();
    ...
    if (!r.ok) {
      renderBuilderKeepingFocus();
      return alert(
        'monday.com did not accept the request (' +
          r.status +
          '). The button stays red — nothing was written.',
      );
    }
    var d = await r.json();
  ...
  async function pullFreightAmount() {
    ...
    if (!item)
      return alert(
        'This proposal needs its Project ID — that is the monday.com deal item the amount is read from.',
      );
    pb.meta.freightBusy = 'amt';
    renderBuilderKeepingFocus();
    var r = await authed(
      '/proposals/' + pb.proposalId + '/freight-amount?itemId=' + encodeURIComponent(item),
    );
    pb.meta.freightBusy = '';
    if (!r.ok) {
      renderBuilderKeepingFocus();
      return alert('Could not read the freight amount from monday.com (' + r.status + ').');
    }
    var d = await r.json();
  ...
  async function autoSyncFreightOnOpen() {
    ...
    try {
      ...
      pb.meta.mondayMatsFreightMinor = d.matsFreightMinor;
      pb.meta.mondayMatsTaxMinor = d.matsTaxMinor;
      if (!pb.meta.matsFreightTouched && d.matsFreightMinor != null)
        pb.meta.matsFreightMinor = d.matsFreightMinor;
      if (!pb.meta.taxTouched && d.matsTaxMinor != null) pb.meta.taxAmountMinor = d.matsTaxMinor;
      renderBuilderKeepingFocus();
    } catch (e) {
      /* silent on open — see above */
    }
  }
  ...
  function repullCatalogFigures() {
    ...
    stale.forEach(function (l) {
      var d = itemDefaults[l.sku],
        bits = [];
      if (!(Number(l.rateMinor) || 0) && d.priceMinor) {
        l.rateMinor = d.priceMinor;
        bits.push('rate');
      }
      if (!(Number(l.costEach) || 0) && d.costMinor) {
        l.costEach = d.costMinor;
        bits.push('cost');
      }
      if (!(Number(l.weightEach) || 0) && d.weightLbs) {
        l.weightEach = d.weightLbs;
        bits.push('weight');
      }
  ...
  function priceWarningHtml() {
    ...
    if (stale.length) {
      out +=
        '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:14px;padding:10px 12px;background:#fdf6e6;border:1px solid #ecd9a6;border-radius:9px;font-size:12.5px;color:#6b5a24;">' +
        '<div style="flex:1;min-width:240px;"><b>' +
        stale.length +
        ' line' +
        (stale.length === 1 ? ' is' : 's are') +
        ' missing a figure the catalog has.</b> ' +
    ...
    if (unpriced.length) {
      out +=
        '<div style="margin-top:10px;padding:10px 12px;background:#fbecea;border:1px solid #e8c4bd;border-radius:9px;font-size:12.5px;color:#7d2f24;">' +
        '<b>' +
        unpriced.length +
        ' line' +
        (unpriced.length === 1 ? ' has' : 's have') +
        ' no price in the catalog either:</b> ' +
        '<code>' +
        unpriced
          .slice(0, 8)
          .map(function (l) {
  ...
  function builderTotals() {
    ...
    pb.lines.forEach(function (l) {
      if (l.lineType === 'GROUP') {
        cur = { name: l.name, optional: l.optional, subtotal: 0, cogs: 0 };
        groups.push(cur);
        return;
      }
    ...
    groups.forEach(function (g) {
      g.margin = g.subtotal - g.cogs;
      g.marginPct = g.subtotal ? Math.round((g.margin / g.subtotal) * 1000) / 10 : 0;
    });
    return {
      subtotal: subtotal,
      ...
      tpFreight: tpFreight,
      tax: tax,
      structureFreight: structureFreight,
      matsFreight: matsFreight,
      stdFreight: stdFreight,
      total: total,
      deposit: deposit,
      groups: groups,
      weight: weight,
      ...
      cogs: cogs,
      margin: revenue - cogs,
      marginPct: revenue ? Math.round(((revenue - cogs) / revenue) * 1000) / 10 : 0,
    };
  }
  ...
  function groupSubtotalMap() {
    ...
    pb.lines.forEach(function (l, i) {
      ...
      if (l.lineType === 'PRODUCT' && curIdx != null) {
        map[curIdx].rev +=
          (Number(l.quantity) || 0) * (Number(l.rateMinor) || 0) + (Number(l.tpFreightMinor) || 0);
  ...
  function renderBuilderKeepingFocus() {
    ...
    var sel =
      el && el.classList && el.classList.contains('bF')
        ? '.bF[data-i="' +
          el.getAttribute('data-i') +
          '"][data-k="' +
  ...
  function renderBuilder() {
    ...
    var view = document.getElementById('view');
    var lineRows = pb.lines
      .map(function (l, i) {
        return builderLineRow(l, i, gsub);
      })
      .join('');
    view.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">' +
      ...
      '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;">' +
      (isMock() ? 'Mock proposal' : 'Prepared for') +
      '</div>' +
      '<div style="font-size:17px;font-weight:600;color:#26303a;">' +
      esc(isMock() ? 'No customer — nothing is saved' : pb.orgName || '—') +
      '</div>' +
      (pb.number
        ? '<div class="muted" style="font-size:12.5px;">' + esc(pb.number) + '</div>'
        : '') +
      ...
      (isMock()
        ...
        : '<span style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;">Version</span>' +
          '<span style="font-size:13px;font-weight:600;color:#3d4a55;">v' +
          (pb.version || 1) +
          '</span>') +
      ...
      fieldRow(
        'Prepared for',
        '<input style="' + IN + 'background:#f2f3ef;" value="' + esc(pb.orgName) + '" disabled>',
      ) +
      fieldRow(
        'Contact name',
        '<input id="mContact" style="' +
          IN +
          '" placeholder="Full name of the customer contact" value="' +
          esc(pb.meta.contactName || '') +
          '">',
      ) +
      fieldRow(
        'Proposal date',
        '<input id="mPropDate" type="date" style="' +
          IN +
          '" value="' +
          esc(pb.meta.proposalDate) +
          '">',
      ) +
      fieldRow(
        'Project ID',
        '<input id="mProj" style="' + IN + '" value="' + esc(pb.meta.projectId) + '">',
      ) +
      fieldRow(
        'Expiration date',
      ...
      '<input type="checkbox" id="mBillSame"' +
      (pb.meta.billSameAsShip ? ' checked' : '') +
      '> same as ship to' +
      '</span>' +
      '</label>' +
      '<textarea id="mBill" rows="2" placeholder="Billing address" style="' +
      IN +
      'resize:vertical;">' +
      esc(pb.meta.billTo || '') +
      '</textarea></div>' +
      '<div class="field" style="margin-top:4px;"><label>Ship to</label><textarea id="mShip" rows="2" style="' +
      IN +
      'resize:vertical;">' +
      esc(pb.meta.shipTo) +
      '</textarea></div>' +
      '<label style="display:flex;align-items:center;gap:8px;font-size:13px;margin-top:8px;cursor:pointer;"><input type="checkbox" id="mShowTitle"' +
      (pb.meta.showTitle !== false ? ' checked' : '') +
      '> Show the proposal title on the customer proposal</label>' +
      priceWarningHtml() +
      ...
      freightControlsHtml() +
      '</div>' +
      '<label style="display:flex;align-items:center;gap:8px;font-size:13px;margin-top:8px;cursor:pointer;"><input type="checkbox" id="mShowProj"' +
      (pb.meta.showProjectId ? ' checked' : '') +
      '> Show Project ID on the customer proposal</label>' +
      ...
      '<label style="display:flex;align-items:center;gap:8px;font-size:13px;margin-top:6px;cursor:pointer;"><input type="checkbox" id="mShowDeposit"' +
      (pb.meta.showDeposit !== false ? ' checked' : '') +
      '> Show the ' +
      depositPct() +
      '% deposit on the customer proposal</label>' +
      '</div>' +
      // quick add
      ...
      (hardwareKitLine()
        ...
        : '') +
      '<select id="bAddNote" style="padding:9px 12px;border:1px solid #dcded7;border-radius:9px;font-size:13.5px;background:#fff;"><option value="">+ Standard Note…</option>' +
      (pb.stdNotes || [])
        .map(function (nn, ni) {
          return (
            '<option value="' +
            ...
            '">' +
            esc(nn.title) +
            (nn.placement === 'FOOTER' ? ' — footer' : '') +
            '</option>'
          );
        })
        .join('') +
      '<option value="__custom">Custom Note…</option></select>' +
      '</div>' +
      ...
      STD_GROUPS.map(function (g) {
        return (
          '<button class="grpChip" data-g="' +
          esc(g) +
          '" style="border:1px solid #dcded7;background:#fff;border-radius:999px;padding:6px 12px;font-size:12.5px;cursor:pointer;color:#3d4a55;">' +
          esc(g) +
          '</button>'
        );
      }).join('') +
      ...
      ((pb.meta.advWarnings || []).length
        ? '<div class="card" style="margin-bottom:12px;background:#fbecea;border:1px solid #f0d5d0;">' +
          ...
          (pb.meta.advWarnings || [])
            .map(function (w) {
              return (
                '<div style="font-size:12.5px;color:#7d2a20;line-height:1.6;">' + esc(w) + '</div>'
              );
            })
            .join('') +
          '</div>'
      ...
      '<div id="bLines" style="display:flex;flex-direction:column;gap:8px;">' +
      (lineRows ||
        '<div class="placeholder" style="padding:26px;"><p class="muted" style="margin:0;">No lines yet. Add a product line or load a template.</p></div>') +
      '</div>' +
      // totals
      '<div class="card" style="margin-top:16px;max-width:390px;margin-left:auto;">' +
      '<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:14px;"><span class="muted">Subtotal</span><span>' +
      fmtUsd(t.subtotal) +
      '</span></div>' +
      ...
      '<option value="PCT"' +
      (pb.meta.discountMode === 'AMT' ? '' : ' selected') +
      '>%</option>' +
      '<option value="AMT"' +
      (pb.meta.discountMode === 'AMT' ? ' selected' : '') +
      '>$</option>' +
      '</select>' +
      '<input id="mDisc" style="width:80px;padding:5px 8px;border:1px solid #dcded7;border-radius:7px;text-align:right;" value="' +
      esc(
        pb.meta.discountMode === 'AMT'
          ? ((Number(pb.meta.discountAmountMinor) || 0) / 100).toFixed(2)
          : pb.meta.discountPct,
      ) +
      '">' +
      '</div></div>' +
      (t.discount
        ? '<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:14px;color:#9c3327;"><span>' +
          discountLabel(t) +
          '</span><span>− ' +
          fmtMoney(t.discount, 'USD') +
          '</span></div>' +
          '<div style="font-size:11px;color:#8a8f85;text-align:right;margin-bottom:2px;">Discount expires ' +
          (pb.meta.expiration ? fmtDate(pb.meta.expiration) : 'with the proposal') +
          '</div>'
        : '') +
      optionalAmountRow(
        'Mat Freight Tax Pass-Through',
        'mTax',
      ...
      (isMock()
        ? ''
        : optionalAmountRow(
            'Structure Crating &amp; Freight $',
            'mStructFreight',
            pb.meta.structureFreightMinor,
            'mStructFreightTbd',
            pb.meta.tbdStructureFreight,
          ) +
          optionalAmountRow(
            'Mats &amp; Padding Freight $',
            'mMatsFreight',
            pb.meta.matsFreightMinor,
            'mMatsFreightTbd',
            pb.meta.tbdMatsFreight,
          ) +
          ...
          stdFreightRow()) +
      '<div style="display:flex;justify-content:space-between;padding:8px 0 0;margin-top:6px;border-top:1px solid #e7e8e3;font-size:16px;font-weight:600;font-family:\'Newsreader\',serif;"><span>Total</span><span>' +
      fmtUsd(t.total) +
      '</span></div>' +
      (isMock()
        ? '<div class="muted" style="font-size:11.5px;text-align:right;margin-top:4px;line-height:1.5;">Product retail only. Crating, freight and tax are quoted on a real proposal.</div>'
        : '') +
      (pb.meta.showDeposit !== false
        ? '<div style="display:flex;justify-content:space-between;padding:6px 0 0;font-size:14px;color:#3d4a55;font-weight:600;"><span>Deposit due (' +
          depositPct() +
          '%)</span><span>' +
          fmtUsd(t.deposit) +
          '</span></div>'
        : '<div style="display:flex;justify-content:space-between;padding:6px 0 0;font-size:12.5px;color:#8a8f85;"><span>Deposit</span><span>Not shown on the proposal</span></div>') +
      ...
      (isMock()
        ? ''
        : '<div id="bMarginRail" style="' +
          marginRailStyle() +
          '">' +
          marginCard(t) +
          '<div id="bRfqRail"></div><div id="bDatesRail"></div><div id="bNotesRail"></div></div>');
    wireBuilder();
  }
  ...
  function footerNotesCard() {
    ...
    var rows = fn
      .map(function (n, i) {
        return (
          ...
          '<div style="flex:1;"><input class="bFN" data-i="' +
          i +
          '" data-k="title" value="' +
          esc(n.title || '') +
          '" placeholder="Note title (optional)" style="width:100%;border:none;background:transparent;font-weight:600;font-size:13.5px;outline:none;margin-bottom:4px;">' +
          '<textarea class="bFN" data-i="' +
          i +
          '" data-k="body" rows="3" placeholder="Note text — **bold** supported" style="width:100%;border:1px solid #ece9db;border-radius:7px;padding:6px 8px;font-size:12.5px;font-family:inherit;resize:vertical;background:#fff;">' +
          esc(n.body || '') +
          '</textarea></div>' +
          '<button class="bFNDel" data-i="' +
          i +
    ...
    return (
      ...
      (rows || '<div class="muted" style="font-size:12.5px;">None yet.</div>') +
      '</div>'
    );
  }
  ...
  function marginCard(t) {
    function stat(label, value, color, big) {
      return (
        ...
        '<span style="font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;">' +
        label +
        '</span>' +
        '<span style="font-size:' +
        (big ? '19px' : '15px') +
        ";font-weight:600;font-family:'Newsreader',serif;color:" +
        (color || '#20241f') +
        ';">' +
        value +
        '</span></div>'
    ...
    var rows = t.groups
      .map(function (g) {
        ...
        return (
          '<div style="padding:8px 0;border-bottom:1px solid #ece7d8;">' +
          '<div style="font-size:12px;font-weight:600;line-height:1.35;">' +
          esc(tc(stripOptional(g.name || 'Untitled section'))) +
          (g.optional ? ' <span style="font-weight:400;color:#8a8f85;">(Optional)</span>' : '') +
          '</div>' +
    ...
    if (isMock()) return '';
    return (
      '<div class="card" style="border:1px solid #e4dfd0;background:#fdfcf7;">' +
      ...
      (rows
        ? '<div style="margin-top:12px;padding-top:10px;border-top:1px solid #ece7d8;">' +
          '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;margin-bottom:2px;">By section</div>' +
          rows +
          '</div>'
        : '<div class="muted" style="font-size:12px;margin-top:10px;">Add a section heading to see per-section margin.</div>') +
      (t.cogs === 0
        ? '<div style="margin-top:10px;font-size:11.5px;color:#8a6d1f;line-height:1.5;">No costs recorded yet — add unit costs in Catalog → Pricing &amp; SKUs, or type a cost on any line.</div>'
        : '') +
      '</div>'
    );
  }
  ...
  function rfqStatusChip(status) {
    var map = {
      DRAFT: ['#8a6d1f', '#fdf6e6', 'Draft'],
      SENT: ['#2f7d5d', '#eaf4ef', 'Sent'],
      SUPERSEDED: ['#8a8f85', '#f2f3ef', 'Superseded'],
    };
  ...
  function rfqOverlay(title, bodyHtml, footHtml) {
    var ov = document.createElement('div');
    ov.style.cssText =
      'position:fixed;inset:0;background:rgba(32,36,31,.34);display:flex;align-items:flex-start;justify-content:center;padding:40px 16px;z-index:60;overflow:auto;';
    ov.innerHTML =
      '<div style="width:100%;max-width:720px;background:#fbfbf9;border:1px solid #e7e8e3;border-radius:16px;box-shadow:0 24px 60px -20px rgba(32,36,31,.4);padding:22px 24px 20px;">' +
    ...
    ov.err = function (msg) {
      ov.querySelector('#rfqErr').innerHTML = '<div class="err">' + esc(msg) + '</div>';
    };
    ov.addEventListener('mousedown', function (e) {
  ...
  function rfqCardHtml() {
    if (!rfqData) {
      return (
        '<div class="card" style="margin-top:14px;"><div class="section-title" style="margin:0;">Freight requests</div>' +
        '<div class="muted" style="font-size:12px;margin-top:6px;">Loading\u2026</div></div>'
      );
    }
    var waiting = rfqData.vendors.filter(function (v) {
      return v.rfqEnabled && !v.existingRfqId;
    });
    var prompt = freightActionHtml();
    if (!prompt && waiting.length) {
      var names = waiting.map(function (v) {
        return v.vendor;
      });
      var list =
        names.length > 1
          ...
          : names[0];
      prompt =
        '<div style="background:#fdf6e6;border:1px solid #ecd9a6;border-radius:10px;padding:10px 11px;margin-bottom:10px;">' +
        '<div style="font-size:12.5px;line-height:1.5;color:#6b5a24;">' +
        esc(list) +
        ' ' +
        (names.length > 1 ? 'supply' : 'supplies') +
        ' parts on this proposal and quote their own freight.</div>' +
    ...
    var rows = rfqData.rfqs
      .map(function (r) {
        var items = r.items
          .slice(0, 4)
          .map(function (i) {
            return i.quantity + '\u00d7 ' + i.sku;
          })
          .join(' \u00b7 ');
        if (r.items.length > 4) items += ' \u00b7 +' + (r.items.length - 4) + ' more';
        return (
          '<div style="padding:9px 0;border-bottom:1px solid #ece7d8;">' +
          ...
          esc(r.reference) +
          '</span><span>' +
          rfqDate(r.requestedAt) +
          '</span></div>' +
          '<div style="font-size:11px;color:#8a8f85;margin-top:3px;line-height:1.45;">' +
          r.itemCount +
          ' item' +
          (r.itemCount === 1 ? '' : 's') +
          (items ? ' \u2014 ' + esc(items) : '') +
          ...
          '<button class="link-btn rfqOpen" data-id="' +
          r.id +
          '" style="width:auto;padding:5px 10px;font-size:12px;">' +
          (r.status === 'DRAFT' ? 'Edit &amp; send' : 'View') +
          '</button>' +
          (r.status === 'SENT'
            ? '<button class="link-btn rfqResend" data-id="' +
              r.id +
              '" style="width:auto;padding:5px 10px;font-size:12px;">Send again</button>'
            : '') +
          (r.status === 'SENT'
            ? '<button class="link-btn rfqRev" data-id="' +
              r.id +
              '" style="width:auto;padding:5px 10px;font-size:12px;">Revise</button>'
            : '') +
          '</div></div>'
        );
      })
    ...
    return (
      '<div class="card" style="margin-top:14px;border:1px solid #e4dfd0;background:#fdfcf7;">' +
      '<div class="section-title" style="margin:0 0 2px;">Freight requests</div>' +
      '<div class="muted" style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px;">Internal only \u2014 not printed</div>' +
      (rfqData.error
        ? '<div style="background:#fbecea;border:1px solid #f0ccc6;color:#9c3327;font-size:12px;line-height:1.5;padding:8px 10px;border-radius:9px;margin-bottom:10px;">' +
          esc(rfqData.error) +
          '</div>'
        : '') +
      prompt +
  ...
  function wireRfqCard() {
    ...
    rail.querySelectorAll('.rfqOpen').forEach(function (b) {
      b.addEventListener('click', function () {
        openRfqEditor(b.getAttribute('data-id'));
      });
    });
    ...
    rail.querySelectorAll('.rfqResend').forEach(function (b) {
      b.addEventListener('click', function () {
        openRfqSend(b.getAttribute('data-id'));
      });
    });
    rail.querySelectorAll('.rfqRev').forEach(function (b) {
      b.addEventListener('click', async function () {
        b.disabled = true;
        try {
          var next = await rfqApi('/rfqs/' + b.getAttribute('data-id') + '/revision', {
            method: 'POST',
            body: {},
          });
  ...
  async function loadCustomerNotes(force) {
    ...
    if (!force && cnData && cnData.organizationId === pb.orgId) {
      renderCustomerRail();
      return;
    }
    ...
    try {
      var r = await authed(
        '/crm/organizations/' +
          pb.orgId +
          '/notes?proposalId=' +
          encodeURIComponent(pb.proposalId || ''),
      );
      cnData = r.ok ? await r.json() : { error: 'Could not load internal notes.' };
    } catch (e) {
      cnData = { error: 'Could not load internal notes.' };
    }
    renderCustomerRail();
  }
  ...
  function cnDatesCardHtml() {
    ...
    var box =
      'width:100%;padding:6px 8px;border:1px solid #dcded7;border-radius:7px;font-size:13px;background:#fff;';
    ...
    return (
      ...
      lbl +
      '">From</label>' +
      '<input type="date" id="cnFrom" value="' +
      esc(d.decisionFrom || '') +
      '" style="' +
      box +
      '"></div>' +
      ...
      '<input type="date" id="cnFollow" value="' +
      esc(d.followUpDate || '') +
      '" style="' +
      box +
      '">' +
  ...
  function cnNoteHtml(n, me, admin) {
    ...
    return (
      '<div style="padding:9px 0;border-bottom:1px solid #ece7d8;">' +
      '<div style="font-size:12.5px;line-height:1.55;white-space:pre-wrap;color:#20241f;">' +
      esc(n.body) +
      '</div>' +
      '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin-top:4px;">' +
      '<span style="font-size:11px;color:#8a8f85;">' +
      esc(n.authorName || 'Unknown') +
      ' · ' +
      esc(cnStamp(n.createdAt)) +
      (n.proposalNumber ? ' · ' + esc(n.proposalNumber) : '') +
      '</span>' +
      (mine || admin
        ? '<button class="cnDel" data-id="' +
          n.id +
          '" title="Remove this note" style="border:none;background:transparent;color:#9c3327;font-size:11px;cursor:pointer;padding:0;">Remove</button>'
        : '') +
      '</div></div>'
    );
  }
  ...
  function cnNotesCardHtml() {
    ...
    if (cnData.error)
      return (
        head +
        '<div style="font-size:12px;color:#9c3327;line-height:1.5;">' +
        esc(cnData.error) +
        '</div></div>'
      );
    ...
    var sub =
      'font-size:11px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;margin:12px 0 2px;';
  ...
  async function cnSaveDates(patch) {
    ...
    try {
      var r = await authed('/crm/organizations/' + pb.orgId + '/dates', {
        method: 'PATCH',
        body: patch,
      });
      if (!r.ok) {
  ...
  function wireCustomerRail() {
    ...
    document.querySelectorAll('.cnDel').forEach(function (b) {
      b.addEventListener('click', async function () {
        if (!confirm('Remove this note? The log cannot be edited, so this cannot be undone.'))
          return;
        b.disabled = true;
        try {
          var r = await authed('/customer-notes/' + b.getAttribute('data-id'), {
            method: 'DELETE',
          });
  ...
  function freightStateOf(sku) {
    ...
    var k = String(sku || '')
      ...
      .toLowerCase();
    if (!cov || !k) return '';
    var hit = (cov.lines || []).filter(function (l) {
      return (
        String(l.sku || '')
          .trim()
          .toLowerCase() === k
  ...
  function freightMarkHtml(sku) {
    ...
    if (st === 'REQUESTED')
      return '<span title="Freight requested" style="font-size:12px;line-height:1;">\uD83D\uDC4D</span>';
    if (st === 'DRAFT')
      return '<span title="On a freight request that has not been sent" style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#d9a520;"></span>';
    if (st === 'PENDING')
      return '<span title="No freight quote requested" style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#c8483a;"></span>';
    return '';
  }
  ...
  function freightActionHtml() {
    ...
    (cov.pendingVendors || []).forEach(function (v) {
      ...
      var btn =
        v.existingStatus === 'SENT'
          ? '<button class="btn rfqRev" data-id="' +
            v.existingRfqId +
            '" style="width:auto;padding:8px 14px;margin-top:9px;font-size:13px;background:#9c3327;">Revise ' +
            esc(v.existingReference || 'the request') +
            '</button>'
          : v.existingStatus === 'DRAFT'
            ? '<button class="btn rfqOpen" data-id="' +
              v.existingRfqId +
              '" style="width:auto;padding:8px 14px;margin-top:9px;font-size:13px;background:#9c3327;">Finish and send the request</button>'
      ...
      out +=
        '<div style="background:#fbecea;border:1px solid #f0ccc6;border-radius:10px;padding:10px 11px;margin-bottom:8px;">' +
        '<div style="font-size:12.5px;line-height:1.5;color:#7d2a20;"><b>' +
        esc(v.vendor) +
        '</b> \u2014 ' +
        v.lineCount +
        ' item' +
        many +
        ' with no freight request' +
        (v.existingStatus === 'SENT'
          ? ' since ' + esc(v.existingReference || 'the last request') + ' went out'
          : '') +
        '.</div>' +
        btn +
        '</div>';
    ...
    Object.keys(drafts).forEach(function (k) {
      var d = drafts[k];
      out +=
        '<div style="background:#fdf6e6;border:1px solid #ecd9a6;border-radius:10px;padding:10px 11px;margin-bottom:8px;">' +
        '<div style="font-size:12.5px;line-height:1.5;color:#6b5a24;"><b>' +
        esc(d.reference || 'A request') +
        '</b> for ' +
        esc(d.vendor || 'this vendor') +
        ' has been raised but never sent \u2014 ' +
        d.count +
        ' item' +
        (d.count === 1 ? '' : 's') +
        '.</div>' +
        '<button class="btn rfqOpen" data-id="' +
        d.id +
        '" style="width:auto;padding:8px 14px;margin-top:9px;font-size:13px;">Finish and send</button></div>';
    });
    return out;
  }
  ...
  function freightRemovedHtml() {
    ...
    return (
      '<div style="border:1px dashed #f0ccc6;background:#fdf7f6;border-radius:10px;padding:10px 11px;margin-bottom:10px;">' +
      ...
      cov.removed
        .map(function (r) {
          return (
            '<div style="font-size:12px;line-height:1.5;color:#9c3327;text-decoration:line-through;text-decoration-color:#c8483a;">' +
            (Number(r.quantity) || 0) +
            '\u00d7 ' +
            esc(r.sku) +
            ' \u2014 ' +
            esc(r.name) +
            '</div>' +
            '<div style="font-size:11px;color:#8a8f85;margin:1px 0 6px;">' +
            esc(r.reference) +
            ' \u00b7 ' +
            esc(r.vendor) +
            '</div>'
  ...
  function rfqLineSignature() {
    ...
    return pb.lines
      .filter(function (l) {
        return (l.lineType || 'PRODUCT') === 'PRODUCT' && l.sku && !l.optional;
      })
      .map(function (l) {
        return l.sku + ':' + (Number(l.quantity) || 0);
      })
      .join('|');
  }
  ...
  async function loadRfqPanel(force) {
    ...
    if (!force && rfqData && rfqData.versionId === pb.versionId && rfqData.sig === sig) {
      renderRfqRail();
      return;
    }
    if (!force) {
      clearTimeout(loadRfqPanel._t);
      loadRfqPanel._t = setTimeout(function () {
        loadRfqPanel(true);
      }, 400);
      renderRfqRail();
      return;
    }
    if (loadRfqPanel._busy) {
      loadRfqPanel._again = true;
      return;
    }
    ...
    try {
      ...
      var vendors = await rfqApi('/proposals/versions/' + versionId + '/rfq/vendors', {
        method: 'POST',
        body: body,
      });
      var rfqs = await rfqApi('/proposals/' + pb.proposalId + '/rfqs');
      var cov = await rfqApi('/proposals/versions/' + versionId + '/freight-coverage', {
        method: 'POST',
        body: body,
      });
      rfqData = {
    ...
    renderRfqRail();
    if (loadRfqPanel._again) {
      loadRfqPanel._again = false;
      loadRfqPanel(true);
    }
  }

  function openRfqVendorPicker() {
    var choices = rfqData.vendors.filter(function (v) {
      return v.rfqEnabled && !v.existingRfqId;
    });
    var others = rfqData.vendors.filter(function (v) {
      return !v.rfqEnabled && !v.existingRfqId;
    });
    var rows = choices
      .map(function (v, i) {
        return (
          '<label style="display:flex;align-items:flex-start;gap:9px;padding:9px 10px;border:1px solid #e7e8e3;border-radius:10px;background:#fff;margin-bottom:7px;cursor:pointer;">' +
          '<input type="checkbox" class="rfqV" data-v="' +
          esc(v.vendor) +
          '" checked style="margin-top:2px;">' +
          '<span style="flex:1;"><b style="font-size:13.5px;">' +
          esc(v.vendor) +
          '</b>' +
    ...
    var note = others.length
      ? '<div class="muted" style="font-size:11.5px;line-height:1.5;margin-top:4px;">' +
        esc(
          others
            .map(function (v) {
              return v.vendor;
        ...
        ' also supply parts here but are not set up for freight requests. Turn that on in Settings \u2192 Manufacturers.</div>'
      : '';

    openModal(
      'Request freight quotes',
      '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-bottom:12px;">One request per vendor. You pick the items on the next screen.</div>' +
        rows +
        note,
      async function (close, showErr) {
        var picked = [].slice
          .call(document.querySelectorAll('.rfqV'))
          .filter(function (c) {
            return c.checked;
          })
          .map(function (c) {
        ...
        try {
          ...
          for (var i = 0; i < picked.length; i++) {
            var made = await rfqApi('/proposals/versions/' + pb.versionId + '/rfqs', {
              method: 'POST',
              body: { vendor: picked[i] },
            });
  ...
  async function openRfqEditor(rfqId) {
    ...
    function lineRows() {
      return m.lines
        .map(function (l) {
          return (
            '<tr style="border-bottom:1px solid #eef0ea;">' +
            '<td style="padding:7px 8px;"><input type="checkbox" class="rfqL" data-id="' +
            l.id +
            '"' +
            (l.included ? ' checked' : '') +
            (editable ? '' : ' disabled') +
            '></td>' +
            '<td style="padding:7px 8px;font-family:ui-monospace,monospace;font-size:11.5px;white-space:nowrap;">' +
            esc(l.sku) +
            '</td>' +
            '<td style="padding:7px 8px;font-size:12.5px;">' +
            esc(l.name) +
            '</td>' +
            '<td style="padding:7px 8px;text-align:right;font-size:12.5px;">' +
            l.quantity +
            '</td>' +
            '<td style="padding:7px 8px;text-align:right;font-size:12.5px;">' +
            fmtMoney(l.unitCostMinor, '') +
            '</td>' +
            '<td style="padding:7px 8px;text-align:right;font-size:12.5px;font-variant-numeric:tabular-nums;">' +
            fmtMoney(l.extendedCostMinor, '') +
            '</td>' +
            '</tr>'
          );
        })
    ...
    var body =
      ...
      esc(m.reference) +
      '</span>' +
      rfqStatusChip(m.status) +
      '</div>' +
      (editable
        ? ''
        : '<div style="background:#f2f3ef;border:1px solid #e7e8e3;border-radius:10px;padding:9px 11px;font-size:12.5px;color:#5c6157;line-height:1.5;margin-bottom:12px;">' +
          'This request has been sent, so its items are locked. <b>Send again</b> emails this same document a second time — it goes out as ' +
          esc(m.reference.replace(/ S\d+$/, '')) +
          ' S' +
          ((m.submission || 1) + 1) +
          '. <b>Revise</b> raises ' +
          esc(m.reference.replace(/ R\d+$/, '').replace(/ S\d+$/, '')) +
          ' R' +
          (m.revision + 1) +
          ' with these items carried over, for when the shipment itself has changed.</div>') +
      ...
      '</tr></thead><tbody id="rfqLines">' +
      lineRows() +
      '</tbody></table>' +
      '<div style="display:flex;justify-content:flex-end;gap:10px;padding:10px 8px 0;font-size:14px;font-weight:600;"><span>Total</span><span id="rfqTotal" style="font-variant-numeric:tabular-nums;min-width:110px;text-align:right;">' +
      fmtMoney(m.totalCostMinor, 'USD') +
      '</span></div>' +
      '<div class="field" style="margin-top:14px;"><label>Special notes for this request</label>' +
      '<textarea id="rfqNotes" rows="3"' +
      (editable ? '' : ' disabled') +
      ' placeholder="Anything the vendor needs to know \u2014 dock access, delivery window, liftgate." style="' +
      IN +
      'resize:vertical;">' +
      esc(m.notes || '') +
      '</textarea></div>';
    ...
    var foot =
      '<button class="link-btn" id="rfqClose" style="width:auto;padding:10px 16px;">Close</button>' +
    ...
    ov.querySelectorAll('.rfqL').forEach(function (c) {
      c.addEventListener('change', async function () {
        try {
          var res = await rfqApi('/rfqs/' + rfqId + '/lines/' + c.getAttribute('data-id'), {
            method: 'PATCH',
            body: { included: c.checked },
          });
  ...
  async function openRfqSend(rfqId) {
    var d;
    try {
      d = await rfqApi('/rfqs/' + rfqId + '/send-defaults');
    } catch (e) {
      alert(e.message);
      return;
    }
    openModal(
      ...
      '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-bottom:12px;">The RFQ is attached as a PDF. Replies come back to sales@summitsensory.com. Sending locks this request.</div>' +
        fieldRow(
          'To',
          '<input id="rfqTo" type="text" style="' +
            IN +
            '" value="' +
            esc(d.to) +
            '" placeholder="freight@vendor.com">',
        ) +
        fieldRow(
          'Cc',
          '<input id="rfqCc" type="text" style="' +
            IN +
            '" value="' +
            esc(d.cc) +
        ...
        fieldRow(
          'Subject',
          '<input id="rfqSubj" style="' + IN + '" value="' + esc(d.subject) + '">',
        ) +
        '<div class="field"><label>Message</label><textarea id="rfqBodyTxt" rows="8" style="' +
        IN +
        'resize:vertical;">' +
        esc(d.body) +
        '</textarea></div>' +
        (d.to
          ? ''
          : '<div class="muted" style="font-size:11.5px;line-height:1.5;">No address is stored for this vendor. Add one in Settings \u2192 Manufacturers and it will be filled in next time.</div>'),
      async function (close, showErr) {
  ...
  function builderLineRow(l, i, gsub) {
    var handle =
      '<div class="bDrag" style="cursor:grab;color:#c2c6bd;font-size:18px;padding:0 4px;user-select:none;" title="Drag to reorder">⋮⋮</div>';
    var del =
      '<button class="bDel" data-i="' +
      i +
      '" style="border:1px solid #e0e1db;background:#fff;border-radius:8px;width:30px;height:30px;color:#9c3327;cursor:pointer;flex:0 0 auto;">✕</button>';
    ...
    var noteBtn =
      '<button class="bAddNote" data-i="' +
      i +
      '" title="Add a note directly under this line" style="border:1px solid #e0e1db;background:#fff;border-radius:8px;height:30px;padding:0 9px;color:#5c6157;cursor:pointer;flex:0 0 auto;font-size:11.5px;white-space:nowrap;">+ Note</button>';
    var noteBtnLight = noteBtn
      .replace(
        'border:1px solid #e0e1db;background:#fff',
        'border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.12)',
      )
      .replace('color:#5c6157', 'color:#e6ebef');
    if (l.lineType === 'GROUP') {
      var g = (gsub && gsub[i]) || { rev: 0, cogs: 0 };
      var gMargin = g.rev - g.cogs;
      var gPct = g.rev ? Math.round((gMargin / g.rev) * 1000) / 10 : 0;
      return (
        '<div class="bRow" draggable="true" data-i="' +
        i +
        '" style="background:#3d4a55;border:1px solid #33404a;border-radius:10px;padding:9px 10px;color:#fff;">' +
        '<div style="display:flex;align-items:center;gap:8px;">' +
        handle.replace('#c2c6bd', '#8fa0ac') +
        '<input class="bF" data-i="' +
        i +
        '" data-k="name" value="' +
        esc(l.name) +
        '" placeholder="SECTION HEADING" style="flex:1;border:none;background:transparent;font-weight:700;font-size:13px;letter-spacing:.03em;text-transform:uppercase;color:#fff;outline:none;">' +
        '<input class="bF" data-i="' +
        i +
        '" data-k="description" value="' +
        esc(l.description || '') +
        '" placeholder="Heading note (e.g. Frame Dimensions: 10\' × 10\')" style="flex:0 1 250px;border:none;background:rgba(255,255,255,.1);border-radius:7px;padding:5px 8px;font-size:11.5px;color:#e6ebef;outline:none;">' +
        '<label style="display:flex;align-items:center;gap:5px;font-size:11px;color:#cdd6dc;white-space:nowrap;cursor:pointer;"><input type="checkbox" class="bChk" data-i="' +
        i +
        '" data-k="optional"' +
        (l.optional ? ' checked' : '') +
        '> Optional</label>' +
        '<span style="font-size:12.5px;font-weight:600;color:#cdd6dc;min-width:90px;text-align:right;">' +
        fmtMoney(g.rev, 'USD') +
        '</span>' +
        noteBtnLight +
        del
          .replace('#9c3327', '#f0b8ae')
          .replace('background:#fff', 'background:rgba(255,255,255,.12)')
          .replace('border:1px solid #e0e1db', 'border:1px solid rgba(255,255,255,.25)') +
        '</div>' +
        (isMock()
          ? ''
          : '<div style="display:flex;gap:16px;justify-content:flex-end;font-size:11px;color:#a9bac6;padding:6px 40px 0 0;">' +
            '<span>Revenue <b style="color:#fff;font-weight:600;">' +
            fmtMoney(g.rev, '') +
            '</b></span>' +
            '<span>COGS <b style="color:#fff;font-weight:600;">' +
            fmtMoney(g.cogs, '') +
            '</b></span>' +
            '<span>Margin <b style="color:' +
            (gMargin >= 0 ? '#9fe0c4' : '#f0b8ae') +
            ';font-weight:600;">' +
            fmtMoney(gMargin, '') +
            ' · ' +
            gPct +
    ...
    if (l.lineType === 'SUBGROUP') {
      return (
        '<div class="bRow" draggable="true" data-i="' +
        i +
        '" style="display:flex;align-items:center;gap:8px;background:#eef0ea;border:1px solid #e2e5dd;border-radius:9px;padding:7px 10px;margin-left:14px;">' +
        handle +
        '<input class="bF" data-i="' +
        i +
        '" data-k="name" value="' +
        esc(l.name) +
        '" placeholder="Sub-heading" style="flex:1;border:none;background:transparent;font-weight:600;font-size:13px;color:#3d4a55;outline:none;">' +
        ...
        '" data-k="description" value="' +
        esc(l.description || '') +
        '" placeholder="Sub-heading note" style="flex:0 1 260px;border:1px solid #dfe3da;background:#fff;border-radius:7px;padding:5px 8px;font-size:11.5px;color:#3d4a55;outline:none;">' +
        noteBtn +
        del +
        '</div>'
      );
    ...
    if (l.lineType === 'NOTE') {
      ...
      return (
        '<div class="bRow" draggable="true" data-i="' +
        i +
        '" style="display:flex;align-items:flex-start;gap:8px;background:#fbfaf4;border:1px solid #ece9db;border-radius:10px;padding:10px;' +
        (noteIndent ? 'margin-left:' + noteIndent + 'px;' : '') +
        '">' +
        handle +
        '<div style="flex:1;"><input class="bF" data-i="' +
        i +
        '" data-k="name" value="' +
        esc(l.name) +
        '" placeholder="Note title" style="width:100%;border:none;background:transparent;font-weight:600;font-size:13.5px;outline:none;margin-bottom:4px;">' +
        '<textarea class="bF" data-i="' +
        i +
        '" data-k="description" rows="3" placeholder="Note text" style="width:100%;border:1px solid #ece9db;border-radius:7px;padding:6px 8px;font-size:12.5px;font-family:inherit;resize:vertical;background:#fff;">' +
        esc(l.description) +
        '</textarea>' +
        '<div style="font-size:10.5px;color:#8a8f85;margin-top:3px;">Formatting: <b>**bold**</b> · <i>*italic*</i> · line breaks are kept · HTML: &lt;ul&gt;&lt;li&gt; &lt;b&gt; &lt;i&gt; &lt;a href&gt;</div></div>' +
        del +
        '</div>'
      );
    }
    ...
    var freightTbdBlock = showsFreightTbd(l)
      ? '<div style="background:#fdf6e6;border:1px solid #ecd9a6;border-radius:8px;padding:9px 11px;margin-bottom:10px;">' +
        ...
        '<div style="font-size:12px;color:#6b5a24;line-height:1.55;">' +
        esc(FREIGHT_TBD_NOTE) +
        '</div>' +
    ...
    var notesPanel = l.showNotes
      ? '<div style="margin-top:10px;padding:10px;background:#f7f8f4;border:1px solid #eef0ea;border-radius:9px;">' +
        ...
        '<div><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Delivery timeline</label><input class="bF" data-i="' +
        i +
        '" data-k="delivery" value="' +
        esc(l.delivery) +
        '" placeholder="e.g. 8–10 weeks" style="' +
        IN +
        'padding:7px 9px;"></div>' +
        '<div><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Returnable</label>' +
        ynSelect(i, 'returnable', l.returnable) +
        '</div>' +
        '<div><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Additional freight charges apply</label>' +
        ynSelect(i, 'addlFreight', l.addlFreight) +
        '</div>' +
        '<div><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Freight charges calculated</label>' +
        ynSelect(i, 'freightCalc', l.freightCalc) +
        '</div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-top:8px;align-items:flex-end;">' +
        '<div style="flex:1;"><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">3rd-party freight line (shown under item)</label><input class="bF" data-i="' +
        i +
        '" data-k="tpFreightLabel" value="' +
        esc(l.tpFreightLabel) +
        '" placeholder="e.g. Steamroller Ramp freight" style="' +
        IN +
        'padding:7px 9px;"></div>' +
        '<div style="width:120px;"><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Freight $</label><input class="bF" data-i="' +
        i +
        '" data-k="tpFreight" value="' +
        m2d(l.tpFreightMinor) +
        '" style="' +
        IN +
        'padding:7px 9px;text-align:right;"></div>' +
        '</div>' +
        '</div>'
      : '';
    return (
      '<div class="bRow" draggable="true" data-i="' +
      i +
      '" style="background:#fff;border:1px solid #e7e8e3;border-radius:10px;padding:10px;">' +
      ...
      '" data-k="name" value="' +
      esc(l.name) +
      '" placeholder="Product / activity" style="flex:1;border:none;font-weight:600;font-size:14px;outline:none;">' +
      '<input class="bF" data-i="' +
      i +
      '" data-k="sku" value="' +
      esc(l.sku) +
      '" placeholder="SKU" style="width:130px;border:1px solid #eef0ea;border-radius:6px;padding:3px 7px;font-size:11.5px;color:#5c6157;font-family:ui-monospace,monospace;">' +
      '</div>' +
      '<textarea class="bF" data-i="' +
      i +
      '" data-k="description" rows="2" placeholder="Description" style="width:100%;border:1px solid #eef0ea;border-radius:7px;padding:6px 8px;font-size:12.5px;font-family:inherit;resize:vertical;color:#4a4f47;">' +
      esc(l.description) +
      '</textarea>' +
      ...
      (l.internalNote
        ? '<div style="margin-top:6px;display:flex;gap:7px;align-items:flex-start;background:#fdf6e6;border:1px solid #ecd9a6;border-radius:7px;padding:6px 9px;font-size:11.5px;color:#6b5a24;line-height:1.5;">' +
          '<b style="font-weight:700;white-space:nowrap;">Internal</b><span>' +
          esc(l.internalNote) +
          '</span></div>'
        : '') +
      '<div style="display:flex;align-items:center;gap:14px;margin-top:6px;flex-wrap:wrap;">' +
      '<button class="bToggleNotes" data-i="' +
      i +
      '" style="border:none;background:transparent;color:#3d4a55;font-size:11.5px;cursor:pointer;padding:0;font-weight:500;">' +
      (l.showNotes
        ? '− Hide delivery / freight notes'
        : hasNotes
      ...
      (l.components && l.components.length
        ? '<button class="bHwLogic" data-i="' +
          i +
          '" style="border:none;background:transparent;color:#3d4a55;font-size:11.5px;cursor:pointer;padding:0;font-weight:500;text-decoration:underline;">Show the ' +
          esc(l.sku || 'kit') +
          ' calculation (' +
          l.components.length +
          ' part numbers) →</button>'
      ...
      '<button class="bAddNote" data-i="' +
      i +
      '" style="border:none;background:transparent;color:#3d4a55;font-size:11.5px;cursor:pointer;padding:0;font-weight:500;">+ Note under this item</button>' +
      '</div>' +
      notesPanel +
      ...
      (l.kind === 'INCLUDED' && l.sku && !(Number(l.rateMinor) || 0)
        ? '<div style="margin-top:6px;font-size:11.5px;color:#8a5a12;">No rate on this line' +
          (itemDefaults[l.sku] && itemDefaults[l.sku].priceMinor
            ? ' — the catalog has $' +
              (itemDefaults[l.sku].priceMinor / 100).toFixed(2) +
              '. Use “Pull from catalog” above.'
            : ' — and no price in the catalog for ' + esc(l.sku) + '.') +
          '</div>'
        : '') +
      '</div>' +
      '<div style="display:flex;flex-direction:column;gap:5px;width:74px;flex:0 0 auto;"><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Qty</label><input class="bF" data-i="' +
      i +
      '" data-k="quantity" value="' +
      esc(l.quantity) +
      '" style="width:100%;padding:6px 8px;border:1px solid #dcded7;border-radius:7px;text-align:right;"></div>' +
      '<div style="display:flex;flex-direction:column;gap:5px;width:104px;flex:0 0 auto;"><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;">Rate</label><input class="bF" data-i="' +
      i +
      '" data-k="rate" value="' +
      m2d(l.rateMinor) +
      '" style="width:100%;padding:6px 8px;border:1px solid #dcded7;border-radius:7px;text-align:right;"></div>' +
      (isMock()
        ? ''
        : '<div style="display:flex;flex-direction:column;gap:5px;width:96px;flex:0 0 auto;"><label style="font-size:10px;color:#8a8f85;text-transform:uppercase;" title="Internal only — never printed">Cost</label><input class="bF" data-i="' +
          i +
          '" data-k="cost" value="' +
          m2d(l.costEach) +
          '" style="width:100%;padding:6px 8px;border:1px solid #e4dfd0;background:#fdfcf7;border-radius:7px;text-align:right;"></div>') +
      '<div style="width:96px;flex:0 0 auto;text-align:right;padding-top:20px;font-weight:600;font-size:14px;">' +
      fmtMoney(amt, 'USD') +
      '</div>' +
      del +
      '</div></div>'
  ...
  function ynSelect(i, k, val) {
    return (
      '<select class="bF" data-i="' +
      i +
      '" data-k="' +
      k +
      '" style="' +
      IN +
      'padding:7px 9px;"><option value="">—</option><option value="YES"' +
      (val === 'YES' ? ' selected' : '') +
  ...
  function wireBuilder() {
    if (!wireBuilder._rail) {
      ...
      window.addEventListener('resize', function () {
        var el = document.getElementById('bMarginRail');
        if (el) el.setAttribute('style', marginRailStyle());
      });
    }
    document.getElementById('bBack').addEventListener('click', function () {
      ...
      if (isMock()) {
        ...
        pbDirty = false;
        activateNav('mock');
        renderMockProposal(u);
        return;
      }
      openProposalDetail(pb.proposalId, pb.user);
    });
    if (isMock()) {
      document.getElementById('bMkReal').addEventListener('click', function () {
        convertMockToProposal(pb.user);
      });
    } else {
      document.getElementById('bSave').addEventListener('click', saveBuilder);
      document.getElementById('bPreview').addEventListener('click', function () {
        previewProposalDoc(builderDoc());
      });
      ...
      document.getElementById('bPdf').addEventListener('click', function () {
        previewProposalDoc(builderDoc(), true);
      });
    ...
    document.getElementById('bFlexSeries').addEventListener('click', function () {
      openLinePicker('Summit Flex');
    });
    document.getElementById('bAddGroup').addEventListener('click', function () {
      pb.lines.push({
        ref: uid(),
        lineType: 'GROUP',
        kind: 'GROUP',
    ...
    document.getElementById('bAddSub').addEventListener('click', function () {
      pb.lines.push({
        ref: uid(),
        lineType: 'SUBGROUP',
        kind: 'SUBGROUP',
    ...
    if (hwTest)
      hwTest.addEventListener('click', function () {
        openHardwareAudit(hardwareKitLine());
      });
    ...
    document.querySelectorAll('.bFNDel').forEach(function (b) {
      b.addEventListener('click', function () {
        (pb.meta.footerNotes || []).splice(+b.getAttribute('data-i'), 1);
        renderBuilder();
      });
    });
    document.getElementById('bAddFooter').addEventListener('click', function () {
      pb.meta.footerNotes = (pb.meta.footerNotes || []).concat([{ title: '', body: '' }]);
      renderBuilder();
    });
    document.querySelectorAll('.grpChip').forEach(function (c) {
      c.addEventListener('click', function () {
        pb.lines.push({
          ref: uid(),
          lineType: 'GROUP',
          kind: 'GROUP',
          name: c.getAttribute('data-g'),
          description: '',
          quantity: 0,
          rateMinor: 0,
          optional: /trolley|adventure|foundation|mat/i.test(c.getAttribute('data-g')),
        });
        renderBuilder();
      });
    });
    ...
    var mct = document.getElementById('mContact');
    if (mct)
      mct.addEventListener('input', function () {
        pb.meta.contactName = mct.value;
      });
    var mp = document.getElementById('mProj');
    if (mp)
      mp.addEventListener('input', function () {
        pb.meta.projectId = mp.value;
      });
    var mpd = document.getElementById('mPropDate');
    if (mpd)
      mpd.addEventListener('input', function () {
        pb.meta.proposalDate = mpd.value;
        pb.meta.expiration = addDays(mpd.value, 7);
        var me2 = document.getElementById('mExp');
        if (me2) me2.value = pb.meta.expiration;
      });
    var msp = document.getElementById('mShowProj');
    if (msp)
      msp.addEventListener('change', function () {
        pb.meta.showProjectId = msp.checked;
      });
    var mdep = document.getElementById('mShowDeposit');
    if (mdep)
      mdep.addEventListener('change', function () {
    ...
    var mst = document.getElementById('mShowTitle');
    if (mst)
    ...
    var mb = document.getElementById('mBill'),
      mbs = document.getElementById('mBillSame');
    if (mb)
    ...
    if (mbs)
      mbs.addEventListener('change', function () {
        ...
        if (mbs.checked) {
          pb.meta.billTo = pb.meta.shipTo || '';
          if (mb) mb.value = pb.meta.billTo;
        }
      });
    var me = document.getElementById('mExp');
    if (me)
      me.addEventListener('input', function () {
        pb.meta.expiration = me.value;
      });
    ...
    if (ms)
      ms.addEventListener('input', function () {
        ...
        if (pb.meta.billSameAsShip) {
          pb.meta.billTo = ms.value;
          if (mb) mb.value = ms.value;
        }
      });
    var mtx = document.getElementById('mTax');
    if (mtx)
      mtx.addEventListener('change', function () {
        pb.meta.taxAmountMinor = d2m(mtx.value);
        pb.meta.taxTouched = true;
        renderBuilderKeepingFocus();
      });
    [
      ['mTaxTbd', 'tbdTax'],
      ['mStructFreightTbd', 'tbdStructureFreight'],
      ['mMatsFreightTbd', 'tbdMatsFreight'],
    ].forEach(function (p) {
      var el = document.getElementById(p[0]);
      if (el) {
    ...
    if (mdisc)
      mdisc.addEventListener('change', function () {
        ...
        if (pb.meta.discountMode === 'AMT') {
          pb.meta.discountAmountMinor = Math.round(v * 100);
          pb.meta.discountPct = 0;
        } else {
          pb.meta.discountPct = v;
          pb.meta.discountAmountMinor = 0;
        }
        renderBuilderKeepingFocus();
      });
    var mdiscMode = document.getElementById('mDiscMode');
    if (mdiscMode)
      mdiscMode.addEventListener('change', function () {
        pb.meta.discountMode = mdiscMode.value === 'AMT' ? 'AMT' : 'PCT';
        pb.meta.discountPct = 0;
        pb.meta.discountAmountMinor = 0;
        renderBuilderKeepingFocus();
      });
    var msf = document.getElementById('mStructFreight');
    if (msf)
      msf.addEventListener('change', function () {
        pb.meta.structureFreightMinor = d2m(msf.value);
        renderBuilderKeepingFocus();
      });
    var mmf = document.getElementById('mMatsFreight');
    if (mmf)
      mmf.addEventListener('change', function () {
        pb.meta.matsFreightMinor = d2m(mmf.value);
        pb.meta.matsFreightTouched = true;
        renderBuilderKeepingFocus();
      });
    ...
    if (mso)
      mso.addEventListener('change', function () {
        pb.meta.stdFreightOn = mso.checked;
        markBuilderDirty();
        renderBuilderKeepingFocus();
      });
    var msf2 = document.getElementById('mStdFreight');
    if (msf2)
      msf2.addEventListener('change', function () {
        pb.meta.stdFreightMinor = d2m(msf2.value);
        markBuilderDirty();
        renderBuilderKeepingFocus();
      });
    var brp = document.getElementById('bRepull');
    if (brp) brp.addEventListener('click', repullCatalogFigures);
    var bfr = document.getElementById('bFreightReq');
    if (bfr) bfr.addEventListener('click', requestFreight);
    var bfs = document.getElementById('bFreightSync');
    if (bfs) bfs.addEventListener('click', pullFreightAmount);
    // line field inputs
    document.querySelectorAll('.bF').forEach(function (el) {
      ...
      if (
        ...
        el.addEventListener('change', renderBuilderKeepingFocus);
    });
    document.querySelectorAll('.bChk').forEach(function (el) {
      el.addEventListener('change', function () {
        markBuilderDirty();
        var l = pb.lines[+el.getAttribute('data-i')];
        if (l) {
          l[el.getAttribute('data-k')] = el.checked;
        }
      });
    });
    document.querySelectorAll('.bToggleNotes').forEach(function (b) {
      b.addEventListener('click', function () {
        var l = pb.lines[+b.getAttribute('data-i')];
        if (l) {
    ...
    document.querySelectorAll('.bHwLogic').forEach(function (b) {
      b.addEventListener('click', function () {
        openHardwareAudit(pb.lines[+b.getAttribute('data-i')]);
      });
    });
    document.querySelectorAll('.bDel').forEach(function (b) {
      b.addEventListener('click', function () {
        markBuilderDirty();
        pb.lines.splice(+b.getAttribute('data-i'), 1);
        renderBuilder();
      });
    });
    ...
    document.querySelectorAll('.bAddNote').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        var next = rows[i + 1];
        if (next) {
          var f = next.querySelector('input.bF');
          if (f) f.focus();
        }
      });
    });
    ...
    document.querySelectorAll('.bRow').forEach(function (row) {
      row.addEventListener('dragstart', function () {
        bDragFrom = +row.getAttribute('data-i');
      ...
      row.addEventListener('drop', function (e) {
        ...
        if (to >= blk.from && to < blk.from + blk.count) {
          bDragFrom = null;
          return;
        }
  ...
  async function openProductPicker() {
    ...
    bundles.forEach(function (b) {
      if (b.sku) bundleSkus[b.sku] = true;
    });
    ...
    async function fetchItems(term) {
      ...
      if (mine !== seq) return;
      items = found.filter(function (i) {
        return i.active && !bundleSkus[i.part];
      });
      loading = false;
      paint();
    }
    ...
    function rowsHtml(term) {
      ...
      if (bnd.length) {
        out +=
          '<div style="padding:6px 12px;background:#f7f8f4;font-size:10px;letter-spacing:.06em;text-transform:uppercase;color:#8a8f85;">Bundles</div>' +
          bnd
            .map(function (b) {
              return (
                '<button type="button" class="pkBundle" data-id="' +
                b.id +
                '" style="display:block;width:100%;text-align:left;border:none;border-bottom:1px solid #f2f3ef;background:#fff;padding:10px 12px;cursor:pointer;font-size:13.5px;">' +
                '<b style="font-weight:600;">' +
                esc(b.name) +
      ...
      out += items
        .map(function (p) {
          ...
          return (
            '<button type="button" class="pkRow" data-part="' +
            esc(p.part) +
            '" style="display:block;width:100%;text-align:left;border:none;border-bottom:1px solid #f2f3ef;background:#fff;padding:10px 12px;cursor:pointer;font-size:13.5px;">' +
            '<b style="font-weight:600;">' +
            esc(p.name || p.part) +
    ...
    function wire() {
      ...
      document.querySelectorAll('.pkBundle').forEach(function (b) {
        b.addEventListener('click', function () {
          var bn = bundles.filter(function (x) {
            return x.id === b.getAttribute('data-id');
          })[0];
          if (!bn) return;
          pb.lines.push(
            applyItemDefaults({
              ...
              sku: bn.sku || '',
              name: bn.name,
              description: bn.proposalDescription || '',
              quantity: 1,
              rateMinor: bn.unitPriceMinor || 0,
              costEach: 0,
              weightEach: 0,
              group: bn.name,
            }),
          ...
          (bn.components || []).forEach(function (c) {
            pb.lines.push(
              applyItemDefaults({
                ...
                rateMinor: 0,
                costEach: c.unitCostMinor || 0,
                weightEach: c.weightLbs || 0,
                group: bn.name,
              }),
            );
          });
    ...
    openModal(
      'Add product line',
      '<input id="pkSearch" placeholder="Search part number or name…" style="' +
        IN +
        'margin-bottom:10px;">' +
  ...
  function builderVersionPayload() {
    ...
    var items = pb.lines.map(function (l, i) {
      return {
        ref: l.ref,
        lineType: l.lineType,
        kind: l.kind,
    ...
    return {
      title: pb.title || undefined,
      sections: sections,
      items: items,
      expirationDate: pb.meta.expiration || undefined,
    };
  }
  ...
  async function saveBuilderQuiet() {
    ...
    var r = await authed('/proposals/versions/' + pb.versionId, {
      method: 'PATCH',
      body: builderVersionPayload(),
    });
    if (!r.ok)
      throw new Error('Could not save the proposal before raising the request (' + r.status + ').');
    clearBuilderDirty();
  }
  ...
  function saveAsTemplate() {
    openModal(
      ...
      fieldRow(
        'Template name',
        '<input id="tplName" style="' +
          IN +
          '" placeholder="e.g. Full Gym, Flex Quote, Soar" required>',
      ) + fieldRow('Description (optional)', '<input id="tplDesc" style="' + IN + '">'),
      async function (close, showErr) {
        var name = document.getElementById('tplName').value.trim();
        if (!name) return showErr('Give the template a name.');
        var data = {
          title: pb.title,
          meta: {
            taxAmountMinor: pb.meta.taxAmountMinor,
            discountPct: pb.meta.discountPct,
            discountMode: pb.meta.discountMode === 'AMT' ? 'AMT' : 'PCT',
            discountAmountMinor: pb.meta.discountAmountMinor || 0,
            structureFreightMinor: pb.meta.structureFreightMinor,
            matsFreightMinor: pb.meta.matsFreightMinor,
            stdFreightOn: !!pb.meta.stdFreightOn,
            stdFreightMinor: pb.meta.stdFreightMinor || 0,
            shipTo: '',
            projectId: '',
            expiration: '',
          },
          lines: pb.lines.map(function (l) {
            return {
              lineType: l.lineType,
              kind: l.kind,
              productId: l.productId,
              ...
              addlFreight: l.addlFreight || '',
              freightCalc: l.freightCalc || '',
              tpFreightMinor: l.tpFreightMinor || 0,
              tpFreightLabel: l.tpFreightLabel || '',
            };
          }),
        };
        var r = await authed('/proposal-templates', {
          method: 'POST',
          body: {
            name: name,
            description: document.getElementById('tplDesc').value.trim() || undefined,
            data: data,
          },
        });
  ...
  async function loadTemplate() {
    ...
    try {
      var r = await authed('/proposal-templates');
      if (r.ok) tpls = await r.json();
    } catch (e) {}
    if (!tpls.length) {
      alert('No saved templates yet. Build a proposal and use “Save as template” first.');
      return;
    }
    openModal(
      'Load template',
      '<div style="max-height:320px;overflow:auto;border:1px solid #e7e8e3;border-radius:10px;">' +
        tpls
          .map(function (t) {
            return (
              '<div style="display:flex;align-items:center;gap:8px;border-bottom:1px solid #f2f3ef;padding:10px 12px;"><button type="button" class="tplRow" data-id="' +
              t.id +
              '" style="flex:1;text-align:left;border:none;background:#fff;cursor:pointer;font-size:13.5px;"><b style="font-weight:600;">' +
              esc(t.name) +
              '</b>' +
              (t.description
                ? '<div class="muted" style="font-size:12px;">' + esc(t.description) + '</div>'
                : '') +
              '</button><button type="button" class="tplDel" data-id="' +
              t.id +
              '" style="border:1px solid #e0e1db;background:#fff;border-radius:7px;color:#9c3327;cursor:pointer;padding:5px 9px;">Delete</button></div>'
            );
          })
          .join('') +
        '</div>' +
    ...
    setTimeout(function () {
      document.querySelectorAll('.tplRow').forEach(function (b) {
        b.addEventListener('click', function () {
          var t = tpls.filter(function (x) {
            return x.id === b.getAttribute('data-id');
          })[0];
          var d = t.data || {};
          if (d.meta) {
            pb.meta.taxRatePct = d.meta.taxRatePct || 0;
            pb.meta.freightMinor = d.meta.freightMinor || 0;
          }
          pb.lines = (d.lines || []).map(function (l) {
            return normalizeLine(l);
          });
          var ov = document.querySelector('div[style*="position:fixed"]');
          if (ov && ov.parentNode) ov.parentNode.removeChild(ov);
          renderBuilder();
        });
      });
      document.querySelectorAll('.tplDel').forEach(function (b) {
        b.addEventListener('click', async function () {
          if (!confirm('Delete this template?')) return;
          await authed('/proposal-templates/' + b.getAttribute('data-id'), { method: 'DELETE' });
          var ov = document.querySelector('div[style*="position:fixed"]');
          if (ov && ov.parentNode) ov.parentNode.removeChild(ov);
          loadTemplate();
        });
  ...
  async function proposalDocData(proposal, version) {
    ...
    try {
      var ro = await authed('/crm/organizations?pageSize=100');
      if (ro.ok) {
        var f = ((await ro.json()).items || []).filter(function (o) {
          return o.id === proposal.organizationId;
        })[0];
        orgName = f ? f.name : '';
      }
    ...
    var tpFreight = 0;
    lines.forEach(function (l) {
      if ((l.lineType || 'PRODUCT') === 'PRODUCT') tpFreight += Number(l.tpFreightMinor) || 0;
    });
    var disc = discountOf(meta, subtotal);
    var discountPct = disc.pct,
      discountMode = disc.mode,
  ...
  function proposalDocHtml(doc) {
    ...
    function lineIndent() {
      return groupOpenSub != null ? (inSub ? 34 : 20) : 8;
    }
    function subtotalRow() {
      if (groupOpenSub == null) return '';
      var r =
        '<tr style="break-inside:avoid;"><td colspan="5" style="padding:5px 8px;text-align:right;font-weight:600;font-size:11px;border-bottom:2px solid #d5d8d2;">Subtotal: ' +
        fmtUsd(groupOpenSub) +
        '</td></tr>';
      groupOpenSub = null;
      return r;
    ...
    (d.lines || []).forEach(function (l) {
      ...
      if (lt === 'GROUP') {
        ...
        inSub = false;
        body +=
          '<tr data-brk="head" style="break-inside:avoid;break-after:avoid;"><td colspan="5" style="padding:6px 10px;font-weight:700;font-size:12px;letter-spacing:.03em;text-transform:uppercase;color:#3d4a55;background:#eef0ea;border-bottom:1px solid #d5d8d2;"><span style="display:inline-flex;align-items:baseline;gap:46px;"><span>' +
          esc(tc(stripOptional(l.name))) +
          (l.optional
            ? ' <span style="font-weight:400;text-transform:none;color:#8a8f85;">(Optional)</span>'
            : '') +
          '</span>' +
          (l.description ? '<span style="color:#20241f;">' + esc(l.description) + '</span>' : '') +
          '</span></td></tr>';
        return;
      }
      if (lt === 'SUBGROUP') {
        ...
        body +=
          '<tr data-brk="head" style="break-inside:avoid;break-after:avoid;"><td colspan="5" style="padding:7px 8px 3px 22px;font-weight:600;font-size:11.5px;color:#3d4a55;border-bottom:1px solid #d5d8d2;">' +
          esc(tc(l.name)) +
          (subNote
            ? '<div style="font-weight:400;font-size:10.5px;color:#5c6157;margin-top:2px;line-height:1.5;">' +
              rt(subNote) +
              '</div>'
            : '') +
          '</td></tr>';
      ...
      if (lt === 'NOTE') {
        body +=
          '<tr style="break-inside:avoid;"><td colspan="5" style="padding:7px 8px 7px ' +
          lineIndent() +
          'px;background:#fbfaf4;font-size:11px;color:#5c6157;line-height:1.5;"><b style="display:block;color:#20241f;margin-bottom:2px;">' +
          esc(tc(l.name)) +
          '</b>' +
          rt(l.description) +
          '</td></tr>';
      ...
      body +=
        '<tr style="break-inside:avoid;"><td style="padding:5px 8px 5px ' +
        indent +
        'px;border-bottom:1px solid #eef0ea;vertical-align:top;"><b style="font-weight:600;">' +
        esc(tc(l.name)) +
        '</b>' +
        (l.description
          ? '<div style="font-size:10.5px;color:#5c6157;line-height:1.45;margin-top:2px;">' +
            esc(l.description) +
            '</div>'
          : '') +
        (l.delivery
          ? '<div style="font-size:10px;color:#7a7f75;margin-top:2px;">Delivery: ' +
            esc(l.delivery) +
            '</div>'
          : '') +
        (showsFreightTbd(l)
          ? '<div style="font-size:10px;color:#5c6157;line-height:1.45;margin-top:3px;font-style:italic;">' +
            esc(FREIGHT_TBD_NOTE) +
            '</div>'
          : '') +
        '</td>' +
        '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;font-size:10px;color:#7a7f75;vertical-align:top;font-family:ui-monospace,monospace;">' +
        esc(l.sku || '') +
        '</td>' +
        '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:center;vertical-align:top;">' +
        (Number(l.quantity) || 0) +
        '</td>' +
        '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;vertical-align:top;">' +
        fmtMoney(l.rateMinor, '') +
        '</td>' +
        '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;vertical-align:top;font-weight:600;">' +
        fmtMoney(amt, '') +
        '</td></tr>';
      if (Number(l.tpFreightMinor) > 0) {
        body +=
          '<tr style="break-inside:avoid;"><td style="padding:2px 8px 5px 20px;border-bottom:1px solid #eef0ea;font-size:10.5px;color:#5c6157;font-style:italic;">+ ' +
          esc(tc(l.tpFreightLabel || 'Third-Party Freight')) +
          '</td><td style="border-bottom:1px solid #eef0ea;"></td><td style="border-bottom:1px solid #eef0ea;"></td><td style="border-bottom:1px solid #eef0ea;"></td><td style="padding:2px 8px 5px;border-bottom:1px solid #eef0ea;text-align:right;font-size:10.5px;color:#5c6157;">' +
    ...
    var bottomNotesHtml = bottomNotes.length
      ? '<div style="margin-top:22px;padding-top:12px;border-top:1px solid #e7e8e3;font-size:10.5px;color:#5c6157;line-height:1.6;break-inside:avoid;"><div style="font-weight:600;color:#20241f;margin-bottom:4px;">Delivery, Returns &amp; Freight Notes</div>' +
        bottomNotes
          .map(function (n) {
            return (
    ...
    var footerNotesHtml = footerNotes.length
      ? '<div style="margin-top:24px;padding-top:13px;border-top:1px solid #e7e8e3;break-inside:avoid;">' +
        footerNotes
          .map(function (fn) {
            return (
              '<div style="margin-bottom:9px;font-size:10.5px;line-height:1.6;color:#5c6157;">' +
              (fn.title
                ? '<div style="font-weight:700;font-size:11px;color:#20241f;margin-bottom:2px;">' +
                  esc(fn.title) +
                  '</div>'
                : '') +
              rt(fn.body) +
    ...
    var preparedBy =
      ...
      '</div>' +
      (preparerLine2
        ? '<div style="color:#5c6157;margin-top:1px;">' + esc(preparerLine2) + '</div>'
        : '') +
    ...
    var html =
      ...
      preparedBy +
      '</div>' +
      '<div style="text-align:right;"><div style="font-family:\'Newsreader\',serif;font-size:22px;font-weight:600;">Proposal</div><div style="font-size:11.5px;color:#5c6157;margin-top:4px;">' +
      esc(d.number || '') +
      ...
      '<div><div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;margin-bottom:4px;">Prepared For</div><div style="font-weight:600;">' +
      esc(d.orgName || '') +
      '</div>' +
      (m.contactName
        ? '<div style="color:#20241f;margin-top:1px;">' + esc(m.contactName) + '</div>'
        : '') +
      (m.billTo
        ? '<div style="color:#5c6157;white-space:pre-line;margin-top:2px;">' +
          esc(m.billTo) +
          '</div>'
        : '') +
      '</div>' +
      (m.shipTo
        ? '<div><div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;margin-bottom:4px;">Ship To</div><div style="color:#5c6157;white-space:pre-line;">' +
          esc(m.shipTo) +
          '</div></div>'
        : '') +
      '</div>' +
      (m.showTitle !== false && d.title
        ? '<div style="font-family:\'Newsreader\',serif;font-size:24px;font-weight:600;margin-bottom:14px;">' +
          esc(d.title) +
          '</div>'
        : '') +
      '<table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr style="color:#8a8f85;font-size:10px;text-transform:uppercase;letter-spacing:.04em;"><th style="text-align:left;padding:6px 8px;border-bottom:2px solid #3d4a55;">Activity / Description</th><th style="text-align:left;padding:6px 8px;border-bottom:2px solid #3d4a55;width:90px;">SKU</th><th style="text-align:center;padding:6px 8px;border-bottom:2px solid #3d4a55;width:44px;">Qty</th><th style="text-align:right;padding:6px 8px;border-bottom:2px solid #3d4a55;width:84px;">Rate</th><th style="text-align:right;padding:6px 8px;border-bottom:2px solid #3d4a55;width:94px;">Amount</th></tr></thead><tbody>' +
      body +
      '</tbody></table>' +
      '<div style="display:flex;justify-content:flex-end;margin-top:16px;break-inside:avoid;"><div style="min-width:260px;">' +
      '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Subtotal</span><span>' +
      fmtUsd(t.subtotal) +
      '</span></div>' +
      (t.discount
        ? '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;color:#9c3327;"><span>' +
          discountLabel(t) +
          '</span><span>− ' +
          fmtUsd(t.discount) +
          '</span></div>' +
          '<div style="padding:0 8px 3px;font-size:10px;color:#8a8f85;text-align:right;">Discount expires ' +
          (m.expiration ? fmtDate(m.expiration) : 'with this proposal') +
          '</div>'
        : '') +
      (t.tpFreight
        ? '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Third-Party Freight</span><span>' +
          fmtUsd(t.tpFreight) +
          '</span></div>'
        : '') +
      '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Mat Freight Tax Pass-Through</span><span>' +
      cellTax +
      '</span></div>' +
      '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Structure Crating &amp; Freight</span><span>' +
      cellStructureFreight +
      '</span></div>' +
      '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Mats &amp; Padding Freight</span><span>' +
      cellMatsFreight +
      '</span></div>' +
      // Standard Freight is opt-in: unticked, the customer never sees the line.
      (m.stdFreightOn
        ? '<div style="display:flex;justify-content:space-between;padding:3px 8px;font-size:12.5px;"><span style="color:#5c6157;">Standard Freight</span><span>' +
          amountCell(t.stdFreight, '') +
          '</span></div>'
        : '') +
      '<div style="display:flex;justify-content:space-between;padding:8px;margin-top:5px;border-top:2px solid #3d4a55;font-size:15px;font-weight:700;"><span>Total</span><span>' +
      fmtUsd(t.total) +
      '</span></div>' +
      (anyTbd
        ? '<div style="padding:2px 8px 0;font-size:10px;color:#8a8f85;text-align:right;line-height:1.5;">Total excludes items marked TBD.</div>'
        : '') +
      (m.showDeposit !== false
        ? '<div style="display:flex;justify-content:space-between;padding:6px 8px 0;font-size:13px;color:#3d4a55;font-weight:700;"><span>Deposit Due (' +
          depositPct() +
          '%)</span><span>' +
          fmtUsd(t.deposit) +
          '</span></div>'
   ...
   */
  var FOUR_LEG_MAX_FT = 10,
    MIN_LEGS_OVER_FOUR_LEG_MAX = 6;
  function legsFor(len) {
    len = Number(len) || 0;
    var banded =
      len <= Number(fxSettings.legsSmallMaxFt)
        ? Number(fxSettings.legsSmallCount)
        : len <= Number(fxSettings.legsMediumMaxFt)
          ? Number(fxSettings.legsMediumCount)
  ...
  function openAdventureConfigurator() {
    ...
    ov.id = 'advOverlay';
    ov.style.cssText =
      'position:fixed;inset:0;background:rgba(32,36,31,.4);z-index:70;overflow:auto;padding:24px 16px;';
    document.body.appendChild(ov);
    renderAdv();
  }
  function advBlank() {
    return {
      ...
      cargoNet8x6: false,
      cargoNet8x6Qty: 1,
      cargoHwCarabiner: true,
      cargoHwCarabinerQty: 1,
      cargoHwVRing: true,
      cargoHwVRingQty: 2,
      climbFrame: false,
      climbWall: false,
  ...
  function advApplyAnswers(a) {
    ...
    adv.legs = Math.max(
      Number(a.legs) || legsFor(adv.length),
      adv.length > FOUR_LEG_MAX_FT ? MIN_LEGS_OVER_FOUR_LEG_MAX : 0,
    );
    adv.monkeyBars = !!a.monkeyBars;
    adv.monkeyBarsQty = Number(a.monkeyBarsQty) || 1;
    adv.ladders = Number(a.ladders) > 0;
    adv.laddersQty = Number(a.ladders) || 1;
    adv.trolley = !!a.trolley;
    adv.trolleyType = a.trolleyType || 'Dual';
    adv.interiorBeams = !!a.interiorBeams;
    adv.interiorBeamsQty = Number(a.interiorBeamsQty) || 1;
    adv.zipLine = !!a.zipLine;
    adv.zipLineQty = Number(a.zipLineQty) || 1;
    adv.ballRack = !!a.ballRack;
    adv.slide = !!a.slide;
    adv.slideGray = !!a.slideGray;
    adv.steamroller = !!a.steamroller;
    ...
    adv.cargoNet10x8 = !!a.cargoNet10x8;
    adv.cargoNet10x8Qty = Number(a.cargoNet10x8Qty) || 1;
    adv.cargoNet8x6 = !!a.cargoNet8x6;
    adv.cargoNet8x6Qty = Number(a.cargoNet8x6Qty) || 1;
    ...
    adv.climbFrame = !!a.climbFrame;
    adv.climbWall = !!a.climbWall;
    adv.climbShield = !!a.climbShield;
    adv.climbMat = !!a.climbMat;
    adv.floorPadding = !!(a.floorPadding || a.matFloor);
    adv.floorPadThickness = a.floorPadThickness === '2' ? '2' : '3.25';
    adv.matColumn = !!a.matColumn;
    adv.uShaped = Number(a.uShaped) || 0;
    adv.completeWrap = Number(a.completeWrap) || 0;
    adv.matLadderLeg = !!a.matLadderLeg;
    adv.matCustom = !!a.matCustom;
    adv.brackets = !!a.brackets;
    adv.bracketsQty = Number(a.bracketsQty) || 0;
    adv.swivel360 = Number(a.swivel360) || 0;
    adv.swivelStandalone = Number(a.swivelStandalone) || 0;
    adv.forged = Number(a.forged) || 0;
    adv.swingHanger = Number(a.swingHanger) || 0;
    adv.vRings = Number(a.vRings) || 0;
    adv.carabiner = Number(a.carabiner) || 0;
    adv.webbingSling = Number(a.webbingSling) || 0;
  ...
  function advAlreadyBuilt() {
    return !!(pb && pb.meta && pb.meta.advAnswers);
  }
  function advGenLabel() {
    return advAlreadyBuilt() ? 'Revise Current Proposal' : 'Generate Proposal';
  }

  function advClose() {
    var o = document.getElementById('advOverlay');
    if (o) document.body.removeChild(o);
  }
  function climbWalls() {
    return (adv.climbFrame ? 1 : 0) + (adv.climbWall ? 1 : 0);
  }
  /**
   ...
   */
  function cargoNetOn() {
    return !!(adv.cargoNet && (adv.cargoNet10x8 || adv.cargoNet8x6));
  }
  function eyeboltSum() {
    var nonSwivel = Math.max(0, (Number(adv.bracketsQty) || 0) - (Number(adv.swivel360) || 0));
    return (
      (Number(adv.swivel360) || 0) +
      nonSwivel +
      (Number(adv.forged) || 0) +
      (Number(adv.swingHanger) || 0)
    );
  }
  ...
  function matMarkup() {
    var v = Number(fxSettings.matMarkupMultiplier);
    return Number.isFinite(v) && v > 0 ? v : 1.4;
  }
  function matOverageIn() {
    var v = Number(fxSettings.matOverageIn);
  ...
  var ADV_HW_ROLLUP = {
    vRings: [
      { part: '6820H-LAE', per: 10 },
      { part: '6820H-LAF', per: 10 },
    ],
  };
  ...
  async function loadAdvOverridable() {
    ...
    try {
      ...
      /* leave empty — nothing is overridable or defaulted if we cannot confirm it */
    }
    advOverridable = map;
    advDefaults = defs;
    applyHwDefaults();
  ...
  function matQuote() {
    ...
    var L = Number(adv.length) || 0,
      W = Number(adv.width) || 0;
    var ov = matOverageIn();
    var li = L * 12 + ov,
      wi = W * 12 + ov;
    var sqIn = li * wi,
    ...
    return {
      ...
      squareFeet: sqFt,
      rate: matRate(th),
      sellRate: matRate(th) * matMarkup(),
      costMinor: costMinor,
      priceMinor: Math.round(costMinor * matMarkup()),
  ...
  function padThicknessPicker() {
    ...
    function opt(val, label, sub) {
      var on = th === val;
      return (
        '<label style="flex:1;display:flex;gap:8px;align-items:flex-start;padding:10px 12px;border:1px solid ' +
        (on ? '#3d4a55' : '#dcded7') +
        ';border-radius:10px;background:' +
        (on ? '#f3f6f8' : '#fff') +
        ';cursor:pointer;">' +
        '<input type="radio" name="advPadTh" data-ak="floorPadThickness" value="' +
        val +
        '"' +
        (on ? ' checked' : '') +
        ' style="margin-top:2px;">' +
        '<span><b style="font-weight:600;font-size:13.5px;">' +
        label +
        '</b><span class="muted" style="display:block;font-size:11.5px;">' +
        sub +
        '</span></span></label>'
      );
    }
    ...
    var per = function (v) {
      return '$' + (matRate(v) * matMarkup()).toFixed(2) + ' / sq ft';
    };
    return (
      '<div style="font-size:11px;color:#8a8f85;text-transform:uppercase;letter-spacing:.03em;margin-bottom:6px;">Padding thickness</div>' +
      '<div style="display:flex;gap:10px;">' +
      opt('3.25', '3.25" thick', per('3.25')) +
      opt('2', '2" thick', per('2')) +
      '</div>'
    );
  }
  function padQuoteHint() {
    var q = matQuote();
    return (
      '<div style="margin-top:10px;background:#f8f9f6;border:1px solid #e7e8e3;border-radius:10px;padding:10px 12px;font-size:12px;color:#5c6157;line-height:1.6;">' +
      '<div style="font-family:ui-monospace,monospace;font-size:11.5px;color:#3d4a55;font-weight:600;">' +
      esc(q.sku) +
      '</div>' +
      q.matLengthIn +
      '" × ' +
      q.matWidthIn +
      '" = ' +
      q.squareInches.toLocaleString() +
      ' sq in ÷ 144 = <b>' +
      q.squareFeet.toFixed(2) +
      ' sq ft</b><br>' +
      q.squareFeet.toFixed(2) +
      ' sq ft × $' +
      q.sellRate.toFixed(2) +
      ' = <b style="color:#2f7d5d;">' +
      fmtMoney(q.priceMinor, 'USD') +
      '</b>' +
      '</div>'
    );
  }
  ...
  function renderAdv() {
    ...
    function sec(title, inner) {
      return (
        '<div style="margin-bottom:18px;"><div style="font-family:\'Newsreader\',serif;font-size:16px;font-weight:600;color:#3d4a55;border-bottom:1px solid #e7e8e3;padding-bottom:6px;margin-bottom:12px;">' +
        title +
        '</div>' +
        inner +
        '</div>'
    ...
    function num(key, label, min, max, extra, hint) {
      return (
        '<div class="af" style="' +
        (extra || '') +
        '"><label style="display:block;font-size:11px;color:#8a8f85;text-transform:uppercase;letter-spacing:.03em;margin-bottom:4px;">' +
        label +
        '</label><input type="number" data-ak="' +
        key +
        '" value="' +
        adv[key] +
        '"' +
        (min != null ? ' min="' + min + '"' : '') +
        (max != null ? ' max="' + max + '"' : '') +
        ' style="width:100%;padding:8px 10px;border:1px solid #dcded7;border-radius:8px;font-size:14px;">' +
    ...
    function tog(key, label, hint) {
      return (
        '<label style="display:flex;align-items:center;gap:9px;padding:8px 0;cursor:pointer;font-size:14px;border-bottom:1px solid #f2f3ef;"><input type="checkbox" data-ak="' +
        key +
        '"' +
        (adv[key] ? ' checked' : '') +
        ' style="width:17px;height:17px;flex:0 0 auto;"><span style="flex:1;"><b style="font-weight:600;">' +
        label +
        '</b>' +
        (hint
          ? '<span class="muted" style="font-size:12px;display:block;">' + hint + '</span>'
          : '') +
        '</span></label>'
      );
    }
    function sel(key, label, opts) {
      return (
        '<div class="af"><label style="display:block;font-size:11px;color:#8a8f85;text-transform:uppercase;letter-spacing:.03em;margin-bottom:4px;">' +
        label +
        '</label><select data-ak="' +
        key +
        '" style="width:100%;padding:8px 10px;border:1px solid #dcded7;border-radius:8px;font-size:14px;background:#fff;">' +
        opts
          .map(function (op) {
            return (
              '<option value="' +
              op +
              '"' +
              (String(adv[key]) === String(op) ? ' selected' : '') +
              '>' +
              op +
              '</option>'
            );
    ...
    function hwPartRow(key) {
      ...
      if (!advOverridable)
        return '<div style="' + lab + 'margin-top:5px;">Part <code>' + esc(p) + '</code></div>';
      if (!(p in advOverridable))
        return (
          '<div style="' + lab + 'margin-top:5px;">Part <code>' + esc(p) + '</code> · fixed</div>'
        );
      var cur = (adv.partOverrides && adv.partOverrides[p]) || p,
        swapped = cur !== p;
      return (
        '<div style="display:flex;align-items:center;gap:6px;margin-top:5px;">' +
        '<span style="' +
        lab +
        'flex:0 0 auto;">Part</span>' +
        '<input data-ovr="' +
        esc(p) +
        '" value="' +
        esc(cur) +
        '" style="flex:1;min-width:0;padding:4px 7px;border:1px solid ' +
        (swapped ? '#c9a227' : '#dcded7') +
        ';border-radius:6px;font-size:12px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;">' +
        (swapped
          ? '<button data-ovrclear="' +
            esc(p) +
            '" style="border:none;background:none;color:#8a8f85;cursor:pointer;font-size:11px;padding:2px 3px;flex:0 0 auto;">reset</button>'
    ...
    function hwDefaultRow(key) {
      ...
      return (
        '<div style="font-size:10.5px;color:#8a8f85;margin-top:4px;">Catalog default ' +
        def +
        (cur !== def
          ? ' · <button data-hwdef="' +
            key +
            '" style="border:none;background:none;padding:0;color:#3d4a55;text-decoration:underline;cursor:pointer;font-size:10.5px;">use default</button>'
          : ' · applied') +
        '</div>'
      );
    }
    ...
    function hwRollRow(key) {
      var r = ADV_HW_ROLLUP[key];
      if (!r) return '';
      var q = Number(adv[key]) || 0;
      if (q <= 0) return '';
      return (
        '<div style="font-size:10.5px;color:#8a8f85;margin-top:3px;">Also adds ' +
        r
          .map(function (x) {
            return q * x.per + '× <code>' + esc(x.part) + '</code>';
          })
          .join(' + ') +
        ' into the H-1000 hardware kit</div>'
      );
    }
    function hwNum(key, label, min, max, hint) {
      return (
        '<div>' +
        num(key, label, min, max, '', hint) +
        hwDefaultRow(key) +
        hwPartRow(key) +
        hwRollRow(key) +
        '</div>'
    ...
    var html =
      ...
      sec(
        ...
        '<div style="' +
          grid +
          '">' +
          sel('length', 'Length (long, ft)', rangeArr(6, 30)) +
          sel('width', 'Width (short, ft)', rangeArr(6, 20)) +
          '</div>',
      ) +
      sec(
        'Frame Configuration',
        '<div style="' +
          grid +
          '">' +
          sel('config', 'Configuration' + (adv.configManual ? ' (overridden)' : ' (auto)'), [
            'Rectangle',
            'Square',
            'L-Shape',
            'T-Shape',
          ]) +
          num('legs', '# of Frame Legs (auto, editable)', 0, 20) +
          '</div>' +
          '<div class="muted" style="font-size:11.5px;margin-top:6px;">' +
          (adv.configManual
            ? 'Manually set — auto would be ' +
              autoConfig() +
              '. <a href="#" id="advCfgReset">Reset to auto</a>'
            : 'Auto from dimensions: ' + autoConfig()) +
          ' · legs auto-set from length (' +
          legsFor(adv.length) +
          ')' +
          (Number(adv.legs) !== legsFor(adv.length)
            ? ' — currently ' + Number(adv.legs) + ', set by hand'
            : '') +
          '</div>',
      ) +
      sec(
        'Frame Options',
        tog('monkeyBars', 'Monkey Bars') +
          (adv.monkeyBars
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              num('monkeyBarsQty', '# of Monkey Bars', 1, 3) +
              '</div>'
            : '') +
          tog('ladders', 'Ladders') +
          (adv.ladders
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              num('laddersQty', '# of Ladders', 1, 4) +
              '</div>' +
              tog(
                'ladderShield',
                'Ladder — Safety Shield',
                'Qty mirrors # of ladders (' + adv.laddersQty + ')',
              )
            : '') +
          tog('trolley', 'Trolley System') +
          (adv.trolley
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              sel('trolleyType', 'Type of Trolley System', ['Dual', 'Single']) +
              '</div>'
            : '') +
          tog('interiorBeams', 'Interior Beams') +
          (adv.interiorBeams
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              num('interiorBeamsQty', '# of Interior Beams', 1, 6) +
              '</div>'
            : ''),
      ) +
      sec(
        'Frame Accessories',
        tog('zipLine', 'Zip Line') +
          (adv.zipLine
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              num('zipLineQty', '# of Zip Line', 1, 3) +
              '</div>'
            : '') +
            ...
            ? '<div style="padding-left:16px;">' +
              ...
              (adv.steamroller
                ? '<div style="padding-left:16px;">' +
                  tog(
                    'slideConvKit',
                    'Adventure Steamroller/Scooter Board — Conversion Kit',
                    'Part A-2349 · required with the ramp',
                  ) +
                  '</div>'
                : '') +
              '</div>'
            ...
            ? '<div style="padding-left:16px;">' +
              ...
              (adv.cargoNet10x8
                ? '<div style="' +
                  grid +
                  'margin:8px 0 4px;">' +
                  num('cargoNet10x8Qty', "# of 10' x 8' nets", 1, 20) +
                  '</div>'
                : '') +
              ...
              (adv.cargoNet8x6
                ? '<div style="' +
                  grid +
                  'margin:8px 0 4px;">' +
                  num('cargoNet8x6Qty', "# of 8' x 6' nets", 1, 20) +
                  '</div>'
                : '') +
              (adv.cargoNet10x8 || adv.cargoNet8x6
                ? '<div style="margin-top:10px;padding:10px 12px;background:#f8f9f6;border:1px solid #e7e8e3;border-radius:9px;">' +
                  ...
                  tog(
                    'cargoHwCarabiner',
                    'Heavy Duty Carabiners — 5/16" Spring Snap (50 Pack)',
                    'Part ' + CARGO_NET_CARABINER_PART,
                  ) +
                  (adv.cargoHwCarabiner !== false
                    ? '<div style="' +
                      grid +
                      'margin:8px 0 4px;">' +
                      num('cargoHwCarabinerQty', '# of carabiner packs', 1, 40) +
                      '</div>'
                    : '') +
                  tog('cargoHwVRing', 'V-Ring Bolt', 'Part ' + ADV_HW_PARTS.vRings) +
                  (adv.cargoHwVRing !== false
                    ? '<div style="' +
                      grid +
                      'margin:8px 0 4px;">' +
                      num('cargoHwVRingQty', '# of V-ring packs', 1, 40) +
                      '</div>'
                    : '') +
                  '</div>'
                : '') +
              '</div>'
            : '') +
          tog('climbFrame', 'Climbing Wall — Frame Mounted') +
          tog('climbWall', 'Climbing Wall — Wall Mounted') +
          (climbWalls() > 0
            ? '<div style="padding-left:16px;">' +
              tog(
                'climbShield',
                'Climbing Wall — Safety Shield',
                'Qty mirrors # climbing walls (' + climbWalls() + ')',
              ) +
              tog(
                'climbMat',
                'Climbing Wall — Mat',
      ...
      sec(
        ...
        '<div style="' +
          ...
          hwNum(
            'carabiner',
            'Auto-Locking Carabiner (4pk)',
            0,
            8,
            'Suggested: ' + carabRec + ' — enter a quantity to include it',
          ) +
          hwNum(
            'webbingSling',
            'Multi-Pocket Webbing Sling',
            0,
            16,
            'Suggested: ' + (Number(adv.legs) || 0) + ' (one per leg)',
          ) +
          '</div>',
      ) +
      sec(
        ...
        tog(
          'floorPadding',
          'Floor Padding',
          'Sized from the frame: 14" added to each side. Priced per sq ft.',
        ) +
          (adv.floorPadding
            ? '<div style="margin:8px 0 12px;padding-left:16px;">' +
              padThicknessPicker() +
              padQuoteHint() +
              '</div>'
            : '') +
          ...
          (adv.matColumn
            ? '<div style="' +
              grid +
              'margin:8px 0 4px;">' +
              num('uShaped', 'U-Shaped Column Wraps (def = # ladders)', 0, 40) +
              num('completeWrap', 'Complete Column Wraps (def = legs − U-shaped)', 0, 40) +
              '</div>'
            : '') +
          tog(
            'matLadderLeg',
            'Adventure Mat System — Ladder Leg',
            'Qty = # of ladders (' + adv.laddersQty + ')',
          ) +
          tog(
            'matCustom',
            'Adventure Mat System — CUSTOM',
            'Mat SKU logic to be provided — added as manual line',
          ),
      ) +
      sec(
        'Hardware',
        '<div style="font-weight:600;font-size:13.5px;color:#3d4a55;margin-bottom:4px;">Quick Shift Saddle Bracket</div>' +
          ...
          (adv.brackets
            ? '<div style="' +
              ...
              '<div class="af"><label style="display:block;font-size:11px;color:#8a8f85;text-transform:uppercase;margin-bottom:4px;"># of 3/8" Non-Swivel Eye Bolts (auto)</label><input value="' +
              nonSwivel +
              '" disabled style="width:100%;padding:8px 10px;border:1px solid #eef0ea;border-radius:8px;font-size:14px;background:#f2f3ef;"></div>' +
              '</div>'
            : '') +
          '<div style="' +
          stack +
          'margin-top:14px;border-top:1px solid #f2f3ef;padding-top:14px;">' +
      ...
      '<button class="btn" id="advGen" style="width:auto;padding:11px 22px;">' +
      advGenLabel() +
      '</button></div>' +
      '</div>' +
      '</div>' +
    ...
    document.getElementById('advTrace').addEventListener('click', function () {
      openAdvTrace();
    });
    o.querySelectorAll('[data-ovr]').forEach(function (el) {
      el.addEventListener('change', function () {
        var base = el.getAttribute('data-ovr'),
          v = el.value.trim();
        adv.partOverrides = adv.partOverrides || {};
        if (!v || v === base) delete adv.partOverrides[base];
        else adv.partOverrides[base] = v;
        renderAdvKeepingTab(null, '[data-ovr="' + base.replace(/"/g, '') + '"]');
      });
    });
    o.querySelectorAll('[data-ovrclear]').forEach(function (b) {
      b.addEventListener('click', function () {
        if (adv.partOverrides) delete adv.partOverrides[b.getAttribute('data-ovrclear')];
        renderAdv();
      });
    });
    o.querySelectorAll('[data-hwdef]').forEach(function (b) {
      b.addEventListener('click', function () {
        var key = b.getAttribute('data-hwdef'),
          p = ADV_HW_PARTS[key];
        if (Array.isArray(p)) p = p[0];
    ...
    o.querySelectorAll('[data-ak]').forEach(function (el) {
      ...
      if (el.type === 'checkbox') {
        el.addEventListener('change', function () {
          adv[k] = el.checked;
          syncAdvDefaults(k);
          renderAdvKeepingTab(k);
        });
      } else {
        el.addEventListener('input', function () {
          adv[k] = el.type === 'number' ? parseFloat(el.value) || 0 : el.value;
          markHwTouched(k);
        });
        el.addEventListener('change', function () {
          adv[k] = el.type === 'number' ? parseFloat(el.value) || 0 : el.value;
  ...
  function renderAdvKeepingTab(key, sel) {
    renderKeepingTab(
      renderAdv,
      function () {
        return document.getElementById('advOverlay');
      },
  ...
  function autoConfig() {
    return (Number(adv.length) || 0) === (Number(adv.width) || 0) ? 'Square' : 'Rectangle';
  }

  function syncAdvDefaults(changed) {
    if (changed === 'config') adv.configManual = true;
    if ((changed === 'length' || changed === 'width') && !adv.configManual)
      adv.config = autoConfig();
    if (changed === 'length') {
    ...
    if (changed === 'brackets' && adv.brackets && !Number(adv.bracketsQty)) {
      adv.bracketsQty = 4;
      adv.swivel360 = 4;
    }
    ...
    if (changed === 'slide' && !adv.slide) {
      adv.slideA2216 = false;
      adv.slideGray = false;
      adv.steamroller = false;
      adv.slideConvKit = false;
    }
    ...
    if (changed === 'cargoNet10x8' || changed === 'cargoNet8x6') {
      if (adv.cargoNet10x8 || adv.cargoNet8x6) {
        adv.cargoHwCarabiner = true;
        adv.cargoHwVRing = true;
    ...
    if (changed === 'cargoNet' && !adv.cargoNet) {
      adv.cargoNet10x8 = false;
      adv.cargoNet8x6 = false;
      adv.cargoHwCarabiner = true;
      adv.cargoHwVRing = true;
    }
    if (changed === 'legs' || changed === 'length') {
      adv.completeWrap = Math.max(0, (Number(adv.legs) || 0) - (Number(adv.uShaped) || 0));
    }
    if (changed === 'ladders' || changed === 'laddersQty') {
      if (adv.ladders && adv.matColumn && !adv.uShaped) adv.uShaped = adv.laddersQty;
    }
    if (changed === 'matColumn' && adv.matColumn) {
      if (!adv.uShaped) adv.uShaped = adv.ladders ? adv.laddersQty : 0;
      adv.completeWrap = Math.max(0, (Number(adv.legs) || 0) - (Number(adv.uShaped) || 0));
    }
    if (changed === 'uShaped') {
      adv.completeWrap = Math.max(0, (Number(adv.legs) || 0) - (Number(adv.uShaped) || 0));
  ...
  function advAnswers() {
    return {
      length: Number(adv.length),
      width: Number(adv.width),
      config: adv.config,
      legs: Number(adv.legs),
      ladders: adv.ladders ? Number(adv.laddersQty) : 0,
      monkeyBars: !!adv.monkeyBars,
      monkeyBarsQty: Number(adv.monkeyBarsQty),
      interiorBeams: !!adv.interiorBeams,
      interiorBeamsQty: Number(adv.interiorBeamsQty),
      trolley: !!adv.trolley,
      trolleyType: adv.trolleyType,
      zipLine: !!adv.zipLine,
      zipLineQty: Number(adv.zipLineQty),
      ballRack: !!adv.ballRack,
      slide: !!adv.slide,
      slideA2216: !!adv.slideA2216,
      slideGray: !!adv.slideGray,
      steamroller: !!adv.steamroller,
      slideConvKit: !!adv.slideConvKit,
      cargoNet: !!adv.cargoNet,
      cargoNet10x8: !!adv.cargoNet10x8,
      cargoNet10x8Qty: Number(adv.cargoNet10x8Qty) || 1,
      cargoNet8x6: !!adv.cargoNet8x6,
      cargoNet8x6Qty: Number(adv.cargoNet8x6Qty) || 1,
      cargoHwCarabiner: adv.cargoHwCarabiner !== false,
      cargoHwCarabinerQty: Number(adv.cargoHwCarabinerQty) || 1,
      cargoHwVRing: adv.cargoHwVRing !== false,
      cargoHwVRingQty: Number(adv.cargoHwVRingQty) || 2,
      climbFrame: !!adv.climbFrame,
      climbWall: !!adv.climbWall,
      climbShield: !!adv.climbShield,
      climbMat: !!adv.climbMat,
      matFloor: !!adv.floorPadding,
      matColumn: !!adv.matColumn,
      uShaped: Number(adv.uShaped),
      completeWrap: Number(adv.completeWrap),
      matLadderLeg: !!adv.matLadderLeg,
      matCustom: !!adv.matCustom,
      floorPadding: !!adv.floorPadding,
      floorPadThickness: adv.floorPadThickness === '2' ? '2' : '3.25',
      brackets: !!adv.brackets,
      bracketsQty: Number(adv.bracketsQty),
      swivel360: Number(adv.swivel360),
      swivelStandalone: Number(adv.swivelStandalone),
      forged: Number(adv.forged),
      swingHanger: Number(adv.swingHanger),
      vRings: Number(adv.vRings),
      carabiner: Number(adv.carabiner),
      webbingSling: Number(adv.webbingSling),
  ...
  async function openAdvTrace(answers) {
    var btn = document.getElementById('advTrace');
    if (btn) {
      btn.disabled = true;
    ...
    var t = null;
    try {
      var r = await authed('/proposals/adventure-series/trace', {
        method: 'POST',
        body: answers || advAnswers(),
      });
      if (r.ok) t = await r.json();
    } catch (e) {}
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'Test the logic →';
    }
    if (!t) {
      alert('Could not reach the logic engine. Is the server running the latest build?');
      return;
    }
    var ov = document.createElement('div');
    ov.style.cssText =
      'position:fixed;inset:0;background:rgba(32,36,31,.5);z-index:80;overflow:auto;padding:24px 16px;';
    function th(label, right) {
      return (
        '<th style="text-align:' +
        (right ? 'right' : 'left') +
        ';padding:6px 8px;border-bottom:2px solid #3d4a55;font-size:10px;text-transform:uppercase;letter-spacing:.04em;color:#8a8f85;white-space:nowrap;">' +
        label +
        '</th>'
      );
    }
    var rows = (t.rows || [])
      .map(function (r) {
        return (
          '<tr style="' +
          (r.rolledIntoH1000 ? 'background:#fdfcf7;' : '') +
          '">' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;font-size:11.5px;color:#5c6157;">' +
          esc(r.rule) +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;font-family:ui-monospace,monospace;font-size:11px;">' +
          esc(r.part) +
          (r.inCatalog ? '' : ' <span style="color:#9c3327;">✕</span>') +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;font-size:11.5px;">' +
          esc(r.description) +
          (r.rolledIntoH1000
            ? '<span style="color:#8a6d1f;font-size:10.5px;"> — rolled into H-1000</span>'
            : '') +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;font-size:11px;color:#5c6157;">' +
          esc(r.formula) +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-weight:600;font-size:12px;">' +
          r.qty +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;">' +
          fmtMoney(r.unitPriceMinor, '') +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;font-weight:600;">' +
          fmtMoney(r.extendedMinor, '') +
          '</td>' +
          '<td style="padding:5px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;color:#5c6157;">' +
          fmtMoney(r.extendedCostMinor, '') +
          '</td></tr>'
        );
      })
      .join('');
    var hw = t.hardware || { components: [] };
    var hwRows = (hw.components || [])
      .map(function (c) {
        return (
          '<tr><td style="padding:4px 8px;border-bottom:1px solid #eef0ea;font-family:ui-monospace,monospace;font-size:11px;">' +
          esc(c.part || '—') +
          '</td>' +
          '<td style="padding:4px 8px;border-bottom:1px solid #eef0ea;font-size:11.5px;">' +
          esc(c.name) +
          (c.inCatalog
            ? ''
            : ' <span style="color:#9c3327;font-size:10.5px;">no SKU record</span>') +
          (c.edited
            ? ' <span style="background:#fdf6e3;border:1px solid #eadfbe;color:#8a6d1f;border-radius:999px;padding:1px 6px;font-size:10px;font-weight:600;">edited formula</span>'
            : '') +
          '</td>' +
          '<td style="padding:4px 8px;border-bottom:1px solid #eef0ea;font-size:11px;color:#5c6157;">' +
          esc(c.formula || '') +
          '</td>' +
          '<td style="padding:4px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;">' +
          c.qty +
          '</td>' +
          '<td style="padding:4px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;">' +
          fmtMoney(c.unitPriceMinor, '') +
          '</td>' +
          '<td style="padding:4px 8px;border-bottom:1px solid #eef0ea;text-align:right;font-size:11.5px;font-weight:600;">' +
          fmtMoney(c.unitPriceMinor * c.qty, '') +
          '</td></tr>'
        );
      })
      .join('');
    var warn = (t.warnings || []).length
      ? '<div style="margin-top:14px;padding:10px 12px;background:#fbecea;border:1px solid #f0d5d0;border-radius:9px;font-size:12px;color:#9c3327;line-height:1.6;">' +
        t.warnings.map(esc).join('<br>') +
        '</div>'
      : '';
    ov.innerHTML =
      ...
      '<div><div style="font-family:\'Newsreader\',serif;font-size:19px;font-weight:600;">Calculation trace — ' +
      esc(t.model) +
      '</div>' +
      '<div style="font-size:12px;color:#cdd6dc;">Frame Dimensions: ' +
      esc(t.dimensions) +
      ' · every quantity, formula and catalog price behind this configuration</div></div>' +
      ...
      '<div><div style="font-size:10px;text-transform:uppercase;color:#8a8f85;letter-spacing:.05em;">Revenue</div><div style="font-size:17px;font-weight:600;">' +
      fmtMoney(t.totals.revenueMinor, 'USD') +
      '</div></div>' +
      '<div><div style="font-size:10px;text-transform:uppercase;color:#8a8f85;letter-spacing:.05em;">COGS</div><div style="font-size:17px;font-weight:600;">' +
      fmtMoney(t.totals.cogsMinor, 'USD') +
      '</div></div>' +
      '<div><div style="font-size:10px;text-transform:uppercase;color:#8a8f85;letter-spacing:.05em;">Margin</div><div style="font-size:17px;font-weight:600;color:' +
      (t.totals.marginMinor >= 0 ? '#2f7d5d' : '#9c3327') +
      ';">' +
      fmtMoney(t.totals.marginMinor, 'USD') +
      ' · ' +
      t.totals.marginPct +
      '%</div></div>' +
      '<div><div style="font-size:10px;text-transform:uppercase;color:#8a8f85;letter-spacing:.05em;">Weight</div><div style="font-size:17px;font-weight:600;">' +
      (t.totals.weightLbs || 0).toLocaleString() +
      ' lbs</div></div>' +
      '</div>' +
      warn +
      ...
      (hwRows
        ? '<div style="overflow:auto;"><table style="width:100%;border-collapse:collapse;"><thead><tr>' +
          ...
          '</tr></thead><tbody>' +
          hwRows +
          '<tr><td colspan="5" style="padding:7px 8px;text-align:right;font-weight:700;font-size:12.5px;">H-1000 total</td><td style="padding:7px 8px;text-align:right;font-weight:700;font-size:12.5px;">' +
          fmtMoney(hw.priceMinor, 'USD') +
          '</td></tr>' +
          '</tbody></table></div>'
        : '<div class="muted" style="font-size:12.5px;">No hardware selected.</div>') +
      '</div></div>';
    document.body.appendChild(ov);
    ov.addEventListener('mousedown', function (e) {
      if (e.target === ov) document.body.removeChild(ov);
    });
    document.getElementById('trClose').addEventListener('click', function () {
      document.body.removeChild(ov);
    });
  }
  ...
  function openHardwareAudit(line) {
    ...
    var comps = line.components.slice().sort(function (x, y) {
      return String(x.part).localeCompare(String(y.part));
    });
    var qtyLine = Number(line.quantity) || 1;
    var sumPrice = comps.reduce(function (s, c) {
      return s + (Number(c.unitPriceMinor) || 0) * (Number(c.qty) || 0);
    }, 0);
    var sumCost = comps.reduce(function (s, c) {
      return s + (Number(c.unitCostMinor) || 0) * (Number(c.qty) || 0);
    }, 0);
    var sumWeight = comps.reduce(function (s, c) {
      return s + (Number(c.weightLbs) || 0) * (Number(c.qty) || 0);
    }, 0);
    var pieces = comps.reduce(function (s, c) {
      return s + (Number(c.qty) || 0);
    }, 0);
    var lineRate = (Number(line.rateMinor) || 0) * qtyLine;
    var lineCost = (Number(line.costEach) || 0) * qtyLine;
    var noPrice = comps.filter(function (c) {
      return !c.unitPriceMinor;
    }).length;
    ...
    var hasPrices = comps.some(function (c) {
      return Number(c.unitPriceMinor) > 0;
    });
    var drift =
      hasPrices && (Math.abs(sumPrice - lineRate) > 1 || Math.abs(sumCost - lineCost) > 1);
    ...
    function stat(label, value, color) {
      return (
        '<div><div style="font-size:10px;text-transform:uppercase;color:#8a8f85;letter-spacing:.05em;">' +
        label +
        '</div>' +
    ...
    var rows = comps
      .map(function (c) {
        ...
        var up = Number(c.unitPriceMinor) || 0,
          uc = Number(c.unitCostMinor) || 0;
        return (
          '<tr>' +
          td(
            esc(c.part || '—') +
              (c.inCatalog === false ? ' <span style="color:#9c3327;">✕</span>' : ''),
            0,
            1,
          ) +
          td(
          ...
          td(
            '<span style="color:#5c6157;font-family:ui-monospace,monospace;font-size:11px;">' +
              esc(c.formula || '—') +
              '</span>',
          ) +
          td('<b>' + q + '</b>', 1) +
    ...
    ov.innerHTML =
      ...
      '<div style="background:#3d4a55;color:#fff;padding:16px 22px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">' +
      '<div><div style="font-family:\'Newsreader\',serif;font-size:19px;font-weight:600;">' +
      esc(line.sku || 'Kit') +
      ' — how this line was calculated</div>' +
      '<div style="font-size:12px;color:#cdd6dc;">' +
      esc(line.name || '') +
      ' · ' +
      comps.length +
      ' part numbers · ' +
      ...
      '<div style="display:flex;gap:8px;align-items:center;">' +
      (hasAnswers
        ? '<button id="hwLive" style="border:1px solid rgba(255,255,255,.3);background:rgba(255,255,255,.12);color:#fff;border-radius:8px;padding:7px 14px;cursor:pointer;">Re-run the live logic →</button>'
        : '') +
      ...
      stat(
        'Cost on the line',
        fmtMoney(lineCost, 'USD'),
        Math.abs(sumCost - lineCost) > 1 ? '#9c3327' : '#2f7d5d',
      ) +
      ...
      (drift
        ...
        : '') +
      (noPrice && hasPrices
        ? '<div style="margin-bottom:14px;padding:10px 12px;background:#fdf6e6;border:1px solid #ecd9a6;border-radius:9px;font-size:12px;color:#6b5a24;line-height:1.6;">' +
          noPrice +
          ' of these part numbers carried no unit price when this line was built, so they added $0.00 to the kit. Set their price in Catalog → Pricing &amp; SKUs and re-generate.</div>'
        : '') +
      (!hasPrices
      ...
      td('<b>' + pieces + '</b>', 1) +
      td('', 1) +
      td('<b>' + (hasPrices ? fmtMoney(sumPrice, 'USD') : '—') + '</b>', 1) +
      td('', 1) +
      td('<b>' + fmtMoney(sumCost, 'USD') + '</b>', 1) +
      td('<b>' + sumWeight.toFixed(2) + '</b>', 1) +
      '</tr></tbody></table></div>' +
      ...
      (hasAnswers
        ? ''
        : ' This proposal was built before the configurator answers were kept with the proposal, so the live re-run is not available here — re-generate the lines from the configurator to enable it.') +
      '</div>' +
      '</div></div>';
    document.body.appendChild(ov);
    ov.addEventListener('mousedown', function (e) {
    ...
    if (live)
      live.addEventListener('click', function () {
        openAdvTrace(pb.meta.advAnswers);
      });
  }

  async function generateAdvLines(replace) {
    var btn = document.getElementById('advGen');
    if (btn) {
      btn.disabled = true;
    ...
    var out = (priced.lines || []).map(function (l) {
      return normalizeLine({
        ...
        sku: l.sku || '',
        description: l.description || '',
        quantity: l.quantity == null ? 0 : l.quantity,
        rateMinor: l.rateMinor || 0,
        costEach: l.costEach || 0,
        weightEach: l.weightEach || 0,
        optional: !!l.optional,
        internalNote: l.internalNote || '',
        components: l.components || null,
        source: 'ADV',
    ...
    advClose();
    renderBuilder();
    var bl = document.getElementById('bLines');
    if (bl) bl.scrollIntoView({ block: 'start' });
  }
  /**
  ...
  function replaceAdvLines(out) {
    var owned = [];
    for (var i = 0; i < pb.lines.length; i++)
      if (pb.lines[i] && pb.lines[i].source === 'ADV') owned.push(i);
  ...
  var SOAR_PAD_FALLBACK = [
    {
      ...
      defaultQty: 0,
      matFor: 'xl',
      description: 'Soar-XL Floor Mat System (138" x 80" x 3.25") - Single Fold',
    },
    {
      key: 'matStdQty',
      part: 'SSM80100',
      defaultQty: 1,
      matFor: 'std',
      description: 'Soar Floor Mat System (100" x 80" x 3.25") - Single Fold',
    },
    {
      key: 'uWrapQty',
      part: 'COLU2812',
      defaultQty: 4,
      description: 'Soar Base Beam U Column Wrap',
    },
  ...
  function soarFrameList() {
    return soarCat && soarCat.frames && soarCat.frames.length
      ? soarCat.frames
      : SOAR_FRAME_FALLBACK;
  }
  function soarPadList() {
    return soarCat && soarCat.padRows && soarCat.padRows.length
      ? soarCat.padRows
      : SOAR_PAD_FALLBACK;
  }
  ...
  function soarClose() {
    var o = document.getElementById('soarOverlay');
    if (o) document.body.removeChild(o);
  }
  ...
  function soarUnits() {
    return soar.rows.reduce(function (s, r) {
  ...
  function soarPadDefaults() {
    var byPart = {};
    soarFrameList().forEach(function (f) {
      byPart[f.part] = f;
    });
    var xl = 0,
      std = 0;
    soar.rows.forEach(function (r) {
      ...
      if (q <= 0) return;
      if (byPart[r.part] && byPart[r.part].xl) xl += q;
      else std += q;
    });
    var units = xl + std || 1,
      out = {};
  ...
  function soarVal(key, fallback) {
    return soar[key] == null || soar[key] === '' ? fallback : Math.max(0, Number(soar[key]) || 0);
  }

  function renderSoar() {
    var o = document.getElementById('soarOverlay');
    if (!o) return;
    var frames = soarFrameList(),
      pads = soarPadList(),
      defs = soarPadDefaults();
    function sec(title, inner, note) {
      return (
        '<div style="margin-bottom:18px;">' +
        '<div style="font-family:\'Newsreader\',serif;font-size:16px;font-weight:600;color:#3d4a55;border-bottom:1px solid #e7e8e3;padding-bottom:6px;margin-bottom:12px;">' +
        title +
        '</div>' +
        inner +
        (note
          ? '<div class="muted" style="font-size:11.5px;margin-top:8px;">' + note + '</div>'
          : '') +
        '</div>'
      );
    }
    var frameRows = soar.rows
      .map(function (r, i) {
        var hit = null;
        frames.forEach(function (f) {
          if (f.part === r.part) hit = f;
        });
        ...
        return (
          ...
          '<select data-sfpart="' +
          i +
          '" style="flex:1;min-width:0;padding:8px 10px;border:1px solid #dcded7;border-radius:8px;font-size:14px;background:#fff;">' +
          frames
            .map(function (f) {
              return (
                '<option value="' +
                esc(f.part) +
                '"' +
                (f.part === r.part ? ' selected' : '') +
                '>' +
                esc(f.part) +
                ' — ' +
                esc(f.label) +
                (f.unitPriceMinor ? '  ' + fmtMoney(f.unitPriceMinor, 'USD') : '') +
                '</option>'
              );
            })
            .join('') +
          '</select>' +
          '<input type="number" min="0" data-sfqty="' +
          i +
          '" value="' +
          r.qty +
          '" title="Quantity" style="width:74px;flex:0 0 auto;padding:8px 10px;border:1px solid #dcded7;border-radius:8px;font-size:14px;text-align:right;">' +
          (soar.rows.length > 1
            ? '<button data-sfdel="' +
              i +
              '" style="border:none;background:none;color:#8a8f85;cursor:pointer;font-size:18px;line-height:1;padding:0 4px;flex:0 0 auto;" title="Remove">×</button>'
          ...
          (missing
            ...
            : price
              ? '<div style="font-size:11px;color:#8a8f85;margin-top:3px;">' +
                fmtMoney(price * (Number(r.qty) || 0), 'USD') +
                ' extended</div>'
              : '') +
          '</div>'
        );
      ...
      : '<div style="margin-top:4px;">' +
        pads
          .map(function (p) {
            ...
            return (
              '<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid #f2f3ef;' +
              (v <= 0 ? 'opacity:.55;' : '') +
              '">' +
              '<div style="flex:1;min-width:0;">' +
              '<div style="font-size:13.5px;font-weight:600;">' +
              esc(p.description || p.part) +
              '</div>' +
              '<div style="font-size:11px;color:#8a8f85;margin-top:2px;"><code>' +
              esc(p.part) +
              '</code>' +
              (p.unitPriceMinor ? ' · ' + fmtMoney(p.unitPriceMinor, 'USD') + ' each' : '') +
              (p.matFor
                ? ' · mat for ' + (p.matFor === 'xl' ? "12' frames" : 'standard frames')
                : '') +
              (isDef
                ? ' · <span style="color:#3f9d78;">workbook default</span>'
                : ' · <span style="color:#b4522e;">overridden</span>') +
              '</div>' +
              '</div>' +
              '<input type="number" min="0" data-sk="' +
              p.key +
              '" value="' +
              v +
              '" style="width:82px;flex:0 0 auto;padding:8px 10px;border:1px solid #dcded7;border-radius:8px;font-size:14px;text-align:right;">' +
              '</div>'
            );
          })
          .join('') +
    ...
    o.innerHTML =
      ...
      sec(
        ...
        soarUnits() +
          ' frame' +
          (soarUnits() === 1 ? '' : 's') +
          ' on this proposal. All eight models are K-40xx parts.',
      ) +
      sec(
        'Padding / Column Wraps',
        '<label style="display:flex;align-items:center;gap:9px;padding:6px 0;cursor:pointer;font-size:14px;">' +
          '<input type="checkbox" data-sk="padding"' +
          (soar.padding ? ' checked' : '') +
          ...
          padBody,
        soar.padding
          ? 'Quantities are the workbook defaults (4 / 2 / 2 per frame). The mat follows frame width — XL frames take CLM325, everything else SSM80100. Edit any number to override.'
          : '',
      ) +
      '<label style="display:flex;align-items:center;gap:9px;font-size:13px;color:#5c6157;cursor:pointer;padding:6px 0;">' +
      '<input type="checkbox" data-sk="includeOverview"' +
      (soar.includeOverview ? ' checked' : '') +
      '> Print the Summit Soar overview &amp; Engineer-of-Record copy on each frame line' +
      '</label>' +
    ...
    o.addEventListener('mousedown', function (e) {
      if (e.target === o) soarClose();
    });
    document.getElementById('soarX').addEventListener('click', soarClose);
    document.getElementById('soarGen').addEventListener('click', function () {
      generateSoarLines(document.getElementById('soarReplace').checked);
    });
    document.getElementById('soarAddFrame').addEventListener('click', function () {
      var used = soar.rows.map(function (r) {
        return r.part;
      });
      var next =
        frames.filter(function (f) {
          return used.indexOf(f.part) < 0;
        })[0] || frames[0];
      soar.rows.push({ part: next.part, qty: 1 });
      renderSoar();
    });
    o.querySelectorAll('[data-sfpart]').forEach(function (el) {
      el.addEventListener('change', function () {
        ...
        renderKeepingTab(
          renderSoar,
          function () {
            return document.getElementById('soarOverlay');
          },
          '[data-sfpart="' + ix + '"]',
        );
      });
    ...
    o.querySelectorAll('[data-sfdel]').forEach(function (el) {
      el.addEventListener('click', function () {
        soar.rows.splice(Number(el.getAttribute('data-sfdel')), 1);
        renderSoar();
      });
    });
  ...
  function soarAnswers() {
    var a = {
      frames: soar.rows
        ...
        .map(function (r) {
          return { part: r.part, qty: Number(r.qty) || 0 };
        }),
    ...
    soarPadList().forEach(function (p) {
      if (soar[p.key] != null && soar[p.key] !== '')
        a[p.key] = Math.max(0, Number(soar[p.key]) || 0);
    });
    return a;
  }
  ...
  async function generateSoarLines(replace) {
    ...
    if (!priced) {
      if (btn) {
        btn.disabled = false;
        btn.textContent = 'Generate proposal lines →';
      }
      alert('Could not reach the pricing engine. Is the server running the latest build?');
      return;
    }
    ...
    out.forEach(applyItemDefaults);
    if (replace) pb.lines = out;
    else pb.lines = pb.lines.concat(out);
    soarClose();
    renderBuilder();
  }
  ...
  function linePickerClose() {
    var o = document.getElementById('linePickerOverlay');
    if (o) document.body.removeChild(o);
    linePicker = null;
  }
    ...
    try {
      ...
      if (r.ok) {
        ...
        nodes.forEach(function (n) {
          if (n.sku) linePicker.qty[n.slug] = n.defaultQuantity != null ? n.defaultQuantity : 1;
        });
      } else {
  ...
  function linePickerSiblingCmp(a, b) {
    ...
    if (d) return d;
    var ra = sizeRank(a.node.name),
      rb = sizeRank(b.node.name);
  ...
  function linePickerTree() {
    ...
    (function sortRec(list) {
      list.sort(linePickerSiblingCmp);
      list.forEach(function (e) {
        sortRec(e.children);
      });
    })(roots);
  ...
  function walkLinePickerTree(entries, depth, visit) {
    entries.forEach(function (e) {
      visit(e.node, depth, e);
      walkLinePickerTree(e.children, depth + 1, visit);
    });
  }
  ...
  function productDescendantSlugs(entry) {
    ...
    entry.children.forEach(function (c) {
      if (c.node.sku) out.push(c.node.slug);
      else out = out.concat(productDescendantSlugs(c));
    });
    return out;
  }
  ...
  function renderLinePicker() {
    ...
    if (linePicker.error) {
      ...
      body =
        '<div class="muted" style="padding:24px;text-align:center;">No items found for ' +
        esc(linePicker.lineName) +
        '.</div>';
    } else {
      var rows = '';
      ...
      walkLinePickerTree(tree, 0, function (n, depth, entry) {
        ...
        if (!n.sku) {
          ...
          if (prodSlugs.length) {
            var checkedN = prodSlugs.filter(function (s) {
              return !!linePicker.checked[s];
            }).length;
            var state = checkedN === 0 ? 'none' : checkedN === prodSlugs.length ? 'all' : 'some';
            headerCheck =
              '<input type="checkbox" class="lpHeaderCheck" data-slug="' +
              esc(n.slug) +
              '" data-state="' +
              state +
              '"' +
              (state === 'all' ? ' checked' : '') +
              ' style="width:16px;height:16px;flex:0 0 auto;margin-right:8px;">';
          }
          rows +=
            '<div style="display:flex;align-items:center;margin-left:' +
            indent +
            'px;padding:' +
            (isTier1 ? '14px 0 6px' : '8px 0 4px') +
            ';' +
            (isTier1 ? 'border-top:1px solid #e7e8e3;' : '') +
            '">' +
            headerCheck +
            ...
            '</span>' +
            (kit
              ? '<span style="margin-left:8px;font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;color:#8a8f85;border:1px solid #dcded7;border-radius:999px;padding:2px 8px;">All parts required</span>'
              : '') +
            '</div>';
          return;
        }
        ...
        rows +=
          '<label style="display:flex;align-items:center;gap:10px;padding:6px 0;margin-left:' +
          indent +
          'px;cursor:' +
          (inKit ? 'default' : 'pointer') +
          ';">' +
          '<input type="checkbox" class="lpCheck" data-slug="' +
          esc(n.slug) +
          '"' +
          (checked ? ' checked' : '') +
          (inKit ? ' disabled title="Part of a kit — the whole kit is selected together"' : '') +
          ' style="width:16px;height:16px;flex:0 0 auto;">' +
          '<span style="flex:1;min-width:0;font-size:13.5px;">' +
          esc(n.name) +
          ' <span class="muted" style="font-size:11.5px;">' +
          esc(n.sku) +
          (price ? ' · ' + fmtMoney(price, 'USD') + ' each' : '') +
          '</span></span>' +
          '<input type="number" min="0" class="lpQty" data-slug="' +
          esc(n.slug) +
          '" value="' +
          qty +
          '"' +
          (checked ? '' : ' disabled') +
          ' style="width:64px;flex:0 0 auto;padding:6px 8px;border:1px solid #dcded7;border-radius:7px;font-size:13px;text-align:right;">' +
          '</label>';
      });
      body = rows;
    }
    var checkedCount = Object.keys(linePicker.checked).filter(function (k) {
      return linePicker.checked[k];
    }).length;
    ...
    o.innerHTML =
      ...
      '<div style="background:#3d4a55;color:#fff;padding:18px 24px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:2;">' +
      '<div><div style="font-family:\'Newsreader\',serif;font-size:20px;font-weight:600;">' +
      esc(linePicker.lineName) +
      '</div>' +
      ...
      '</div>' +
      '<div style="padding:22px 24px;max-height:60vh;overflow:auto;">' +
      body +
      '</div>' +
      ...
      (checkedCount === 1 ? '' : 's') +
      ' selected</span>' +
      '<button class="btn" id="lpGen" style="width:auto;padding:11px 22px;"' +
      (checkedCount ? '' : ' disabled') +
      '>Insert into proposal →</button>' +
      '</div>' +
      '</div>';

    o.addEventListener('mousedown', function (e) {
      if (e.target === o) linePickerClose();
    });
    var x = document.getElementById('lpX');
    if (x) x.addEventListener('click', linePickerClose);
    var gen = document.getElementById('lpGen');
    if (gen) gen.addEventListener('click', insertLinePickerSelection);
    ...
    o.querySelectorAll('.lpQty').forEach(function (el) {
      el.addEventListener('change', function () {
        linePicker.qty[el.getAttribute('data-slug')] = Math.max(0, Number(el.value) || 0);
      });
    });
    o.querySelectorAll('.lpHeaderCheck').forEach(function (el) {
      ...
      el.addEventListener('change', function () {
        ...
        if (!entry) return;
        productDescendantSlugs(entry).forEach(function (slug) {
          linePicker.checked[slug] = el.checked;
        });
        renderLinePicker();
      });
    });
  ...
  function insertLinePickerSelection() {
    ...
    function hasCheckedDescendant(entry) {
      return entry.children.some(function (c) {
        return (
          (c.node.sku && linePicker.checked[c.node.slug]) ||
          (!c.node.sku && hasCheckedDescendant(c))
        );
      });
    }
    function walk(entries) {
      entries.forEach(function (e) {
        ...
        var qty =
          linePicker.qty[n.slug] != null ? Math.max(0, Number(linePicker.qty[n.slug]) || 0) : 1;
        out.push(
          applyItemDefaults({
            ref: uid(),
            lineType: 'PRODUCT',
  ...
  function openLockForm(versionId, user) {
    openModal(
      'Lock to operational order',
      fieldRow(
        'Approval method',
        selectEl(
          'aMethod',
          ['SIGNATURE', 'COUNTERSIGNED_PROPOSAL', 'PURCHASE_ORDER', 'EMAIL', 'VERBAL', 'PORTAL'],
        ...
        fieldRow(
          'Approved on',
          '<input id="aDate" type="date" value="' + todayISO() + '" style="' + IN + '">',
        ) +
        fieldRow(
          'Notes',
          '<textarea id="aNotes" rows="2" style="' + IN + 'resize:vertical;"></textarea>',
        ),
      async function (close, showErr) {
        var name = document.getElementById('aName').value.trim();
        if (!name) return showErr('Approver name is required.');
        var body = {
          method: document.getElementById('aMethod').value,
          approverName: name,
          approverTitle: document.getElementById('aTitle').value.trim() || undefined,
          poNumber: document.getElementById('aPo').value.trim() || undefined,
          approvedAt: new Date(document.getElementById('aDate').value || Date.now()).toISOString(),
        ...
        close();
        alert('Operational order created.');
        var nb = document.querySelector('[data-view="orders"]');
        if (nb) nb.click();
      },
      'Lock order',
    );
  ...
  function openUnlockForm(order, user) {
    openModal(
      'Unlock ' + (order.number || 'order') + ' for changes',
      '<div style="background:#fdf6e3;border:1px solid #eadfbe;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#8a6d1f;line-height:1.55;margin-bottom:12px;">' +
        ...
        '</div>' +
        '<div class="field"><label>Reason (required)</label><textarea id="ulReason" rows="3" placeholder="e.g. Customer added a second zip line before production started" style="' +
        IN +
      ...
      async function (close, showErr) {
        ...
        if (reason.length < 4)
          return showErr('Please give a reason — it goes on the order record.');
        var r = await authed('/orders/' + order.id + '/unlock', {
          method: 'POST',
          body: { reason: reason, createRevision: document.getElementById('ulRevision').checked },
        });
        if (!r.ok) {
          var msg = '';
          try {
  ...
  var ORDER_COLS = [
    {
      ...
      fixed: true,
      cell: function (o) {
        return '<b style="font-weight:600;">' + esc(o.customer || '—') + '</b>';
      },
      plain: function (o) {
        return o.customer || '';
      },
    ...
    },
    {
      ...
      label: 'Status',
      cell: function (o) {
        return '<span class="chip">' + titleCase(o.status) + '</span>';
      },
      plain: function (o) {
    ...
    },
    {
      ...
      label: 'Total',
      cell: function (o) {
        return fmtMoney(o.grandTotalMinor, o.currency);
      },
      plain: function (o) {
        return money(o.grandTotalMinor);
      },
    },
    {
      key: 'deposit',
      label: 'Deposit',
      cell: function (o) {
        return o.depositRequired ? fmtMoney(o.depositDueMinor, o.currency) : '—';
      },
      plain: function (o) {
        return o.depositRequired ? money(o.depositDueMinor) : '';
      },
    },
    {
      key: 'createdAt',
    ...
    },
    {
      ...
      label: 'Balance due',
      cell: function (o) {
        return fmtMoney(o.balanceDueMinor, o.currency);
      },
      plain: function (o) {
        return money(o.balanceDueMinor);
      },
    },
    {
      key: 'proposalNumber',
      label: 'Proposal #',
      cell: function (o) {
        return esc(o.proposalNumber || '—');
      },
      plain: function (o) {
        return o.proposalNumber || '';
      },
    },
    {
      key: 'proposalTitle',
      label: 'Project',
      cell: function (o) {
        return esc(o.proposalTitle || '—');
      },
      plain: function (o) {
        return o.proposalTitle || '';
      },
    },
    {
      key: 'acceptedVersion',
      ...
      cell: function (o) {
        return o.acceptedVersion ? 'v' + o.acceptedVersion : '—';
      },
      plain: function (o) {
    ...
    },
    {
      ...
      cell: function (o) {
        return o.approvalMethod ? titleCase(o.approvalMethod) : '—';
      },
      plain: function (o) {
        return titleCase(o.approvalMethod || '');
      },
    },
    {
      key: 'poNumber',
    ...
    },
    {
      ...
      label: 'Open tasks',
      cell: function (o) {
        return (o.openTasks || 0) + ' / ' + (o.taskCount || 0);
      },
      plain: function (o) {
        return (o.openTasks || 0) + ' of ' + (o.taskCount || 0);
      },
    },
    {
      key: 'requirements',
      ...
      cell: function (o) {
        return (o.openRequirements || 0) + ' / ' + (o.requirementCount || 0);
      },
      plain: function (o) {
        return (o.openRequirements || 0) + ' of ' + (o.requirementCount || 0);
      },
    },
    {
      key: 'procurement',
      label: 'Sourced',
      cell: function (o) {
        return (o.procurementSourced || 0) + ' / ' + (o.procurementCount || 0);
      },
      plain: function (o) {
        return (o.procurementSourced || 0) + ' of ' + (o.procurementCount || 0);
      },
    },
    {
      key: 'qbo',
      label: 'QuickBooks',
      cell: function (o) {
        return o.qboEstimateTxnId
          ? '<span class="chip">Linked</span>'
          : '<span class="muted">Not pushed</span>';
      },
      plain: function (o) {
        return o.qboEstimateTxnId ? 'Linked' : 'Not pushed';
      },
    },
    {
      key: 'monday',
      label: 'monday.com',
      cell: function (o) {
        return o.mondayProjectId
          ? '<span class="chip">Linked</span>'
          : '<span class="muted">—</span>';
      },
      plain: function (o) {
        return o.mondayProjectId ? 'Linked' : '';
      },
    },
    {
      key: 'updatedAt',
  ...
  function orderColKeys() {
    var saved = null;
    try {
      saved = JSON.parse(localStorage.getItem(ORDER_COLS_KEY) || 'null');
    } catch (e) {}
  ...
  function orderCol(key) {
    for (var i = 0; i < ORDER_COLS.length; i++) if (ORDER_COLS[i].key === key) return ORDER_COLS[i];
    return null;
  }
  ...
  async function renderOrders(user) {
    ...
    document.getElementById('ordCols').addEventListener('click', function () {
      openOrderColumnPicker(user);
    });
    ...
    try {
      var r = await authed('/orders');
      if (!r.ok) {
        document.getElementById('ordList').innerHTML =
          '<div class="err">Could not load (' + r.status + ').</div>';
        return;
      }
      ...
      var rows = ordersData
        ...
        .join('');
      document.getElementById('ordList').innerHTML = tableShell(
        cols.map(function (c) {
          return c.label;
        }),
        rows,
        cols.length,
        'No operational orders yet. Lock an accepted proposal to create one.',
      );
      document.querySelectorAll('#ordList tr[data-id]').forEach(function (tr) {
        tr.addEventListener('click', function () {
          openOrderDetail(tr.getAttribute('data-id'), user);
        });
      });
    } catch (e) {
      document.getElementById('ordList').innerHTML =
        '<div class="err">Could not reach the server.</div>';
    }
  }
  ...
  function openOrderColumnPicker(user) {
    ...
    var body =
      '<div class="muted" style="font-size:13px;margin-bottom:12px;">Customer and signed date always lead the table. Choose what else to show.</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px 18px;">' +
      ORDER_COLS.filter(function (c) {
        return !c.fixed;
      })
        .map(function (c) {
          return (
            '<label style="display:flex;gap:8px;align-items:center;font-size:13.5px;"><input type="checkbox" class="ordColChk" value="' +
            c.key +
            '"' +
            (chosen.indexOf(c.key) > -1 ? ' checked' : '') +
            '>' +
            esc(c.label) +
            '</label>'
          );
    ...
    openModal(
      ...
      function (close) {
        var keys = [];
        document.querySelectorAll('.ordColChk').forEach(function (chk) {
          if (chk.checked) keys.push(chk.value);
        });
  ...
  async function openOrderDetail(id, user) {
    ...
    try {
      ...
      if (!r1.ok) {
        var msg = '';
        try {
          msg = ((await r1.json()) || {}).message || '';
        } catch (e0) {}
        view.innerHTML =
          '<div class="err">Could not load this order (' +
          r1.status +
          ').' +
      ...
      order = await r1.json();
      var r2 = await authed('/orders/' + id + '/status');
      st = r2.ok ? await r2.json() : {};
      var r3 = await authed('/orders/' + id + '/audit');
      audit = r3.ok ? await r3.json() : [];
    } catch (e) {
      view.innerHTML = '<div class="err">Could not load order.</div>';
      return;
    }
    ...
    view.innerHTML =
      ...
      (hasRole(ORDERS_MANAGE_ROLES, user.role) &&
      order.status !== 'CANCELLED' &&
      order.status !== 'COMPLETE'
        ? '<button class="link-btn" id="ordUnlock" style="width:auto;padding:8px 15px;color:#9c3327;">Unlock for changes</button>'
        : '') +
      '</div>' +
      '<div class="card" style="margin-bottom:16px;"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;"><div><div class="k">' +
      esc(order.number) +
      '</div><h2 style="font-size:22px;margin-top:2px;">Operational order</h2><div class="muted" style="font-size:13px;margin-top:4px;">Accepted proposal v' +
      (order.acceptedVersion || '') +
      '</div></div>' +
      '<div style="text-align:right;"><span class="chip">' +
      titleCase(order.status) +
      '</span><div style="margin-top:8px;font-size:13px;">' +
      (integ.ok
        ? '<span class="dot ok"></span>Integrity verified'
        : '<span class="dot bad"></span>Integrity drift') +
      '</div></div></div>' +
      '<div class="grid" style="margin-top:16px;"><div><div class="k">Total</div><div class="v small">' +
      fmtMoney(order.grandTotalMinor, order.currency) +
      '</div></div>' +
      '<div><div class="k">Deposit</div><div class="v small">' +
      (order.depositRequired ? fmtMoney(order.depositDueMinor, order.currency) : '—') +
      '</div></div>' +
      '<div><div class="k">Balance due</div><div class="v small">' +
      fmtMoney(order.balanceDueMinor, order.currency) +
      '</div></div>' +
      '<div><div class="k">Customer approval</div><div class="v small">' +
      (order.customerApproval ? esc(order.customerApproval.approverName) : '—') +
      '</div></div></div></div>' +
      (hasRole(ORDERS_MANAGE_ROLES, user.role)
        ? sectionBlock(
            'Manufacturing',
            '<div id="mfgBox"><div class="muted" style="padding:16px;">Loading…</div></div>',
          )
        : '') +
      ...
      (hasRole(QBO_VIEW_ROLES, user.role)
        ? sectionBlock(
            'QuickBooks',
            '<div id="qboBox"><div class="muted" style="padding:16px;">Loading…</div></div>',
          )
      ...
      sectionBlock(
        'Bill of Materials',
        '<div id="bomBox"><div class="muted" style="padding:16px;">Loading…</div></div>',
      ) +
      sectionBlock('Audit timeline', auditRows(audit));
    document.getElementById('ordBack').addEventListener('click', function () {
      renderOrders(user);
    });
  ...
  function hoStatusSelect(kind, id, opts, sel) {
    return (
      '<select data-kind="' +
      kind +
      '" data-id="' +
      id +
      '" class="hoStatus" style="padding:6px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;background:#fff;">' +
      opts
        .map(function (o) {
          return (
            '<option value="' +
  ...
  function changedBy(row) {
    ...
    return (
      '<div class="muted" style="font-size:11.5px;margin-top:5px;line-height:1.4;">' +
      esc(row.updatedByName) +
      ' · ' +
  ...
  function reqRows(reqs, edit) {
    var rows = reqs
      .map(function (r) {
        var cell = edit
          ? hoStatusSelect(
              'req',
              r.id,
              ['OPEN', 'IN_PROGRESS', 'BLOCKED', 'COMPLETE', 'WAIVED'],
              r.status,
            )
          : '<span class="chip">' + titleCase(r.status) + '</span>';
        return (
          '<tr>' +
          td(esc(titleCase(r.category))) +
          td(esc(r.title)) +
          td(cell + changedBy(r)) +
          td(
            r.isException
              ? '<span class="chip" style="background:#fbecea;color:#9c3327;">Exception</span>'
              : '—',
          ) +
          '</tr>'
        );
      ...
      .join('');
    return tableShell(
      ['Category', 'Requirement', 'Status & last change', 'Flag'],
      rows,
      4,
      'No requirements.',
    );
    ...
    var rows = tasks
      .map(function (t) {
        var cell = edit
          ...
          : '<span class="chip">' + titleCase(t.status) + '</span>';
        return (
          '<tr>' +
          td('<b style="font-weight:600;">' + esc(t.title) + '</b>') +
          td(esc(t.assigneeRole ? titleCase(t.assigneeRole) : 'Unassigned')) +
          td(cell + changedBy(t)) +
          td(t.dueDate ? fmtDate(t.dueDate) : '—') +
          '</tr>'
        );
      })
      .join('');
  ...
  async function loadManufacturing(order, user) {
    ...
    try {
      ...
      if (!r.ok) {
        box.innerHTML =
          '<div class="muted" style="padding:16px;">Could not read the manufacturing gate (' +
          r.status +
          ').</div>';
        return;
      }
      ...
      box.innerHTML =
        '<div class="muted" style="padding:16px;">Could not read the manufacturing gate.</div>';
      return;
    }
    ...
    var state = released
      ? '<div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;">' +
        ...
        '<span class="muted" style="font-size:12.5px;">' +
        fmtDateTime(order.manufacturingReleasedAt) +
        (order.manufacturingReleasedByName ? ' · ' + esc(order.manufacturingReleasedByName) : '') +
        '</span>' +
        '</div>'
      : '<div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;">' +
    ...
    var gateLine = gate.invoiceCreated
      ? '<div style="font-size:13px;color:#2f6b4f;"><span class="dot ok"></span>Invoice ' +
        esc(gate.invoiceDocNumber || '') +
        ' created in QuickBooks' +
        (gate.invoiceCreatedAt ? ' on ' + fmtDate(gate.invoiceCreatedAt) : '') +
        '.</div>'
      : gate.waived
    ...
    var docs = (gate.documents || []).length
      ? '<div class="muted" style="font-size:11.5px;margin-top:8px;">In QuickBooks: ' +
        gate.documents
          .map(function (d) {
            return (
              esc(titleCase(d.type)) +
              ' ' +
              esc(d.docNumber || '—') +
              ' (' +
              esc(titleCase(d.status)) +
              ')'
            );
          })
          .join(' · ') +
    ...
    var actions =
      released || cancelled
        ...
        : '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px;">' +
          '<button class="btn" id="mfgRelease" style="width:auto;padding:9px 16px;"' +
          (gate.satisfied ? '' : ' disabled') +
          '>Push to Manufacturing</button>' +
          (gate.satisfied
            ? ''
            : '<button class="link-btn" id="mfgWaive" style="width:auto;padding:9px 16px;color:#9c3327;">Waive the invoice requirement…</button>') +
          '</div>';
    ...
    if (rel)
      rel.addEventListener('click', async function () {
        if (
          !confirm(
            'Release ' +
              (order.number || 'this order') +
              ' to manufacturing?\n\nThe shop treats this as the go-ahead to build.',
          )
        )
          return;
        rel.disabled = true;
        var rr = await authed('/orders/' + order.id + '/manufacturing/release', {
          method: 'POST',
          body: {},
        });
        if (!rr.ok) {
          rel.disabled = false;
          alert(await serverMessage(rr, 'Could not release this order (' + rr.status + ').'));
          return;
        }
      ...
      wv.addEventListener('click', function () {
        openModal(
          ...
          '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">Manufacturing normally waits for a QuickBooks invoice. Waiving that is recorded against your name on the order timeline, and Accounting is emailed straight away. Say why.</div>' +
            fieldRow(
              'Reason',
              '<input id="mfgWaiveReason" placeholder="e.g. PO received, invoice to follow Monday" style="' +
                IN +
                '">',
            ),
          async function (close, showErr) {
            ...
            var rw = await authed('/orders/' + order.id + '/manufacturing/waive-invoice', {
              method: 'POST',
              body: { reason: reason },
            });
            if (!rw.ok) {
              var m = '';
              try {
                m = ((await rw.json()) || {}).message || '';
              } catch (e) {}
              return showErr(m || 'Could not waive (' + rw.status + ').');
            }
            var out = await rw.json();
            close();
            if (out && out.notifyError)
              alert('Waived, but Accounting was not emailed: ' + out.notifyError);
            openOrderDetail(order.id, user);
          },
          'Waive',
        );
  ...
  function bomFieldStyle(w) {
    return (
      'width:' +
      (w || '100%') +
      ';padding:7px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;background:#fff;color:#20241f;outline:none;'
    );
  }
  ...
  function bomFieldStyle(w, locked) {
    return (
      ...
      (locked ? '#e7e8e3' : '#dcded7') +
      ';border-radius:8px;font-size:13px;background:' +
      (locked ? '#f6f7f4' : '#fff') +
      ';color:' +
      (locked ? '#8a8f85' : '#20241f') +
      ';outline:none;'
    );
  }

  async function loadBomSections(order, user, canHandoff) {
    var box = document.getElementById('bomBox');
    if (!box) return;
    bomOrder = order;
    ...
    try {
      ...
      box.innerHTML = '<div class="err">Could not load the Bill of Materials.</div>';
      return;
    }

    if (!procData.length) {
      box.innerHTML =
        '<div class="placeholder" style="padding:20px;"><p class="muted" style="margin:0;">No Bill of Materials lines.</p></div>';
      return;
    }
    box.innerHTML =
      ...
      bomSectionData
        .map(function (s, i) {
          return sectionCard(s, i, canHandoff);
        })
        .join('') +
  ...
  function sectionCard(s, idx, canHandoff) {
    ...
    var statusChip = locked
      ? '<span class="chip" style="background:#eaf1ec;color:#2f6b4f;">Submitted ' +
        (s.submittedOn ? fmtDate(s.submittedOn) : '') +
        '</span>'
      : '<span class="chip">Draft</span>';

    var confirmLine =
      locked && s.confirmedBy
        ? '<div class="muted" style="font-size:11.5px;margin-top:4px;">Confirmed by ' +
          esc(s.confirmedBy) +
          ' · ' +
          fmtDate(s.confirmedAt) +
          '</div>'
        : s.unlockedBy
          ? '<div class="muted" style="font-size:11.5px;margin-top:4px;">Reopened by ' +
            esc(s.unlockedBy) +
            ' · ' +
            fmtDate(s.unlockedAt) +
            '</div>'
          : '';
    ...
    var hasBag = lines.some(function (p) {
      return p.packagingBag;
    });
    ...
    var rowHtmlFor = function (p) {
      ...
      var buy = p.productUrl
        ? ' <a href="' +
          esc(p.productUrl) +
          '" target="_blank" rel="noopener" style="font-size:11.5px;margin-left:6px;">Buy ↗</a>'
        : '';
      return (
        '<tr>' +
        ...
        td(
          edit
            ? '<input class="bomLine" data-id="' +
                p.id +
                '" data-f="vendorNotes" value="' +
                esc(p.vendorNotes || '') +
                '" placeholder="—" style="' +
                bomFieldStyle('160px') +
                '">'
        ...
        td(
          edit
            ? '<select class="bomLine" data-id="' +
                p.id +
                '" data-f="sourced" style="' +
                bomFieldStyle('110px') +
                '">' +
                '<option value="false"' +
                (p.sourced ? '' : ' selected') +
                ...
                '<option value="true"' +
                (p.sourced ? ' selected' : '') +
                '>Ordered</option></select>'
            : p.sourced
              ? '<span class="chip">Ordered</span>'
              : '<span class="muted">Pending</span>',
        ) +
        (edit
          ? td(
              '<button class="link-btn" data-line-del="' +
                p.id +
                '" data-line-name="' +
                esc(p.name) +
                '" title="Take this part off the order" style="width:auto;padding:5px 10px;font-size:12px;color:#a2402f;">Remove</button>',
            )
          : '') +
        '</tr>'
      );
    ...
    var prodLines = lines.filter(function (p) {
      return !p.isHardwareComponent;
    });
    var hwLines = lines.filter(function (p) {
      return p.isHardwareComponent;
    });
    var divider = function (label) {
      return (
        '<tr><td colspan="' +
        cols +
        '" style="padding:11px 16px 5px;font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#5c6157;background:#f4f5f1;border-top:1px solid #e7e8e3;">' +
        label +
        '</td></tr>'
      );
    };
    var rows =
      prodLines.map(rowHtmlFor).join('') +
      (hwLines.length ? divider('Hardware') + hwLines.map(rowHtmlFor).join('') : '') +
      '<tr><td style="padding:12px 16px;border-top:1px solid #e7e8e3;font-weight:600;">Total — ' +
      s.lineCount +
      ' line' +
      (s.lineCount === 1 ? '' : 's') +
      '</td>' +
      '<td style="padding:12px 16px;border-top:1px solid #e7e8e3;"></td>' +
      '<td style="padding:12px 16px;border-top:1px solid #e7e8e3;font-weight:600;">' +
      s.unitCount +
      '</td>' +
      '<td colspan="' +
      (2 + (showColor ? 1 : 0) + (showBag ? 1 : 0)) +
      '" style="padding:12px 16px;border-top:1px solid #e7e8e3;"></td>' +
      '<td style="padding:12px 16px;border-top:1px solid #e7e8e3;font-weight:600;">' +
      money2(s.extendedCostMinor) +
      '</td>' +
      '<td colspan="' +
      (edit ? 3 : 2) +
      '" style="padding:12px 16px;border-top:1px solid #e7e8e3;"></td></tr>';
    ...
    var warn = s.missingColorSkus.length
      ? '<div style="background:#fdf6e6;border:1px solid #ecd9a6;border-radius:9px;padding:9px 12px;font-size:12.5px;color:#6b5a24;margin-bottom:10px;">' +
        s.missingColorSkus.length +
        ' part' +
        (s.missingColorSkus.length === 1 ? '' : 's') +
        ' still need a colour: <code>' +
        s.missingColorSkus.slice(0, 6).map(esc).join('</code>, <code>') +
        '</code>' +
        (s.missingColorSkus.length > 6 ? ' and ' + (s.missingColorSkus.length - 6) + ' more' : '') +
        '</div>'
      : '';
    ...
    return (
      ...
      s.id +
      '" style="margin-bottom:22px;padding:0;overflow:hidden;border-left:3px solid ' +
      (locked ? '#3f9d78' : '#c9a227') +
      ';">' +
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;padding:16px 18px;background:' +
      (locked ? '#f6f7f4' : '#fbfbf9') +
      ';border-bottom:1px solid #e7e8e3;">' +
      '<div>' +
      '<div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;">' +
      '<h3 style="font-size:16px;margin:0;font-weight:650;">' +
      esc(s.vendor) +
      '</h3>' +
      statusChip +
      '</div>' +
      ...
      (canHandoff && idx > 0
        ? '<button class="link-btn" data-sec-move="up" data-id="' +
          s.id +
          '" title="Move this section earlier in the export" style="width:auto;padding:6px 10px;">↑</button>'
        : '') +
      (canHandoff && idx < bomSectionData.length - 1
        ? '<button class="link-btn" data-sec-move="down" data-id="' +
          s.id +
          '" title="Move this section later in the export" style="width:auto;padding:6px 10px;">↓</button>'
        : '') +
      '<button class="link-btn" data-proc="csv" data-vendor="' +
      esc(s.vendor) +
      '" style="width:auto;padding:7px 13px;">Excel</button>' +
      '<button class="link-btn" data-proc="pdf" data-vendor="' +
      esc(s.vendor) +
      '" style="width:auto;padding:7px 13px;">PDF</button>' +
      (canHandoff
        ? '<button class="btn" data-sec-email="' +
          s.id +
          '" title="Emails this vendor their sheet and submits the section" style="width:auto;padding:8px 14px;">Email vendor</button>'
        : '') +
      (canHandoff && !locked
        ? '<button class="link-btn" data-sec-confirm="' +
          s.id +
          '" title="Use when the sheet went out some other way" style="width:auto;padding:8px 14px;">Mark sent by hand</button>'
        : '') +
      (canHandoff && locked
        ? '<button class="link-btn" data-sec-unlock="' +
          s.id +
          '" style="width:auto;padding:8px 14px;color:#9c3327;">Unlock for revisions</button>'
        : '') +
      '</div>' +
      '</div>' +
      '<div style="padding:16px 18px;">' +
      ...
      '<div><div class="k">Job name</div><input class="secF" data-id="' +
      s.id +
      '" data-f="jobName" value="' +
      esc(s.jobName || '') +
      '" style="' +
      bomFieldStyle(null, locked) +
      '"' +
      dis +
      '></div>' +
      '<div><div class="k">Ship to</div><select class="secF" data-id="' +
      s.id +
      '" data-f="shipTo" style="' +
      bomFieldStyle(null, locked) +
      '"' +
      ...
      (s.shipTo === 'SUMMIT' ? '' : ' selected') +
      '>Customer site</option>' +
      '<option value="SUMMIT"' +
      (s.shipTo === 'SUMMIT' ? ' selected' : '') +
      '>Summit Sensory Gym</option></select></div>' +
      '<div><div class="k">Submission date</div><input class="secF" data-id="' +
      s.id +
      '" data-f="submittedOn" type="date" value="' +
      esc(dateVal) +
      '" style="' +
      bomFieldStyle(null, locked) +
      (placeholderDate ? 'color:#8a8f85;' : '') +
      '"' +
      dis +
      '>' +
      (placeholderDate
        ? '<div class="muted" style="font-size:11px;margin-top:3px;">Today, until you confirm or change it</div>'
        : '') +
      '</div>' +
      '<div><div class="k">Delivery type</div><input class="secF" data-id="' +
      s.id +
      '" data-f="deliveryType" value="' +
      esc(s.deliveryType || '') +
      '" placeholder="e.g. Lift Gate" style="' +
      bomFieldStyle(null, locked) +
      '"' +
      dis +
      '></div>' +
      '<div><div class="k">Estimated shipment quote</div><input class="secF" data-id="' +
      s.id +
      '" data-f="shipmentQuote" value="' +
      esc(s.shipmentQuote || '') +
      '" placeholder="TBD" style="' +
      bomFieldStyle(null, locked) +
      '"' +
      dis +
      '></div>' +
      ...
      '<textarea class="secF" data-id="' +
      s.id +
      '" data-f="notes" rows="2" placeholder="Prints beneath the line items" style="' +
      bomFieldStyle(null, locked) +
      'resize:vertical;"' +
      dis +
      '>' +
      ...
      '<label style="display:flex;align-items:center;gap:8px;font-size:12.5px;color:#5c6157;margin-top:10px;' +
      (edit ? 'cursor:pointer;' : 'opacity:.6;') +
      '">' +
      '<input type="checkbox" class="secColorCol" data-id="' +
      s.id +
      '"' +
      (s.showPowderColor ? ' checked' : '') +
      (edit ? '' : ' disabled') +
      '>' +
      ...
      (hasBag
        ? '<label style="display:flex;align-items:center;gap:8px;font-size:12.5px;color:#5c6157;margin-top:6px;' +
          (edit ? 'cursor:pointer;' : 'opacity:.6;') +
          '">' +
          '<input type="checkbox" class="secBagCol" data-id="' +
          s.id +
          '"' +
          (s.showPackagingBag ? ' checked' : '') +
          (edit ? '' : ' disabled') +
          '>' +
      ...
      tableShell(
        ['Item', 'Part #', 'Qty'].concat(
          showBag ? ['Bag #'] : [],
          showColor ? ['Powder color'] : [],
          ['Weight (lb)', 'Cost each', 'Total cost', 'Notes', 'Status'],
          edit ? [''] : [],
        ),
        rows,
        cols,
      ...
      (edit
        ? '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:11px;flex-wrap:wrap;">' +
          ...
          '<button class="link-btn" data-line-add="' +
          esc(s.vendor) +
          '" style="width:auto;padding:8px 14px;white-space:nowrap;">Add a part</button>' +
          '</div>'
        : '') +
      sendHistory(s) +
      '</div>' +
  ...
  function qtyCell(p, edit) {
    ...
    var badge = changed
      ? '<div class="muted" style="font-size:11px;margin-top:3px;color:#8a6d1f;" title="' +
        (p.quantityEditedBy ? esc(p.quantityEditedBy) + ' · ' : '') +
        (p.quantityEditedAt ? esc(fmtDateTime(p.quantityEditedAt)) : '') +
        '">Was ' +
        orig +
        '</div>'
      : '';
    ...
    return (
      '<input class="bomLine" data-id="' +
      p.id +
      '" data-f="quantity" type="number" min="1" step="1" value="' +
      Number(p.quantity) +
      '" style="' +
      bomFieldStyle('72px') +
      (changed ? 'border-color:#c9a227;' : '') +
      '">' +
      badge
    );
  }
  ...
  function colorCell(p) {
    ...
    return (
      ...
      '<input class="bomLine" data-id="' +
      p.id +
      '" data-f="powderColorCode" value="' +
      esc(p.powderColorCode || '') +
  ...
  function colorApplyRow(s) {
    var codes = [];
    bomBrands.forEach(function (b) {
      (b.recentCodes || []).forEach(function (c) {
        codes.push(c);
      });
    });
    return (
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:12px;padding:11px 13px;background:#fbfbf9;border:1px solid #eceee8;border-radius:9px;">' +
      '<span style="font-size:12.5px;color:#5c6157;">Set a colour across this section:</span>' +
      '<select class="secColorBrand" data-id="' +
      s.id +
      '" style="' +
      bomFieldStyle('130px') +
      '">' +
      ...
      bomBrands
        ...
        .join('') +
      '</select>' +
      '<input class="secColorCode" data-id="' +
      s.id +
      '" list="bomCodeList" placeholder="Colour code" style="' +
      bomFieldStyle('140px') +
      '">' +
      (codes.length
        ? '<datalist id="bomCodeList">' +
          codes
            .map(function (c) {
          ...
          '</datalist>'
        : '') +
      '<button class="link-btn" data-sec-color="' +
      s.id +
      '" style="width:auto;padding:7px 13px;">Apply</button>' +
  ...
  function questionBlock(s, edit) {
    ...
    var fields = qs
      .map(function (q) {
        var name = 'q_' + q.id,
          v = q.value || '',
          st = bomFieldStyle(null, !edit),
          dis = edit ? '' : ' disabled';
        var input;
        if (q.type === 'SELECT') {
          input =
            '<select class="secQ" data-id="' +
            q.id +
            '" style="' +
            st +
            '"' +
          ...
          input =
            ...
            q.options
              .map(function (o) {
                return (
                  '<label style="display:flex;gap:5px;align-items:center;font-size:13px;cursor:' +
                  (edit ? 'pointer' : 'default') +
                  ';">' +
                  '<input type="checkbox" class="secQM" data-id="' +
                  q.id +
                  '" value="' +
                  esc(o) +
                  '"' +
                  (picked.indexOf(o) >= 0 ? ' checked' : '') +
                  dis +
                  '> ' +
                  esc(o) +
                  '</label>'
        ...
        return (
          '<div><div class="k">' +
          esc(q.label) +
          (q.required ? ' <span style="color:#9c3327;">*</span>' : '') +
          (edit && !q.fromTemplate
            ? ' <button class="link-btn" data-q-del="' +
              q.id +
              '" title="Remove this question" style="width:auto;padding:0 4px;font-size:11px;color:#9c3327;display:inline;">×</button>'
            : '') +
          '</div>' +
          input +
          '</div>'
      ...
      .join('');
    return (
      '<div style="margin-top:14px;padding-top:14px;border-top:1px dashed #e2e4de;">' +
      ...
      (edit
        ? '<button class="link-btn" data-q-add="' +
          s.id +
          '" style="width:auto;padding:6px 11px;font-size:12px;">Add a question</button>'
        : '') +
      '</div>' +
      (qs.length
  ...
  function sendHistory(s) {
    ...
    var rows = s.sends
      .map(function (x) {
        var chip =
          x.status === 'FAILED' || x.status === 'BOUNCED'
            ? '<span class="chip" style="background:#fbecea;color:#9c3327;">' +
              titleCase(x.status) +
              '</span>'
            : '<span class="chip" style="background:#eaf1ec;color:#2f6b4f;">' +
              titleCase(x.status) +
              '</span>';
        ...
        var delivered = x.deliveredAt
          ? fmtDateTime(x.deliveredAt) +
            (x.openedAt
              ? '<div class="muted" style="font-size:11px;margin-top:2px;">Opened ' +
                fmtDateTime(x.openedAt) +
                '</div>'
              : '')
          : '<span class="muted">Not confirmed</span>';
        return (
          '<tr>' +
          td(fmtDateTime(x.sentAt)) +
          td(esc(x.sentBy || '—')) +
          td(esc(x.toEmail)) +
          td(esc(x.format)) +
          td(
            chip +
              (x.error
                ? '<div class="muted" style="font-size:11px;color:#9c3327;margin-top:3px;">' +
                  esc(x.error) +
                  '</div>'
                : ''),
          ) +
  ...
  function wireBom(order, user, canHandoff) {
    ...
    document.querySelectorAll('.secColorCol').forEach(function (el) {
      el.addEventListener('change', async function () {
        var r = await authed('/bom/sections/' + el.getAttribute('data-id'), {
          method: 'PATCH',
          body: { showPowderColor: el.checked },
        });
        if (!r.ok) {
          alert('Could not change that (' + r.status + ').');
          el.checked = !el.checked;
          return;
        }
        loadBomSections(order, user, canHandoff);
    ...
    document.querySelectorAll('.secBagCol').forEach(function (el) {
      el.addEventListener('change', async function () {
        var rb = await authed('/bom/sections/' + el.getAttribute('data-id'), {
          method: 'PATCH',
          body: { showPackagingBag: el.checked },
        });
        if (!rb.ok) {
          alert(
    ...
    document.querySelectorAll('.secF').forEach(function (el) {
      el.addEventListener('change', async function () {
        var f = el.getAttribute('data-f'),
          body = {};
        if (f === 'submittedOn')
          body[f] = el.value ? new Date(el.value + 'T12:00:00').toISOString() : null;
    ...
    document.querySelectorAll('.bomLine').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        el.style.borderColor = r.ok ? '#3f9d78' : '#c2452f';
        if (!r.ok) {
          await fail(r, 'Could not save the line');
          reload();
          return;
        }
        ...
        var line = (procData || []).filter(function (x) {
    ...
    var refreshLines = async function () {
      ...
      if (rr.ok) {
        var oo = await rr.json();
        order.procurement = oo.procurement;
      }
      reload();
    };
    ...
    document.querySelectorAll('[data-line-del]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        if (
          !confirm(
            'Take “' +
              bt.getAttribute('data-line-name') +
              '” off this order?\n\nIt leaves the Bill of Materials but stays on the accepted proposal. The removal is recorded on the order timeline.',
          )
        )
          return;
        bt.disabled = true;
        var r = await authed('/orders/procurement/' + bt.getAttribute('data-line-del'), {
          method: 'DELETE',
        });
        bt.disabled = false;
      ...
      bt.addEventListener('click', function () {
        ...
        openModal(
          ...
          '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">Type a part number and the catalog fills in the rest. Leave cost blank to use the catalog’s.</div>' +
            fieldRow(
              'Part #',
              '<input id="plSku" placeholder="e.g. 6820H-114" style="' +
                IN +
            ...
            fieldRow(
              'Qty',
              '<input id="plQty" type="number" min="1" step="1" value="1" style="' + IN + '">',
            ) +
            fieldRow(
              'Cost each',
              '<input id="plCost" type="number" min="0" step="0.01" placeholder="From the catalog" style="' +
                IN +
                '">',
            ),
          async function (close, showErr) {
            ...
            var r = await authed('/orders/' + order.id + '/procurement', {
              method: 'POST',
              body: body,
            });
            if (!r.ok) {
              ...
              return showErr(m || 'Could not add the part (' + r.status + ').');
            }
            close();
            refreshLines();
          },
        ...
        if (skuEl)
          skuEl.addEventListener('blur', async function () {
            var q = skuEl.value.trim();
            if (!q) return;
            var rs = await authed('/skus?q=' + encodeURIComponent(q) + '&pageSize=5');
            if (!rs.ok) return;
            var items = ((await rs.json()) || {}).items || [];
            var hit = items.filter(function (k) {
              return String(k.part).toUpperCase() === q.toUpperCase();
            })[0];
            if (!hit) return;
            var nameEl = document.getElementById('plName'),
              costEl = document.getElementById('plCost');
            if (nameEl && !nameEl.value.trim()) nameEl.value = hit.description || '';
            if (costEl && !costEl.value.trim() && hit.unitCostMinor != null)
              costEl.value = (Number(hit.unitCostMinor) / 100).toFixed(2);
          });
      });
    });
    ...
    document.querySelectorAll('.secQ').forEach(function (el) {
      el.addEventListener('change', async function () {
        el.style.borderColor = '#c9a227';
        var r = await authed('/bom/questions/' + el.getAttribute('data-id'), {
          method: 'PATCH',
          body: { value: el.value },
        });
        el.style.borderColor = r.ok ? '#3f9d78' : '#c2452f';
    ...
    document.querySelectorAll('.secQM').forEach(function (el) {
      el.addEventListener('change', async function () {
        ...
        var picked = [];
        document.querySelectorAll('.secQM[data-id="' + id + '"]').forEach(function (c) {
          if (c.checked) picked.push(c.value);
        });
        var r = await authed('/bom/questions/' + id, {
          method: 'PATCH',
          body: { value: JSON.stringify(picked) },
        });
        if (!r.ok) return fail(r, 'Could not save the answer');
      });
    });
    document.querySelectorAll('[data-q-del]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        ...
        var r = await authed('/bom/questions/' + bt.getAttribute('data-q-del'), {
          method: 'DELETE',
        });
    ...
    document.querySelectorAll('[data-q-add]').forEach(function (bt) {
      bt.addEventListener('click', function () {
        openQuestionForm(bt.getAttribute('data-q-add'), reload);
      });
    });
    ...
    document.querySelectorAll('[data-sec-color]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        ...
        if (!brand || !code) {
          alert('Pick a brand and type the colour code.');
          return;
        }
        var sec = bomSectionData.filter(function (x) {
          return x.id === id;
        })[0];
        var r = await authed('/orders/' + order.id + '/bom/apply-color', {
          method: 'POST',
          body: { brandId: brand, code: code, vendor: sec.vendor, overwrite: false },
        });
        ...
        alert(
          d.applied +
            ...
            d.color +
            (d.skipped
              ? '. ' + d.skipped + ' skipped — already coloured, or in another section.'
              : '.'),
        );
        var o = await (await authed('/orders/' + order.id)).json();
        order.procurement = o.procurement;
        reload();
      });
    });
    ...
    document.querySelectorAll('[data-sec-move]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        var id = bt.getAttribute('data-id'),
          dir = bt.getAttribute('data-sec-move');
        var ids = bomSectionData.map(function (x) {
          return x.id;
        });
        ...
        ids[i] = ids[j];
        ids[j] = id;
        var r = await authed('/orders/' + order.id + '/bom/sections/reorder', {
          method: 'POST',
          body: { ids: ids },
        });
      ...
      bt.addEventListener('click', async function () {
        ...
        if (
          !confirm(
            'Record the ' +
              sec.vendor +
              ' Bill of Materials as sent?\n\nUse this only when the sheet went out some other way — emailing it from here submits the section on its own. Its fields lock until you unlock them for revisions.',
          )
        )
          return;
        bt.disabled = true;
      ...
      bt.addEventListener('click', function () {
        ...
        openModal(
          ...
          '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">This reopens the section for editing. The vendor already has the version you sent, so note what is changing — it goes on the order timeline.</div>' +
            fieldRow(
              'Reason',
              '<input id="secUnlockReason" placeholder="e.g. customer changed two colours" style="' +
                IN +
                '">',
            ),
          async function (close, showErr) {
            ...
            var r = await authed('/bom/sections/' + id + '/unlock', {
              method: 'POST',
              body: { reason: reason },
            });
            if (!r.ok) {
              ...
              return showErr(m || 'Could not unlock.');
            }
            close();
            reload();
          },
    ...
    document.querySelectorAll('[data-sec-email]').forEach(function (bt) {
      bt.addEventListener('click', function () {
        var sec = bomSectionData.filter(function (x) {
          return x.id === bt.getAttribute('data-sec-email');
        })[0];
    ...
    document.querySelectorAll('[data-proc]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        ...
        var qs =
          ...
          (zero && zero.checked ? '&includeZeroQty=true' : '');
        ...
        bt.disabled = false;
        if (!doc) {
          alert('Could not build the Bill of Materials.');
          return;
        }
  ...
  var Q_TYPES = [
    ...
    ['MULTI_SELECT', 'Dropdown — pick several'],
    ['BOOLEAN', 'Yes / No'],
  ];
  ...
  function openQuestionForm(sectionId, done) {
    openModal(
      ...
      '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">Asked on this vendor’s section of this order. For a question you ask the same vendor every time, add it under Administration instead.</div>' +
        fieldRow(
          'Question',
          '<input id="qLabel" placeholder="e.g. What gauge steel?" style="' + IN + '">',
        ) +
        fieldRow(
          'Answer type',
          '<select id="qType" style="' +
            IN +
            '">' +
        ...
        fieldRow(
          'Options',
          '<textarea id="qOpts" rows="3" placeholder="One per line" style="' +
            IN +
        ...
        fieldRow(
          'Required',
          '<select id="qReq" style="' +
            IN +
            '"><option value="false">Optional</option><option value="true">Required before the section can be confirmed</option></select>',
        ),
      async function (close, showErr) {
        ...
        var opts = document
          .getElementById('qOpts')
          .value.split('\n')
          .map(function (x) {
            return x.trim();
          })
          .filter(Boolean);
        if ((type === 'SELECT' || type === 'MULTI_SELECT') && !opts.length)
          return showErr('A dropdown needs at least one option.');
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'Could not add.');
        }
        close();
        done();
      },
    ...
    t.addEventListener('change', function () {
      document.getElementById('qOptWrap').style.display =
        t.value === 'SELECT' || t.value === 'MULTI_SELECT' ? 'block' : 'none';
    });
  }
  ...
  function openSendForm(order, sec, done) {
    var e = sec.email;
    openModal(
      'Email the ' + sec.vendor + ' Bill of Materials',
      (sec.editable
        ...
        (e.hasDefault
          ? ''
          : '<div style="background:#fdf6e6;border:1px solid #ecd9a6;border-radius:9px;padding:9px 12px;font-size:12.5px;color:#6b5a24;margin-bottom:12px;">No saved address for this vendor. Type one below, then set a default under Manufacturers so it is pre-filled next time.</div>') +
        fieldRow(
          'To',
          '<input id="sndTo" value="' +
            esc(e.to) +
            '" placeholder="purchasing@vendor.com" style="' +
            IN +
            '">',
        ) +
        fieldRow(
          'Cc',
          '<input id="sndCc" value="' + esc(e.cc) + '" placeholder="Optional" style="' + IN + '">',
        ) +
        fieldRow(
          'Subject',
          '<input id="sndSubject" value="' + esc(e.subject) + '" style="' + IN + '">',
        ) +
        fieldRow(
          'Attach',
          '<select id="sndFormat" style="' +
            IN +
            '">' +
            '<option value="PDF"' +
            (e.format === 'PDF' ? ' selected' : '') +
            '>PDF</option>' +
            '<option value="EXCEL"' +
            (e.format === 'EXCEL' ? ' selected' : '') +
            '>Excel</option>' +
            '<option value="BOTH"' +
            (e.format === 'BOTH' ? ' selected' : '') +
            '>Both</option></select>',
        ) +
        fieldRow(
          'Message',
          '<textarea id="sndBody" rows="8" style="' +
            IN +
      ...
      async function (close, showErr) {
        ...
        var r = await authed('/bom/sections/' + sec.id + '/send', {
          ...
          body: {
            to: to,
            cc: document.getElementById('sndCc').value.trim(),
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'Could not send.');
        }
        close();
        done();
      },
  ...
  function bomFileSlug(vendor, order) {
    ...
    return [
      part(o.customerName || o.organizationName || ''),
      part(o.number || ''),
      part(vendor === '*' ? 'All Vendors' : vendor),
    ]
      .filter(Boolean)
      .join('-');
  }
  ...
  function downloadBomCsv(doc, vendor) {
    ...
    var body = (doc.lines || []).map(function (l) {
      var base = [
        ...
        String(l.quantity),
        l.powderColor,
        (Number(l.extendedWeightLbs) || 0).toFixed(2),
        (l.unitCostMinor / 100).toFixed(2),
        (l.extendedCostMinor / 100).toFixed(2),
      ];
      return all ? [l.vendor].concat(base) : base;
    });
    var t = doc.totals || {};
    var totalRow = (all ? [''] : []).concat([
      'Total',
      '',
      String(t.unitCount || 0),
      '',
      (Number(t.totalWeightLbs) || 0).toFixed(2),
      '',
      ((t.extendedCostMinor || 0) / 100).toFixed(2),
    ]);
    var meta = [
      ['Bill of Materials', doc.order.number],
      ['Job', doc.order.jobName],
      ['Vendor', all ? 'All vendors' : vendor],
      [
        'Submission date',
        doc.order.submittedOn ? String(doc.order.submittedOn).slice(0, 10) : todayISO(),
      ],
      ['Ship to', doc.shipTo.name],
      ['Delivery type', doc.order.deliveryType],
      ['Powder coat brand', doc.order.powderCoatBrand],
      ['Estimated shipment quote', doc.order.shipmentQuote],
      ['Total steel weight (lb)', (Number(t.steelWeightLbs) || 0).toFixed(2)],
      ['Prepared by', (doc.createdBy && doc.createdBy.name) || ''],
      ['Prepared on', new Date(doc.createdAt).toLocaleString()],
      [],
    ];
    downloadCsv(
      bomFileSlug(vendor, {
        number: doc.order.number,
        customerName: doc.customer && doc.customer.name,
      }) + '.csv',
      meta.concat([head]).concat(body).concat([totalRow]),
    );
  }
  ...
  function printBom(doc, vendor) {
    ...
    var t = doc.totals || {};
    var c = doc.company;
    var money2 = function (minor) {
      return '$' + (Number(minor || 0) / 100).toFixed(2);
    };
    var dateStr = function (v) {
      return v ? new Date(v).toLocaleDateString() : '—';
    };
    var block = function (label, name, lines, contact, phone, email) {
      return (
        '<div style="flex:1;min-width:200px;">' +
        '<div style="font-size:9.5px;letter-spacing:.09em;text-transform:uppercase;color:#8a8f85;margin-bottom:3px;">' +
        esc(label) +
        '</div>' +
    ...
    var field = function (label, value) {
      return (
        '<div><div style="font-size:9.5px;letter-spacing:.09em;text-transform:uppercase;color:#8a8f85;">' +
        esc(label) +
        '</div>' +
    ...
    var thead = cols
      .map(function (h, i) {
        return (
          '<th style="text-align:' +
          (i > rightFrom ? 'right' : 'left') +
          ';padding:6px 8px;font-size:9px;letter-spacing:.05em;text-transform:uppercase;color:#5c6157;border-bottom:1.5px solid #3d4a55;white-space:nowrap;">' +
          esc(h) +
          '</th>'
        );
      })
    ...
    var tbody = (doc.lines || [])
      .map(function (l) {
        var cells = (all ? [esc(l.vendor)] : []).concat([
          '<code style="font-size:10.5px;">' + esc(l.lineNo) + '</code>',
          esc(l.name),
          String(l.quantity),
          esc(l.powderColor || '—'),
          (Number(l.extendedWeightLbs) || 0).toFixed(2),
          money2(l.unitCostMinor),
          money2(l.extendedCostMinor),
          esc(l.vendorNotes || ''),
        ]);
        ...
        return (
          ...
          cells
            .map(function (v, i) {
              return (
                '<td style="padding:5px 8px;border-bottom:1px solid #e7e8e3;font-size:10.5px;text-align:' +
                (i > rightFrom ? 'right' : 'left') +
                ';">' +
                v +
                '</td>'
    ...
    var totalCells = (all ? [''] : []).concat([
      ...
      (Number(t.totalWeightLbs) || 0).toFixed(2),
      '',
      money2(t.extendedCostMinor),
      '',
    ]);

    w.document.write(
      '<!doctype html><html><head><meta charset="utf-8"><title>' +
        esc('BOM ' + doc.order.number + ' — ' + (all ? 'all vendors' : vendor)) +
        '</title>' +
        ...
        '<img src="' +
        location.origin +
        '/logo.png" alt="" style="height:52px;width:auto;">' +
        '<div><div style="font-family:Georgia,serif;font-size:20px;font-weight:700;letter-spacing:-.01em;">' +
        esc(c.name) +
        '</div>' +
        '<div style="font-size:10.5px;color:#5c6157;line-height:1.45;">' +
        esc(c.addressLine1) +
        ' · ' +
        esc(c.city + ', ' + c.region + ' ' + c.postalCode) +
        '<br>' +
        esc(c.phone) +
        ' · ' +
        esc(c.email) +
        ...
        '<div style="font-size:11px;color:#5c6157;margin-top:2px;">' +
        esc(doc.order.number) +
        ' · accepted proposal v' +
        (doc.order.acceptedVersion || '') +
        '</div>' +
        '<div style="font-size:11px;color:#5c6157;">' +
        esc(all ? 'All vendors' : vendor) +
        '</div>' +
        '</div>' +
        '</div>' +
        // Ship from / ship to
        ...
        block(
          ...
          doc.vendor
            ? [
                streetLine(doc.vendor.addressLine1, doc.vendor.addressLine2),
                [
                  doc.vendor.city,
                  [doc.vendor.region, doc.vendor.postalCode].filter(Boolean).join(' '),
                ]
                  .filter(Boolean)
                  .join(', '),
              ].filter(Boolean)
            : [],
          doc.vendor
            ? [doc.vendor.contactName, doc.vendor.contactTitle].filter(Boolean).join(', ')
            : '',
          doc.vendor ? doc.vendor.contactPhone : '',
          doc.vendor ? doc.vendor.contactEmail : '',
        ) +
        block(
          'Ship to (' + doc.shipTo.label + ')',
          doc.shipTo.name,
          doc.shipTo.lines,
        ...
        totalCells
          .map(function (v, i) {
            return (
              '<td style="padding:7px 8px;border-top:1.5px solid #3d4a55;font-size:11px;font-weight:700;text-align:' +
              (i > rightFrom ? 'right' : 'left') +
              ';">' +
              esc(v) +
              '</td>'
        ...
        '</tr></tbody></table>' +
        (doc.order.notes
          ? '<div style="margin-top:12px;padding:10px 12px;border:1px solid #e7e8e3;border-radius:8px;font-size:11px;line-height:1.55;"><b>Notes:</b> ' +
            esc(doc.order.notes) +
            '</div>'
          : '') +
        // Sign-off footer
        ...
        '<div>Prepared by <b>' +
        esc((doc.createdBy && doc.createdBy.name) || '—') +
        '</b>' +
        (doc.createdBy && doc.createdBy.email ? ' · ' + esc(doc.createdBy.email) : '') +
        '<br>Created ' +
        esc(new Date(doc.createdAt).toLocaleString()) +
        '</div>' +
        '<div style="text-align:right;">' +
        esc(c.name) +
        ' · ' +
  ...
  function qboTypeLabel(t) {
    for (var i = 0; i < QBO_TYPES.length; i++) if (QBO_TYPES[i][0] === t) return QBO_TYPES[i][1];
    return titleCase(t);
  }
  function qboStatusChip(s) {
    var tone =
      { PAID: '#3f9d78', PARTIALLY_PAID: '#b7873a', OVERDUE: '#c2452f', OPEN: '#5c6357' }[s] ||
      '#82877d';
    return (
  ...
  async function loadQbo(order, user) {
    var box = document.getElementById('qboBox');
    if (!box) return;
    var txns = [],
      billing = null,
      conn = null;
    try {
      var rs = await authed('/integrations/quickbooks/status');
      conn = rs.ok ? await rs.json() : null;
      var r = await authed(
        '/integrations/quickbooks/transactions?proposalId=' + encodeURIComponent(order.proposalId),
      );
      if (r.status === 403) {
        box.innerHTML =
          '<div class="placeholder" style="padding:18px;"><p class="muted" style="margin:0;">Your role cannot view QuickBooks documents.</p></div>';
        return;
      }
      ...
      var rb = await authed(
        '/integrations/quickbooks/billing/' + encodeURIComponent(order.proposalId),
      );
      if (rb.ok) billing = await rb.json();
    } catch (e) {
      box.innerHTML = '<div class="err">Could not reach QuickBooks.</div>';
      return;
    }
    ...
    if (!hasRole(QBO_TXN_ROLES, user.role)) {
      blockReason =
        'Your role (' +
        titleCase(user.role || 'unknown') +
        ') cannot create QuickBooks documents. Accounting or System Admin can.';
    } else if (!conn) {
      blockReason =
        'This app could not read the QuickBooks connection status. Your role may lack the QuickBooks management permission, or Integrations is unreachable.';
    } else if (!conn.configured) {
      blockReason =
        'QuickBooks is not configured on this deployment — the QBO environment variables are missing.';
    } else if (!connected) {
      blockReason =
        'No active QuickBooks connection for the ' +
        titleCase(conn.environment || 'current') +
        ' environment. Connect it under Integrations.';
    } else if (order.status === 'CANCELLED') {
      blockReason =
        'This order is cancelled, so its totals are no longer a thing to bill. Re-accept the proposal first.';
    } else if (!order.proposalVersionId) {
      blockReason =
        'This record has no linked proposal version, so there are no accepted totals to freeze.';
    }
    ...
    var writeGate =
      connected && conn && conn.environment === 'PRODUCTION' && !conn.productionWritesEnabled
        ...
        : '';
    var billByTxn = {};
    ((billing && billing.documents) || []).forEach(function (d) {
      billByTxn[d.id] = d;
    });
    ...
    var pending = txns.filter(function (t) {
      return t.status !== 'CREATED';
    });
    var live = txns.filter(function (t) {
      return t.status === 'CREATED';
    });
    ...
    var pendingRows = pending
      .map(function (t) {
        var step = '';
        if (canTransact) {
          if (t.status === 'DRAFT' || t.status === 'PENDING_AUTHORIZATION')
            step =
              '<button class="link-btn" data-qbo="authorize" data-id="' +
              t.id +
              '" style="width:auto;padding:7px 13px;">Step 2 · Authorize</button>';
          else if (t.status === 'AUTHORIZED')
            step =
              '<button class="btn" data-qbo="execute" data-id="' +
              t.id +
              '" style="width:auto;padding:7px 13px;">Step 3 · Create in QuickBooks</button>';
          else if (t.status === 'FAILED')
            step =
              '<button class="link-btn" data-qbo="retry" data-id="' +
              t.id +
              '" style="width:auto;padding:7px 13px;color:#9c3327;">Retry</button>';
        }
        return (
          '<tr>' +
          td(
            '<b style="font-weight:600;">' +
              esc(qboTypeLabel(t.type)) +
              '</b>' +
              (t.error
                ? '<div style="font-size:12px;color:#9c3327;">' + esc(t.error) + '</div>'
                : ''),
          ) +
    ...
    var liveRows = live
      .map(function (t) {
        ...
        var sent = d.sentAt
          ? '<b style="font-weight:600;color:#3f9d78;">Sent</b><div style="font-size:12px;color:#82877d;">' +
            esc(fmtStamp(d.sentAt)) +
            (d.sentToEmail ? '<br>' + esc(d.sentToEmail) : '') +
            (d.sentBy ? '<br>by ' + esc(d.sentBy) : '') +
            '</div>'
          : (isEstimate
            ...
            (d.sendError
              ? '<div style="font-size:12px;color:#9c3327;">' + esc(d.sendError) + '</div>'
              : '');
        var moneyCell = isEstimate
          ? '<span class="muted">—</span>'
          : d.balanceMinor == null
            ? '<span class="muted">Not synced yet</span>'
            : qboStatusChip(d.qboStatus) +
              '<div style="font-size:12px;color:#82877d;margin-top:3px;">' +
              fmtMoney(d.paidMinor || '0', t.currency) +
              ' paid of ' +
              fmtMoney(d.qboTotalMinor || t.amountMinor, t.currency) +
              (Number(d.balanceMinor) > 0
                ? '<br><b style="color:#20241f;">' +
                  fmtMoney(d.balanceMinor, t.currency) +
                  ' outstanding</b>'
                : '') +
              (d.dueDate ? '<br>Due ' + esc(fmtDate(d.dueDate)) : '') +
              '</div>';
        var acts = [
          '<button class="link-btn" data-qbob="pdf" data-id="' +
            t.id +
            '" style="width:auto;padding:6px 11px;">View</button>',
        ];
        ...
        return (
          ...
          td(
            '<b style="font-weight:600;">' +
              esc(qboTypeLabel(t.type)) +
              '</b><div style="font-size:12px;color:#82877d;">' +
              esc(t.qboDocNumber || t.qboId || '') +
              '</div>',
          ) +
          td(sent) +
          td(moneyCell) +
    ...
    var payRows = ((billing && billing.payments) || [])
      .map(function (p) {
        return (
          '<tr>' +
          td(esc(fmtDate(p.txnDate))) +
          td(
            '<b style="font-weight:600;">' +
              fmtMoney(p.amountMinor, p.currency) +
              '</b>' +
              (p.totalAmountMinor !== p.amountMinor
                ? '<div style="font-size:12px;color:#82877d;">of a ' +
                  fmtMoney(p.totalAmountMinor, p.currency) +
                  ' payment</div>'
                : ''),
          ) +
          td(esc(p.method || '—')) +
          td(esc(p.referenceNumber || '—')) +
          td(esc(p.depositToAccount || '—')) +
          '</tr>'
        );
      })
      .join('');

    var remRows = ((billing && billing.reminders) || [])
      .map(function (m) {
        return (
          '<tr>' +
          td(esc(fmtStamp(m.at))) +
          td(esc(m.toEmail)) +
          td(
            esc(m.subject) +
              (m.error
                ? '<div style="font-size:12px;color:#9c3327;">' + esc(m.error) + '</div>'
                : ''),
          ) +
          td(fmtMoney(m.balanceMinor, 'USD')) +
          td(
            m.status === 'sent'
              ? '<span class="chip" style="color:#3f9d78;border-color:#3f9d7833;">Sent</span>'
              : '<span class="chip" style="color:#c2452f;border-color:#c2452f33;">Failed</span>',
          ) +
          '</tr>'
        );
      })
    ...
    var lastSync = ((billing && billing.documents) || [])
      .map(function (d) {
    ...
    box.innerHTML =
      ...
      (!connected
        ? '<div class="placeholder" style="padding:16px;margin-bottom:10px;"><p class="muted" style="margin:0;">QuickBooks is not connected — connect it under Integrations first.</p></div>'
        : '') +
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">' +
      (order.organizationId
        ? '<button class="link-btn" id="qboProfile" style="width:auto;padding:9px 14px;">Check customer profile</button>'
        : '') +
      '<button class="link-btn" id="qboRefresh" style="width:auto;padding:9px 14px;">Refresh from QuickBooks</button>' +
      (lastSync
        ? '<span class="muted" style="font-size:12px;">Last read ' +
          esc(fmtStamp(lastSync)) +
          '</span>'
        : '') +
      '<div style="margin-left:auto;">' +
      '<button class="btn" id="qboPrepare"' +
      (canTransact
        ? ''
        : ' disabled title="' +
          esc(blockReason) +
          '" style="width:auto;padding:9px 15px;opacity:.45;cursor:not-allowed;"') +
      (canTransact ? ' style="width:auto;padding:9px 15px;"' : '') +
      '>Step 1 · Prepare a document</button>' +
      '</div>' +
      '</div>' +
      (blockReason
        ? '<div style="background:#fbe9e6;border:1px solid #f0cdc7;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#9c3327;line-height:1.55;margin-bottom:10px;"><b style="font-weight:600;">Step 1 is unavailable.</b> ' +
          esc(blockReason) +
          '</div>'
        : '') +
      (writeGate
        ? '<div style="background:#fdf6e3;border:1px solid #eadfbe;border-radius:10px;padding:11px 13px;font-size:12.5px;color:#8a6d1f;line-height:1.55;margin-bottom:10px;">' +
          writeGate +
          '</div>'
        : '') +
      (pending.length
        ? '<div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:#8a8f85;margin:14px 0 6px;">Waiting to be created</div>' +
      ...
      tableShell(
        ['Document', 'Delivery', 'Payment', ''],
        liveRows,
        4,
        'Nothing has been created in QuickBooks for this order yet.',
      ) +
      (payRows
        ? '<div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:#8a8f85;margin:18px 0 6px;">Payments received</div>' +
          tableShell(['Date', 'Applied', 'Method', 'Reference', 'Deposited to'], payRows, 5, '')
        : '') +
      (remRows
        ? '<div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:#8a8f85;margin:18px 0 6px;">Payment reminders</div>' +
    ...
    if (pb && canTransact)
      pb.addEventListener('click', function () {
        openQboPrepare(order, user, txns);
      });
    var prof = document.getElementById('qboProfile');
    if (prof)
      prof.addEventListener('click', function () {
        openQboProfile(order, user);
      });
    var rf = document.getElementById('qboRefresh');
    if (rf)
      rf.addEventListener('click', async function () {
        rf.disabled = true;
        rf.textContent = 'Reading QuickBooks…';
        var rr = await authed(
          '/integrations/quickbooks/billing/' + encodeURIComponent(order.proposalId) + '?refresh=1',
        );
    ...
    document.querySelectorAll('[data-qbo]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        ...
        if (act === 'execute' && !confirm('This creates the document in QuickBooks. Continue?'))
          return;
        bt.disabled = true;
        bt.textContent = 'Working…';
        var r = await authed(
          '/integrations/quickbooks/transactions/' + bt.getAttribute('data-id') + '/' + act,
          { method: 'POST', body: {} },
        );
        if (!r.ok) {
          var m = '';
  ...
  async function openQboPdf(txnId, bt) {
    var label = bt.textContent;
    bt.disabled = true;
    bt.textContent = 'Fetching…';
    try {
      var r = await authed('/integrations/quickbooks/transactions/' + txnId + '/pdf');
      if (!r.ok) {
        alert('QuickBooks did not return a PDF (' + r.status + ').');
        return;
      }
  ...
  function openQboSend(txnId, doc, order, user) {
    openModal(
      doc.sentAt ? 'Resend this document' : 'Send to the customer',
      '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">QuickBooks emails the document and records the delivery on its side. ' +
        (doc.sentAt
          ? 'It was last sent ' + esc(fmtStamp(doc.sentAt)) + '.'
          : 'It has not been sent yet.') +
        '</div>' +
        fieldRow(
          'Send to',
          '<input id="qbSendTo" type="email" value="' +
            esc(doc.sentToEmail || '') +
            '" placeholder="Leave blank to use the address on the invoice" style="' +
            IN +
            '">',
        ),
      async function (close, showErr) {
        var r = await authed('/integrations/quickbooks/transactions/' + txnId + '/send', {
          method: 'POST',
          body: { to: document.getElementById('qbSendTo').value.trim() || null },
        });
        if (!r.ok) {
          var m = '';
          try {
          ...
          return showErr(m || 'Could not send (' + r.status + ').');
        }
        close();
        loadQbo(order, user);
      },
  ...
  async function openQboReminder(txnId, order, user) {
    ...
    if (!r.ok) {
      alert('Could not prepare a reminder (' + r.status + ').');
      return;
    }
    var d = await r.json();
    if (d.blockers && d.blockers.length) {
      alert(d.blockers.join('\n\n'));
      return;
    }
    openModal(
      'Remind ' +
      ...
      '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">' +
        ...
        fieldRow(
          'Cc',
          '<input id="qbRemCc" type="text" placeholder="Optional" style="' + IN + '">',
        ) +
        fieldRow(
          'Subject',
          '<input id="qbRemSubj" type="text" value="' + esc(d.subject) + '" style="' + IN + '">',
        ) +
        fieldRow(
          'Message',
          '<textarea id="qbRemBody" rows="12" style="' +
            IN +
      ...
      async function (close, showErr) {
        var rr = await authed('/integrations/quickbooks/transactions/' + txnId + '/reminder', {
          method: 'POST',
          body: {
        ...
        if (!rr.ok) {
          ...
          return showErr(m || 'Could not send the reminder (' + rr.status + ').');
        }
        close();
        loadQbo(order, user);
      },
  ...
  async function openQboProfile(order, user) {
    var r = await authed(
      '/integrations/quickbooks/customers/' + encodeURIComponent(order.organizationId) + '/profile',
    );
    if (!r.ok) {
      var m = '';
      try {
      ...
      alert(m || 'Could not read the QuickBooks profile (' + r.status + ').');
      return;
    }
    var d = await r.json();
    ...
    function profileRows(fields) {
      return (fields || [])
        .map(function (f) {
          ...
          return (
            '<tr>' +
            '<td style="padding:8px 10px;border-bottom:1px solid #e7e8e3;vertical-align:top;color:#5c6357;white-space:nowrap;">' +
            esc(f.label) +
            '</td>' +
            '<td style="padding:8px 10px;border-bottom:1px solid #e7e8e3;vertical-align:top;">' +
            (esc(f.crm) || '<span class="muted">—</span>') +
            '</td>' +
            '<td style="padding:8px 10px;border-bottom:1px solid #e7e8e3;vertical-align:top;' +
            (f.differs ? 'color:#9c3327;' : '') +
            '">' +
            (esc(f.qbo) || '<span class="muted">—</span>') +
            '</td>' +
            '<td style="padding:8px 10px;border-bottom:1px solid #e7e8e3;vertical-align:top;font-size:12px;color:' +
            tone +
            ';white-space:nowrap;">' +
            note +
            '</td>' +
    ...
    openModal(
      ...
      (!d.linked
        ...
        : '<div class="muted" style="font-size:12.5px;margin-bottom:12px;line-height:1.55;">QuickBooks customer ' +
          esc(d.qboCustomerId) +
          (d.qboActive === false ? ' <b style="color:#9c3327;">(inactive)</b>' : '') +
          (d.qboBalanceMinor != null
            ? ' · ' + fmtMoney(d.qboBalanceMinor, 'USD') + ' owed across all their invoices'
            : '') +
          '</div>') +
        (d.warnings.length
          ? '<div style="background:#fbf6ec;border:1px solid #e6d9bd;border-radius:6px;padding:11px 13px;margin-bottom:12px;font-size:13px;color:#6b5a34;line-height:1.55;">' +
            d.warnings
              .map(function (w) {
                return esc(w);
              })
        ...
        '<thead><tr>' +
        ['', 'In this CRM', 'In QuickBooks', '']
          .map(function (h) {
            return (
              '<th style="text-align:left;padding:6px 10px;font-size:11px;letter-spacing:.07em;text-transform:uppercase;color:#8a8f85;font-weight:600;border-bottom:1px solid #cfd3ca;">' +
              h +
              '</th>'
            );
          })
          .join('') +
        '</tr></thead><tbody id="ctxRows">' +
        rows +
        '</tbody></table>' +
        /**
        ...
        (canEditTax
          ? '<div style="border:1px solid #dcded7;border-radius:10px;padding:13px 14px;margin-top:14px;">' +
            ...
            '<input type="checkbox" id="ctxExempt"' +
            (d.taxExempt ? ' checked' : '') +
            '>' +
            ...
            '<input id="ctxNum" value="' +
            esc(d.taxExemptId || '') +
            '" placeholder="As printed on the certificate" style="' +
            IN +
            '"' +
            (d.taxExempt ? '' : ' disabled') +
            '>' +
            '</div>' +
    ...
    if (ctxSave)
      ctxSave.addEventListener('click', async function () {
        ...
        var rp = await authed('/crm/organizations/' + order.organizationId, {
          method: 'PATCH',
          body: {
            taxExempt: ctxEx.checked,
            taxExemptId: ctxEx.checked ? ctxNum.value.trim() : null,
          },
        });
        if (!rp.ok) {
          var pm = '';
          try {
            pm = ((await rp.json()) || {}).message || '';
          } catch (e) {}
        ...
        var rs = await authed(
          '/integrations/quickbooks/customers/' + order.organizationId + '/sync',
          { method: 'POST' },
        );
        ctxSave.disabled = false;
        if (!rs.ok) {
          var sm = '';
          try {
            sm = ((await rs.json()) || {}).message || '';
          } catch (e) {}
          return say(
            'Saved in the CRM, but the push failed: ' +
              (sm || 'HTTP ' + rs.status) +
              '. Retry with Sync customer to QuickBooks under Integrations.',
            '#9c3327',
          );
        }
        ...
        var rr = await authed(
          '/integrations/quickbooks/customers/' +
            encodeURIComponent(order.organizationId) +
            '/profile',
        );
        if (!rr.ok)
          return say(
            'Pushed to QuickBooks. Close and re-open this panel to see the result.',
            '#2f7d5d',
          );
        ...
        say(
          nd.differenceCount
            ? 'Pushed. ' + nd.differenceCount + ' field(s) still differ above.'
            : 'Pushed. QuickBooks now matches the CRM.',
  ...
  function openQboPrepare(order, user, txns) {
    txns = txns || [];
    function priorOf(type) {
      return txns.filter(function (t) {
        return t.type === type;
      });
    }
    function createdOf(type) {
      return priorOf(type).filter(function (t) {
        return t.status === 'CREATED';
      });
    }
    function nextSeq(type) {
      return (
        priorOf(type).reduce(function (n, t) {
          return Math.max(n, qboSeqOf(t));
        }, 0) + 1
      );
    }
    ...
    function noteHtml(type) {
      ...
      var pending = priorOf(type).filter(function (t) {
        return t.status !== 'CREATED';
      });
      if (!made.length) {
        return pending.length
          ? '<div class="muted" style="font-size:12.5px;line-height:1.55;">A ' +
              esc(qboTypeLabel(type).toLowerCase()) +
              ' is already prepared and waiting to be created. Preparing again reopens that one rather than starting a second.</div>'
          : '';
      }
      var list = made
        .map(function (t) {
          return (
            ...
            (t.createdAt ? ' · ' + esc(fmtStamp(t.createdAt)) : '') +
            '</div>'
          );
        })
        .join('');
      return (
        '<div style="background:#fbf6ec;border:1px solid #e6d9bd;border-radius:8px;padding:11px 13px;font-size:13px;color:#6b5a34;line-height:1.55;">' +
        '<b style="font-weight:600;">Already in QuickBooks</b>' +
        list +
    ...
    openModal(
      'Prepare a QuickBooks document',
      '<div class="muted" style="font-size:13px;margin-bottom:12px;">This freezes the accepted totals of ' +
        esc(order.number) +
        ' against an idempotency key. Nothing is sent to QuickBooks until you authorize and create it.</div>' +
        fieldRow(
          'Document type',
          '<select id="qboType" style="' +
            IN +
            '">' +
            QBO_TYPES.map(function (t) {
              return '<option value="' + t[0] + '">' + t[1] + '</option>';
            }).join('') +
            '</select>',
        ) +
      ...
      async function (close, showErr) {
        ...
        if (createdOf(type).length && !(another && another.checked)) {
          return showErr(
            'A ' +
              qboTypeLabel(type).toLowerCase() +
              ' already exists in QuickBooks for this version. Tick “Create another copy” to make a second one.',
          );
        }
        var body = { proposalVersionId: order.proposalVersionId, type: type };
        if (another && another.checked) body.sequence = nextSeq(type);
        var r = await authed('/integrations/quickbooks/transactions/prepare', {
          method: 'POST',
          body: body,
        });
        if (!r.ok) {
          ...
          return showErr(m || 'Could not prepare (' + r.status + ').');
        }
        close();
        loadQbo(order, user);
      },
  ...
  function printTable(title, subtitle, head, rows, totalRow) {
    ...
    var cell = function (v, b) {
      return (
        '<td style="padding:7px 10px;border-bottom:1px solid #e7e8e3;' +
        (b ? 'font-weight:600;border-top:1px solid #cfd3ca;' : '') +
        '">' +
        esc(v) +
        '</td>'
      );
    ...
    w.document.write(
      '<!doctype html><meta charset="utf-8"><title>' +
        ...
        '<h1 style="font-size:21px;margin:4px 0 2px;">' +
        esc(title) +
        '</h1>' +
        '<div style="font-size:12.5px;color:#6c7266;margin-bottom:16px;">' +
        esc(subtitle) +
        '</div>' +
        ...
        head
          .map(function (h) {
            return (
              '<th style="text-align:left;padding:7px 10px;font-size:10px;letter-spacing:.05em;text-transform:uppercase;color:#8a8f85;border-bottom:1px solid #cfd3ca;">' +
              esc(h) +
              '</th>'
            );
          })
  ...
  function auditRows(events) {
    if (!events || !events.length)
      return '<div class="placeholder" style="padding:20px;"><p class="muted" style="margin:0;">No events recorded.</p></div>';
    return (
      '<div class="card">' +
      events
        .map(function (e, i) {
          return (
            ...
            (i ? '10px' : '0') +
            ' 0 0;border-top:' +
            (i ? '1px solid #f2f3ef;margin-top:10px;' : 'none;') +
            'font-size:13.5px;"><span style="color:#8a8f85;min-width:150px;">' +
            fmtDate(e.at) +
  ...
  async function loadQuestionTemplates() {
    ...
    try {
      ...
      if (!qtVendors.length) {
        ...
        if (rm.ok)
          qtVendors = ((await rm.json()) || []).map(function (m) {
            return m.name;
          });
      }
    } catch (e) {
      box.innerHTML = '<div class="err">Could not load questions.</div>';
      return;
    }

    var body = rows
      .map(function (q) {
        var typeLabel = (QT_TYPES.filter(function (t) {
          return t[0] === q.type;
        })[0] || ['', q.type])[1];
        var opts = Array.isArray(q.options) ? q.options : [];
        return (
          '<tr' +
          (q.active ? '' : ' style="opacity:.55;"') +
          '>' +
          td(
            '<b style="font-weight:600;">' +
              esc(q.label) +
          ...
          td(
            q.required
              ? '<span class="chip" style="background:#fdf6e6;color:#6b5a24;">Required</span>'
              : '<span class="muted">Optional</span>',
          ) +
          td(
            '<div style="display:flex;gap:6px;justify-content:flex-end;">' +
              '<button class="link-btn" data-qt-edit="' +
              q.id +
              '" style="width:auto;padding:6px 11px;font-size:12px;">Edit</button>' +
              '<button class="link-btn" data-qt-del="' +
              q.id +
              '" style="width:auto;padding:6px 10px;font-size:12px;color:#9c3327;">✕</button>' +
              '</div>',
          ) +
          '</tr>'
        );
    ...
    box.querySelectorAll('[data-qt-edit]').forEach(function (bt) {
      bt.addEventListener('click', function () {
        openQuestionTemplateForm(
          rows.filter(function (x) {
            return x.id === bt.getAttribute('data-qt-edit');
          })[0],
        );
      });
    });
    box.querySelectorAll('[data-qt-del]').forEach(function (bt) {
      bt.addEventListener('click', async function () {
        if (
          !confirm(
            'Remove this question?\n\nSections that already ask it keep their copy and their answers — only new sections stop asking.',
          )
        )
          return;
        var r = await authed('/bom/question-templates/' + bt.getAttribute('data-qt-del'), {
          method: 'DELETE',
        });
        if (!r.ok) {
          alert('Could not remove (' + r.status + ').');
          return;
        }
  ...
  function openQuestionTemplateForm(q) {
    ...
    openModal(
      q.id ? 'Edit question' : 'New vendor question',
      fieldRow(
        'Question',
        '<input id="qtLabel" style="' +
          IN +
          '" value="' +
          esc(q.label || '') +
          '" placeholder="e.g. What gauge steel?">',
      ) +
        fieldRow(
          'Asked of',
          '<select id="qtVendor" style="' +
            IN +
        ...
        fieldRow(
          'Answer type',
          '<select id="qtType" style="' +
            IN +
            '">' +
            QT_TYPES.map(function (t) {
              return (
                '<option value="' +
                t[0] +
                '"' +
        ...
        '<div id="qtOptWrap" style="display:' +
        (isChoice ? 'block' : 'none') +
        ';">' +
        fieldRow(
          'Options',
          '<textarea id="qtOpts" rows="4" placeholder="One per line" style="' +
            IN +
            'resize:vertical;">' +
            esc(opts) +
            '</textarea>',
        ...
        fieldRow(
          'Help text',
          '<input id="qtHelp" style="' +
            IN +
            '" value="' +
            esc(q.helpText || '') +
            '" placeholder="Optional — shown under the question">',
        ) +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;margin-top:4px;"><input type="checkbox" id="qtReq"' +
        (q.required ? ' checked' : '') +
        '> Required — blocks the section from being confirmed until answered</label>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;margin-top:6px;"><input type="checkbox" id="qtActive"' +
        (q.id ? (q.active !== false ? ' checked' : '') : ' checked') +
        '> Active — asked on new sections</label>',
      async function (close, showErr) {
        var type = document.getElementById('qtType').value;
        var label = document.getElementById('qtLabel').value.trim();
        if (!label) return showErr('Type the question.');
        var list = document
          .getElementById('qtOpts')
          .value.split('\n')
          .map(function (x) {
            return x.trim();
          })
          .filter(Boolean);
        if ((type === 'SELECT' || type === 'MULTI_SELECT') && !list.length)
          return showErr('A dropdown needs at least one option.');
        var payload = {
          label: label,
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'Could not save.');
        }
        close();
        loadQuestionTemplates();
      },
    ...
    t.addEventListener('change', function () {
      document.getElementById('qtOptWrap').style.display =
        t.value === 'SELECT' || t.value === 'MULTI_SELECT' ? 'block' : 'none';
    });
  }

  async function loadFinancingAdmin() {
    var box = document.getElementById('finAdmin');
    if (!box) return;
    var d = null;
    try {
      var r = await authed('/admin/financing');
      if (r.ok) d = await r.json();
    } catch (e) {}
    if (!d) {
      box.innerHTML = '<div class="err">Could not load financing settings.</div>';
      return;
    }

    var factorRows = d.factors
      .map(function (f) {
        ...
        return (
          ...
          td(
            '<input class="finFactor" data-term="' +
              f.termMonths +
              '" type="number" step="0.000001" min="0.0001" max="1" value="' +
              Number(f.factor).toFixed(6) +
              '" style="width:130px;padding:7px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;">',
          ) +
          td(
            '<span class="muted" style="font-size:12.5px;">$100,000 → <b style="color:#20241f;">$' +
              Math.round(100000 * f.factor).toLocaleString() +
              '</b>/mo</span>',
          ) +
          td(
            '<label style="display:flex;gap:6px;align-items:center;font-size:12.5px;cursor:pointer;"><input type="checkbox" class="finActive" data-term="' +
              f.termMonths +
              '"' +
              (f.active ? ' checked' : '') +
              '> Offered</label>',
    ...
    var settingRows = d.settings
      .map(function (s) {
        return (
          ...
          esc(s.label) +
          '</div>' +
          '<div class="muted" style="font-size:12px;line-height:1.5;max-width:560px;margin-top:2px;">' +
          esc(s.help) +
          '</div></div>' +
          '<input class="finSetting" data-key="' +
          s.key +
          '" type="number" step="' +
          s.step +
          '" min="' +
          ...
          '" value="' +
          s.value +
          '" style="width:140px;padding:7px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;text-align:right;">' +
          '</div></div>'
        );
      })
      .join('');
    ...
    box.innerHTML =
      ...
      '<div style="margin-top:18px;padding:14px 16px;background:#fbfbf9;border:1px solid #e7e8e3;border-radius:10px;">' +
      settingRows +
      '</div>' +
      '<div class="muted" style="font-size:12px;margin-top:10px;">Financing enquiries are sent to <b>' +
      esc(d.partnerEmail) +
      '</b>.</div>';
    ...
    document.querySelectorAll('.finFactor').forEach(function (el) {
      el.addEventListener('change', function () {
        save(el, '/admin/financing/factors/' + el.getAttribute('data-term'), {
          factor: Number(el.value),
        }).then(loadFinancingAdmin);
      });
    });
    document.querySelectorAll('.finActive').forEach(function (el) {
      el.addEventListener('change', function () {
        authed('/admin/financing/factors/' + el.getAttribute('data-term'), {
          method: 'PUT',
    ...
    document.querySelectorAll('.finSetting').forEach(function (el) {
      el.addEventListener('change', function () {
        save(el, '/admin/financing/settings/' + el.getAttribute('data-key'), {
  ...
  async function loadFinancing(p, user) {
    var box = document.getElementById('finBox');
    if (!box) return;
    var d = null;
    try {
      var r = await authed('/proposals/' + p.id + '/financing');
      if (r.ok) d = await r.json();
    } catch (e) {}
    finCache = d;
    if (!d) {
      box.innerHTML =
        '<div class="placeholder" style="padding:18px;"><p class="muted" style="margin:0;">No released version to quote from yet.</p></div>';
      return;
    }
    var q = d.quote,
      s = q.section179;
    var m0 = function (x) {
      return '$' + Math.round(Number(x || 0) / 100).toLocaleString();
    };
    var lowest = q.terms.reduce(function (a2, b) {
      return b.monthlyPaymentMinor < a2.monthlyPaymentMinor ? b : a2;
    }, q.terms[0]);

    box.innerHTML =
      '<div class="muted" style="font-size:12.5px;margin:-4px 0 12px;line-height:1.55;">Calculated from the ' +
      fmtMoney(d.proposal.grandTotalMinor) +
      ' project total. Nothing to fill in — the sheet is generated on demand.</div>' +
      '<div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:14px;">' +
      q.terms
        .map(function (t) {
          var best = t.termMonths === lowest.termMonths;
          return (
            '<div style="flex:1;min-width:112px;border:1px solid ' +
            (best ? '#c9a227' : '#e7e8e3') +
            ';border-radius:11px;padding:12px 11px;background:' +
            (best ? '#fdfaf0' : '#fff') +
            ';text-align:center;">' +
            '<div class="k" style="margin:0;">' +
            t.termMonths +
            ' months</div>' +
            '<div style="font-family:Georgia,serif;font-size:19px;font-weight:700;margin:5px 0 1px;">' +
            m0(t.monthlyPaymentMinor) +
            '</div>' +
            '<div class="muted" style="font-size:11px;">per month</div>' +
            '<div class="muted" style="font-size:11px;margin-top:7px;padding-top:7px;border-top:1px solid #eceee8;">' +
            m0(t.totalOfPaymentsMinor) +
            ' total</div>' +
            '</div>'
          );
        })
      ...
      '<div><div class="k">Section 179 deduction</div><div class="v small">' +
      fmtMoney(s.deductionMinor) +
      '</div></div>' +
      '<div><div class="k">Estimated savings at ' +
      s.taxRatePct +
      '%</div><div class="v small" style="color:#2f6b4f;">' +
      fmtMoney(s.estimatedSavingsMinor) +
      '</div></div>' +
      '<div><div class="k">Net cost</div><div class="v small">' +
      fmtMoney(s.netCostMinor) +
      '</div></div>' +
      (s.exceedsCap
        ? '<div class="muted" style="font-size:11.5px;align-self:center;max-width:230px;">Above the ' +
          m0(s.capMinor) +
          ' cap — the excess may still be depreciated.</div>'
        : '') +
      '</div>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
    ...
    document.getElementById('finPreview').addEventListener('click', async function () {
      ...
      if (!r.ok) {
        alert('Could not build the sheet.');
        return;
      }
    ...
    document.getElementById('finPdf').addEventListener('click', async function () {
      ...
      try {
        ...
        alert('Could not reach the renderer.');
      }
      bt.disabled = false;
      bt.textContent = 'Download PDF';
    });
    var sb = document.getElementById('finSend');
    if (sb)
      sb.addEventListener('click', function () {
        openSendDocuments(p, d, 'partner');
      });
    ...
    if (sc)
      sc.addEventListener('click', function () {
        openSendDocuments(p, d, 'financing');
      });
  }
  ...
  async function openSendDocuments(p, fin, preset) {
    ...
    try {
      var rc = await authed('/proposals/' + p.id + '/send-context');
      if (rc.ok) ctx = await rc.json();
    } catch (e) {}
      ...
      ? '<div style="margin-top:14px;padding-top:12px;border-top:1px solid #e7e8e3;">' +
        ...
        ctx.history
          ...
          .map(function (h) {
            ...
            var docs = Array.isArray(det.documents)
              ? det.documents.length + ' file' + (det.documents.length === 1 ? '' : 's')
              : 'financing sheet';
            return (
              '<div style="font-size:12px;color:' +
              (h.failed ? '#9c3327' : '#5c6157') +
              ';line-height:1.6;">' +
              (h.failed ? '✕ Failed — ' : '✓ ') +
              esc(docs) +
              ' to ' +
    ...
    var contactChips = ctx.contacts.length
      ? '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:7px;">' +
        ctx.contacts
          .map(function (c) {
            return (
              '<button type="button" class="sdWho link-btn" data-email="' +
              esc(c.email) +
              '" style="width:auto;padding:5px 10px;font-size:12px;">' +
              esc(c.name || c.email) +
              (c.isDecisionMaker ? ' ★' : '') +
          ...
          .join('') +
        '<button type="button" class="sdWho link-btn" data-email="' +
        esc(ctx.partnerEmail) +
        '" style="width:auto;padding:5px 10px;font-size:12px;">Ryan Capital</button>' +
        '</div>'
      : '';

    var check = function (id, on, label, note, disabled) {
      return (
        '<label style="display:flex;gap:9px;align-items:flex-start;padding:10px 12px;border:1px solid #e7e8e3;border-radius:9px;background:' +
        (disabled ? '#f4f5f1' : '#fff') +
        ';' +
        (disabled ? 'opacity:.6;' : 'cursor:pointer;') +
        '">' +
        ...
        (disabled ? ' disabled' : '') +
        ' style="margin-top:2px;">' +
        '<span><b style="font-weight:600;font-size:13px;">' +
        label +
        '</b>' +
        '<div class="muted" style="font-size:11.5px;line-height:1.5;margin-top:1px;">' +
        note +
        '</div></span></label>'
      );
    };
    ...
    openModal(
      ...
      '<div class="muted" style="font-size:13px;margin-bottom:12px;line-height:1.55;">' +
        ...
        check(
          ...
          wantProposal,
          'Proposal',
          'The customer document, exactly as the preview shows it.',
        ) +
        check(
          'sdFin',
          wantFinancing && hasFinancing,
          'Financing options',
          hasFinancing
            ? 'Monthly payments and the Section 179 position, calculated from this proposal.'
            : 'Not available until the proposal has a saved price.',
          !hasFinancing,
        ) +
        '</div>' +
        fieldRow(
          ...
          '<input id="sdTo" value="' +
            esc(defaultTo) +
            '" placeholder="name@company.com" style="' +
            IN +
            '">' +
            contactChips,
        ) +
        fieldRow(
          'Cc',
          '<input id="sdCc" placeholder="Optional — comma separated" style="' + IN + '">',
        ) +
        fieldRow(
          'Subject',
          '<input id="sdSubject" placeholder="Leave blank for the default" style="' + IN + '">',
        ) +
        fieldRow(
          'Message',
          '<textarea id="sdMsg" rows="5" placeholder="Leave blank for a short default note" style="' +
            IN +
            'resize:vertical;"></textarea>',
        ) +
        histHtml,
      async function (close, showErr) {
        ...
        if (wantP) {
          ...
          if (!doc)
            return showErr(
              'Could not prepare the proposal. Open the proposal preview once, then try again.',
            );
          body.proposalHtml = doc.html;
          body.proposalFilename = doc.filename;
        }

        var r = await authed('/proposals/' + p.id + '/send-documents', {
          method: 'POST',
          body: body,
        });
        if (!r.ok) {
    ...
    document.querySelectorAll('.sdWho').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        if (cur.indexOf(em) === -1) cur.push(em);
        else
  ...
  async function buildProposalDocForSend(p, versionId) {
    try {
      ...
      var versions = (full.versions || []).slice().sort(function (x, y) {
  ...
  async function renderAdmin(user) {
    ...
    document.getElementById('qtNew').addEventListener('click', function () {
      openQuestionTemplateForm(null);
    });
    loadUsers();
    loadStandardNotes();
    loadFormulas();
    ...
    try {
      ...
      box.querySelectorAll('.snDel').forEach(function (b) {
        b.addEventListener('click', async function () {
          ...
          var rr = await authed('/standard-notes/' + b.getAttribute('data-id'), {
            method: 'DELETE',
          });
          if (!rr.ok && rr.status !== 204) {
  ...
  function fxSet(kind) {
    return fx.data ? fx.data[kind] : null;
  }
  function fxInputs(kind) {
    var set = fxSet(kind);
    return (set && set.inputs) || [];
  }
  function fxSourceLabel(kind, src) {
    if (!src) return '';
    if (src.indexOf('in:') === 0 || src.indexOf('flag:') === 0) {
      var k = src.slice(src.indexOf(':') + 1);
      var f = fxInputs(kind).filter(function (i) {
        return i.key === k;
      })[0];
      return f ? f.label : k;
    }
  ...
  function fxFormulaText(kind, r) {
    ...
    var body =
      (r.terms || [])
        .map(function (t, i) {
          ...
          var base = t.source
            ? fxSourceLabel(kind, t.source) + (mag === 1 ? '' : ' × ' + mag)
            : String(mag);
    ...
    if (r.mode === 'PRESENCE') body = '(' + body + ') > 0 ? ' + r.constant + ' : 0';
      ...
      if (r.roundMode === 'CEIL')
        body = 'ceil(' + body + (Number(r.roundStep) > 1 ? ', step ' + r.roundStep : '') + ')';
      else if (r.roundMode === 'ROUND')
        body = 'round(' + body + (Number(r.roundStep) > 1 ? ', step ' + r.roundStep : '') + ')';
    }
    return body + (r.when ? '  —  only ' + fxCondText(kind, r.when) : '');
  }
  async function loadFormulas() {
    var box = document.getElementById('fxBody');
    if (!box) return;
    ...
    try {
      ...
      if (!r.ok) {
        box.innerHTML =
          '<div class="err">Could not load formulas (' +
          r.status +
          '). Run the 0021 migration if this persists.</div>';
        return;
      }
  ...
  function drawFxTabs() {
    var box = document.getElementById('fxTabs');
    if (!box) return;
    box.innerHTML = FX_TABS.map(function (t) {
      ...
      return (
        ...
        ';border-radius:999px;padding:7px 13px;font-size:12.5px;cursor:pointer;">' +
        esc(t.label) +
        (count ? ' <span style="opacity:.65;">' + count + '</span>' : '') +
        '</button>'
      );
    }).join('');
    box.querySelectorAll('[data-fxt]').forEach(function (b) {
      b.addEventListener('click', function () {
        fx.tab = b.getAttribute('data-fxt');
        drawFxTabs();
        drawFormulas();
      });
    });
  ...
  function fxConfirmChange(opts) {
    ...
    return new Promise(function (resolve) {
      ...
      wrap.style.cssText =
        'position:fixed;inset:0;background:rgba(30,34,30,.45);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px;';
      var card = document.createElement('div');
      card.style.cssText =
        'background:#fff;border:1px solid #dcded7;border-radius:14px;max-width:560px;width:100%;padding:22px 24px;box-shadow:0 18px 50px rgba(0,0,0,.18);max-height:88vh;overflow:auto;';
      ...
      var rows = (opts.changes || [])
        .map(function (c) {
          return (
            '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:14px;padding:8px 0;border-top:1px solid #f2f3ef;font-size:13.5px;">' +
            '<span>' +
            esc(String(c.label)) +
            '</span>' +
            '<span style="font-family:ui-monospace,monospace;font-size:12.5px;white-space:nowrap;">' +
            '<span style="color:#8a8f85;text-decoration:line-through;">' +
            esc(String(c.from)) +
            '</span>' +
            ' &rarr; <b style="color:#9c3327;">' +
            esc(String(c.to)) +
            '</b>' +
            ' <span class="muted">' +
            esc(String(c.unit || '')) +
      ...
      card.innerHTML =
        '<div style="font-family:\'Newsreader\',serif;font-size:19px;font-weight:600;color:#3d4a55;">Type ' +
        esc(word) +
        ' to ' +
        esc(opts.what) +
        '</div>' +
        ...
        '<input id="fxcWord" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="' +
        esc(word) +
        '" style="width:100%;margin-top:14px;padding:11px 13px;border:1px solid #dcded7;border-radius:9px;font-size:15px;font-family:ui-monospace,monospace;letter-spacing:.1em;">' +
      ...
      (async function () {
        ...
        var qs = (opts.parts || [])
          .map(function (p) {
            return 'part=' + encodeURIComponent(p);
          })
          .join('&');
        try {
          var r = await authed('/formulas/impact' + (qs ? '?' + qs : ''));
          if (!r.ok) {
            el.textContent = 'Could not check which open orders this affects.';
            return;
          }
          var d = await r.json();
          var list = (d.orders || [])
          ...
          el.innerHTML =
            ...
            (list
              ? '<div style="margin-top:6px;font-family:ui-monospace,monospace;font-size:11.5px;">' +
                list +
                ((d.orders || []).length > 8
                  ? '<div class="muted" style="padding:3px 0;">and ' +
                    ((d.orders || []).length - 8) +
                    ' more</div>'
                  : '') +
                '</div>'
      ...
      var input = card.querySelector('#fxcWord'),
        go = card.querySelector('#fxcGo');
      function sync() {
        ...
        go.style.cssText =
          ...
          ';color:' +
          (ok ? '#fff' : '#9aa093') +
          ';border-radius:9px;padding:9px 16px;font-size:13px;cursor:' +
      ...
      input.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' && sync()) done(input.value.trim());
      });
      card.querySelector('#fxcCancel').addEventListener('click', function () {
        done(null);
      });
      go.addEventListener('click', function () {
        if (!go.disabled) done(input.value.trim());
      });
      input.focus();
    });
  }
  ...
  async function drawRevisionLog() {
    var box = document.getElementById('fxLog');
    if (!box) return;
    ...
    var head =
      ...
      '<input type="checkbox" id="fxLogAll"' +
      (fxLogAll ? ' checked' : '') +
      '> All sets</label>' +
      '</div>';
    box.innerHTML =
      head +
      '<div class="card" style="padding:14px;"><div class="muted" style="font-size:12.5px;">Loading…</div></div>';
    ...
    try {
      ...
      if (!r.ok) {
        box.innerHTML =
          head +
          '<div class="card" style="padding:14px;"><div class="muted" style="font-size:12.5px;">Could not load the log (' +
          r.status +
          ').</div></div>';
        wireToggle();
        return;
      ...
      box.innerHTML =
        head +
        '<div class="card" style="padding:14px;"><div class="muted" style="font-size:12.5px;">Could not reach the server.</div></div>';
      wireToggle();
      return;
    }
    ...
    if (!rows.length) {
      box.innerHTML =
        head +
        '<div class="card" style="padding:14px;"><div class="muted" style="font-size:12.5px;line-height:1.55;">' +
    ...
    box.innerHTML =
      ...
      rows
        .map(function (x) {
          ...
          return (
            '<div style="padding:12px 14px;border-bottom:1px solid #f2f3ef;' +
            (undone ? 'opacity:.55;' : '') +
            '">' +
            '<div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline;">' +
            '<code style="font-size:12px;color:#4a4f47;">' +
            esc(x.target) +
            '</code>' +
            '<span class="muted" style="font-size:11px;white-space:nowrap;">' +
            esc(KIND_LABEL[x.kind] || x.kind) +
            '</span>' +
            '</div>' +
            ...
            (x.impactedCount
              ? ' · <span style="color:#9c3327;">' +
                ...
                '</span>'
              : '') +
            (undone ? ' · undone' + (x.undoneByName ? ' by ' + esc(x.undoneByName) : '') : '') +
            '</div>' +
            '<div style="display:flex;gap:10px;margin-top:6px;">' +
            '<button class="fxRevDetail" data-id="' +
            x.id +
            '" style="border:none;background:none;padding:0;font-size:11.5px;color:#3d4a55;text-decoration:underline;cursor:pointer;">Before / after</button>' +
            (x.undoable && isSystemAdmin()
              ? '<button class="fxRevUndo" data-id="' +
                x.id +
                '" style="border:none;background:none;padding:0;font-size:11.5px;color:#9c3327;text-decoration:underline;cursor:pointer;">Undo</button>'
              : '') +
            '</div>' +
            '</div>'
          );
    ...
    box.querySelectorAll('.fxRevDetail').forEach(function (b) {
      b.addEventListener('click', async function () {
        ...
        if (!rr.ok) {
          alert('Could not load that revision (' + rr.status + ').');
          return;
        }
        var d = await rr.json();
        var pane = function (label, v) {
          return (
            '<div style="flex:1;min-width:0;"><div class="k">' +
            label +
            '</div>' +
            ...
            esc(
              v == null
                ? 'No override — the workbook default applied.'
        ...
        openModal(
          ...
          '<div class="muted" style="font-size:12.5px;line-height:1.55;margin-bottom:12px;">' +
            ...
            (d.confirmedWord ? ' · typed ' + esc(d.confirmedWord) : '') +
            '</div>',
          null,
          null,
        );
    ...
    box.querySelectorAll('.fxRevUndo').forEach(function (b) {
      b.addEventListener('click', async function () {
        ...
        if (
          !confirm(
            'Undo this change?\n\nThe value goes back to what it was immediately before this edit — not to the workbook default. The undo is itself recorded.',
          )
        )
          return;
        var rr = await authed('/formulas/revisions/' + b.getAttribute('data-id') + '/undo', {
          method: 'POST',
          body: {},
        });
        if (!rr.ok) {
          alert(await serverMessage(rr, 'Could not undo (' + rr.status + ').'));
          return;
        }
  ...
  function drawFormulas() {
    ...
    if (fx.tab === 'code') {
      box.innerHTML =
        ...
        (fx.data.inCode || [])
          .map(function (c) {
            return (
              '<div class="card" style="margin-bottom:10px;"><div style="font-weight:600;font-size:14px;">' +
              esc(c.name) +
              '</div>' +
              '<div class="muted" style="font-size:12px;font-family:ui-monospace,monospace;margin-top:2px;">' +
              esc(c.where) +
              '</div>' +
              '<div style="font-size:13px;line-height:1.55;margin-top:6px;">' +
              esc(c.what) +
              '</div>' +
              '<div class="muted" style="font-size:12.5px;line-height:1.5;margin-top:4px;">Why it is not a setting: ' +
              esc(c.why) +
              '</div></div>'
            );
          })
    ...
    if (fx.tab === 'settings') {
      ...
      box.innerHTML =
        '<div class="muted" style="font-size:12.5px;margin-bottom:12px;">The business numbers the proposal math uses. Changing one affects new proposals; documents already saved keep their own figures. Saving asks you to type ' +
        esc((fx.data && fx.data.confirmWord) || 'CONFIRMED') +
        ' and lists the open orders built on the current figures.</div>' +
        groups
          .map(function (g) {
            return (
              '<div class="card" style="margin-bottom:12px;"><div class="section-title" style="margin:0 0 10px;">' +
              esc(g) +
              '</div>' +
              st.defs
                .filter(function (d) {
                ...
                .map(function (d) {
                  ...
                  return (
                    '<div style="display:flex;align-items:flex-start;gap:12px;padding:8px 0;border-top:1px solid #f2f3ef;">' +
                    '<div style="flex:1;"><div style="font-size:13.5px;font-weight:600;">' +
                    esc(d.label) +
                    (changed
                      ? ' <span style="background:#fdf6e3;border:1px solid #eadfbe;color:#8a6d1f;border-radius:999px;padding:1px 7px;font-size:10.5px;font-weight:600;">changed from ' +
                        st.defaults[d.key] +
                        '</span>'
                      : '') +
                    (d.confirm
                      ? ' <span title="Feeds the price of every Adventure mat quoted from now on" style="background:#fbf0ee;border:1px solid #e6cbc6;color:#9c3327;border-radius:999px;padding:1px 7px;font-size:10.5px;font-weight:600;">affects mat pricing</span>'
                      : '') +
                    '</div>' +
                    '<div class="muted" style="font-size:12px;line-height:1.5;margin-top:2px;">' +
                    esc(d.help) +
                    '</div></div>' +
                    '<div style="display:flex;align-items:center;gap:6px;flex:0 0 auto;">' +
                    '<input class="fxSet" data-k="' +
                    d.key +
                    '" type="number" min="' +
                    d.min +
                    '" max="' +
                    ...
                    '" value="' +
                    esc(v) +
                    '" style="width:92px;padding:7px 9px;border:1px solid #dcded7;border-radius:8px;text-align:right;font-size:13.5px;">' +
                    '<span class="muted" style="font-size:12px;min-width:34px;">' +
                    esc(d.unit) +
                    '</span></div></div>'
                  );
                })
      ...
      document.getElementById('fxSaveSet').addEventListener('click', async function () {
        var body = {};
        document.querySelectorAll('.fxSet').forEach(function (el) {
          body[el.getAttribute('data-k')] = Number(el.value);
        });
        ...
        if (!word) {
          msg.textContent = 'Cancelled — nothing was saved.';
          return;
        }
        body.confirm = word;
    ...
    var rows = (set.rules || [])
      .map(function (rl) {
        ...
        if (kind === 'frame' && rl.group && rl.group !== lastGroup) {
          lastGroup = rl.group;
          head =
            '<tr><td colspan="4" style="padding:9px 14px 5px;background:#f7f8f4;border-bottom:1px solid #e7e8e3;font-size:11px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;color:#3d4a55;">' +
            esc(rl.group) +
            '</td></tr>';
        }
        return (
          head +
          '<tr' +
          (rl.active ? '' : ' style="opacity:.6;"') +
          '>' +
          td(
            '<code style="font-size:12.5px;color:#4a4f47;">' +
              esc(rl.part) +
              '</code>' +
              (edited
                ? '<div><span style="display:inline-block;margin-top:3px;background:#fdf6e3;border:1px solid #eadfbe;color:#8a6d1f;border-radius:999px;padding:2px 8px;font-size:11px;font-weight:600;">Edited</span></div>'
                : ''),
          ) +
          td('<span style="font-size:13px;">' + esc(rl.name) + '</span>') +
          td(
            '<span style="font-size:12.5px;color:#4a4f47;font-family:ui-monospace,monospace;line-height:1.45;">' +
              esc(fxFormulaText(kind, rl)) +
              '</span>',
          ) +
          td(
            '<div style="display:flex;gap:6px;justify-content:flex-end;"><button class="fxEdit" data-part="' +
              esc(rl.part) +
              '" style="border:1px solid #dcded7;background:#fff;border-radius:8px;padding:6px 12px;font-size:12.5px;color:#3d4a55;cursor:pointer;">Edit</button>' +
              (edited
                ? '<button class="fxRevert" data-part="' +
                  esc(rl.part) +
                  '" style="border:1px solid #e0e1db;background:#fff;border-radius:8px;padding:6px 12px;font-size:12.5px;color:#9c3327;cursor:pointer;">Reset</button>'
                : '') +
              '</div>',
          ) +
          '</tr>'
    ...
    box.innerHTML =
      ...
      '<div class="muted" style="font-size:12.5px;max-width:760px;line-height:1.55;">' +
      esc(set.blurb || '') +
      ' Quantities stay formula-driven — these are the numbers each formula multiplies by. Rows you have changed are badged <b>Edited</b> and can be reset individually.</div>' +
    ...
    document.getElementById('fxNewRule').addEventListener('click', function () {
      ...
      if (!part) return;
      if (
        (set.rules || []).some(function (x) {
          return x.part === part;
        })
      ) {
        alert('That part already has a rule — edit it in the list.');
        return;
      }
      openFormulaForm(kind, {
        ...
        active: true,
        when: null,
        group: kind === 'frame' ? 'Zip line' : 'Hardware',
      });
    });
    document.getElementById('fxResetKind').addEventListener('click', async function () {
      ...
      if (!word) return;
      var r = await authed('/formulas/reset', {
        method: 'POST',
        body: { kind: kind.toUpperCase(), confirm: word },
      });
      if (!r.ok) {
        alert(await serverMessage(r, 'Could not reset (' + r.status + ').'));
        return;
      }
    ...
    box.querySelectorAll('.fxEdit').forEach(function (b) {
      b.addEventListener('click', function () {
        openFormulaForm(
          kind,
          (set.rules || []).filter(function (x) {
            return x.part === b.getAttribute('data-part');
          })[0],
        );
      });
    });
    box.querySelectorAll('.fxRevert').forEach(function (b) {
      b.addEventListener('click', async function () {
        var part = b.getAttribute('data-part');
        var word = await fxConfirmChange({
          what: 'reset ' + part + ' to its workbook default',
          parts: [part],
        });
        if (!word) return;
        var rr = await authed(
        ...
        if (!rr.ok && rr.status !== 204) {
          alert(await serverMessage(rr, 'Could not reset (' + rr.status + ').'));
          return;
        }
  ...
  function fxSourceOptions(kind, sel) {
    ...
    var numeric = inputs
      .filter(function (i) {
        return i.kind !== 'flag' && i.kind !== 'choice';
      })
      .map(function (i) {
        return ['in:' + i.key, i.label];
      });
    var flags = inputs
      .filter(function (i) {
        return i.kind === 'flag';
      ...
      .map(function (i) {
        return ['flag:' + i.key, i.label + ' (yes = 1)'];
      });
    var out =
      '<option value=""' +
      (sel ? '' : ' selected') +
      '>— a plain number —</option>' +
      group('Configurator answers', numeric) +
      group('Yes / no answers', flags);
    if (kind === 'hardware') {
      out += group(
        'Frame quantities',
        (src.frameParts || [])
          .concat(
            (src.skuParts || []).filter(function (p) {
              return (src.frameParts || []).indexOf(p) === -1 && p.indexOf('6820H-') !== 0;
            }),
          )
          .map(function (p) {
            return ['bom:' + p, p];
      ...
      out += group(
        'Other fastener rows',
        (src.hardwareParts || []).map(function (p) {
          return ['hw:' + p, p];
        }),
      );
    }
  ...
  function openFormulaForm(kind, rule) {
    if (!rule) return;
    var terms = (rule.terms || []).map(function (t) {
      return {
        source: t.source || '',
        coefficient: t.coefficient,
        when: t.when ? { input: t.when.input, op: t.when.op, value: t.when.value } : null,
      };
    });
    var ruleWhen = rule.when
      ? { input: rule.when.input, op: rule.when.op, value: rule.when.value }
      : null;
    ...
    var condValueField = function (cls, i, cond) {
      ...
      var def = inputs.filter(function (x) {
        return x.key === key;
      })[0];
      if (def && def.kind === 'flag') {
        return (
          ...
          '" style="' +
          IN +
          'width:110px;"><option value="true"' +
          (cond && cond.value !== false ? ' selected' : '') +
      ...
      return (
        ...
        i +
        '" value="' +
        esc(cond && cond.value != null ? cond.value : 0) +
        '" style="' +
        IN +
    ...
    var termsHtml = function () {
      return (
        terms
          .map(function (t, i) {
            return (
              '<div style="border:1px solid #eef0ea;border-radius:9px;padding:8px;margin-bottom:7px;background:#fff;">' +
              '<div style="display:flex;gap:6px;align-items:center;">' +
              '<select class="fxTermSrc" data-i="' +
              i +
              '" style="' +
              IN +
              'flex:1;">' +
              fxSourceOptions(kind, t.source) +
              '</select>' +
              '<span class="muted" style="font-size:13px;">×</span>' +
              '<input class="fxTermK" data-i="' +
              i +
              '" value="' +
              esc(t.coefficient) +
              '" style="width:84px;padding:9px 10px;border:1px solid #dcded7;border-radius:9px;text-align:right;font-size:14px;">' +
              '<button type="button" class="fxTermDel" data-i="' +
              i +
              '" style="border:1px solid #e0e1db;background:#fff;border-radius:8px;width:32px;height:32px;color:#9c3327;cursor:pointer;">✕</button>' +
              '</div>' +
              '<div style="display:flex;gap:6px;align-items:center;margin-top:6px;">' +
              '<span class="muted" style="font-size:11.5px;min-width:44px;">Count</span>' +
              '<select class="fxTermCond" data-i="' +
              i +
              '" style="' +
              IN +
              'flex:1;">' +
              condOptions(t.when ? t.when.input : '') +
              '</select>' +
              (t.when
                ? '<select class="fxTermOp" data-i="' +
                  i +
                  '" style="' +
                  IN +
                  'width:74px;">' +
                  opOptions(t.when.op) +
                  '</select>' +
                  condValueField('fxTermCond', i, t.when)
                : '') +
              '</div></div>'
            );
          })
          .join('') ||
        '<div class="muted" style="font-size:12.5px;margin-bottom:6px;">No drivers — this rule produces its constant only.</div>'
      );
    };
    var readTerms = function () {
      document.querySelectorAll('.fxTermSrc').forEach(function (el) {
        terms[+el.getAttribute('data-i')].source = el.value;
      });
      document.querySelectorAll('.fxTermK').forEach(function (el) {
        terms[+el.getAttribute('data-i')].coefficient = Number(el.value) || 0;
      });
    ...
    var wireTerms = function () {
      var host = document.getElementById('fxTerms');
      if (!host) return;
      ...
      host.querySelectorAll('.fxTermDel').forEach(function (b) {
        b.addEventListener('click', function () {
          readTerms();
          terms.splice(+b.getAttribute('data-i'), 1);
          wireTerms();
        });
      });
    };
    openModal(
      'Quantity formula — ' + rule.part,
      '<div class="muted" style="font-size:12.5px;margin-bottom:10px;">' +
        esc(rule.name) +
        (rule.group ? ' · ' + esc(rule.group) : '') +
        '</div>' +
        ...
        '<div style="display:flex;gap:8px;">' +
        '<div class="field" style="flex:1;"><label>Constant</label><input id="fxConst" value="' +
        esc(rule.constant) +
        '" style="' +
        IN +
        'text-align:right;"></div>' +
        '<div class="field" style="flex:1;"><label>Overage factor</label><input id="fxFactor" value="' +
        esc(rule.factor) +
        '" style="' +
        IN +
        'text-align:right;"></div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;">' +
        '<div class="field" style="flex:1;"><label>Rounding</label><select id="fxRound" style="' +
        IN +
        '">' +
        [
          ['NONE', 'None'],
          ...
          .join('') +
        '</select></div>' +
        '<div class="field" style="flex:1;"><label>Sold in multiples of</label><input id="fxStep" value="' +
        esc(rule.roundStep) +
        '" style="' +
        IN +
        'text-align:right;"></div>' +
        '<div class="field" style="flex:1;"><label>Mode</label><select id="fxMode" style="' +
        IN +
        '">' +
        [
          ['SUM', 'Sum of drivers'],
          ['PRESENCE', 'If any, use constant'],
        ]
          .map(function (o) {
            return (
              '<option value="' +
        ...
        '<select id="fxRuleCond" style="' +
        IN +
        'flex:1;">' +
        condOptions(ruleWhen ? ruleWhen.input : '') +
        '</select>' +
        '<select id="fxRuleOp" style="' +
        IN +
        'width:74px;' +
        (ruleWhen ? '' : 'display:none;') +
        '">' +
        opOptions(ruleWhen ? ruleWhen.op : '=') +
        '</select>' +
        '<span id="fxRuleValWrap">' +
        (ruleWhen ? condValueField('fxRuleCond', 0, ruleWhen) : '') +
        '</span>' +
        '</div></div>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;margin:2px 0;cursor:pointer;"><input type="checkbox" id="fxMinZero"' +
        (rule.minZero !== false ? ' checked' : '') +
        '> Never go below zero</label>' +
        '<label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;"><input type="checkbox" id="fxActive"' +
        (rule.active !== false ? ' checked' : '') +
        '> Include this item at all</label>' +
      ...
      async function (close, showErr) {
        ...
        body.confirm = word;
        var r = await authed(
          '/formulas/' + kind.toUpperCase() + '/' + encodeURIComponent(rule.part),
        ...
        if (!r.ok) {
          ...
          return showErr(m || 'Could not save (' + r.status + ').');
        }
        fxReportRevision(await r.json());
        close();
        loadFormulas();
    ...
    wireTerms();
    document.getElementById('fxAddTerm').addEventListener('click', function () {
      readTerms();
      terms.push({ source: '', coefficient: 1, when: null });
      wireTerms();
    });
    ...
    document.getElementById('fxPreview').addEventListener('click', async function () {
      ...
      var body = fxFormBody();
      body.part = rule.part;
      var r = await authed('/formulas/preview', {
        method: 'POST',
        body: { kind: kind.toUpperCase(), answers: answers, overrides: [body] },
      });
      if (!r.ok) {
        out.textContent = 'Preview failed (' + r.status + ').';
        return;
      }
      var d = await r.json();
      var changed = (d.rows || []).filter(function (x) {
        return x.changed;
      });
      var mine = (d.rows || []).filter(function (x) {
        return x.part === rule.part;
      })[0];
      out.innerHTML =
        (mine
          ? '<div style="color:#20241f;"><b>' +
            esc(rule.part) +
            '</b>: ' +
            mine.qtyBefore +
            ' → <b>' +
            mine.qtyAfter +
            '</b> · <span style="font-family:ui-monospace,monospace;font-size:11.5px;">' +
            esc(mine.formula) +
            '</span></div>'
          : '') +
        (changed.filter(function (x) {
          return x.part !== rule.part;
        }).length
          ? '<div style="margin-top:5px;">Knock-on changes: ' +
            changed
              .filter(function (x) {
                return x.part !== rule.part;
              })
              .map(function (x) {
                return esc(x.part) + ' ' + x.qtyBefore + '→' + x.qtyAfter;
              })
              .join(', ') +
            '</div>'
          : changed.length
            ? ''
            : '<div style="margin-top:5px;">No change on this configuration.</div>');
    });
  }
  ...
  async function loadUsers() {
    ...
    if (!box) return;
    try {
      var r = await authed('/admin/users');
      if (!r.ok) {
        box.innerHTML = '<div class="err">Could not load (' + r.status + ').</div>';
        return;
      }
        ...
        .map(function (u) {
          return (
            ...
            td(
              '<select data-id="' +
                u.id +
                '" class="roleSel" style="padding:6px 9px;border:1px solid #dcded7;border-radius:8px;font-size:13px;background:#fff;">' +
                ROLES.map(function (rl) {
                  return (
                    '<option value="' +
                    rl +
                    '"' +
            ...
            td(
              '<div style="display:flex;gap:6px;justify-content:flex-end;">' +
                ...
                u.id +
                '" style="width:auto;padding:6px 11px;">Edit</button>' +
                '<button class="link-btn" data-pwd="' +
                u.id +
                '" data-email="' +
                esc(u.email) +
                '" style="width:auto;padding:6px 11px;">Reset password</button>' +
                (u.isActive
                  ? '<button class="link-btn" data-deact="' +
                    u.id +
                    '" style="width:auto;padding:6px 11px;">Deactivate</button>'
                  : '<button class="link-btn" data-react="' +
                    u.id +
                    '" style="width:auto;padding:6px 11px;">Reactivate</button>') +
                '</div>',
            ) +
            '</tr>'
          );
      ...
      document.querySelectorAll('.roleSel').forEach(function (sel) {
        sel.addEventListener('change', async function () {
          var r2 = await authed('/admin/users/' + sel.getAttribute('data-id') + '/role', {
            method: 'PATCH',
            body: { role: sel.value },
          });
          if (!r2.ok) {
            alert('Could not change role (' + r2.status + ').');
            loadUsers();
          }
        });
      });
      document.querySelectorAll('[data-deact]').forEach(function (bt) {
        bt.addEventListener('click', async function () {
          if (!confirm('Deactivate this user?')) return;
          var r2 = await authed('/admin/users/' + bt.getAttribute('data-deact') + '/deactivate', {
            method: 'PATCH',
            body: {},
          });
          if (!r2.ok) alert('Could not deactivate.');
          loadUsers();
        });
      });
      document.querySelectorAll('[data-react]').forEach(function (bt) {
        bt.addEventListener('click', async function () {
          var r2 = await authed('/admin/users/' + bt.getAttribute('data-react') + '/reactivate', {
            method: 'PATCH',
            body: {},
          });
          if (!r2.ok) alert('Could not reactivate.');
          loadUsers();
        });
      });
      document.querySelectorAll('[data-pwd]').forEach(function (bt) {
        bt.addEventListener('click', function () {
          openResetPasswordForm(bt.getAttribute('data-pwd'), bt.getAttribute('data-email'));
        });
      });
      document.querySelectorAll('[data-edit]').forEach(function (bt) {
        bt.addEventListener('click', function () {
          openEditUserForm(admUsers[bt.getAttribute('data-edit')]);
        });
      });
    } catch (e) {
      box.innerHTML = '<div class="err">Could not reach the server.</div>';
  ...
  function openEditUserForm(u) {
    ...
    openModal(
      ...
      fieldRow('Name', '<input id="eName" style="' + IN + '" value="' + esc(u.name || '') + '">') +
        fieldRow(
          'Email',
          '<input id="eEmail" type="email" style="' +
            IN +
            '" value="' +
            esc(u.email || '') +
            '" required>',
        ...
        (isMe
          ? 'This is the address you sign in with. Changing it takes effect immediately — you stay signed in here.'
          : 'This is the address they sign in with. Tell them before you change it.') +
        '</div>' +
        fieldRow(
          'Title',
          '<input id="eTitle" style="' +
            IN +
            '" value="' +
            esc(u.title || '') +
        ...
        fieldRow(
          'Phone',
          '<input id="ePhone" style="' +
            IN +
            '" value="' +
            esc(u.phone || '') +
            '" placeholder="720-457-5500">',
        ),
      async function (close, showErr) {
        ...
        var updated = await r.json().catch(function () {
          return null;
        });
        close();
        ...
        if (isMe && updated && currentUser) {
          ...
          currentUser.title = updated.title;
  ...
  function openUserForm() {
    openModal(
      ...
      fieldRow('Email', '<input id="uEmail" type="email" style="' + IN + '" required>') +
        ...
        fieldRow(
          'Title',
          '<input id="uTitle" style="' + IN + '" placeholder="e.g. Sales Director">',
        ) +
        fieldRow('Phone', '<input id="uPhone" style="' + IN + '" placeholder="720-457-5500">') +
        fieldRow(
          'Temporary password',
          '<input id="uPass" style="' + IN + '" placeholder="at least 12 characters" required>',
        ) +
      ...
      async function (close, showErr) {
        ...
        var body = {
          email: email,
          name: document.getElementById('uName').value.trim() || undefined,
          password: pass,
        ...
        var created = await r.json().catch(function () {
          return null;
        });
        ...
        if (created && created.id && (title || phone)) {
          await authed('/admin/users/' + created.id, {
            method: 'PATCH',
            body: { title: title, phone: phone },
          });
        }
        close();
        loadUsers();
  ...
  function openResetPasswordForm(id, email) {
    openModal(
      'Reset password',
      '<div class="muted" style="font-size:12.5px;margin-bottom:12px;">Sets a new password for <b>' +
        esc(email) +
        '</b> and signs them out everywhere. Give them the new password directly — they can change it themselves from Change password once signed in.</div>' +
        fieldRow(
          'New password',
          '<input id="rPass" style="' + IN + '" placeholder="at least 12 characters" required>',
        ),
      async function (close, showErr) {
        var pass = document.getElementById('rPass').value;
        if (pass.length < 12) return showErr('Password must be at least 12 characters.');
        var r = await authed('/admin/users/' + id + '/reset-password', {
          method: 'POST',
          body: { password: pass },
        });
  ...
  function openProfileForm(user) {
    openModal(
      ...
      fieldRow(
        'Full name',
        '<input id="upName" style="' + IN + '" value="' + esc(user.name || '') + '" required>',
      ) +
        fieldRow(
          'Title',
          '<input id="upTitle" style="' +
            IN +
            '" placeholder="e.g. Director of Sales" value="' +
            esc(user.title || '') +
            '">',
        ) +
        fieldRow(
          'Phone',
          '<input id="upPhone" style="' +
            IN +
            '" placeholder="e.g. (720) 457-5500" value="' +
            esc(user.phone || '') +
            '">',
        ) +
        fieldRow(
  ...
  function openPasswordForm() {
    openModal(
      ...
      fieldRow(
        'Current password',
        '<input id="pwCur" type="password" autocomplete="current-password" style="' +
          IN +
          '" required>',
      ) +
        fieldRow(
          'New password',
          '<input id="pwNew" type="password" autocomplete="new-password" minlength="12" style="' +
            IN +
            '" required>',
        ) +
        fieldRow(
          'Confirm new password',
          '<input id="pwNew2" type="password" autocomplete="new-password" minlength="12" style="' +
            IN +
            '" required>',
        ) +
  ...
  async function logout() {
    ...
    try {
      if (rt)
        await api('/auth/logout', { method: 'POST', noAuth: true, body: { refreshToken: rt } });
    } catch (e) {}
    clearTokens();
    renderLogin();
  }
