...
import { loadCatalogSeed, validateCatalogSeed, tiersInInsertOrder } from '../../src/catalog/workbook-import.js';
import seedJson from '../../prisma/seed-catalog.json' with { type: 'json' };

const base = {
  productLines: [{ name: 'Adventure Series', slug: 'adventure-series', sortOrder: 10, isActive: true }],
  manufacturers: [{ name: 'Goldberg Brothers', code: 'goldberg-brothers', isThirdParty: true, isActive: true }],
  products: [
    { sku: 'A-2245', name: 'Vertical Tall', productLine: 'Adventure Series', defaultQuantity: 1, unitPriceMinor: 55775, showDimensions: false },
  ],
  tiers: [
...
describe('catalog seed validation', () => {
  ...
  it('rejects a tier pointing at an unknown SKU', () => {
    ...
    expect(report.issues.some((i) => i.message.includes('not in Products'))).toBe(true);
  });
  ...
  it('rejects a product placed at tier 1', () => {
    const d = clone();
    d.tiers.push({ slug: 'bad', name: 'Bad', productLine: 'Adventure Series', tierLevel: 1, parentSlug: null, sku: 'A-2245', sortOrder: 30 });
    expect(loadCatalogSeed(d).report.ok).toBe(false);
  });
  ...
  it('rejects two primary manufacturers for one SKU', () => {
    const d = clone();
    d.manufacturers.push({ name: 'TFH', isThirdParty: true, isActive: true });
  ...
  it('orders tiers parents-before-children', () => {
    const ordered = tiersInInsertOrder(base.tiers as never);
...
describe('the real workbook seed', () => {
  ...
  it('every tier parent resolves', () => {
    const data = loadCatalogSeed(seedJson).data;
    const slugs = new Set(data.tiers.map((t) => t.slug));
    const orphans = data.tiers.filter((t) => t.parentSlug && !slugs.has(t.parentSlug));
