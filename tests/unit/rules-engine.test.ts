...
function evalOne(
  rule: Partial<RuleDef> & Pick<RuleDef, 'type' | 'outcome'>,
  config: Configuration,
) {
  const full: RuleDef = { id: 'r1', version: 1, target: {}, params: {}, ...rule };
  return evaluateConfiguration([full], config);
}
const line = (productId: string, quantity = 1, extra: Record<string, unknown> = {}) => ({
  productId,
  quantity,
  ...extra,
});
...
describe('rules engine — one test per rule type', () => {
  ...
  it('INCOMPATIBLE_WITH: warns when both present', () => {
    const rule = {
      type: 'INCOMPATIBLE_WITH' as const,
  ...
  it('MIN_ROOM_DIMENSIONS: requests info when missing, blocks when too small', () => {
    const rule = {
      type: 'MIN_ROOM_DIMENSIONS' as const,
      outcome: 'BLOCK' as const,
      target: { productId: 'A' },
      params: { minLengthIn: 120, minWidthIn: 100 },
    };
    expect(evalOne(rule, { lines: [line('A')] }).requests.length).toBe(1);
    expect(
      evalOne(rule, { lines: [line('A')], context: { room: { lengthIn: 100, widthIn: 100 } } })
        .blocked,
    ).toBe(true);
    expect(
      evalOne(rule, { lines: [line('A')], context: { room: { lengthIn: 120, widthIn: 100 } } })
        .blocked,
    ).toBe(false);
  });
  ...
  it('MIN_CEILING_HEIGHT: requests info then blocks', () => {
    const rule = {
      type: 'MIN_CEILING_HEIGHT' as const,
      outcome: 'BLOCK' as const,
      target: { productId: 'A' },
      params: { minCeilingHeightIn: 108 },
    };
    expect(evalOne(rule, { lines: [line('A')] }).requests.length).toBe(1);
    expect(
      evalOne(rule, { lines: [line('A')], context: { room: { ceilingHeightIn: 96 } } }).blocked,
    ).toBe(true);
  });
  ...
  it('CLEARANCE: requests info then warns', () => {
    const rule = {
      ...
      outcome: 'WARN' as const,
      target: { productId: 'A' },
      params: { minClearanceIn: 24 },
    };
    expect(evalOne(rule, { lines: [line('A')] }).requests.length).toBe(1);
    expect(
      evalOne(rule, { lines: [line('A')], context: { room: { clearanceIn: 12 } } }).findings[0]!
        .outcome,
    ).toBe('WARN');
  });
  ...
  it('STRUCTURAL/INSTALLATION/FREIGHT: request info when fact missing; no assumption made', () => {
    const rule = {
      ...
      target: { productId: 'A' },
      params: { factKey: 'wallRated', expected: true },
    };
  ...
  it('AUTO_INCLUDE_COMPONENT: auto-adds per unit', () => {
    const rule = {
      type: 'AUTO_INCLUDE_COMPONENT' as const,
      outcome: 'AUTO_ADD' as const,
      target: { productId: 'A' },
      params: { componentProductId: 'MNT', perUnit: 2 },
    };
    const res = evalOne(rule, { lines: [line('A', 3)] });
    expect(res.autoAdds).toEqual([
      { productId: 'MNT', quantity: 6, sources: [{ ruleId: 'r1', ruleVersion: 1 }] },
    ]);
  });

  it('AUTO_CALCULATED_COMPONENT: integer ceil, no float drift', () => {
    const rule = {
      type: 'AUTO_CALCULATED_COMPONENT' as const,
      outcome: 'AUTO_ADD' as const,
      target: { productId: 'A' },
      params: { componentProductId: 'BOLT', ratioNum: 1, ratioDen: 3 },
    };
    // 7 units * 1/3 -> ceil = 3
  ...
  it('SUGGESTED_ACCESSORY / SUGGESTED_UPGRADE: recommend when absent', () => {
    const acc = {
      type: 'SUGGESTED_ACCESSORY' as const,
      outcome: 'RECOMMEND' as const,
      target: { productId: 'A' },
      params: { productId: 'ACC' },
    };
  ...
  it('MISSING_INFORMATION: requests info until provided', () => {
    const rule = {
      ...
      outcome: 'REQUEST_INFORMATION' as const,
      target: {},
      params: { infoKey: 'installDate', label: 'Install date' },
    };
    expect(evalOne(rule, { lines: [line('A')] }).requests.length).toBe(1);
    expect(
      evalOne(rule, { lines: [line('A')], context: { provided: ['installDate'] } }).requests.length,
    ).toBe(0);
  });
});
...
describe('rules engine — edge cases', () => {
  ...
  it('prevents duplicate automatic additions across multiple rules', () => {
    const r1: RuleDef = {
      ...
      outcome: 'AUTO_ADD',
      target: { productId: 'A' },
      params: { componentProductId: 'X', perUnit: 1 },
    };
    const r2: RuleDef = {
      id: 'r2',
      version: 1,
      ...
      outcome: 'AUTO_ADD',
      target: { productId: 'B' },
      params: { componentProductId: 'X', perUnit: 3 },
    };
  ...
  it('throws on a misconfigured threshold rather than assuming a value', () => {
    ...
    expect(() => evaluateConfiguration([rule], { lines: [line('A')] })).toThrow(
