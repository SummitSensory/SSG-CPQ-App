...
| 2026-08-29 | AUD-023: 194 parts existed as half of themselves; 192 ACTIVE and selectable in the proposal builder at $0.00 | **Fixed and verified.** `prisma/repair-half-created-parts.ts` completed all 194 (243 → 437 priced records). Exposure closed; no released proposal was ever mispriced, because the builder snapshots the rate onto the line |
| 2026-08-29 | AUD-024: `POST /catalog/tree/import` and `/catalog/import` silently discarded price, cost, weight and vendor columns | **Fixed.** Zod strips unknown keys without error, so imports "succeeded" while dropping the data. Both ends of the workbook round trip now carry the priced columns |
| 2026-08-29 | AUD-025: `PATCH /catalog/items/:part` created a Manufacturer row for any unrecognised name | **Fixed.** Refused, before any write, naming the vendors on record. Catalog UI now offers a dropdown. Two import paths that wrote only `Sku.manufacturer` now write `ProductSourcing` too |
| 2026-08-29 | AUD-026: nine integration tests guarding accepted-order locking and QuickBooks idempotency had never run | **Fixed and verified.** Their hand-written Prisma stubs omitted models the code reaches, so all nine threw before their first assertion. 80/80 integration tests now pass. Note: AUD-011 above claims this was already repaired with a Proxy — that fix was not present in the code |
| 2026-08-29 | Catalog integrity gate added to `pnpm check` | **Live.** `src/catalog/partIntegrity.ts` + `tests/integration/part-integrity.test.ts` (15 cases). A half-created part, or a part ordered from an unlisted vendor, now fails the build instead of reaching a quote |
| 2026-08-29 | AUD-003 step 3: Catalog screen extracted to `public/catalog.js` | Shipped. `app.js` 16,083 → 13,138 lines. Browser retest still outstanding |
| 2026-08-29 | Proposal reordering, unified New part form, history CSV export, manufacturer dropdown | Shipped; **browser verification outstanding.** Verified only by unit assertions and `tsc` |

## 13. Current overall status

**Pass 1 complete. Pass 2 executed against catalog data integrity only (2026-08-29) — see section 14.**

The catalog's blocking defects are closed and guarded. `pnpm check` now includes a data-integrity gate and passes. The integration suite runs 80/80, having previously run 71/80 with nine failures nobody had seen.

Not yet done: runtime/browser verification of the 2026-08-29 UI work, the security-probe pass, and the performance pass. No readiness score should be quoted until the browser pass is executed.

---

## 14. Pass 2 — catalog data integrity (2026-08-29)

A targeted pass, prompted by a user report: products added to the CRM were arriving without
manufacturer, cost or price, and adding one product required visiting two separate tabs.

### What the report turned out to be

Not a UI problem. **A part is two rows** — `Product` (name, category, tree position,
sourcing) and `Sku` (price, cost, weight, vendor) — and **three of the four ways to create
one wrote only a single half**:

| path                         | writes                                                          |
| ---------------------------- | --------------------------------------------------------------- |
| `POST /catalog/import`       | `Product` only                                                  |
| `POST /catalog/tree/import`  | `Product` only (status DRAFT); touches `Sku` only to deactivate |
| `POST /skus`, `/skus/import` | `Sku` only                                                      |
| `POST /catalog/items`        | both — added during this pass                                   |

413 products, 243 priced records. **194 products had no priced record.** Having to visit two
tabs was not a workflow; it was the shape of the defect.

### The part that mattered most

`GET /catalog/items` reports a Sku-less `Product` as `unitPriceMinor: 0, active: true`, and
the proposal builder's part picker keeps every row it marks active. **192 of the 194 were
ACTIVE, so every one was offered to a rep at $0.00** with nothing to flag it.

Measured before acting: 9 proposal lines referenced a part with no priced record, all 9 on
released proposals — and **all 9 carried correct non-zero rates**. The builder snapshots the
rate onto the line when it is added rather than reading the catalog at print time. That one
design decision is why an eight-month-old defect never reached a customer with a wrong
number. It is the single best thing in this codebase.

### Fixed

- **194 parts completed.** Priced records created at $0.00, INACTIVE for import casualties
  and ACTIVE for the five a proposal had already used (a discount at quantity -1, hourly
  labour, a bundle — these have no catalog price by nature and hiding them would have
  stopped reps adding them). `Product` rows untouched, so nothing left the catalog or tree.
- **The workbook round trip carries prices** in both directions. It previously exported no
  price columns and, because Zod strips unknown keys without erroring, silently discarded
  any that were added by hand while reporting success.
- **Vendor identity cannot diverge.** `PATCH /catalog/items/:part` created a Manufacturer row
  for any unrecognised name — no address, no terms, no BOM defaults, indistinguishable in
  the vendor list from a real vendor. Now refused before any write. The two import paths
  that wrote only `Sku.manufacturer` now write `ProductSourcing` as well.
- **One place to add a part.** `POST /catalog/items` writes both rows in one transaction,
  with section and manufacturer as dropdowns and tree placement in the same form.
- **Nine dead tests revived** — see AUD-026.

### The guard

`src/catalog/partIntegrity.ts` holds the invariants once; `prisma/check-part-integrity.ts`
runs them against the database and is wired into `pnpm check`;
`tests/integration/part-integrity.test.ts` runs them against fixtures so the rules cannot
silently stop detecting a class of drift.

| rule                                                                 | severity |
| -------------------------------------------------------------------- | -------- |
| `product-without-sku` (ACTIVE)                                       | blocking |
| `vendor-not-sourced`                                                 | blocking |
| `vendor-multiple-primary`, `vendor-not-primary`, `vendor-not-linked` | warning  |
| `sku-without-product`, `active-mismatch`                             | warning  |

Blocking means it can put a wrong number on a customer document. A check that blocks on
everything gets switched off.

**Current state: 0 blocking, 213 warning.** The 189 `active-mismatch` warnings are the
completed-but-unpriced parts — a worklist, not a fault.

### Three findings that were not defects, and cost time

Recorded because the reasoning errors are more instructive than the fixes:

1. **`Sku.category` vs `Product.categoryId`.** Flagged as a duplicated fact; a repair was
   written that would have overwritten 219 parts' type codes with tree section names.
   `src/reporting/dataset.ts` documents the truth: `Sku.proposalGroup` is the proposal
   heading, `Sku.category` is a part TYPE (FRAME, TROLLEY, ACCESSORY), `Product.categoryId`
   is tree position. Three fields, three jobs. "ADVENTURE SERIES FRAME" in the tree and
   "FRAME" as the type is correct on both counts.
2. **`ProductSourcing` cardinality.** Flagged 7 parts as vendor disagreements. It is a
   many-to-many with `@@unique([productId, manufacturerId])`; the tracking-rail hardware
   lists two vendors deliberately. The check had collapsed a list to one value.
3. **`isPrimary`.** Then flagged the same 7 as mis-flagged. `isPrimary` is
   `@default(true)`, so both rows were primary and "the primary source" did not exist for
   those parts.

Each was the same mistake: inferring a data model from field names instead of reading
`prisma/schema.prisma`. In one case a comment written earlier in the same session was later
read back as corroboration. **The schema is the authority; a field's shape is not its
meaning.**

### Outstanding

- **Browser verification** of the 2026-08-29 UI work: proposal reordering, the New part
  form, history CSV export, the manufacturer dropdown, and the six Catalog tabs after the
  `catalog.js` extraction. None has been opened in a browser.
- **189 parts need prices.** Export the tree workbook, fill `unitPrice`/`unitCost`,
  dry-run, commit, set active.
- **`SVC-CON-HR` and `SVC-DES-SITE`** went inactive (no proposal had used them). Give them
  a rate if reps quote consulting hours or site visits.
- **`isPrimary` should default to `false`**, with create paths setting it explicitly. Small
  migration; prevents the next multi-source part acquiring two primaries.
- **24 `sku-without-product` warnings** — kit and bundle lines, several likely deliberate.
- **`Product` and `Sku` remain two rows.** Drift is now detected and unshippable, but the
  duplication is policed rather than removed. Collapsing them is the honest fix and needs
  the proposal builder, BOM and QuickBooks paths run against it, not read.
